...
1. Project guidance files -- look for a `CMAKE_PREFIX_PATH` or explicit Qt path.
...
- No shell-execution tool is configured in this session.
- A sandbox blocks executing binaries outside the project tree (e.g.
  a host application sandbox).
...
4. Mention the alternative: if the user would prefer the skill to
   run qmlprofiler automatically, a shell-enabled Cline session can
   typically do this on their machine when the Qt binary path is
...
3. Animation / frame-time summary (if `animations` key is present in
   ...
   using `frame_count`: N = round(5% x frame_count) for p95, round(1%
   x frame_count) for p99 -- e.g. "p95 = 66.67 ms -> ~45 frames >= 67
   ms".
