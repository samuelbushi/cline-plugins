...
Explicit-override fast path. If the user's prompt combines (a) an explicit deliverable ("give me the Bicep", "just print the az CLI", "apply it via MCP") and (b) an explicit refusal of dialog ("no questions", "skip planning", "no plan", "just do it"), respect them. Skip the Plan Card table and the approval question. Instead:
