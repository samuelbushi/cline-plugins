...
Render the Plan Card markdown inline in the chat first so the user can read it, then ask one combined approval, output-format, and delivery question:
...
Implementation: ask once with the six likely combinations above, and let the user provide another answer such as "give me both bicep and terraform".
