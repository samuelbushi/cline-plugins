...
|-------------|---------|
| "Create Foundry" / "Set up Foundry" (ambiguous) | Ask the user: (a) just an AI Services resource, (b) a project with public access, or (c) a project with network isolation? Route: (a) -> [resource/create](resource/create/create-foundry-resource.md), (b) -> [project/create](project/create/create-foundry-project.md), (c) -> [private-network](resource/private-network/private-network.md) |
...
Ask the user in chat only for values not resolved from the user's message, session context, metadata, or azd bootstrap. Common values skills may need:
...
- Ask the user in chat whenever collecting information from the user
