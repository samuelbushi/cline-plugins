---
name: microsoft-foundry-capacity
...
Pre-built scripts handle the complex REST API calls and data processing. Use these instead of constructing commands manually. The Bash scripts require `jq`; use the PowerShell versions or install `jq` first if it is unavailable.
