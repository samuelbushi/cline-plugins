...
1. Plan first - MANDATORY - Before creating or updating `.azure/deployment-plan.md`, tell the user the exact path that will be written and ask for approval unless they already explicitly asked you to prepare Azure deployment files. After approval, physically write an initial skeleton in the workspace root directory (not the session-state folder) before any code generation or execution begins. Populate it progressively as Phase 1 analysis and research unfold; finalize it with all decisions at Phase 1 Step 6. This file must exist on disk throughout. azure-validate and azure-deploy depend on it and will fail without it. Do not skip or defer this step.
...
6. Confirm Azure context - Ask the user to confirm subscription and location per [Azure Context](references/azure-context.md)
7. ❌ Destructive actions require explicit user confirmation - [Global Rules](references/global-rules.md)
...
> 2. CREATE SKELETON - Ask before writing unless the user already explicitly requested Azure deployment file preparation. Then write an initial `.azure/deployment-plan.md` skeleton to disk before any code generation or execution begins, populate it progressively as Phase 1 steps 1-5 reveal details, and finalize it at Step 6
...
| 8 | Destructive actions require explicit user confirmation | [Global Rules](references/global-rules.md) |
