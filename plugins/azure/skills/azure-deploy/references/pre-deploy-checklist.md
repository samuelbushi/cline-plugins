...
You MUST ask the user to confirm the subscription. Find the default subscription (marked `isDefault: true`) from Step 1 results and present it as the recommended choice.
...
2. Alternative - Delete conflicts: Ask the user to confirm deletion of old resources (required by global rules).
...
For environments with a `provisioningState` of `Succeeded`, use explicit user confirmation to present the conflict and offer choices:
...
3. Delete and start fresh - Delete the conflicting environment (requires explicit user confirmation confirmation per global-rules):
   ```bash
...
You MUST use explicit user confirmation with regions that support ALL services in the architecture.
