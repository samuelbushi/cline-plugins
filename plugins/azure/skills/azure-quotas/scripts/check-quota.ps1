...
.DESCRIPTION
    Checks that the quota CLI extension is available, then queries quota limits
    and current usage in a single call. Returns a table with limit, usage, and
...
# Ensure the quota extension is available. Do not install extensions from this
# helper; Cline should ask the user before changing local tooling.
$ext = az extension list --query "[?name=='quota'].name" -o tsv 2>$null
if (-not $ext) {
    Write-Host "Azure CLI quota extension is not installed."
    Write-Host "Ask the user before running: az extension add --name quota --yes"
    exit 2
}
