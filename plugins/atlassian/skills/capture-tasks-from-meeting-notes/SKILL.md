...
```
[OK] Found: Sarah Johnson (sarah.johnson@company.com)
-> Use accountId from result
```

Scenario B: No Match (0 results)
```
[WARN] Couldn't find user "Sarah" in Jira.
...
```
[WARN] Found multiple users named "Sarah":
...
- [OK] "Create user stories for chat feature"
- [OK] "Update architecture documentation"
- [OK] "Review and approve design mockups"
- [NO] "Do the thing" (too vague)
...
### Do:
[OK] Use consistent @mention format in notes
[OK] Include full names when possible
[OK] Be specific in action item descriptions
[OK] Add context (why/what/when)
[OK] Review parsed tasks before confirming

### Don't:
[NO] Mix multiple tasks for one person in one bullet
[NO] Use ambiguous names (just "John" if you have 5 Johns)
[NO] Skip action verbs (unclear what to do)
[NO] Forget to specify project
...
Don't use for:
[NO] Summarizing meetings (no task creation)
[NO] Finding meeting notes (use search skill)
[NO] Creating calendar events
[NO] Sending meeting notes via email
[NO] General note-taking
...
- `references/action-item-patterns.md` - additional action-item formats and extraction examples for meeting notes.
