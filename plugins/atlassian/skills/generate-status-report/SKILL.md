...
Before calling `createConfluencePage` or `updateConfluencePage`, show the target space/page, title, and report body preview. Wait for explicit user confirmation to publish or update.

Use the `createConfluencePage` tool only after that confirmation.
...
2. Show the updated body and wait for explicit user confirmation, then update the page with new content:
```
