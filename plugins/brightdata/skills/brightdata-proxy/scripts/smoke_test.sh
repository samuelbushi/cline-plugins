...
if [[ -f "$CA_CERT" ]]; then
  ...
  echo "x ca cert not found at ${CA_CERT}" >&2
  echo "  Set BD_CA_CERT to a valid Bright Data Proxy Root CA path or restore the bundled assets/brightdata_proxy_ca.crt file." >&2
  exit 1
fi
echo
