...
Do not disable certificate verification. If TLS verification fails, install or
load the Bright Data CA certificate, or use a product surface that does not need
raw proxy TLS interception.
...
For CA verification on Node, set `NODE_EXTRA_CA_CERTS=/path/to/brightdata_proxy_ca.crt` in the environment, or pass a custom `Agent` with `ca` set.
...
Residential and Mobile have a network access policy that Datacenter and ISP don't. The user must pick an approved verification path before requests will succeed against arbitrary HTTPS targets:
...
Generated code for Residential/Mobile should default to path 1a (load CA in code) with the cert path as a placeholder the user fills in. Mention KYC as the alternative path when appropriate.
...
- TLS / certificate errors on Residential/Mobile: you skipped the network-access setup. Install the cert or load it in code; do not disable certificate verification.
