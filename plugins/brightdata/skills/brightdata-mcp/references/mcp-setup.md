...
Add a Bright Data MCP server through the Cline MCP configuration UI or the
user-managed Cline MCP settings file. The exact settings location depends on
the host application, so do not mutate it from this skill; give the user the
server details to apply themselves.
...
1. Ask Cline: "Use the Bright Data MCP to search for 'test query'"
