---
name: brightdata-brand-listening
description: >
...
> Pipeline names change. Always confirm with `bdata pipelines list` before hardcoding a type. Names are inconsistent across platforms (`tiktok_posts` plural, `reddit_posts` plural, `x_posts`). The `brightdata-data-feeds` skill has the verified list.
