...
Use this workflow without a session ID when the user wants a recent-session
health check.
...
5. Bandwidth outliers -- group sessions by `target_url` domain. For each domain with 3+ sessions, calculate the median bandwidth. Flag any session whose bandwidth exceeds 2x the median for that domain as an outlier, and note if it was a failed session that burned unusually high bandwidth before dying.
...
Use this workflow with a specific Bright Data browser session ID when the user
wants a deep dive into one failed or suspicious session.
...
- Dynamic bandwidth outliers: compares sessions per domain using median, flags sessions exceeding 2x the median
