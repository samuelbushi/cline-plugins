...
Canonical install / login / troubleshooting reference for the `bdata` (aka `brightdata`) CLI. Linked from the `brightdata-scrape`, `brightdata-search`, and `brightdata-data-feeds` skills.
...
These are still honored by legacy `curl`-based paths documented in each skill's local patterns reference. The CLI path is preferred; env vars are retained for environments where Node/CLI aren't available.
