...
Reads from .env in the repo root (parent of scripts/) or current working directory:
    ...
    TENANT_ID          - required
    DATAVERSE_USERNAME - optional, selects one account when the shared cache has several
...
# be silently reused by Python scripts.
...
def _build_shared_msal_cache():
    ...
    platform, empty cache, corrupt cache) so the caller can surface a setup
    error.
    """
    try:
        import msal
    ...
    try:
        ...
        # Any failure (permissions, unsupported keyring, corrupt cache) is
        # handled by the caller as a setup/authentication error.
        return None
...
def _get_credential():
    ...
    if not tenant_id or not dataverse_url:
        ...
        print("  Ask Cline to initialize Dataverse setup with the dv-connect skill.", flush=True)
        sys.exit(1)
    ...
    if bool(client_id) != bool(client_secret):
        ...
        print("  service principal auth. Falling back to the shared Dataverse CLI cache.", flush=True)
    ...
    if shared is not None:
        app, accounts = shared
        account_hint = os.environ.get("DATAVERSE_USERNAME", "").strip().lower()
        if account_hint:
            matches = [
                account for account in accounts
                if account_hint in str(account.get("username", "")).lower()
                or account_hint in str(account.get("home_account_id", "")).lower()
            ]
            if len(matches) != 1:
                print("ERROR: DATAVERSE_USERNAME did not uniquely match a cached account.", flush=True)
                print("  Set DATAVERSE_USERNAME to the exact account username or re-run `dataverse auth create`.", flush=True)
                sys.exit(1)
            accounts = matches
        elif len(accounts) > 1:
            usernames = ", ".join(
                str(account.get("username", account.get("home_account_id", "<unknown>")))
                for account in accounts
            )
            print("ERROR: Multiple Dataverse CLI accounts are cached.", flush=True)
            print(f"  Cached accounts: {usernames}", flush=True)
            print("  Set DATAVERSE_USERNAME in .env so Python scripts use the intended account.", flush=True)
            sys.exit(1)
        _credential = _MsalSharedCacheCredential(app, accounts)
        return _credential

    print("ERROR: No shared Dataverse CLI token cache was found.", flush=True)
    print("  Run `dataverse auth create --environment "
          f"{dataverse_url}` after approval, then retry.", flush=True)
    sys.exit(1)
...
def get_token(scope=None):
    ...
    then the shared DataverseCLI MSAL cache (silent).
...
_ALLOWED_AGENTS = frozenset({
    "cline", "copilot", "cursor", "codex", "unknown",
})
...
if __name__ == "__main__":
    get_token()
    print("Dataverse auth helper ready. Access token acquired but not printed.")
