...
- If BOTH exist: workspace is initialized. Proceed to the relevant task after confirming the target environment when a write is involved.
- If EITHER is missing: load the `dv-connect` skill, inspect the current setup, present the exact setup plan, and wait for explicit approval before installing tools, authenticating, writing `.env`, copying scripts, or configuring MCP.

Do not create `requirements.txt`, `.env.example`, or scaffold files manually unless the approved `dv-connect` plan calls for them. The connect flow owns the workspace setup shape.
...
NEVER:
- Run `npm init` or create app package files for Dataverse automation
- Use Node.js or JavaScript for Dataverse scripts
- Install project/runtime packages via `npm`, `yarn`, or `pnpm`
...
- Use `npm install -g @microsoft/dataverse@latest` only when the approved `dv-connect` plan needs the Microsoft Dataverse CLI

If you find yourself about to run `npm` for anything other than the Microsoft Dataverse CLI installation approved by the user, STOP. You are going off-rails. Re-read Hard Rule 1 above.
...
3. Load the `dv-connect` skill and present the optional MCP setup plan
4. Wait for explicit approval before editing MCP settings or running setup commands
5. After MCP is configured, stop here - the session may need to be restarted or resumed for MCP tools to appear. Do not proceed with SDK unless the user asks for a fallback.
...
| --- | --- |
| dv-connect | Connect to Dataverse: inspect tools, authenticate, create `.env`, copy helper scripts, optionally guide MCP setup, verify connection |
...
When running in a Unix-style shell on Windows, adapt path and quoting syntax as needed:
...
- Background job output may be empty on Windows. Agent background task runners on Windows (e.g., Cline's "Running in the background") can silently produce no output. Always use `python -u` (unbuffered stdout) and `print(..., flush=True)` in long-running scripts. For foreground execution with logging: `python -u scripts/import_data.py 2>&1 | tee /tmp/out.txt`. Do NOT assume a background task succeeded just because it appeared to finish.
...
Then show the resulting diff and ask the user before staging, committing, or pushing. Do not push unless the user explicitly asks for it.
