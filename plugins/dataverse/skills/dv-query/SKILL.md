...
- Cross-table aggregation: `$apply` only works within one entity set. Use `client.dataframe.get(entity, select=[...])` per table -> `pd.merge()` -> `groupby()`. Always pass `select=`; without it transfers 10-20x more data.
