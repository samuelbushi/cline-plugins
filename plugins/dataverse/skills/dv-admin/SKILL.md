...
> ## WARNING: Critical safety rules - read first
>
...
`SearchAndCopilotIndexMode` is one int (0-3) that encodes two UI toggles - Dataverse search x M365 Copilot search:
...
- Bad: "The command requires approval. Please confirm to pause the job."
- Good: "I'll pause bulk delete job `<job-id>` on the active environment. Confirm to proceed."

Audit status across N environments (read-only, multi-call):
- Bad: Sequential `pac org fetch` per env, or starting with Python/SDK because it "feels like a query."
- Good: "I'll run `pac org list-settings --filter audit` in parallel across all N environments (one `&`-batch, single `wait`)."
