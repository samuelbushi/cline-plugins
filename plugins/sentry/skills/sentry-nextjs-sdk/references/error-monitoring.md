...
|-----------|-----------|-----------|
| Unhandled client JS exceptions | [recommended] Yes | `window.onerror` (GlobalHandlers integration) |
| Unhandled promise rejections (client) | [recommended] Yes | `window.onunhandledrejection` |
| Server Component render errors (Next.js 15+) | [recommended] Yes | `onRequestError` hook in `instrumentation.ts` |
| Unhandled API route crashes (server) | [recommended] Yes | Node.js uncaught exception handler |
| Re-thrown errors from `try/catch` | [recommended] Yes | Bubbles to global handler |
| `error.tsx` boundary errors | No No | Next.js catches before Sentry |
| `global-error.tsx` boundary errors | No No | Next.js catches before Sentry |
| Caught + swallowed `try/catch` errors | No No | Must call `captureException` manually |
| Server Action graceful error returns | No No | Must call `captureException` or use wrapper |
| Caught edge middleware errors | No No | Must call `captureException` manually |
...
| Context | No No | No No | Structured debug info (nested objects) |
| Extra | No No | No No | Arbitrary debug values |
...
|----------|---------------|---------|
| Unhandled client JS exception | [recommended] Yes | - |
| Unhandled promise rejection | [recommended] Yes | - |
| Server Component error (Next.js 15+) | [recommended] Yes | `onRequestError` hook |
| Unhandled API route crash | [recommended] Yes | - |
| `app/error.tsx` boundary | No No | `captureException` in `useEffect` |
| `app/global-error.tsx` | No No | `captureException` in `useEffect` |
| `try/catch` with graceful return | No No | `captureException` before return |
| `try/catch` with re-throw | [recommended] Yes | - |
| Server Action graceful error | No No | `captureException` or `withServerActionInstrumentation` |
| Caught edge middleware error | No No | `captureException` manually |
