...
> Warning: Not available via CDN/loader snippet - NPM install required.
...
> Warning: `logger.fmt` must be used as a tagged template literal - not a function call. `Sentry.logger.fmt("text")` will not produce structured parameters.
...
// Warning: CRITICAL for Next.js server-side: use isolation scope (not global)
...
> Warning: Next.js server-side isolation: Always use `getIsolationScope()` for per-request data on the server. The isolation scope is unique per request, preventing attributes from one user's request from bleeding into another's concurrent request.
...
```typescript
// [recommended] Preferred - one wide log with full context
...
// No Avoid - fragmented logs that are hard to correlate
