...
> Warning: Tracing must be enabled. AI monitoring piggybacks on tracing infrastructure. `tracesSampleRate` must be > 0.
...
|---------|----------------|----------------------------|----------------|
| OpenAI (`openai`) | `openAIIntegration` / `instrumentOpenAiClient` | [recommended] Yes | 10.53.0 |
| Vercel AI SDK (`ai`) | `vercelAIIntegration` | [recommended] Yes (Node), No Edge manual | 10.53.0 |
| Anthropic (`@anthropic-ai/sdk`) | `anthropicAIIntegration` / `instrumentAnthropicAiClient` | [recommended] Yes | 10.53.0 |
...
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY, // Warning: Never expose this in the browser!
});
...
const anthropic = new Anthropic({
  apiKey: process.env.ANTHROPIC_API_KEY, // Warning: Never expose in the browser!
});
...
> Warning: PII warning: Prompts often contain user-supplied text. If users include personal data in prompts, enabling `recordInputs` will send that data to Sentry. Review your privacy policy before enabling.
