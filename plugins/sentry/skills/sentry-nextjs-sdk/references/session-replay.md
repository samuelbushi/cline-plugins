...
> Warning: Browser-only feature. Add `replayIntegration()` only in `instrumentation-client.ts`. Never in `sentry.server.config.ts` or `sentry.edge.config.ts`.
...
> Warning: v8 Breaking Change: In SDK v8+, `unblock` and `unmask` no longer automatically add `sentry-unblock`/`sentry-unmask` class selectors. To restore v7 behavior:
> ```typescript
> Sentry.replayIntegration({
...
> Warning: Tree-shaking only works with webpack builds. Turbopack is not supported.

---

## Canvas Recording

> Warning: There is currently NO PII scrubbing in canvas recordings. Use with caution.
