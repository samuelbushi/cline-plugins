...
- [recommended] Error Monitoring - always; captures server errors, client errors, server actions, and unhandled promise rejections
- [recommended] Tracing - server-side request tracing + client-side navigation spans across all runtimes
- [recommended] Session Replay - recommended for user-facing apps; records sessions around errors

Optional (enhanced observability):
- [optional] Logging - structured logs via `Sentry.logger.*`; recommend when `pino`/`winston` or log search is needed
- [optional] Profiling - continuous profiling; requires `Document-Policy: js-profiling` header
- [optional] AI Monitoring - OpenAI, Vercel AI SDK, Anthropic; recommend when AI/LLM calls detected
- [optional] Crons - detect missed/failed scheduled jobs; recommend when cron patterns detected
- [optional] Metrics - custom metrics via `Sentry.metrics.*`; recommend when custom KPIs or business metrics needed
