...
|-----------|---------|-------------------|
| HTTP request lifecycle | `http.server` | [recommended] Always |
| Route handler dispatch | `http.route` | [recommended] Always |
| SQL queries | `db.sql.query` | [recommended] (`tracing.sql_queries`) |
| DB transactions | `db.transaction` | [recommended] Always |
| Blade view rendering | `view.render` | [recommended] (`tracing.views`) |
| Outgoing HTTP client | `http.client` | [recommended] (`tracing.http_client_requests`, Laravel >= 8.45) |
| Cache operations | `cache.*` | [recommended] (`tracing.cache`, Laravel >= 11.11) |
| Queue job processing | `queue.process` | Config `tracing.queue_jobs: true` |
| Queue job as transaction | `queue.process` | Config `tracing.queue_job_transactions: true` |
| Redis commands | (redis spans) | Config `tracing.redis_commands: true` |
| Livewire components | (livewire spans) | Config `tracing.livewire: true` |
| Notifications | (notification spans) | [recommended] (`tracing.notifications`) |
| Lighthouse GraphQL | (graphql spans) | [recommended] When Lighthouse installed |
| Laravel Folio routes | transaction name | [recommended] When Folio installed |
| Filesystem disk operations | (file spans) | Config Opt-in via `Storage\Integration::configureDisks()` |
...
```
Effective profiling rate = traces_sample_rate x profiles_sample_rate

Examples:
  0.5 x 0.5 = 25% of all requests profiled
  1.0 x 1.0 = 100% of all requests profiled
  0.1 x 1.0 = 10% of all requests profiled
```
