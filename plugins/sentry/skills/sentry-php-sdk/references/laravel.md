...
Warning: Limitation: Tasks using `between`, `unlessBetween`, `when`, and `skip` methods are not supported. Use `cron('...')` for the schedule frequency instead.
...
Warning: Important: If using `withScope()` / `configureScope()` for per-request context, ensure you're using `withScope()` (not `configureScope()`) in Octane environments - `configureScope()` persists across requests in long-running workers.
