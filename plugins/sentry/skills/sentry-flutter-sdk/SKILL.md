...
- [recommended] Error Monitoring - captures Dart exceptions, Flutter framework errors, and native crashes (iOS + Android)
- [recommended] Tracing - auto-instruments navigation, app start, network requests, and UI interactions
- [recommended] Session Replay - captures widget tree screenshots for debugging (iOS + Android only)

Optional (enhanced observability):
- [optional] Profiling - CPU profiling; iOS and macOS only (alpha)
- [optional] Logging - structured logs via `Sentry.logger.*` and `sentry_logging` integration
- [optional] Metrics - counters, gauges, distributions (SDK >=9.11.0)
...
> Warning: Platform limitations in debug mode:
