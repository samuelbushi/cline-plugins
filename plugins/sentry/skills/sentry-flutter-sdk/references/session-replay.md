...
| Text in replay | [recommended] Selectable, searchable | No Pixel-only - text is in screenshots |
...
> Warning: Production warning: Always verify your masking config before enabling in production. The SDK masks aggressively by default, but any customizations require thorough testing with your actual app UI. If you discover unmasked PII, disable Session Replay until resolved.
...
|-------------|-----------------|
| `Text`, `RichText`, `EditableText` | [recommended] Masked by default |
| `Image` (including asset images) | [recommended] Masked by default |
| `TextFormField`, `TextField` | [recommended] Masked by default |
...
- [recommended] Captured: Tap position, tapped widget, timestamp
- No Not captured: Swipe paths, gesture velocity, multi-touch sequences
