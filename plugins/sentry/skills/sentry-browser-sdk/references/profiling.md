...
> Warning: Beta status - breaking changes may occur. Browser support is limited to Chromium-based browsers only (Chrome, Edge). Firefox and Safari are not supported.
...
| Firefox | No No | JS Self-Profiling API not implemented |
| Safari / iOS Safari | No No | JS Self-Profiling API not implemented |

> Warning: Sampling bias: Profile data is collected only from Chromium users. Firefox and Safari sessions are silently excluded - no error is thrown, no overhead is added.
