...
|-----------|----------|-------------|
| `message` | [recommended] | The feedback text from the user |
| `name` | No | User's name |
| `email` | No | User's email |
| `associatedEventId` | No | Links feedback to a specific Sentry event (use `Sentry.lastEventId()` or the return value of `captureException`) |
...
|--------|----------|-------------|
| `eventId` | [recommended] | The Sentry event ID to associate the feedback with |
| `user.name` | No | Pre-fill user's name |
| `user.email` | No | Pre-fill user's email |
| `lang` | No | Dialog language code (e.g., `"de"`, `"fr"`) |
| `title` | No | Override dialog title |
| `subtitle` | No | Override dialog subtitle |
| `subtitle2` | No | Override second subtitle line |
| `labelSubmit` | No | Override submit button label |
