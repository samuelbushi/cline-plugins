...
|--------|-------------|---------------|
| LCP - Largest Contentful Paint | Perceived load speed | [recommended] Always |
| CLS - Cumulative Layout Shift | Visual stability | [recommended] Always |
| TTFB - Time to First Byte | Server responsiveness | [recommended] Always |
| INP - Interaction to Next Paint | Responsiveness to user inputs | [recommended] (SDK >=8.x) |
...
|--------|------|----------|-------------|
| `name` | `string` | [recommended] | Human-readable identifier shown in the Sentry UI |
...
Sentry.init({
  ...
  integrations: [
    Sentry.browserTracingIntegration({
      enableInp: true,
      interactionsSampleRate: 0.1, // Effective rate: 50% x 10% = 5% of interactions
    }),
  ],
});
```
