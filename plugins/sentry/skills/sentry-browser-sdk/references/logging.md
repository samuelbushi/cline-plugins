...
> Warning: NPM or CDN logs bundle required - Sentry logging is not available via the Loader Script. Use npm/yarn/pnpm (`@sentry/browser`) or a CDN bundle with `.logs.` in its name (e.g., `bundle.logs.metrics.min.js`).
...
> Warning: `logger.fmt` must be used as a tagged template literal - not as a function call. `Sentry.logger.fmt("text")` will not produce structured parameters.
