...
- [recommended] Error Monitoring - always; captures unhandled errors and promise rejections
- [recommended] Tracing - recommended for any interactive site; tracks page load and user interactions
- [recommended] Session Replay - recommended for user-facing apps; records sessions around errors

Optional (enhanced observability):
- [optional] User Feedback - capture reports directly from users after errors
- [optional] Logging - structured logs via `Sentry.logger.*`; requires npm or CDN logs bundle (not available via Loader Script)
- [optional] Profiling - JS Self-Profiling API; beta, Chromium-only, requires `Document-Policy: js-profiling` response header
...
> Warning: esbuild plugin does not fully support `splitting: true`. Use `sentry-cli` instead if code splitting is enabled.
...
- No No `Sentry.logger.*` (logging) - npm path only
- No No framework-specific features (React ErrorBoundary, Vue Router tracking, etc.)
- No Tracing headers only added to fetch calls made after SDK loads
- No Version changes take a few minutes to propagate via CDN cache
- Warning: Use `defer` (not `async`) on all other scripts when using the loader
