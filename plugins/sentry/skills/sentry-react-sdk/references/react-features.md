...
> Warning: Critical: `sentryReduxEnhancer` is a store enhancer, not Redux middleware. Do NOT pass it inside `applyMiddleware()`.
...
> Warning: Warning: If `stateTransformer` returns `null`, error events will lack Redux state context. Debugging large state-dependent bugs becomes much harder. Prefer returning a filtered copy over returning `null`.
...
|---------|---------------------|
| Vite | [recommended] Supported |
| Webpack | [recommended] Supported |
| Rollup | [recommended] Supported |
| esbuild | No Not supported |
...
> Warning: Place `sentryVitePlugin` after all other plugins (especially `@vitejs/plugin-react`) to ensure correct source map generation.
...
> Warning: Never commit `SENTRY_AUTH_TOKEN` to source control. Always read it from environment variables.
