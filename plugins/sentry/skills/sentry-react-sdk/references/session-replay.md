...
> Warning: Browser-only feature. Never add `replayIntegration()` to SSR entry points, Node.js server code, or web workers. Only add it where browser APIs are available.
...
|-----------|-----------------|
| All text content | [recommended] Masked - replaced with `*` characters (length-preserving) |
| All `<input>` values | [recommended] Masked |
| `<img>`, `<video>`, `<audio>`, `<svg>`, `<picture>`, `<embed>`, `<map>`, `<object>` | [recommended] Blocked - replaced with same-size placeholder rectangle |
| Network request/response bodies | No Not captured - opt-in required |
| Network headers beyond Content-Type/Length/Accept | No Not captured - opt-in required |
...
|-------|-----------|
| URL | [recommended] Yes |
| HTTP method | [recommended] Yes |
| HTTP status code | [recommended] Yes |
| Request body size (bytes) | [recommended] Yes |
| Response body size (bytes) | [recommended] Yes |
| Request/response body content | No No |
| Custom headers | No No |
...
| Format | Captured |
|--------|----------|
| `application/json` | [recommended] Yes |
| XML | [recommended] Yes |
| `text/plain` | [recommended] Yes |
| `multipart/form-data` (text fields) | [recommended] Yes |
| Binary / byte arrays | No No |
| File uploads | No No |
| Blobs / media streams | No No |
...
Sentry.replayIntegration({
  ...
  networkRequestHeaders: [
    ...
    'X-Request-ID',
    'X-Correlation-ID',
    // Warning: Avoid: 'Authorization', 'Cookie' - contain credentials
  ],
...
> Warning: Never capture `Authorization`, `Cookie`, or `Set-Cookie` headers in production. They contain bearer tokens and session credentials.
...
> Warning: No PII scrubbing in canvas recordings. If your canvas renders sensitive data (user documents, medical images, profile photos), either avoid canvas recording or sanitize the canvas content before each snapshot.
