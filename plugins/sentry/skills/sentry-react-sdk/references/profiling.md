...
> Warning: Beta status - breaking changes may occur. Browser support is limited to Chromium-based browsers only.
...
|---------|-----------|-------|
| Chrome / Chromium | [recommended] Yes | Primary support target |
| Edge (Chromium) | [recommended] Yes | Same engine as Chrome |
| Firefox | No No | Does not implement JS Self-Profiling API |
| Safari / iOS Safari | No No | Does not implement JS Self-Profiling API |

> Warning: Sampling bias: Profile data is collected only from Chromium users. Firefox and Safari sessions are silently not profiled. Consider this when drawing performance conclusions.
...
> Warning: Static hosting that disallows custom headers (some CDNs, GitHub Pages) will prevent profiling entirely.
...
> Warning: Chrome DevTools conflict: When `browserProfilingIntegration` is active, Chrome DevTools profiles incorrectly show profiling overhead mixed into rendering work. Disable the integration when doing local DevTools profiling sessions.
