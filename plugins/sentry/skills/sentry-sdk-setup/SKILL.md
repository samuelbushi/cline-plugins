...
This plugin bundles each SDK skill locally. When this router selects a platform, read the bundled `../<skill-name>/SKILL.md` file from the table below and follow it directly. Do not fetch remote skill content unless the user explicitly asks for the latest upstream copy.
...
3. Wait for confirmation before loading the selected bundled skill and proceeding.
...
Match your project to a skill by keywords. Use the bundled skill path in the table below.
...
Looking for workflows or feature configuration instead? Use the bundled `sentry-workflow` or `sentry-feature-setup` skill.
