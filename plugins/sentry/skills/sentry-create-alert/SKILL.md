...
Stop before running the mutation. Show the user the target org, region, alert name, trigger conditions, action targets, and exact request payload. Ask for explicit confirmation before running any `POST`, `PUT`, or `DELETE` request that creates, updates, enables/disables, or deletes a Sentry workflow or alert.
...
List/get requests are read-only. For update and delete requests, show the workflow id, current state when available, proposed payload or deletion target, and ask for explicit confirmation immediately before running the command.
