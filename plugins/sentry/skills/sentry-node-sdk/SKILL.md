...
- [recommended] Error Monitoring - always; captures unhandled exceptions, promise rejections, and framework errors
- [recommended] Tracing - automatic HTTP, DB, and queue instrumentation via OpenTelemetry

Optional (enhanced observability):
- [optional] Logging - structured logs via `Sentry.logger.*`; recommend when `winston`/`pino`/`bunyan` or log search is needed
- [optional] Profiling - continuous CPU profiling (Node.js only; not available on Bun or Deno); not available with OTLP path
- [optional] AI Monitoring - OpenAI, Anthropic, LangChain, Vercel AI SDK; recommend when AI/LLM calls detected
- [optional] Crons - detect missed or failed scheduled jobs; recommend when node-cron, Bull, or Agenda is detected
- [optional] Metrics - custom counters, gauges, distributions; recommend when custom KPIs needed
- [optional] Runtime Metrics - automatic collection of memory, CPU, and event loop metrics; `nodeRuntimeMetricsIntegration()` (Node.js) / `bunRuntimeMetricsIntegration()` (Bun)
...
|---------|-------------|-------|
| Error Monitoring | [recommended] Full | Same API as Node |
| Tracing | [recommended] Via `@sentry/node` OTel | Most auto-instrumentations work |
| Logging | [recommended] Full | `enableLogs: true` + `Sentry.logger.*` |
| Profiling | No Not available | `@sentry/profiling-node` uses native addons incompatible with Bun |
| Metrics | [recommended] Full | `Sentry.metrics.*` |
| Runtime Metrics | [recommended] Full | `bunRuntimeMetricsIntegration()` - memory, CPU, event loop (no event loop delay percentiles) |
| Crons | [recommended] Full | `Sentry.withMonitor()` |
| AI Monitoring | [recommended] Full | OpenAI, Anthropic integrations work |
...
|---------|-------------|-------|
| Error Monitoring | [recommended] Full | Unhandled exceptions + `captureException` |
| Tracing | [recommended] Custom OTel | Automatic spans for `Deno.serve()` and `fetch` |
| Logging | [recommended] Full | `enableLogs: true` + `Sentry.logger.*` |
| Profiling | No Not available | No profiling addon for Deno |
| Metrics | [recommended] Full | `Sentry.metrics.*` |
| Runtime Metrics | No Not available | No runtime metrics integration for Deno |
| Crons | [recommended] Full | `denoCronIntegration()` + `Sentry.withMonitor()` |
| AI Monitoring | [recommended] Partial | Vercel AI SDK integration works; OpenAI/Anthropic via `npm:` |
...
|--------|--------------------------|----------------------------------|
| OTel bundled | [recommended] Yes - built-in TracerProvider | No No - uses your existing provider |
| Tracing control | `tracesSampleRate` in `Sentry.init()` | OTel SDK controls sampling |
| Auto-instrumentation | [recommended] Built-in (HTTP, DB, etc.) | No You manage OTel instrumentations |
| Profiling | [recommended] Available | No Not compatible |
| Error <-> trace linking | [recommended] Automatic | [recommended] Automatic (via `otlpIntegration`) |
