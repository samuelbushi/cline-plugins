...
|---------|-----------|-------|
| Node.js >=18 | [recommended] | Full support via `@sentry/profiling-node` |
| Bun | No | Native addon not supported |
| Deno | No | Native addon not supported |
...
> Warning: Version pinning is required. `@sentry/profiling-node` must exactly match your `@sentry/node` version. Mismatched versions cause silent failures or startup crashes.
...
> No FreeBSD, 32-bit systems, Bun, and Deno are not supported.  
