...
- [recommended] Error Monitoring - always; auto-captures unhandled errors on client and server
- [recommended] Tracing - SvelteKit has both client-side navigation spans and server-side request spans; always recommend
- [recommended] Session Replay - recommended for user-facing SvelteKit apps (client-side only)

Optional (enhanced observability):
- [optional] Logging - structured logs via `Sentry.logger.*`; recommend when app uses server-side logging or needs log-to-trace correlation
...
| Tracing | `references/tracing.md` | API calls / distributed tracing needed |
| Session Replay | `references/session-replay.md` | User-facing app |
| Logging | `references/logging.md` | Structured logs / log-to-trace correlation |
...
|------|---------|--------|--------|
| `src/instrumentation.server.ts` | Server `Sentry.init()` - runs once at startup | [recommended] Required | No |
| `src/hooks.client.ts` | Client `Sentry.init()` + `handleError` | [recommended] Required | [recommended] Required |
| `src/hooks.server.ts` | `handleError` + `sentryHandle()` (no init) | [recommended] Required | [recommended] Init goes here |
| `svelte.config.js` | Enable `experimental.instrumentation.server` | [recommended] Required | No |
| `vite.config.ts` | `sentrySvelteKit()` plugin for source maps | [recommended] Recommended | [recommended] Recommended |
| `.env` | `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, `SENTRY_PROJECT` | [recommended] For source maps | [recommended] For source maps |
...
| Adapter | Support |
|---------|---------|
| `@sveltejs/adapter-auto` / adapter-vercel (Node) | [recommended] Full |
| `@sveltejs/adapter-node` | [recommended] Full |
| `@sveltejs/adapter-cloudflare` | Warning: Partial - requires extra setup |
| Vercel Edge Runtime | No Not supported |
