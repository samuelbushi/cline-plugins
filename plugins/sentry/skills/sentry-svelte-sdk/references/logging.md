...
| Server-side logging | No N/A | [recommended] Full - `Sentry.logger.*` in any server code |
| Isolation scope per request | No N/A | [recommended] Set in `hooks.server.ts` for per-request context |
