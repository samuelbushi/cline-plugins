...
```csharp
// No These lose exception details
activity.RecordException(ex);
...
```csharp
// No Raw URL - creates unbounded unique groups, defeats sampling
SentrySdk.StartTransaction("/users/12345/orders/9876", "http.server");

// [recommended] Parameterized - clean grouping, correct dynamic sampling
...
|---|---|---|
| `Route` | Low [recommended] | Parameterized route templates (e.g., `GET /users/{id}`) |
| `Custom` | Low [recommended] | User-defined names (background jobs, tasks) |
| `View` | Low [recommended] | Controller / view class names |
| `Component` | Medium | Function / component names |
| `Url` | High No | Raw URLs - avoid for dynamic sampling |
| `Task` | Low [recommended] | Background task names |
...
> Warning: Unit consistency: `("latency", 60, Second)` and `("latency", 3, Minute)` are stored as separate measurements, not aggregated. Always use the same unit per measurement name.
