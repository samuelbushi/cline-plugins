...
| .NET 8+ on Linux | EventPipe CPU sampling | `Sentry.Profiling` Warning: see Linux note |
...
| Blazor WebAssembly | No Not supported | - |
| Native AOT (non-iOS) | No Not supported | - |
...
```
TracesSampleRate x ProfilesSampleRate = net profiling rate
...
```csharp
// No Problem: profiler may not be ready when first transaction starts
SentrySdk.Init(options => {
...
```csharp
// [recommended] Fix: provide a timeout to block until profiler is ready
SentrySdk.Init(options => {
