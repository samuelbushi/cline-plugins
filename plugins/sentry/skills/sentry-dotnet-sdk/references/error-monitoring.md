...
|-----------|-----------|-----------|
| Unhandled exceptions (all platforms) | [recommended] Yes | `AppDomain.CurrentDomain.UnhandledException` |
| Unobserved Task exceptions | [recommended] Yes | `TaskScheduler.UnobservedTaskException` |
| ASP.NET Core request errors | [recommended] Yes | Sentry middleware |
| WPF Dispatcher unhandled exceptions | [recommended] Yes | `Application.DispatcherUnhandledException` (with hook) |
| MAUI unhandled exceptions | [recommended] Yes | Platform-specific native integrations |
| WinForms exceptions | [recommended] Yes | Requires `SetUnhandledExceptionMode(ThrowException)` |
| Caught + swallowed `try/catch` | No No | Must call `SentrySdk.CaptureException()` manually |
| Graceful error returns | No No | Must call `SentrySdk.CaptureException()` manually |
...
```csharp
// [recommended] Automatically captured - unhandled, bubbles up
throw new Exception("Unhandled");

// [recommended] Automatically captured - re-thrown
try
{
...
// No NOT captured - swallowed by graceful return
try
{
    await DoSomethingAsync();
}
...
|---------|------------|---------|---------|
| Tags | [recommended] Yes | [recommended] Yes | Filtering, grouping, alerting |
| Contexts | No No | No No | Structured debug info (nested objects) |
| Extra (deprecated) | No No | No No | Prefer `Contexts` instead |
| User | [recommended] Partially | [recommended] Yes | User attribution and filtering |
...
|----------|--------------|---------|
| Unhandled exception (all frameworks) | [recommended] Yes | `AppDomain.UnhandledException` integration |
| Unobserved Task exception | [recommended] Yes | `TaskScheduler.UnobservedTaskException` integration |
| ASP.NET Core request error | [recommended] Yes | Sentry middleware |
| WPF `DispatcherUnhandledException` | [recommended] Yes | Hook in `App()` constructor |
| MAUI unhandled exception | [recommended] Yes | Platform-specific native integrations |
| WinForms unhandled exception | [recommended] Yes | Requires `SetUnhandledExceptionMode(ThrowException)` |
| `try/catch` with graceful return | No No | `SentrySdk.CaptureException(ex)` before return |
| `try/catch` with re-throw | [recommended] Yes | Bubbles to unhandled exception handler |
| Background thread exception | [recommended] Yes | `IsGlobalModeEnabled = true` for desktop apps |
