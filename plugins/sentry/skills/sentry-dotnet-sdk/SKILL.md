...
- [recommended] Error Monitoring - always; captures unhandled exceptions, structured captures, scope enrichment
- [recommended] Tracing - always for ASP.NET Core and hosted apps; auto-instruments HTTP requests and EF Core queries
- [recommended] Logging - recommended for all apps; routes ILogger / Serilog / NLog entries to Sentry as breadcrumbs and events

Optional (enhanced observability):
- [optional] Profiling - CPU profiling; recommend for performance-critical services running on .NET 6+
- [optional] Metrics - counters, gauges, distributions linked to traces; recommend for apps that need custom business metrics
- [optional] Crons - detect missed/failed scheduled jobs; recommend when Hangfire, Quartz.NET, or scheduled endpoints detected
...
> Warning: Critical: Initialize in the constructor, NOT in `OnStartup()`. The constructor fires earlier, catching more failure modes.
...
| Option | Type | Default | Notes |
|--------|------|---------|-------|
| `IncludeTextInBreadcrumbs` | `bool` | `false` | Text from `Button`, `Label`, `Entry` elements. Warning: PII risk. |
| `IncludeTitleInBreadcrumbs` | `bool` | `false` | Titles from `Window`, `Page` elements. Warning: PII risk. |
| `IncludeBackgroundingStateInBreadcrumbs` | `bool` | `false` | `Window.Backgrounding` event state. Warning: PII risk. |
