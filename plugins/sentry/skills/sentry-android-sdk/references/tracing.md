...
|---------|-------------------|----------------|-----------------|---------------|
| Activity `ui.load` + TTID | [recommended] | None | No | No |
| TTFD | No | Manifest + `reportFullyDisplayed()` | 1 call | No |
| Fragment spans | No | `addIntegration()` | No | No |
| OkHttp spans | No | Add to builder | Yes | Optional |
| OkHttp sub-spans (DNS/TLS) | No | Add `SentryOkHttpEventListener` | Yes | Optional |
| SQLite / Room | No | Wrap factory | Yes | Optional |
| Compose navigation | No | `.withSentryObservableEffect()` | Yes | Optional |
| File I/O | No | Use `Sentry*` wrappers | Yes | Optional |
| User interaction | No | Manifest flag | No | No |
| Slow/frozen frames | [recommended] | None (API 24+) | No | No |
