...
Transaction starts (sampled)
  |
  --- Profiler starts: Debug.startMethodTracingSampling(file, 3MB, ~9901 us)
...
```
All transactions
    -- x tracesSampleRate  -> Traced transactions
              -- x profilesSampleRate -> Profiled transactions
```
...
Android SDK constants:
  ...
  Sampling interval    ~ 9,901 us   (101 Hz)
```
...
| App start coverage | [recommended] `isProfilingStartOnAppStart = true` | No Profile starts with first transaction |
