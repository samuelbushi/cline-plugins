...
- [recommended] Error Monitoring - captures uncaught exceptions, ANRs, and native NDK crashes automatically
- [recommended] Tracing - auto-instruments Activity lifecycle, app start, HTTP requests, and database queries
- [recommended] Session Replay - records screen captures and user interactions for debugging (API 26+)

Optional (enhanced observability):
- [optional] Profiling - continuous UI profiling (recommended) or transaction-based sampling
- [optional] Logging - structured logs via `Sentry.logger()`, with optional Timber bridge
- [optional] User Feedback - collect user-submitted bug reports from inside the app
...
> Warning: Manifest meta-data is a convenient alternative but does not support the full option set. For complex configuration (session replay, profiling, hooks), use `SentryAndroid.init()`.
