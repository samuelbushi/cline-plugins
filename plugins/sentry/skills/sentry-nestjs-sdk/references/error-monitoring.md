...
|-----------|-----------|--------|
| Unhandled exceptions from controllers | [recommended] Yes | `SentryGlobalFilter` intercepts |
| `HttpException` (4xx errors) | No No | `isExpectedError()` skips them |
| `HttpException` subclasses (`BadRequestException`, etc.) | No No | Duck-typed as expected |
| Caught + swallowed in service `try/catch` | No No | Never reaches the filter |
| Re-thrown from `try/catch` | [recommended] Yes | Reaches filter as unhandled |
...
```typescript
// [recommended] Automatically captured - reaches SentryGlobalFilter
throw new Error("Unhandled database error");

// [recommended] Automatically captured - re-thrown reaches filter
try {
  await db.query(sql);
} catch (err) {
...
// No NOT captured - swallowed before reaching filter
try {
  await db.query(sql);
} catch (err) {
...
// [recommended] Manually captured before graceful return
try {
  await db.query(sql);
} catch (err) {
...
|---------|------------|---------|
| Tags | [recommended] Yes | Filtering, grouping, alerting |
| Context | No No | Structured debug info (nested objects) |
| User | [recommended] Partially | User attribution and filtering |
...
|----------|---------------|---------|
| Unhandled controller exception | [recommended] Yes | `SentryGlobalFilter` intercepts |
| `HttpException` (4xx, 5xx) | No No | Expected by design; capture manually if needed |
| `try/catch` with graceful return | No No | `Sentry.captureException()` before return |
| `try/catch` with re-throw | [recommended] Yes | Reaches filter as unhandled |
| GraphQL resolver error | [recommended] Yes | `SentryGlobalFilter` captures + re-throws |
| RPC microservice error | Warning: Partial | Use dedicated `RpcExceptionFilter` |
| Background job (`@Cron`, `@OnEvent`) | No No | Wrap with `withIsolationScope()` + manual capture |
| WebSocket gateway error | No No | Catch manually in gateway methods |
| Caught + swallowed error | No No | Always call `captureException` before swallowing |
