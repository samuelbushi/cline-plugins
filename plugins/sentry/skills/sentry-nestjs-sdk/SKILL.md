...
- [recommended] Error Monitoring - captures unhandled exceptions across HTTP, GraphQL, RPC, and WebSocket contexts
- [recommended] Tracing - auto-instruments middleware, guards, pipes, interceptors, filters, and route handlers

Recommend when detected:

- [recommended] Profiling - production apps where CPU performance matters (`@sentry/profiling-node`)
- [recommended] Logging - structured Sentry Logs + optional console capture
- [recommended] Crons - `@nestjs/schedule`, Bull, or BullMQ detected
- [recommended] Metrics - business KPIs or SLO tracking
- [recommended] AI Monitoring - OpenAI/Anthropic/LangChain/etc. detected (auto-instrumented, zero config)
...
| Tracing          | Always - NestJS lifecycle is auto-instrumented | `references/tracing.md`          |
| Profiling        | Production + CPU-sensitive workloads               | `references/profiling.md`        |
| Logging          | Always; enhanced for structured log aggregation    | `references/logging.md`          |
| Metrics          | Custom business KPIs or SLO tracking               | `references/metrics.md`          |
| Crons            | `@nestjs/schedule`, Bull, or BullMQ detected       | `references/crons.md`            |
...
> Warning: Do NOT install `@sentry/node` alongside `@sentry/nestjs` - `@sentry/nestjs` re-exports everything from `@sentry/node`. Installing both causes duplicate registration.
...
> Warning: Do NOT register `SentryModule.forRoot()` twice. If Phase 1 found it already imported in a shared library module (e.g. a `SentryProxyModule` or `AnalyticsModule`), do not add it again in `AppModule`. Duplicate registration causes every span to be intercepted twice, bloating trace data.

> Warning: Two entrypoints, different imports:
>
...
| Tracing          | `references/tracing.md`          | Always (NestJS routes are auto-traced) |
| Profiling        | `references/profiling.md`        | CPU-intensive production apps          |
| Logging          | `references/logging.md`          | Structured log aggregation needed      |
| Metrics          | `references/metrics.md`          | Custom KPIs / SLO tracking             |
| Crons            | `references/crons.md`            | Scheduled jobs or task queues          |
| AI Monitoring    | `references/ai-monitoring.md`    | OpenAI/Anthropic/LangChain detected    |
