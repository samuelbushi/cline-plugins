...
- [recommended] Error Monitoring - always; captures unhandled exceptions in fetch, scheduled, queue, email, and Durable Object handlers
- [recommended] Tracing - automatic HTTP request spans, outbound fetch tracing, D1 query spans

Optional (enhanced observability):
- [optional] Logging - structured logs via `Sentry.logger.*`; recommend when log search is needed
- [optional] Crons - detect missed/failed scheduled jobs; recommend when cron triggers are configured
- [optional] D1 Instrumentation - automatic query spans and breadcrumbs; recommend when D1 is bound
- [optional] Durable Objects - automatic error capture and spans for DO methods; recommend when DOs are configured
- [optional] Workflows - automatic span creation for workflow steps; recommend when Workflows are configured
- [optional] AI Monitoring - Vercel AI SDK, OpenAI, Anthropic, LangChain; recommend when AI libraries detected
