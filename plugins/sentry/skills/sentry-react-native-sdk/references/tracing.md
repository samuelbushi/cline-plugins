...
```typescript
// No Eager import - executes at bundle parse time
import { HeavyModule } from './heavy-module';

// [recommended] Lazy import - deferred until actually needed
const loadHeavy = () => import('./heavy-module');

// No Synchronous AsyncStorage read at startup
const theme = await AsyncStorage.getItem('theme'); // blocks JS thread

// [recommended] Use a synchronous-safe default, hydrate later
...
Tab navigators preload screens, so auto-instrumentation only creates a transaction for the initial tab visit. For subsequent tab switches, use the `TimeToInitialDisplay` and `TimeToFullDisplay` components explicitly (see section5).
...
| Scenario | Captured? |
|----------|-----------|
| `profile/[id]` with `{ id: '123' }` | [recommended] `route.params.id = "123"` |
| `posts/[...slug]` with `{ slug: ['tech', 'rn'] }` | [recommended] `route.params.slug = "tech/rn"` |
| `StaticScreen` with `{ utm_source: 'email' }` | No No dynamic segment - nothing captured |
| Any route with `sendDefaultPii: false` | No Opted out - nothing captured |
...
```typescript
// No Synchronous heavy computation on the JS thread - causes stalls
const result = items.reduce((acc, item) => {
  return acc + expensiveComputation(item); // blocks JS thread
}, 0);

// [recommended] Offload to InteractionManager or requestAnimationFrame
...
// [recommended] Or better - move to a native module / worklet (Reanimated)
```
...
|---|---|---|
| App cold start tracking | No | [recommended] `measurements.app_start_cold` |
| App warm start tracking | No | [recommended] `measurements.app_start_warm` |
| Slow frames (Mobile Vital) | No | [recommended] Auto (requires `reactNativeTracingIntegration`) |
| Frozen frames (Mobile Vital) | No | [recommended] Auto (requires `reactNativeTracingIntegration`) |
| JS event loop stall tracking | No | [recommended] Auto (3 metrics: count, longest, total) |
| Time to Initial Display (TTID) | No | [recommended] `enableTimeToInitialDisplay: true` |
| Time to Full Display (TTFD) | No | [recommended] `<Sentry.TimeToFullDisplay record={...} />` |
| Touch interaction tracing | No | [recommended] `enableUserInteractionTracing: true` |
| Gesture tracing (RNGH v2) | No | [recommended] `sentryTraceGesture()` |
| Hermes JS profiling | No | [recommended] `profilesSampleRate` + `hermesProfilingIntegration` |
| Native platform profiling | No | [recommended] `platformProfilers: true` |
| Navigation transactions | [recommended] (SPA routers) | [recommended] React Navigation - Expo Router - RNN |
| Network span tracing | [recommended] | [recommended] fetch + XHR auto-instrumented |
| Distributed tracing | [recommended] | [recommended] `tracePropagationTargets` |
| Web Vitals (LCP, FID, CLS) | [recommended] | No (replaced by Mobile Vitals) |
