...
- Competitor detected (`appsignal`, `honeybadger`, `bugsnag`, `rollbar`, `airbrake`) -> load `references/migration.md` first; delete the competitor initializer as part of migration
...
- Puma detected -> queue time capture is automatic (v6.4.0+), but the reverse proxy must set `X-Request-Start` header; see `references/tracing.md` -> "Request Queue Time"
...
| Profiling | Warning: Beta - performance profiling requested; requires `stackprof` or `vernier` gem; skip if OTel tracing detected (requires `traces_sample_rate`, incompatible with OTLP) |
