...
|------|----------------|-----------|
| Client-side framework errors | [recommended] Yes | `HydratedRouter onError={Sentry.sentryOnError}` |
| Unhandled client exceptions | [recommended] Yes | Browser global handlers from SDK init |
| Server request/render errors | [recommended] Yes | `createSentryHandleRequest` + `createSentryHandleError` |
| Custom handled server errors | No Not always | Call `Sentry.captureException` manually |
| Custom loader/action failures | Warning: Depends | Use `wrapServerLoader` / `wrapServerAction` or manual capture |
