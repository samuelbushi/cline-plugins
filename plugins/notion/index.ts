...
const plugin: AgentPlugin = {
	...
	setup(api) {
		...
		api.registerCommand({
			...
			handler: (input) =>
				promptFor(
					"Use the Notion MCP server to create a Notion page. Resolve the parent page or database first, ask if ambiguous, avoid overwriting existing pages, and confirm the created page link.",
					input,
				),
		})
		...
		api.registerCommand({
			...
			handler: (input) =>
				promptFor(
					"Use the Notion MCP server to create a task in an appropriate Notion tasks database. Resolve the database first, map available properties such as status, due date, owner, and project, ask before guessing required fields, and confirm the task link.",
					input,
				),
		})
		...
		api.registerCommand({
			...
			handler: (input) =>
				promptFor(
					"Use the Notion MCP server to insert a row into a Notion database. Resolve the database, map natural-language key=value properties to actual schema properties, validate required fields, and confirm the created row.",
					input,
				),
		})
