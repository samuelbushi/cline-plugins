...
description: Prepares meeting materials by gathering context from Notion, enriching with Cline research, and creating both an internal pre-read and external agenda saved to Notion. Helps you arrive prepared with comprehensive background and structured meeting docs.
---

# Meeting Intelligence

Prepares you for meetings by gathering context from Notion, enriching it with Cline research, and creating comprehensive meeting materials. Generates both an internal pre-read for attendees and an external-facing agenda for the meeting itself.

## Write Safety

Draft the meeting documents, destination pages/databases, linked resources, and any page updates first. Show the proposed Notion writes to the user and ask for approval before creating pages, adding links, writing agendas, updating project pages, or sharing any Notion link externally. External agendas must exclude private/internal links by default.

## Quick Start

When asked to prep for a meeting:

1. Gather Notion context: Use the available Notion MCP search tool to find related pages
2. Fetch details: Use the available Notion MCP fetch tool to read relevant content
3. Enrich with research: Use Cline's knowledge to add context, industry insights, or best practices
4. Create internal pre-read: Use the available Notion MCP page-write tool for background context document (for attendees)
5. Create external agenda: Use the available Notion MCP page-write tool for meeting agenda (shared with all participants)
...
```
Use the available Notion MCP search tool to find:
...
1. Fetch with the available Notion MCP fetch tool
...
### Step 4: Enrich with Cline research
...
```
Use the available Notion MCP page-write tool for internal doc:
...
- Current Status:
  ...
  - Key metrics or data
- Context & Insights (from Cline research):
...
```
Use the available Notion MCP page-write tool for meeting doc:
...
- Related Resources:
  ...
  - Do not include internal pre-read or private workspace links unless the user explicitly approves sharing them externally
...
3. Cross-reference:
   - Agenda mentions pre-read only for internal-only meetings, or when the user explicitly approves sharing that link externally
...
Beyond Notion content, add value through Cline's capabilities:
...
Note: Use general knowledge and analytical capabilities. Don't fabricate specific facts. Clearly distinguish Notion facts from Cline insights.
...
2. Distinguish sources: Label what's from Notion vs. Cline research
...
6. Enrich thoughtfully: Cline research should add real value, not fluff
