...
description: Turns product or tech specs into concrete Notion tasks that Cline can implement. Breaks down spec pages into detailed implementation plans with clear tasks, acceptance criteria, and progress tracking to guide development from requirements to completion.
---
...
Draft the implementation plan, target task database, task list, property mapping, status changes, and progress updates first. Show the proposed Notion writes to the user and ask for approval before creating plan pages, creating tasks, changing statuses, updating specs, adding comments, or modifying database schema. If multiple specs or task databases match, ask the user to choose.

## Quick Start

When asked to implement a specification:

1. Find spec: Use the available Notion MCP search tool to locate specification page
2. Fetch spec: Use the available Notion MCP fetch tool to read specification content
3. Extract requirements: Parse and structure requirements from spec
4. Create plan: Use the available Notion MCP page-write tool for implementation plan
5. Find task database: Use the available Notion MCP search tool to locate tasks database
6. Create tasks: Use the available Notion MCP page-write tool for individual tasks in task database
7. Track progress: Use the available Notion MCP page-update tool to log progress and update status
...
```
1. Search for spec:
   - Use the available Notion MCP search tool with spec name or topic
...
```
1. Fetch spec page:
   - Use the available Notion MCP fetch tool with spec URL/ID from search results
...
```
Use the available Notion MCP page-write tool:
...
1. Search for task database:
   - Use the available Notion MCP search tool to find "Tasks" or "Task Management" database
...
2. Fetch database schema:
   - Use the available Notion MCP fetch tool with database URL/ID
...
1. Create task in task database using the available Notion MCP page-write tool
...
"Can't find spec": Use the available Notion MCP search tool with spec name/topic, try broader terms, or ask user for URL
