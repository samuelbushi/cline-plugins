...
Draft the research output, destination, source citations, and any page/database changes first. Show the proposed Notion writes to the user and ask for approval before creating pages, updating existing pages, inserting database rows, or sharing workspace links outside the current conversation. If the user only wants an answer, summarize in chat without writing to Notion.

## Quick Start

When asked to research and document a topic:

1. Search for relevant content: Use the available Notion MCP search tool to find pages
2. Fetch detailed information: Use the available Notion MCP fetch tool to read full page content
3. Synthesize findings: Analyze and combine information from multiple sources
4. Create structured output: Use the available Notion MCP page-write tool to write documentation
...
```
Use the available Notion MCP search tool with the research topic
...
```
Use the available Notion MCP fetch tool for each relevant page URL
