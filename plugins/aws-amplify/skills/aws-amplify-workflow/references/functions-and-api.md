...
Set secrets via CLI with interactive input: `npx ampx sandbox secret set MY_API_KEY`. Do not pass secret values through `echo`, command arguments, shell history, logs, or committed files. If the user explicitly approves reading from a secure source, pipe directly to `npx ampx sandbox secret set MY_API_KEY --from-stdin`.
