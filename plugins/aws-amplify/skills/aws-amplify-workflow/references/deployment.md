...
Prefer connecting the repository through the Amplify console or the GitHub App flow so tokens are not passed through shell command arguments. If the user explicitly chooses CLI setup, ask for approval, use a short-lived token from a secure source, avoid printing it, and do not store it in files or shell history.

```bash
REPO="github.com/<user>/<repo>"
read -rsp "GitHub access token: " GITHUB_ACCESS_TOKEN
echo
