...
Use this only for starter/demo data or data the user explicitly confirms is intentionally public and non-sensitive. For user or business data, prefer `userPool` with `allow.owner()` or `allow.authenticated()` instead of public API-key access.
