...
Run the commands in this file only after explicit user approval. They can clone repositories, install packages, initialize git repositories, scaffold Amplify backends, create mobile projects, or run native dependency installers.
...
Ask before running it, and show the target project directory first.
...
Ask before running React Native, Expo, Amplify, npm install, or pod install commands, and confirm the target directory first.
