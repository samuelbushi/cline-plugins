...
description: 'Build and deploy full-stack web and mobile apps with AWS Amplify Gen2
  ...
  or direct Bedrock work without Amplify AI Kit. Use a dedicated serverless or Bedrock
...
## Cline Safety and AWS Operations

This plugin registers the shared `aws-mcp` documentation/context server through `uvx mcp-proxy-for-aws@1.6.0` with `--skip-auth`. Use it for current AWS and Amplify guidance when available. Live AWS work should still go through explicit user-approved local commands such as `npx ampx` or the AWS CLI, using the user's configured credential chain.

Prefer read-only documentation or local project inspection first, and confirm the AWS account, region, profile, app ID, branch, and expected resource changes before any live AWS operation.

Ask for explicit approval before installing packages, scaffolding projects, editing infrastructure files, running `npm create amplify`, running `npx ampx`, starting sandboxes, setting secrets, deploying, creating Amplify apps or branches, changing IAM, invoking live AWS APIs, or running commands that can incur cost.

Never paste secret values into chat, command arguments, shell history, logs, or committed files. Use `npx ampx sandbox secret set NAME` interactively for sandbox secrets, `npx ampx secret set NAME --branch <branch> --app-id <app-id>` interactively for branch secrets, or pipe from an approved secure source with `--from-stdin`.
...
- Data authorization: ask before using public or guest access. Use
  `allow.publicApiKey()` only for starter/demo data or data the user explicitly
  confirms is intentionally public and non-sensitive. Prefer `allow.owner()` or
  `allow.authenticated()` for user or business data.
