    ...
    try:
        ...
        with console.status(f"[bold blue]Creating assistant '{name}'...[/bold blue]"):
            ...
            assistant = pc.assistant.create_assistant(
                ...
                region=region,
                timeout=timeout,
                metadata={"agentic-ide-source":"cline-plugin"}
            )

        # Success message
        console.print(f"\n[bold green]OK Assistant '{name}' created successfully![/bold green]\n")
...
1. Use the pinecone-assistant skill to upload files from [path] to {name}
2. Use the pinecone-assistant skill to ask {name} about [your question]
3. Use the pinecone-assistant skill to search {name} for context about [topic]"""
