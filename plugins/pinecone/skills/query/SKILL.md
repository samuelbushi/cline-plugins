...
Required:
1. OK Pinecone MCP server must be configured - Check if MCP tools are available
2. OK PINECONE_API_KEY environment variable must be set - Get a free API key at https://app.pinecone.io/?sessionType=signup
3. OK Index must be an integrated index - Uses Pinecone embedding models (e.g., multilingual-e5-large, llama-text-embed-v2, pinecone-sparse-english-v0)

### When NOT to use this skill

Use the CLI skill instead if:
- BAD: Your index is a standard index (no integrated embedding model)
- BAD: You need to query with custom vector values (not text)
- BAD: You need advanced vector operations (fetch by ID, list vectors, bulk operations)
- BAD: Your index uses third-party embedding models (OpenAI, HuggingFace, Cohere)
...
IMPORTANT At the moment, the /query command can only be used with integrated indexes, which use Pinecone-managed embedding models to embed and search for data.
...
- Ask the user to clarify missing information when needed.
