...
- Gemini: pass `config=types.EmbedContentConfig(output_dimensionality=768)`. The model uses Matryoshka representations, so smaller dimensions are valid truncations of the native output. 768 is usually a 4x payload reduction vs. the native 3072 and costs very little quality.
- OpenAI `text-embedding-3-*`: pass `dimensions=768` (or similar) to `embeddings.create`.
- Pinecone-managed fixed-dim models: dimension is fixed; the only levers are `batch_size` (halve it to 25) and per-document body size.
