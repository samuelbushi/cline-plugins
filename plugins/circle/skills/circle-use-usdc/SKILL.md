...
For Arc-specific setup and configuration, see the `circle-use-arc` skill.
...
- Use `circle-bridge-stablecoin` skill (CCTP / Bridge Kit) for transferring USDC between chains. Bridge Kit handles approve, burn, attestation, and mint in a single `kit.bridge()` call.
- Use `circle-use-gateway` skill for unified USDC balance across chains with instant transfers (<500ms). Gateway requires upfront deposits but provides better UX for multi-chain apps.
- Stick with `circle-use-usdc` for single-chain USDC operations (balance checks, payments, approvals) or when you need low-level control over transfers.
