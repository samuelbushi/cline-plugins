---
name: circle-use-developer-controlled-wallets
...
- Trigger `circle-use-user-controlled-wallets` skill when end users should custody their own keys via social login, email OTP, or PIN authentication.
- Trigger `circle-use-modular-wallets` skill for passkey-based smart accounts with extensible module architecture (multisig, session keys, etc.).
