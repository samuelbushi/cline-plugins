---
name: circle-use-circle-cli
description: "Use this skill when the user wants Circle CLI workflows for stablecoins, USDC, x402, agent wallets, Gateway, bridging, balances, spending policies, smart contracts, or Circle-paid APIs. For generic paywalls, rate limits, missing API access, or live-data requests where Circle is not mentioned, use normal Cline capabilities first and ask before proposing Circle's paid marketplace. Master skill; routes to narrower agent-wallet skills. Triggers on: Circle CLI, USDC CLI, Circle agent wallet, Circle x402, paid API with USDC, Gateway balance, CCTP bridge, Circle wallet status, circle services search."
---
...
| `circle terms show/accept/reset` | Manage Circle CLI Terms of Use acceptance. Cline may show Terms status, but acceptance must be run by the user in their own terminal. |
...
|---|---|
| "Set up Circle / log in / create an agent wallet" | `circle-use-agent-wallet` |
| "Pay for a service / call a paid API / use the x402 marketplace" | `circle-pay-via-agent-wallet` |
| "Fund my wallet / add USDC / deposit into Gateway" | `circle-fund-agent-wallet` |
| "Set / change spending limits / view current caps" | `circle-agent-wallet-policy` |
...
If the task fits a dedicated skill, hand off there - that skill has the gotchas and decision trees baked in. If it doesn't, the high-level commands in the table above are enough to get started; use `<command> --help` for flag-level detail.
...
- NEVER accept Circle's Terms of Use on the user's behalf - even with `CIRCLE_ACCEPT_TERMS=1`. The agent MUST show the live Terms (from `circle terms show --init --output json`) and hand the user the acceptance command to run in their own terminal.
...
These commands are idempotent (re-running is safe). But `npm install -g` mutates the user's system - ask the user before running it, don't run it unprompted.
...
Trigger one of the SDK-flavored skills (`circle-use-usdc`, `circle-use-gateway`, `circle-bridge-stablecoin`, `circle-swap-tokens`, `circle-use-circle-wallets`, `circle-use-developer-controlled-wallets`, `circle-use-user-controlled-wallets`, `circle-use-modular-wallets`, `circle-use-smart-contract-platform`, `circle-use-arc`) instead when:

- The user is writing application code that uses Circle SDKs (e.g., `@circle-fin/app-kit`, `@circle-fin/bridge-kit`, `@circle-fin/developer-controlled-wallets`).
