...
For an overview of the Circle CLI's full capability set - bridging, smart contract execution, x402 payment, and more - see the `circle-use-circle-cli` master skill. This skill is the narrower funding flow.
...
This skill covers funding both pools. Explain the relevant tradeoff clearly, then pick the shortest time-to-result only after the user has chosen to fund the Circle wallet or Gateway balance.
...
`--open` renders the EIP-681 QR code on a local HTML page in the user's default browser. Use this by default. Terminal-rendered QR codes are frequently truncated or unscannable inside terminal-based agent UIs; the browser page renders the QR at full resolution and works on both desktop and mobile.
...
Once the deposit verifies, hand off to the `circle-pay-via-agent-wallet` skill for the actual payment - that skill owns the `circle services pay` flow and gotchas.
...
| `Wallet not deployed` | First tx on this chain - SCA needs deployment | Trigger deployment by performing any real transaction from this wallet on this chain - typically the deposit / transfer step in this skill, or a normal payment via `circle-pay-via-agent-wallet`. There is no dedicated "deploy" command today; consult `circle wallet --help` for current options. |
...
- ALWAYS prefer `--method crypto --open` over rendering QR codes in the terminal - terminal QR codes get truncated inside terminal-based agent UIs and become unscannable.
...
- Sizing formula: `amount = max(price x N + fee + slack, Gateway minimum)`, where N is the workflow's expected call count. Typically reserve ~50% of vanilla as a soft headroom floor. Gateway enforces a server-side minimum deposit; sizing below it returns an error, so floor your suggestion to that minimum. Run `circle gateway deposit --help` for current bounds.
- Surface to the user before depositing when the required amount is materially larger than the workflow's total cost (cheap-endpoint case: $0.0024/call x 5 = $0.012, but the Gateway minimum sets the floor) or above the user's stated `--max-amount` cap. Don't silently deposit ~100x the task cost.
