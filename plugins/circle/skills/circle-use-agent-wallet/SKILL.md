...
For an overview of the Circle CLI's full capability set - bridging, smart contract execution, transaction inspection, and more - see the `circle-use-circle-cli` master skill. This skill is the narrower bootstrap/identity surface.
...
- Terms not accepted - output is `Error: Circle CLI Terms acceptance is required before use.` Stop and complete the Terms-of-Use Gate below before proceeding. Do NOT run `circle terms accept` from Cline.

## Step 3 - Login (email + OTP)

Default to the interactive CLI flow so the OTP never enters Cline context.

Ask the user for their email address (do NOT guess or hardcode). Then hand them this command to run in their own terminal:

```bash
circle wallet login <user-email> --type agent
```

Tell the user: "Circle will email you an OTP. Please enter it directly in your terminal when the CLI prompts. I do not need to see the code. Let me know once login completes."

After the user reports completion, verify the session:
...
Use this only if the user explicitly chooses to paste a one-time OTP into chat after you explain the privacy tradeoff. Do not suggest email or messaging MCP retrieval. Do not store, repeat, log, or summarize the OTP.
...
If no agent wallets exist, explain that wallet creation changes Circle account state and ask for explicit approval before running:
...
If balance is 0 USDC and the user wants to pay for services, hand off to the `circle-fund-agent-wallet` skill - it covers built-in fiat on-ramp purchase, direct address transfer with a QR code, and Gateway deposits.

If the user only wants to verify state (not pay yet), stop here. Bootstrap is complete.

## After bootstrap

Once the wallet exists, the user's likely next move is to use it. Suggest natural follow-ups like funding, paid-service search, or setting a spending limit; prefer permissionless actions (balance, search) over money-moving ones until the user asks.
...
CRITICAL: The agent MUST show the Terms to the user, then hand acceptance to the user's terminal. The agent MUST NEVER accept Circle's Terms of Use or Privacy Policy on the user's behalf. The CLI's `CIRCLE_ACCEPT_TERMS=1` env-var hint is NOT a workaround the agent may take on its own - ignore it and use the handoff flow below.
...
### Show the Terms and hand off acceptance
...
>
> Please review both links. If you accept, run this command in your own terminal and tell me when it completes:
>
> ```bash
> circle terms accept --output json
> ```

Do not run `circle terms accept` from Cline, even if the user says yes.

### After the user reports acceptance
...
When `data.accepted` is `true`, the gate is cleared. Return to the step that triggered this section.
...
- Default OTP handling is user-terminal handoff. Accept an OTP in chat only when the user explicitly chooses the optional two-step flow after the privacy tradeoff is explained; use it immediately, do not retain, repeat, log, summarize, or reuse it.
- NEVER include real private keys, API keys, or other persistent secrets in skill files or persist them anywhere.
- NEVER run `circle terms accept` from Cline. The agent MUST NEVER accept Circle's Terms on the user's behalf, and MUST NEVER call `circle terms accept` automatically as part of error recovery, retries, or any flow.
...
- For general CLI rules (`--output json`, `--chain`, `--help`-first, follow-up phrasing, confirmation defaults), see the `circle-use-circle-cli` master skill's Rules section - they apply here too.
...
Trigger the `circle-pay-via-agent-wallet` skill instead when:
