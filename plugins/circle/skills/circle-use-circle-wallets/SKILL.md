...
|--------------------------------------------------|-----------------------------|------------------------------------|
| Payment backend, programmatic payouts, high TPS  | Developer-controlled + EOA  | `circle-use-developer-controlled-wallets` |
| Consumer app with Google/Apple login, gasless UX | User-controlled + SCA on L2 | `circle-use-user-controlled-wallets`      |
| DeFi app with biometric auth, custom modules     | Modular Wallet on L2        | `circle-use-modular-wallets`              |
| NFT marketplace on Ethereum L1                   | User-controlled + EOA       | `circle-use-user-controlled-wallets`      |
| AI agent, autonomous multi-chain transactions    | Developer-controlled + EOA  | `circle-use-developer-controlled-wallets` |

## Implementation Patterns

Once a wallet type has been determined, TRIGGER the corresponding skill:

- Developer-controlled -> `circle-use-developer-controlled-wallets` skill
- User-controlled -> `circle-use-user-controlled-wallets` skill
- Modular (Passkey) -> `circle-use-modular-wallets` skill
...
- ALWAYS delegate to the specific wallet skill (`circle-use-developer-controlled-wallets`, `circle-use-user-controlled-wallets`, or `circle-use-modular-wallets`) for implementation.
