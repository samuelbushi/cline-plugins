...
- USDC only -- Unified Balance Kit works exclusively with USDC. For other tokens, use the `circle-swap-tokens` skill to convert first.
...
Trigger the `circle-use-gateway` skill instead when:
...
Trigger the `circle-bridge-stablecoin` skill instead when:
...
Trigger the `circle-swap-tokens` skill instead when:
