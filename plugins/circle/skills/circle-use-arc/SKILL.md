...
Use CCTP to bridge USDC from other chains. Arc's CCTP domain is `26`. See the `circle-bridge-stablecoin` skill for the complete bridging workflow.
...
|---------|-------|--------------|
| Wallets (overview) | `circle-use-circle-wallets` | Compare wallet types and choose the right one for your app |
| Modular Wallets | `circle-use-modular-wallets` | Passkey-authenticated smart accounts with gasless transactions and batch operations |
| User-Controlled Wallets | `circle-use-user-controlled-wallets` | Non-custodial wallets with social login, email OTP, and PIN authentication |
| Developer-Controlled Wallets | `circle-use-developer-controlled-wallets` | Custodial wallets your app manages on behalf of users |
| Smart Contract Platform | `circle-use-smart-contract-platform` | Deploy, interact with, and monitor smart contracts using audited templates or custom bytecode |
| CCTP Bridge | `circle-bridge-stablecoin` | Bridge USDC to and from Arc using Crosschain Transfer Protocol |
| Gateway | `circle-use-gateway` | Unified USDC balance across chains with instant crosschain transfers |
