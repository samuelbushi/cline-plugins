...
Aurora DSQL is a serverless, PostgreSQL-compatible distributed SQL database. This skill covers schema management, migrations, multi-tenant isolation, IAM auth, bulk data loading via `aurora-dsql-loader`, and direct query execution when the optional Aurora DSQL MCP server is configured.
...
This plugin registers the `aws-knowledge` MCP server for AWS documentation and service-limit lookup.

The Aurora DSQL database MCP server is optional and is not registered automatically. Add it manually only after the user selects a cluster endpoint, region, database user, AWS profile, and write-mode policy. If configured, the `aurora-dsql` MCP server provides these tools:
...
Fallback: If `aws-knowledge` is unavailable, use the defaults above and flag that limits should be verified against [DSQL documentation](https://docs.aws.amazon.com/aurora-dsql/latest/userguide/).

## Local Scripts And CLIs

This Cline plugin does not bundle cluster-management shell scripts. When users ask for cluster creation, deletion, psql access, or `aurora-dsql-loader` data loading, provide explicit AWS CLI, psql, or loader commands only after confirming the target account, region, cluster, and destructive/write intent. Prefer dry runs and read-only discovery before running local commands.

---

## Quick Start

If the optional `aurora-dsql` MCP server is not configured, provide design, migration, review, setup, and command-planning guidance without attempting database tool calls.

1. Explore: When MCP is configured, use `readonly_query` with `information_schema` to list tables. Use `get_schema` for table structure.
2. Query: When MCP is configured, use `readonly_query` for SELECT queries. MUST include `tenant_id` in WHERE for multi-tenant apps. MUST build SQL with `safe_query.build()`.
3. Schema changes: When MCP write mode is explicitly enabled by the user, use `transact` with one DDL per transaction. MUST batch DML under 3,000 rows. MUST use `CREATE INDEX ASYNC` in a separate call. Use `dsql_lint` to validate first.
...
- `aws-knowledge` returns no results: Use the default limits in the table above and note that limits should be verified against [DSQL documentation](https://docs.aws.amazon.com/aurora-dsql/latest/userguide/).
