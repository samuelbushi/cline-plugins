...
- SHOULD implement connection pooling - Reuse connections to minimize token overhead; respect 10,000 max per cluster (verify via `aws-knowledge`: `aurora dsql connection limits`)
- PREFER initial pool size 10-50 per instance - Generate fresh tokens in pool hooks (e.g., `BeforeConnect`) for 15-minute expiration (verify via `aws-knowledge`: `aurora dsql authentication token`)
...
- PREFER batches of 500-1,000 rows - Balance throughput and transaction limits (3,000 rows, 10 MiB, 5 minutes max - verify via `aws-knowledge`: `aurora dsql transaction limits`)
...
REQUIRED: Specify CACHE explicitly when creating sequences or identity columns. Supported values are 1 or >= 65536 (verify via `aws-knowledge`: `aurora dsql sequence cache`).
