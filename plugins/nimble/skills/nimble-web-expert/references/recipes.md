...
> [warning] CSS selectors may break on Amazon redesigns. Prefer the `amazon_pdp` agent when available.
```bash
...
> [warning] CSS selectors may break on Yelp redesigns. Prefer the `yelp_serp` agent when available.
```bash
nimble extract \
...
> [warning] CSS selectors may break on Target redesigns. If a Target agent becomes available, prefer it.
```bash
nimble extract \
