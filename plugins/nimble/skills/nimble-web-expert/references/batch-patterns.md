...
cat keywords.txt | xargs -P 8 -I{} sh -c \
  'nimble --transform "data.parsing" agent run --agent amazon_serp --params "{\"keyword\": \"{}\"}" > ".nimble/amazon-{}.json" && echo "ok {}"'
```
...
async def extract_one(url: str) -> dict:
    ...
    if proc.returncode != 0:
        print(f"  x {url}: {stderr.decode().strip()[:80]}")
