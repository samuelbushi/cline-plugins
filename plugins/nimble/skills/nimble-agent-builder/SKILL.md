...
Try the registered Nimble MCP server first:

```bash
Use the registered Cline MCP server named `nimble` if it is authorized.
```

- MCP connected -> proceed using Nimble MCP tools.

Use the CLI only if MCP is unavailable, the workflow needs unsupported flags or local batch scripts, or the user explicitly chooses shell-based workflows:
...
Neither available -> use the setup guidance in `references/nimble-playbook.md`. The install path depends on the host:
- Any Cline surface -> `cline plugin install nimble` (one command, registers the Nimble MCP server, OAuth handles auth).
- terminal-only agents -> `npm i -g @nimble-way/nimble-cli` + API key.

Plugin installed but registered MCP server not connected: if Nimble MCP tools are listed, verify with one read-only Nimble MCP agents-list probe before any work. An auth/not-connected error or an OAuth authorization URL means not connected -- surface the Cline MCP authorization guidance from `references/nimble-playbook.md` and stop. Never substitute WebFetch, WebSearch, curl, or any other tool.
...
Step 2 -- Spawn a background subtask (`a background subtask`):
...
- ask the user at every decision point in the foreground -- no exceptions. Always present the standard `ask the user` prompts shown in each step. Never skip them, never auto-advance without asking. Never present choices as plain numbered lists. Constraints: 2--4 options, header max 12 chars, label 1--5 words. Recommended option goes first with "(Recommended)". Note: background subtasks NEVER use ask the user -- all decisions are pre-made before launching the Task.
...
- Agent creation/update runs in background subtasks. Generation takes 1-3 minutes -- run the generate -> poll -> publish loop as a background subtask so the user isn't blocked. See [Delegation model](#delegation-model).

## Delegation model

The foreground conversation orchestrates and presents results. background subtasks handle
...
background subtasks -- long-running operations:

| Phase                                                                                            | background subtask | Foreground does        |
| ------------------------------------------------------------------------------------------------ | ---------- | ---------------------- |
...
Agent creation runs in a background subtask because generation takes 1-3 minutes (poll loop).
The background subtask uses CLI commands via Bash. If CLI is unavailable, it falls back to MCP
tools.

For multi-source workflows, launch background subtasks sequentially (one per source/phase). Gather reports, then present the combined plan.

### CLI commands for background subtasks

All background subtask prompts should include this block so the subagent knows the available commands:
...
MCP equivalents for Cline installs:
| CLI command | MCP tool |
|---|---|
| `nimble agent generate` | the Nimble MCP `nimble_agents_generate` tool |
| `nimble agent get-generation` | the Nimble MCP `nimble_agents_status` tool |
| `nimble agent publish` | the Nimble MCP `nimble_agents_publish` tool |
...
| Result                                                            | Route                                                                                                                                                                                                          |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Exact match                                                       | Show schema summary + `ask the user`: "Use this agent" (Recommended) / "Create new agent" -> Step 3                                                                                                          |
| Close match (same domain/type, missing fields or different scope) | Show schema gaps + `ask the user`: "Update this agent" (Recommended) / "Create new agent". Always recommend update -- it preserves existing extraction logic and is faster than generating from scratch. |
| 2+ plausible matches                                              | Show table + `ask the user` with top matches + "Update closest agent" (Recommended). Pick the agent with the most field overlap.                                                                            |
| 0 matches                                                         | Launch Discovery background subtask (Step 1D) -> results inform Step 3. This is the ONLY case where generating a new agent is appropriate.                                                                      |
...
1. Clarify -- `ask the user` to resolve critical unknowns (max 2 questions). Focus on: what site(s), what data fields, what output format.
2. Explore -- `nimble agent list --limit 100 --search "<domain>"` (CLI, once per domain). For unfamiliar domains, launch Discovery background subtasks (Step 1D).
...
4. Execute -- Step 2 for existing agents, Step 3 for generations (as background subtasks).

### Step 1D: Discovery (background subtask -- for unfamiliar domains)

Launch when `nimble agent list --limit 100 --search "<domain>"` returns 0 matches for the target domain and it needs exploration. Runs as `a background subtask`. The foreground tells the user: _"Exploring {domain} to understand available data..."_
...
Do NOT use ask the user. Do NOT use nimble_find_search_agent, nimble_run_search_agent, or nimble_url_extract.
...
2. If data gaps exist (e.g., missing ratings), asks the user via `ask the user` how to proceed.
...
2A-2. Always confirm before running via `ask the user`:
...
2B-2. Launch script generation background subtask: `a background subtask`.
...
Do NOT use ask the user. Do NOT use nimble_find_search_agent or nimble_run_search_agent.
...
2B-3. Present the generated script and confirm execution via `ask the user` (foreground):
...
Updates an existing agent (preferred) or creates a new one on Nimble's platform. Default to update when a close-match agent was found in Step 1 -- pass the existing agent name to the background subtask so it uses `nimble_agents_update_from_agent` instead of `nimble_agents_generate`. Only create a new agent when Step 1 returned 0 matches. This is NOT code/script generation -- it creates/modifies an extraction definition callable via Step 2A or 2B. ALWAYS runs as a background subtask as a bounded background subtask.
...
### 3-3. Launch background subtask

Set `refine_validate` to the user's choice. Launch `a background subtask` using the prompt template from `references/generate-update-and-publish.md` (includes MCP tool registry, lifecycle phases, and all rules). Tell the user: "Agent generation started. I'll report results when complete."

The background subtask executes a closed-loop lifecycle: Discovery -> Create/Update -> Poll -> Validate -> Publish -> Report. On failure, it auto-triggers an update loop (max 2 cycles, 15-minute wall-clock timeout). See the reference file for complete details.

### 3-4. Present report

When the background subtask completes, present the report. On success, route to Step 2A or 2B. On failure after max cycles, offer:
...
- Agent creation/update runs in background subtasks. Generation takes 1-3 minutes -- use a background subtask for the generate -> poll -> publish loop. See [Delegation model](#delegation-model).
- Prefer Nimble MCP tools in Cline. Use CLI only when MCP is unavailable, missing a needed capability, or the user chooses shell workflows.
- For Nimble web-data steps, use Nimble MCP tools or `nimble search`/`nimble extract`. Do not substitute unrelated web fetch/search tools unless the user explicitly changes the task away from Nimble.
- Every background subtask prompt MUST include the CLI commands block (see [Delegation model](#delegation-model)).
