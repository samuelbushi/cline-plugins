...
3. Present the pivot to the user via `ask the user` -- offer `nimble search` exploration, generating a custom agent, or waiting for the service to recover.
...
Poll with `nimble_agents_status` (read-only GET endpoint) from a background subtask. After 20 consecutive `processing` responses (~10 minutes), inform the user:
