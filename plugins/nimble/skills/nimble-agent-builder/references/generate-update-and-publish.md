...
background subtask workflow for creating, updating, and refininf agents with automated validation.

This entire workflow runs inside a background subtask because generation takes 1-3 minutes (poll loop). The background subtask uses CLI commands (`nimble agent generate`, `get-generation`) via Bash. If CLI is unavailable, it falls back to MCP tools.
...
CRITICAL: ask the user happens ONCE in the foreground conversation, BEFORE launching the background subtask. The background subtask receives `refine_validate=true|false` and NEVER asks the user anything.
...
This determines `refine_validate=true|false` for the background subtask. Even with `false`, failures still auto-trigger the loop.
...
| Status | Action |
|--------|--------|
| `waiting` | Relay follow-up questions to user via `ask the user`. Call `nimble_agents_update_session` with same `session_id` and user's answer as `prompt`. |
...
Max 2 update cycles. After 2 cycles without reaching 80%, the background subtask stops and reports the best result. The foreground conversation presents recovery options to the user.

## background subtask prompt template

```
Launch a background subtask with this prompt:
...
|---|---|
| nimble agent generate | Nimble MCP nimble_agents_generate |
| nimble agent get-generation | Nimble MCP nimble_agents_status |
...
IMPORTANT: Do NOT use ask the user. All decisions are pre-made.
...
When generating multiple agents (e.g., multi-store comparison), launch background subtasks sequentially -- one per session_id. Each task runs the full closed-loop lifecycle independently. Gather reports when all complete.

## Key rules

- Generate/update runs as a background subtask. Generation takes 1-3 minutes -- use a background subtask for the poll loop.
- ask the user happens ONCE in the foreground before launching. The background subtask NEVER asks the user anything.
