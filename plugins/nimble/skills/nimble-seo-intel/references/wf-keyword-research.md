...
`nimble-researcher` sub-agents (`references/nimble-playbook.md#background-research-subtasks`) with
`normal Cline tool approval settings` to extract and analyze these pages.
...
Follow `references/memory-and-distribution.md` for registered MCP server detection and sharing flow.
