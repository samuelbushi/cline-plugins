...
Prompt 2 -- locale and depth (use ask the user):
...
agents (`references/nimble-playbook.md#background-research-subtasks`) with `normal Cline tool approval settings`.
...
- Do not launch detached background processes. Keep background subtasks bounded and return structured outputs.
...
```bash
After persistent-output approval: mkdir -p ~/.nimble/memory/seo/rank-snapshots/{domain}
```
...
Follow `references/memory-and-distribution.md` for registered MCP server detection and sharing.
