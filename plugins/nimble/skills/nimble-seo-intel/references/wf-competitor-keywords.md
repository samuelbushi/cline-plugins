...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-seo-intel <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
...
Spawn one `nimble-researcher` agent per domain with `normal Cline tool approval settings`.
...
`references/memory-and-distribution.md` for registered MCP server detection, sharing flow,
...
- Analyze competitor messaging -> "Run `nimble-competitor-positioning` for messaging"
- Business context -> "Run `nimble-competitor-intel` for business signals"
...
> - Run `nimble-competitor-positioning` to understand competitor messaging strategy
> - Run `nimble-competitor-intel` for business context (funding, hiring, launches)
...
- Run `nimble-competitor-positioning` for messaging and positioning analysis
- Run `nimble-competitor-intel` for business context (funding, hiring, launches)
```
