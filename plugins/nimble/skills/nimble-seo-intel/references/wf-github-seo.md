...
- `gh` not installed -> warn: "GitHub CLI (`gh`) not found. Install with
  install `gh` with the user's platform package manager and run `gh auth login`. Without it, metadata and community
...
Prompt 2 -- confirm scope and gather inputs (use ask the user):
...
Use sub-agents (`references/nimble-playbook.md#background-research-subtasks`) with `normal Cline tool approval settings`
