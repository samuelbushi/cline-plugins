...
Prompt 2 -- skill-specific setup (use ask the user):
...
Prompt 1 -- ask in plain text (NOT ask the user):
...
Prompt 2 -- confirm scope (use ask the user):
...
`normal Cline tool approval settings`. One agent per domain. Replace template placeholders
...
> - Run `nimble-competitor-positioning` to understand how competitors message these topics
...
- Run `seo-keyword-research` to expand keywords for the top gap topics
- Run `seo-site-audit` to ensure existing content is technically healthy
- Run `nimble-competitor-positioning` to understand competitor messaging angles
