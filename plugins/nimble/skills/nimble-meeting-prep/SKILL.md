...
description: |
  ...
  or single-company deep dives without attendees (use nimble-company-deep-dive).
metadata:
  author: Nimbleway
  version: 0.21.3
---
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-meeting-prep <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> read `~/.nimble/memory/people/index.md` to identify existing
  ...
  recent intel from nimble-competitor-intel runs). Also check `~/.nimble/memory/companies/`
  for cached company research.
  No same-day report check -- nimble-meeting-prep is per-meeting, not per-day. Users
...
Calendar shortcut: If the user didn't specify attendees and a calendar registered MCP server
...
Value Positioning section is generated, so it's worth one question. Use ask the user:
...
`normal Cline tool approval settings`. One agent per attendee. Pass discovered WSA names
...
calls: ~5 searches per attendee + ~4 company searches + 3-5 extractions = ~(5 x N) + 9
...
of nimble-company-deep-dive -- focused on what's useful for the conversation, not a full 360 degrees.
...
- Report -> `~/.nimble/memory/reports/nimble-meeting-prep-[company-slug]-[date].md`
...
- Company profile -> update `~/.nimble/memory/companies/[company-slug].md` if new
  ...
  (e.g., `[[people/alex-kim]]`).
- Profile -> update `last_runs.nimble-meeting-prep` in `~/.nimble/business-profile.json`
...
- "Looks good" -> done
- Sibling skills: `nimble-company-deep-dive` for a full 360 on the company,
  `nimble-competitor-intel` to track them as a competitor, `nimble-competitor-positioning`
...
- Not competitor monitoring -- use `nimble-competitor-intel` for tracking competitors
- Not a company deep dive -- use `nimble-company-deep-dive` for research without attendees
