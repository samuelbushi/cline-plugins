---
name: nimble-competitor-positioning
description: |
  ...
  Do NOT use for business signals like funding/hiring (use nimble-competitor-intel),
  single-company deep dives (use nimble-company-deep-dive), or meeting prep (use
  nimble-meeting-prep).
metadata:
  author: Nimbleway
  version: 0.21.3
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-competitor-positioning <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> load prior data from two sources:
  ...
  - `~/.nimble/memory/competitors/*.md` -- business signals from nimble-competitor-intel
  ...
  - Same-day repeat: if `last_runs.nimble-competitor-positioning` is today, check for
    existing report at `~/.nimble/memory/reports/nimble-competitor-positioning-[today].md`.
...
This skill shares the competitor list from `nimble-competitor-intel`. If a profile already
...
- More than 4 -> ask which to focus on (use ask the user):

  > You have [N] competitors tracked. Which ones should I analyze?
  > - All [N] (~[3-5 x N] Nimble API credits, ~[N x 2] min)
...
calls: ~1 map + 3 extractions + 3 searches per competitor = ~7 x N calls (plus baseline
...
`normal Cline tool approval settings` and inline the prompt from
...
Do NOT use `references/nimble-playbook.md#background-research-subtasks` -- that agent is scoped for raw data gathering
...
- Report -> `~/.nimble/memory/reports/nimble-competitor-positioning-[date].md`
...
- Profile -> update `last_runs.nimble-competitor-positioning` in
...
> Next steps:
> - Run `nimble-competitor-intel` for business signals (funding, hiring, product launches)
> - Run `nimble-company-deep-dive` for a full 360 profile on any competitor
