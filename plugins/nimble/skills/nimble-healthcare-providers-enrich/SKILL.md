---
name: nimble-healthcare-providers-enrich
description: |
  ...
  Do NOT use for extracting providers from practice URLs -- use nimble-healthcare-providers-extract instead.
  Do NOT use for validating credentials -- use nimble-healthcare-providers-verify instead.
  Do NOT use for discovering practices -- use nimble-market-finder or nimble-local-places instead.
...
`nimble-healthcare-providers-extract` ran earlier in this session by following the Sibling
...
- Before creating persistent Nimble memory, get persistent-output approval from `references/nimble-playbook.md`. If approved: `mkdir -p ~/.nimble/memory/{reports,nimble-healthcare-providers-enrich/checkpoints}`; otherwise use workspace `.nimble/`.
- `ls ~/.nimble/memory/nimble-healthcare-providers-enrich/checkpoints/ 2>/dev/null`
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-healthcare-providers-enrich <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> note it for context. Determine mode using smart date windowing
  ...
  - Same-day repeat: if `last_runs.nimble-healthcare-providers-enrich` is today, check
    for existing report at `~/.nimble/memory/reports/nimble-healthcare-providers-enrich-*[today].md`.
...
```bash
ls ~/.nimble/memory/reports/nimble-healthcare-providers-extract-*$(date +%Y-%m-%d).md 2>/dev/null
```
If a same-day report exists, parse the `{slug}` from the filename and load
`~/.nimble/memory/nimble-healthcare-providers-extract/{slug}/providers.json`. The practice
...
If input is ambiguous, use ask the user (counts as 1 of max 2 prompts):
...
Checkpoint: before writing a persistent home-dir checkpoint, get persistent-output approval from `references/nimble-playbook.md`. If not approved, use workspace `.nimble/` for resumable checkpoint files.
Interrupted runs with 20+ providers waste significant API credits without resume.
```bash
echo '{...}' > ~/.nimble/memory/nimble-healthcare-providers-enrich/checkpoints/{slug}/search.json
```
...
Checkpoint: before writing a persistent home-dir checkpoint, get persistent-output approval from `references/nimble-playbook.md`. If not approved, use workspace `.nimble/` for resumable checkpoint files.
```bash
echo '{...}' > ~/.nimble/memory/nimble-healthcare-providers-enrich/checkpoints/{slug}/extraction.json
```
...
- Report -> `~/.nimble/memory/reports/nimble-healthcare-providers-enrich-{slug}-{date}.md`
- Enriched data -> `~/.nimble/memory/nimble-healthcare-providers-enrich/{slug}/enriched.json`
- Profile -> update `last_runs.nimble-healthcare-providers-enrich` in
...
`references/memory-and-distribution.md` for registered MCP server detection and sharing flow.
...
> Next steps:
> - Run `nimble-healthcare-providers-verify` to validate the enriched credentials and
>   license status
> - Run `nimble-healthcare-providers-extract` to discover more providers from practice
>   websites
> - Run `nimble-market-finder` to find additional practices in this area
...
(`references/nimble-playbook.md#background-research-subtasks`) to parallelize search and extraction.
