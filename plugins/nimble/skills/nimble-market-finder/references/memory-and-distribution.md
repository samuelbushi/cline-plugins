...
- Skills read only their directory's index in preflight. nimble-competitor-intel reads
  `competitors/index.md`; nimble-meeting-prep reads `people/index.md`. Cross-directory
...
- Skills follow links to enrich output. When nimble-meeting-prep finds
  `[[competitors/widgetco]]` in a person's file, it loads that competitor file for
  additional context. When nimble-competitor-intel finds `[[people/alex-kim]]` in a
...
|------|---------|----------------|
| `competitive-landscape.md` | Market positioning, feature gaps, pricing comparison | nimble-competitor-intel after 3+ competitors |
...
sources:
  ...
  - path: competitors/acme-rival.md
    updated: 2026-03-18
generated_by: nimble-competitor-intel
...
nimble-competitor-intel generates `competitive-landscape.md` when:
...
## Open
- [ ] WidgetCo pricing for enterprise tier -- couldn't find public pricing [2026-03-20, nimble-competitor-intel]
- [ ] GizmoTech Series B details -- rumored but unconfirmed [2026-03-20, nimble-competitor-intel]
- [ ] Alex Kim's LinkedIn activity -- profile was private [2026-03-15, nimble-meeting-prep]

## Resolved
- [x] WidgetCo new CTO name -- confirmed: Sarah Chen [2026-03-22, nimble-competitor-intel]
```
...
One file per contact. Used by nimble-meeting-prep skill.
...
per day (e.g., nimble-meeting-prep for different companies), add a qualifier:
`{skill-name}-{qualifier}-{YYYY-MM-DD}.md`. The qualifier is defined in each skill's
SKILL.md (e.g., company slug for nimble-meeting-prep).
...
```bash
After persistent-output approval: mkdir -p ~/.nimble/memory/{competitors,people,companies,reports,positioning,synthesis}
```
...
1. Step 0 (Preflight): Load relevant memory files for context. This tells the skill
   ...
   research. For example, nimble-competitor-intel loads `~/.nimble/memory/competitors/*.md`;
   nimble-meeting-prep loads `~/.nimble/memory/people/*.md`.
...
After presenting output, offer sharing based on available MCP registered MCP servers.

### registered MCP server Detection

Check before presenting options:
- Notion: `a Notion MCP page-creation tool, if available`
- Slack: Any Slack MCP tool

### Sharing Flow

Use `ask the user` with only the available options:
