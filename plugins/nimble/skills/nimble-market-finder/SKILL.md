...
description: |
  ...
  Do NOT use for competitor monitoring -- use nimble-competitor-intel instead.
  Do NOT use for company deep dives -- use nimble-company-deep-dive instead.
  Do NOT use for neighborhood-level exploration with social enrichment
  -- use nimble-local-places instead.
metadata:
  author: Nimbleway
  version: 0.21.3
---
...
Also simultaneously:
- Before creating persistent Nimble memory, get persistent-output approval from `references/nimble-playbook.md`. If approved: `mkdir -p ~/.nimble/memory/{reports,nimble-market-finder/checkpoints}`; otherwise use workspace `.nimble/`.
- Check for existing checkpoints: `ls ~/.nimble/memory/nimble-market-finder/checkpoints/ 2>/dev/null`
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-market-finder <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
...
Use ask the user with up to 3 questions:
...
Check: `cat ~/.nimble/memory/nimble-market-finder/checkpoints/{slug}/discovery.json 2>/dev/null`
...
4. Save checkpoint: `~/.nimble/memory/nimble-market-finder/checkpoints/{slug}/discovery.json`
...
Save checkpoint: `~/.nimble/memory/nimble-market-finder/checkpoints/{slug}/enrichment.json`
...
4. Calculate coverage score -- `matched / reference_count x 100`
...
Discovery mode:
- Report -> `~/.nimble/memory/reports/nimble-market-finder-{slug}-{date}.md`
- Entity data -> `~/.nimble/memory/nimble-market-finder/{slug}/entities.json`

Audit mode:
- Report -> `~/.nimble/memory/reports/nimble-market-finder-audit-{slug}-{date}.md`
- Structured data -> `~/.nimble/memory/nimble-market-finder/{slug}/audit-{date}.json`
  (all three categories with match metadata)

Both modes:
- Profile -> update `last_runs.nimble-market-finder` in `~/.nimble/business-profile.json`
...
- "Run nimble-company-deep-dive on new discoveries?" -> deep research on discovered_only
...
> - Run `nimble-competitor-positioning` to compare top players in this market
> - Run `nimble-local-places` for neighborhood-level discovery with social enrichment and maps
...
Use `nimble-researcher` agents (`references/nimble-playbook.md#background-research-subtasks`) when:
...
(normal Cline tool approvals, batch max 4, fallback on failure).
