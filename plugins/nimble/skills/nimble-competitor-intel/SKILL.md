...
description: |
  ...
  Requires the Nimble MCP server or Nimble CLI for live web data.
  Do NOT use for single-company deep dives (use nimble-company-deep-dive), meeting
  prep with attendees (use nimble-meeting-prep), or non-business queries.
metadata:
  author: Nimbleway
  version: 0.21.3
---
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-competitor-intel <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> read `~/.nimble/memory/competitors/index.md` to identify which
  ...
  - Same-day repeat: if `last_runs.nimble-competitor-intel` is today, check if a report
    already exists at `~/.nimble/memory/reports/nimble-competitor-intel-[today].md`. If so,
...
Prompt 1 -- ask in plain text (NOT ask the user with options):
...
Prompt 2 -- confirm company + choose competitor method (use ask the user):
...
`normal Cline tool approval settings`. Customize the prompt template with each competitor's
...
calls: ~6 searches per competitor x N competitors + ~2 industry searches + extractions.
...
- Report -> `~/.nimble/memory/reports/nimble-competitor-intel-[date].md` (save the full
...
- Profile -> update `last_runs.nimble-competitor-intel` in `~/.nimble/business-profile.json`
...
Use the `nimble-analyst` agent (`references/nimble-playbook.md#background-analysis-subtasks`) with
`normal Cline tool approval settings` to synthesize patterns across all competitor files. The
...
> Next steps:
> - Run `nimble-competitor-positioning` to analyze how competitors present themselves online
> - Run `nimble-company-deep-dive` for a full 360 profile on any competitor from this report
> - Run `nimble-meeting-prep` if you're meeting with someone at a competitor
