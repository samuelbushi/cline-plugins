...
description: |
  ...
  Requires the Nimble MCP server or Nimble CLI for live web data via WSAs and fallback search.
  Do NOT use for competitor analysis or monitoring (use nimble-competitor-intel),
  company research or deep dives (use nimble-company-deep-dive), general web search
...
Also simultaneously:
- Before creating persistent Nimble memory, get persistent-output approval from `references/nimble-playbook.md`. If approved: `mkdir -p ~/.nimble/memory/{reports,nimble-local-places/checkpoints}`; otherwise use workspace `.nimble/`.
- Check for existing checkpoints: `ls ~/.nimble/memory/nimble-local-places/checkpoints/ 2>/dev/null`
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-local-places <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> note the user's location preferences if any. Determine mode
  ...
  - Quick refresh: last run < 14 days ago (skip social enrichment, reviews only
    for new places)
  - Same-day repeat: if `last_runs.nimble-local-places` is today, check if a report
    already exists at `~/.nimble/memory/reports/nimble-local-places-*[today].md`. If so,
...
If the user provided both but you want to scope further, use ask the user
...
Check: `cat ~/.nimble/memory/nimble-local-places/checkpoints/{slug}/discovery.json 2>/dev/null`
...
2. Deduplicate following the Entity Deduplication pattern from
   ...
   fuzzy name + city
3. Save checkpoint:
   `~/.nimble/memory/nimble-local-places/checkpoints/{slug}/discovery.json`
...
Save checkpoint: `~/.nimble/memory/nimble-local-places/checkpoints/{slug}/social.json`
...
Save checkpoint:
`~/.nimble/memory/nimble-local-places/checkpoints/{slug}/reviews.json`
...
Save to: `~/.nimble/memory/nimble-local-places/{slug}-map-{date}.html`

Provide this generated file path to the user: `~/.nimble/memory/nimble-local-places/{slug}-map-{date}.html`
...
- Report -> `~/.nimble/memory/reports/nimble-local-places-{slug}-{date}.md`
- Per-place data -> `~/.nimble/memory/nimble-local-places/{slug}/places.json`
  (structured JSON with all enriched records)
- Profile -> update `last_runs.nimble-local-places` in `~/.nimble/business-profile.json`
...
> Next steps:
> - Run `nimble-company-deep-dive` for a full 360 profile on any business from this list
> - Run `nimble-meeting-prep` if you're meeting with someone at one of these businesses
> - Run `nimble-competitor-positioning` to compare businesses in this area
...
(`references/nimble-playbook.md#background-research-subtasks`) to parallelize enrichment.
