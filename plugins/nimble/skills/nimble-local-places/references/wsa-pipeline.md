...
`~/.nimble/memory/nimble-local-places/{slug}-map-{YYYY-MM-DD}.html`

Open automatically after generation:
```bash
Provide this generated file path to the user: ~/.nimble/memory/nimble-local-places/{slug}-map-{YYYY-MM-DD}.html
```
