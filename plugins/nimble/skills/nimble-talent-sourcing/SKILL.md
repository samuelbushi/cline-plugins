...
description: |
  ...
  candidate list with profiles and skills. Contact signals are opt-in and require
  ...
  nimble-market-finder instead. Do NOT use for researching a single known person
  -- use nimble-company-deep-dive or nimble-meeting-prep instead.
metadata:
  author: Nimbleway
  version: 0.21.3
---
...
Also simultaneously:
- Before creating persistent Nimble memory, get persistent-output approval from `references/nimble-playbook.md`. If approved: `mkdir -p ~/.nimble/memory/{reports,nimble-talent-sourcing}`; otherwise use workspace `.nimble/`.
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-talent-sourcing <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> note industry keywords if any; proceed to Step 1
- No profile -> fine, nimble-talent-sourcing doesn't require onboarding; proceed to Step 1
...
If role is missing or ambiguous, ask with `ask the user`:

> "What role are you hiring for, and where? (e.g. 'Senior ML Engineer, remote US'
> or paste a job description)"

Once parameters are clear, confirm with the user using `ask the user`:
...
`normal Cline tool approval settings`, max 4 concurrent. Assign one agent per platform:
...
Before collecting contact signals, ask for explicit approval to process personal
contact data and explain that exports or persistence are opt-in. If the user does
not approve, collect role, skills, experience, education, and source links only.
...
- Contact signals (email, personal site, GitHub handle) only if approved
...
Before presenting results, check `~/.nimble/memory/nimble-talent-sourcing/[role-slug].md` --
...
- Profile: [URL]
- Contact signals: [email / personal site / GitHub, only if approved]
...
- Report -> `~/.nimble/memory/reports/nimble-talent-sourcing-{YYYY-MM-DD}.md` (full candidate report with all tiers)
- Per-role -> `~/.nimble/memory/nimble-talent-sourcing/[role-slug].md` (candidate list; write or update)
- Profile -> update `last_runs.nimble-talent-sourcing` in `~/.nimble/business-profile.json` using the python3 snippet in `references/profile-and-onboarding.md`. Skip if the file does not exist.

Update `~/.nimble/memory/nimble-talent-sourcing/index.md` with a row for this search.
...
Offer next steps using `ask the user`:
...
> - Run `nimble-company-deep-dive` on a candidate's current employer for deal context
> - Run `nimble-meeting-prep` before reaching out to a Tier 1 candidate
