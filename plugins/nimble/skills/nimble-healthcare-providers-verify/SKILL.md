---
name: nimble-healthcare-providers-verify
description: |
  ...
  Do NOT use for filling data gaps -- use nimble-healthcare-providers-enrich instead.
...
`nimble-healthcare-providers-extract` or `nimble-healthcare-providers-enrich` ran earlier in this
...
- Before creating persistent Nimble memory, get persistent-output approval from `references/nimble-playbook.md`. If approved: `mkdir -p ~/.nimble/memory/{reports,nimble-healthcare-providers-verify/checkpoints}`; otherwise use workspace `.nimble/`.
- `ls ~/.nimble/memory/nimble-healthcare-providers-verify/checkpoints/ 2>/dev/null`
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-healthcare-providers-verify <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> note it for context. Determine mode using smart date windowing
  ...
  - Same-day repeat: if `last_runs.nimble-healthcare-providers-verify` is today, check
    for existing report at `~/.nimble/memory/reports/nimble-healthcare-providers-verify-*[today].md`.
...
ls ~/.nimble/memory/reports/nimble-healthcare-providers-enrich-*$(date +%Y-%m-%d).md 2>/dev/null
```
...
main context must get persistent-output approval before writing a home-dir checkpoint, then write the checkpoint before spawning the next step or presenting
results:
1. Receive sub-agent results
2. Write checkpoint: `echo '[results]' > ~/.nimble/memory/nimble-healthcare-providers-verify/checkpoints/{slug}/batch-{n}.json`
...
- Report -> `~/.nimble/memory/reports/nimble-healthcare-providers-verify-{slug}-{date}.md`
- Verification data -> `~/.nimble/memory/nimble-healthcare-providers-verify/{slug}/verified.json`
- Profile -> update `last_runs.nimble-healthcare-providers-verify` in
...
`~/.nimble/memory/nimble-healthcare-providers-verify/{slug}/verified-{date}.csv` with all
...
> Next steps:
> - Run `nimble-healthcare-providers-extract` on unverified providers' practice URLs
>   to get fresh data
> - Run `nimble-healthcare-providers-enrich` to fill gaps in verified providers' records
...
(`references/nimble-playbook.md#background-research-subtasks`) to parallelize NPI lookups and extraction.
