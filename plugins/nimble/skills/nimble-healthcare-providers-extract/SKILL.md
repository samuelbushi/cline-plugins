---
name: nimble-healthcare-providers-extract
description: |
  ...
  Do NOT use for credential validation -- use nimble-healthcare-providers-verify instead.
...
- Before creating persistent Nimble memory, get persistent-output approval from `references/nimble-playbook.md`. If approved: `mkdir -p ~/.nimble/memory/{reports,nimble-healthcare-providers-extract/checkpoints}`; otherwise use workspace `.nimble/`.
- `ls ~/.nimble/memory/nimble-healthcare-providers-extract/checkpoints/ 2>/dev/null`
...
- Tag all `nimble` CLI calls: `nimble --client-source skill-nimble-healthcare-providers-extract <subcommand>`. MCP tools are available through the registered Nimble server when the CLI is unavailable; prefer CLI only for workflows that need unsupported flags or generated scripts.
- Profile exists -> note it for context. Determine mode using smart date windowing
  ...
  - Same-day repeat: if `last_runs.nimble-healthcare-providers-extract` is today, check
    for existing report at `~/.nimble/memory/reports/nimble-healthcare-providers-extract-*[today].md`.
...
Path B -- Market-finder handoff. User ran `nimble-market-finder` first and wants to
extract providers from those results. Read the nimble-market-finder output:

```bash
cat ~/.nimble/memory/nimble-market-finder/{slug}/entities.json 2>/dev/null
```
...
Save checkpoint: `~/.nimble/memory/nimble-healthcare-providers-extract/checkpoints/{slug}/mapping.json`
...
Save checkpoint: `~/.nimble/memory/nimble-healthcare-providers-extract/checkpoints/{slug}/extraction.json`
...
- Report -> `~/.nimble/memory/reports/nimble-healthcare-providers-extract-{slug}-{date}.md`
- Provider data -> `~/.nimble/memory/nimble-healthcare-providers-extract/{slug}/providers.json`
- Profile -> update `last_runs.nimble-healthcare-providers-extract` in
...
> Next steps:
> - Run `nimble-healthcare-providers-enrich` to fill data gaps (NPI lookup, board
>   certification verification, additional contact info)
> - Run `nimble-healthcare-providers-verify` to validate credentials and license status
> - Run `nimble-market-finder` to discover more practice URLs in this area
...
(`references/nimble-playbook.md#background-research-subtasks`) to parallelize site mapping and extraction.
