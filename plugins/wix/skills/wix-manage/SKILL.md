...
- Treat every Wix REST call as operating on live site/business data unless the user clearly identifies a sandbox or test site.
- Do not create, update, delete, publish, install, purchase, upload, bulk mutate, or otherwise change Wix resources without an explicit user request for that exact action.
- Ask before broad reads that could expose contacts, customers, orders, bookings, payment data, private CMS content, domains, media libraries, or other sensitive business information.
- Keep API keys, OAuth tokens, bearer tokens, site IDs, account IDs, and request headers out of commits and final responses. Use placeholders in examples.
- For destructive or bulk operations, first present the target site, endpoint, filter/body, expected effect, and rollback or verification plan.
