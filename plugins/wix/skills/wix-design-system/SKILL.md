...
- Use the bundled `scripts/wds.cjs` helper only against the user's current project or a path the user explicitly provides.
- Do not install `@wix/design-system`, `@wix/wix-ui-icons-common`, testkit packages, or other dependencies unless the user asks for that setup.
- Prefer read-only component lookup before editing UI files. When edits are needed, keep them scoped to the requested Wix UI work.
