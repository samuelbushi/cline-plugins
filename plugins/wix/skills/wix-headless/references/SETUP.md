...
Always invoke `wix-manage` here - it is the only thing that publishes `<wix-manage-root>` into scratch and loads the recipe files into context (Step 4's installs and the whole Seed phase reuse them; `SEED.md` does not re-invoke). A known directory path is not a substitute - the recipes must be in context. Use the host's available skill-invocation primitive when one is available; otherwise load the `wix-manage` skill files directly. The prose instruction is the contract - don't hardcode a tool-call snippet.
