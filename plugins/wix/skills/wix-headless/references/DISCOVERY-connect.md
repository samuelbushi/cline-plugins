...
Read the entry HTML (and other pages) - markup, copy, headings/`<title>`, `<form>`s, repeated structures, and the CSS custom-property token block (`:root { --… }`). Opportunistic enrichment: if a design-handoff bundle is present (`README.md`, transcripts, or exported project notes), read it to sharpen intent - but never require it; the inference must stand on markup alone.
