...
1. CONNECT/IMPLEMENT-AN-EXISTING-DESIGN intent in the prompt → operation: connect,
   ...
   implement (v0, Lovable, a static-site generator, or any imported design source). The design arrives by fetch into
...
## Pre-flight - Verify CLI auth (BEFORE project-changing work)

The first project-changing Wix touch is the post-approval project bootstrap - create/astro: `scaffold.sh` → `npm create @wix/new@latest headless`; connect: `npm create @wix/new@latest init` - which creates a business + project against the user's Wix account and so requires an active CLI session. Without one it fails. A read-only auth check is fine before planning, but starting an interactive login is not.
...
- Exit non-zero → tell the user Wix CLI authentication is required before this workflow can create, initialize, or release Wix resources. Ask for explicit permission before starting `npx @wix/cli@latest login`.
- If the user approves login, start `npx @wix/cli@latest login` using the host's background-command mechanism when available, surface the device-login URL and code, wait for completion, confirm with `whoami`, then open the matching operation discovery file.
- If the user does not approve login, stop and give them the command to run later.
