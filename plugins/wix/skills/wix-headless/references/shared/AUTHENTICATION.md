...
The primary place this check runs is DISCOVERY.md § "Pre-flight". `scaffold.sh` repeats the check defensively for its standalone-invocation path. If the check fails, do not start login silently. Tell the user Wix authentication is required and ask for explicit permission before running `npx @wix/cli@latest login`.

If the user approves, use the host's background-command mechanism when available so the login process can wait while the user completes the browser/device-code step. If the host cannot run a long-lived background command, stop and give the user the login command to run manually.
...
The shell command is just the CLI invocation, nothing else:

```bash
npx @wix/cli@latest login
```

Run it through the Cline host's background-command mechanism when available. Do not add shell `&`, do not redirect to a custom temp file unless the host requires it, and do not chain unrelated commands around login. The goal is one login process whose stdout/stderr can be read until the `awaiting_user` event appears.

### Reading the output

Read the background command output until the `awaiting_user` JSON line appears. The first useful event is usually on line 1 (a Node `TimeoutNaNWarning` may follow; ignore it). Parse JSON lines defensively and extract `verificationUri` + `userCode`.
...
3. Then wait for the host to report that the background login command completed. Do not re-run `whoami` in a sleep loop while waiting.

When login exits 0, run `whoami` once to confirm and proceed. On non-zero exit (rare - typically a timeout if the user took >600 s, or a network error), surface the relevant output to the user and stop.
