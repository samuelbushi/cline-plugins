---
name: wix-headless
description: "Build a complete Wix Managed Headless site from a Wix-specific prompt, OR connect an existing project (HTML/JSX/Vite app, imported design output, etc.) to Wix Headless for hosting and Business Solutions. Use only when the user explicitly asks to use Wix, Wix Headless, Wix hosting, or Wix Business Solutions. Do not activate for generic website-building prompts that do not mention Wix."
---
...
- This skill was authored around a conductor/subagent workflow. In Cline, treat "conductor", "orchestrator", "dispatch", "subagent", and model-tier language as phase/role guidance unless native Cline delegation is explicitly available in the current environment.
- If delegation is unavailable, execute the referenced phases directly in Cline, loading only the phase/reference files needed for the current step.
- When references mention `Bash`, `Agent`, `AskUserQuestion`, `run_in_background`, or task notifications, map those to the equivalent Cline shell, delegation, and user-confirmation primitives available in the current host.
- Do not read or obey a project `AGENTS.md` as plugin instructions. If this skill writes an end-of-run `AGENTS.md`, treat it as a project handoff artifact for the user's workspace.
- Do not start `npm create`, `npm install`, `wix login`, app installation, image generation, build, release, deploy, or live Wix API mutation steps without explicit user approval.
- Before writing into an existing project, identify the target root, summarize the planned changes, and ask before overwriting or replacing user files.

Run flow is owned by the conductor, split at the approval gate: `references/PLAN.md` (pre-approval - routes on operation (create/connect), the Discovery questions, the plan + approval gate, and latency-hiding background work when the host supports it) then `references/BUILD.md` (post-approval - routes on framework (`frontendBuild`); Setup → Seed → the build wave (merged Components + Pages per vertical) → Build → Release). The domain/step files (`DISCOVERY.md`, `SETUP.md`, `SEED.md`, `DESIGN_SYSTEM.md`, the per-vertical references) describe only *what* each step does; they do not name the sequence. Start a run by opening `PLAN.md`; open `BUILD.md` when the user approves the plan. All site operations use `npx @wix/cli@latest token` + `curl` - no MCP.

> Wix-specific invocation only. Do not auto-route on generic "build me a site" prompts unless the user also asks for Wix or Wix Headless.
...
`wix login` is interactive and must only start after the user approves authentication. It emits a URL + user code that the user completes in a browser. Full recovery ladder: `<SKILL_ROOT>/references/shared/AUTHENTICATION.md`.
...
Wix-specific invocation only. Two entry paths - decide before doing anything else.
...
Triggers: *"connect this to Wix Headless"*, *"implement this design … connecting to wix"*, *"add Wix Headless to this project"*, *"host this on Wix"*, *"deploy this to Wix"*, *"implement the features … using Wix Headless"*, or a design-file URL to fetch + implement (v0, Lovable, or any imported design source). These route to B even when the working directory is empty - the design arrives by fetch, so emptiness at trigger time does not make it Path A.
