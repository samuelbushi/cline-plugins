...
Verify all pricing and model availability before making a final recommendation. Use a user-managed pricing MCP when available, otherwise use `references/shared/pricing-cache.md` with a staleness warning. Treat competitive claims in this file as a starting point for analysis, not as authoritative current pricing.
