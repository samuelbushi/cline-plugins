...
- Gateway/router type (`gateway_type`): Detect whether AI calls go through a gateway, router, or framework. This critically affects migration effort (gateway users can migrate in 1-3 days vs 1-3 weeks for direct SDK).
  ...
  | Env var `OPENAI_BASE_URL` containing `openrouter.ai` in non-secret config, CI config, or shell scripts             | `llm_router`     | OpenRouter - env-configured router        |
  | Env var `OPENROUTER_API_KEY` or `OR_API_KEY` present in non-secret config, CI config, or shell scripts             | `llm_router`     | OpenRouter - API key detected             |
  ...
  Env var scan scope for gateway detection: Check these non-secret files for `OPENAI_BASE_URL`, `OPENROUTER_API_KEY`, `OR_API_KEY`, `LITELLM_PROXY_BASE_URL`, `LITELLM_API_KEY`:
  ...
  Never read `.env`, `.env.local`, `.env.production`, `.env.staging`, or any `.env.*` file by default. If gateway classification depends on an environment file, ask the user to confirm with a redacted variable-name list instead of opening the file. Log: "Detected [KEY_NAME] in [file] - classified as llm_router (OpenRouter/LiteLLM)."
