...
| Compliance               | `compliance.value` (if present)                        | Triggers Config + Security Hub recommendations in `baseline-plan.md` |
...
For detailed cost breakdown: See Appendix B (cost estimates include security baseline recommendations as a line item).

To enable: Review `terraform/baseline-plan.md`, then ask for active `baseline.tf` only after confirming account-wide controls will not collide with existing AWS account settings.
