...
Generate deployment runbook script. It must print commands for review only; it must not execute deployment commands itself.
...
if ! command -v agentcore &> /dev/null; then
    ...
    echo "Review and install prerequisites before running the commands below."
fi

# Step 2: Create project
show_cmd agentcore create --name "$HARNESS_NAME"

# Step 3: Add harness with model and tools
show_cmd agentcore add harness --name "$HARNESS_NAME" \\
  --model-id "$MODEL_ID" \\
  --system-prompt "{harness_config.system_prompt}" \\
  --tools "{comma-separated tool types from harness_config.tools}"

# Step 4: Deploy
show_cmd agentcore deploy

# Step 5: Test invocation
echo ""
echo "=== Test Invocation ==="
show_cmd agentcore invoke --harness "$HARNESS_NAME" \\
...
Generate AgentCore Runtime deployment runbook script. It must print commands for review only; it must not install packages or deploy by itself.
...
echo "--- Prerequisites ---"
echo "Install prerequisites after review if needed:"
show_cmd pip install strands-agents strands-agents-tools boto3 bedrock-agentcore-cli

# Step 2: Create AgentCore project
show_cmd agentcore create --name "$PROJECT_NAME"
...
echo ""
echo "--- Test Invocation ---"
show_cmd agentcore invoke --name "$PROJECT_NAME" "Hello, test invocation"
...
echo "  3. Review the commands printed by ./deploy_strands.sh, then run approved commands manually"
