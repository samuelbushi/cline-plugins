...
| `baseline-plan.md` | security baseline | Recommended account-wide security baseline: alternate contacts, password policy, S3 PAB, EBS encryption, Access Analyzer, IMDSv2 default, CloudTrail, AWS Budget, GuardDuty, and compliance-conditional Config/Security Hub guidance. Emit active `baseline.tf` only when the user explicitly asks for account-wide Terraform. |
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| Account Alternate Contacts, IAM Account Password Policy, S3 Account Public Access Block, EBS Default Encryption, IAM Access Analyzer (ACCOUNT), EC2 Instance Metadata Defaults, CloudTrail, AWS Budgets, GuardDuty Detector, S3 buckets for CloudTrail and Config logs, AWS Config recorder/delivery/role, AWS Security Hub + standards | `baseline-plan.md` by default; `baseline.tf` only on explicit user request |
...
> By default, emit `baseline-plan.md` instead of active account-wide Terraform. Account-wide controls are useful but can collide with existing CloudTrail, Config, Security Hub, budgets, and account settings. Emit `baseline.tf` only if the user explicitly asks for account-wide baseline Terraform after reviewing the plan.
...
- `terraform` block: `required_version >= 1.5.0`, `hashicorp/aws ~> 5.80`; do not enable a remote backend by default
...
## Step 1a: Remote state backend guidance

Do not emit an active S3 backend block by default. Local state is acceptable for review-only generated artifacts, and an active backend can fail or collide with existing user infrastructure. Instead, document a reviewed backend template in `terraform/README.md` and ask the user before converting it into active Terraform.

Document the following backend block as an opt-in template:
...
If the user explicitly asks for active backend Terraform, emit the following resources in `baseline.tf` or a dedicated backend module after confirming bucket naming and ownership:
...
## Step 1.5: Generate security baseline plan

Always emit `terraform/baseline-plan.md`. The baseline applies account-wide security controls that should be reviewed before any Terraform is applied. Emit active `terraform/baseline.tf` only when the user explicitly asks for it after reviewing the plan.
...
4. Emit `baseline-plan.md` starting with the file-header comment block and a `cloudtrail_retention_days` recommendation:
...
5. List the recommended always-on resources, in this order. If the user requested active `baseline.tf`, each resource carries the plugin's standard four default tags plus `Component = "security-baseline"`:
...
6. If `compliance` contains any of `soc2`, `pci`, `hipaa`, `fedramp`, append a compliance-conditional section to the plan. If the user requested active `baseline.tf`, wrap the Terraform section in `########## Compliance-Conditional ##########` / `########## End Compliance-Conditional ##########` dividers:
...
9. compute.tf modification (runs during Step 3 compute domain, not here): every `aws_launch_template` emitted for ECS-EC2, EKS node groups, or bare EC2 receives IMDSv2 enforcement unconditionally. This workload-level control is safe even when account-wide `baseline.tf` is not emitted:
...
- Emit `baseline-plan.md` even when `aws-design.json` contains only AI or billing-only resources (no infrastructure clusters). The baseline is workload-independent.
- Do NOT probe for existing account resources (CloudTrail trails, Config recorders, Security Hub enrollment). Collision risk is surfaced by the review notes listed in item 8.
...
- [ ] `baseline-plan.md` exists and clearly marks account-wide controls as review-before-apply recommendations.
- [ ] If the user explicitly requested active baseline Terraform, `baseline.tf` contains `aws_account_alternate_contact` for each of OPERATIONS, BILLING, SECURITY, plus `aws_iam_account_password_policy`, `aws_s3_account_public_access_block`, `aws_ebs_encryption_by_default`, `aws_cloudtrail`, `aws_guardduty_detector`, `aws_accessanalyzer_analyzer`, `aws_ec2_instance_metadata_defaults`, and `aws_budgets_budget`.
- [ ] If active `baseline.tf` exists, it contains a `locals` block with `cloudtrail_retention_days` set to a positive integer, and the lifecycle `expiration.days` on `aws_s3_bucket_lifecycle_configuration.cloudtrail_logs` equals `local.cloudtrail_retention_days`.
- [ ] If active `baseline.tf` exists, `aws_budgets_budget.monthly_spend.limit_amount` equals `max(50, ceil(estimation-infra.json.projected_costs.breakdown.total.mid * 1.2))` as a string.
- [ ] If `compute.tf` contains any `aws_launch_template`, every such launch template has `metadata_options { http_tokens = "required", http_put_response_hop_limit = 1 }` - this applies unconditionally even when active `baseline.tf` is not emitted.
- [ ] Every `aws_ecr_repository` in `compute.tf` has `image_scanning_configuration { scan_on_push = true }`.
- [ ] If active `baseline.tf` exists and `compliance` contains soc2/pci/hipaa/fedramp, it contains `aws_config_configuration_recorder`, `aws_config_delivery_channel`, `aws_config_configuration_recorder_status`, `aws_securityhub_account`, `aws_securityhub_standards_subscription` for FSBP.
- [ ] If active `baseline.tf` exists and `compliance` contains pci, an additional `aws_securityhub_standards_subscription` for PCI DSS exists.
- [ ] If active `baseline.tf` exists, it does NOT contain any `aws_securityhub_standards_subscription` whose `standards_arn` references `nist-800-53`, regardless of compliance values.
- [ ] If active `baseline.tf` exists and `compliance` is empty, absent, or contains only gdpr, it does NOT contain any `aws_config_*` or `aws_securityhub_*` resources.
- [ ] If active `baseline.tf` exists, it does NOT contain any invented SSB control IDs. Search for `ACCT.IAM`, `ACCT.S3`, `ACCT.EBS`, `ACCT.CT`, `ACCT.GD`, `ACCT.CFG`, `ACCT.SH`, `WKLD.EC2.01` - all MUST have zero matches. Only bare `ACCT.01` through `ACCT.13` identifiers are permitted.
- [ ] If active `baseline.tf` exists, it does NOT mention "Trusted Advisor" anywhere (Trusted Advisor is docs-action only and out of scope).
- [ ] If active `baseline.tf` exists, Security Hub subscribes to FSBP (always when the compliance-conditional section is emitted) and PCI DSS (only when `compliance` contains `pci`). No other standards subscriptions.
...
- `terraform/baseline-plan.md` MUST exist. `terraform/baseline.tf` is optional and requires explicit user request.
...
- validation-report.json (status: <validation_status>, if Terraform validation ran)
