...
Review `terraform/baseline-plan.md` before creating account-wide baseline Terraform. If active `terraform/baseline.tf` was explicitly requested, review it separately before running `terraform apply`.
...
If active `terraform/baseline.tf` was explicitly requested, review the compliance-conditional section between `########## Compliance-Conditional ##########` and `########## End Compliance-Conditional ##########` before running `terraform apply`. Otherwise keep the account-wide baseline as guidance in `terraform/baseline-plan.md`.
