...
2. Stage 2 route gates (fail closed):
   - If infra artifact route is active (`generation-infra.json` AND `aws-design.json`) -> require `terraform/` and `scripts/`. If `validation-report.json` exists, its `status` must be in `{passed, passed_degraded_offline, skipped_user_continue}`.
...
3. Validation status - If `$MIGRATION_DIR/validation-report.json` exists, report its `status` field (`passed`, `passed_degraded_offline`, or `skipped_user_continue`). If it is missing, report: "Terraform validation was not run; review generated files before apply." If `status == "passed_degraded_offline"`, add: "Provider registry was unreachable; `terraform validate` was skipped. Re-run `terraform init && terraform validate` from a network-connected shell to complete validation."
