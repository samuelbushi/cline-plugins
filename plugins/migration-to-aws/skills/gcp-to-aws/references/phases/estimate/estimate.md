...
### Step 0b: Optional Live Pricing Check (only if cache stale or service not listed)

If the user has separately configured and authorized an AWS Pricing MCP server, attempt a pricing lookup with up to 2 retries (3 total attempts):
...
If no user-managed pricing MCP is available, do not try to install one or run local package managers. Use the bundled pricing cache and surface the appropriate staleness warning.
...
- If cache > 90 days and a user-managed pricing MCP is available: "Pricing source: live API. Cache is stale ([date]) - using real-time pricing."
- If cache > 90 days and no user-managed pricing MCP is available: "Pricing source: stale cache only (updated [date]). No live pricing MCP is configured. Proceeding with cached pricing; accuracy may be ±15-25% for AI models."
...
2. User-managed pricing MCP (secondary) - Real-time pricing for services NOT in pricing-cache.md (±5-10% accuracy, more precise). Set `pricing_source: "live"`. Only called when the cache lacks the needed service or model and the user has already configured such a server. Always pass the user's target region (from `preferences.json`) in pricing queries when the tool supports it.
