---
name: turnstile-spin
description: Set up Cloudflare Turnstile end-to-end in a project  -  scan the codebase, create the widget via the Cloudflare API, deploy the managed siteverify Worker, write the frontend snippets, and validate. Load this when a user asks to add Turnstile, set up CAPTCHA, protect a form from bots, or fix a Turnstile integration. Mirrors developers.cloudflare.com/turnstile/spin.
references:
  - vanilla-html
  - nextjs-app
  - nextjs-pages
...
8. Widget creation. Run `scripts/widget-create.sh --account-id <id> --name <name> --domains <list> --mode managed`. Report the sitekey. The script writes the widget secret to a chmod 600 temp file and returns `secret_file`; do not print or cat that file.

9. Worker deploy. Run `WIDGET_SECRET_FILE=<secret_file> scripts/worker-deploy.sh --name turnstile-siteverify-<project-slug>`. Report the Worker URL on `status: ok`; the deploy script deletes `secret_file` after the Worker secret is set. On `set_secret_failed`, the Worker deployed but `TURNSTILE_SECRET_KEY` is not set on it; surface the error and retry with the same `WIDGET_SECRET_FILE` before running validation.
...
- Do not print or paste the Turnstile secret. The helper scripts may keep it in a chmod 600 temp file only long enough for `worker-deploy.sh` to pass it to `wrangler secret put`; delete that file if the workflow is abandoned.
...
2. Fetch the widget metadata via `scripts/fetch-secret.sh --account-id <id> --sitekey <key>`. Branch on `status`:
   - `ok`: read `secret_file`, `clearance_level`, and `domains` from the response. Confirm `domains` includes the user's production hostname; if not, surface the gap before proceeding.
...
| Step 11 returns `missing-input-secret` | Secret didn't propagate. Re-run `worker-deploy.sh` with the same `WIDGET_SECRET_FILE`, wait 10s, then re-validate. Use the `worker_name` field returned by `worker-deploy.sh`; do not rely on a `$WORKER_NAME` env var. |
| `worker-deploy.sh` returns `set_secret_failed` | Worker is deployed but secret is not set. Re-run `worker-deploy.sh` with the same `WIDGET_SECRET_FILE` and returned `worker_name`, or manually pipe the temp file into `wrangler secret put` without printing it. Surface the `detail` field to the user  -  it carries the wrangler error. |
