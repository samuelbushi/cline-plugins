...
#   ok:        {"status":"ok","secret_file":"<chmod-600 temp file>","clearance_level":"<level>","domains":[<list>]}
...
if [ "$http_code" = "200" ]; then
  ...
  if [ -n "$secret" ] && [ "$secret" != "null" ]; then
    secret_file=$(mktemp "${TMPDIR:-/tmp}/turnstile-secret.XXXXXX")
    chmod 600 "$secret_file"
    printf '%s' "$secret" >"$secret_file"
    echo "{\"status\":\"ok\",\"secret_file\":\"$secret_file\",\"clearance_level\":\"$clearance\",\"domains\":$domains}"
    exit 0
  fi
fi
