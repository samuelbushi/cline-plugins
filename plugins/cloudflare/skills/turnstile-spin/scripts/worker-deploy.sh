...
#   $WIDGET_SECRET        (required unless $WIDGET_SECRET_FILE is set)
#   $WIDGET_SECRET_FILE   (preferred; chmod-600 temp file from widget-create.sh)
#
# Args:
...
TEMPLATE_DIR=$(cd "$SCRIPT_DIR/../templates/worker" && pwd)
START_DIR=$(pwd)
while [[ $# -gt 0 ]]; do
  case $1 in
    --name)       NAME="$2"; shift 2 ;;
...
if [ -z "${WIDGET_SECRET:-}" ] && [ -n "${WIDGET_SECRET_FILE:-}" ]; then
  WIDGET_SECRET=$(cat "$WIDGET_SECRET_FILE")
fi
: "${WIDGET_SECRET:?WIDGET_SECRET or WIDGET_SECRET_FILE must be set}"
...
deploy() {
  ...
  (cd "$DEPLOY_DIR" && run_wrangler deploy --name "$target_name") >"$deploy_log" 2>&1
}
...
secret_log=$(mktemp)
set_secret() {
  echo "$WIDGET_SECRET" | (cd "$DEPLOY_DIR" && run_wrangler secret put TURNSTILE_SECRET_KEY --name "$NAME") >"$secret_log" 2>&1
}
...
rm -f "$secret_log"
if [ -n "${WIDGET_SECRET_FILE:-}" ]; then
  rm -f "$WIDGET_SECRET_FILE"
fi
sleep 5
