...
Ask before writing an audit report to disk. If the user wants a saved report, ask for or propose a path such as `reports/security-audit-<cluster-name>-<YYYY-MM-DD>-<sequence>.md`, verify whether that path is ignored before relying on it, and redact secrets or customer data before writing. If the user does not approve a file write, provide the report in the conversation only.
