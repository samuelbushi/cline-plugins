...
### Example 2: Table Creation With Explicit Approval
Input: "Create a users table with email and password, then apply it to my local dev database"

Process:
1. Generate the CockroachDB SQL.
2. Explain that `CREATE TABLE` is a schema change.
3. Ask for explicit approval before execution, including the exact target database and command.
4. If approved, use the plugin-owned MCP server when it supports the operation, or use a user-requested `cockroach sql` command with connection details redacted in responses.

Output:
```
## Connection Method
Target: local development database (connection details redacted)
...
## Approval Required
This is a schema change. I will not execute it unless you confirm the exact target database and approve the command.

## Next Steps
- Insert sample data: INSERT INTO users (email, password_hash) VALUES (...)
- Query the table with an explicit column list, not SELECT *.
...
2. Use the plugin-owned read-only `cockroachdb-toolbox` MCP server when connected
3. Generate and execute a bounded read-only query

Output:
```
## Connection Method
Using: plugin-owned cockroachdb-toolbox MCP server
