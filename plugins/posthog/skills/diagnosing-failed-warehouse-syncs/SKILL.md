...
- Report: "Your Stripe source's API key is no longer authenticating.
   ...
   To fix:
   1. Rotate the restricted API key in the Stripe dashboard.
   2. Update the stored credential through PostHog's UI or your team's secure credential workflow.
   3. Syncs should resume automatically  --  no data loss.

   Don't paste the new key into chat. Tell me when the credential has been updated and I can re-check the source."

User: "I updated it in PostHog."

Agent:
- external-data-sources-reload({id}) to trigger retry
- Report: "Re-triggered the sync. Check back in a few minutes  --  latest_error should clear."
```
