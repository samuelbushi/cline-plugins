...
description: >
  Debugs why session recordings aren't appearing in the PostHog monorepo local dev environment.
  Use only when a developer is working inside the PostHog application repo and reports that local replay ingestion isn't working,
...
This is a PostHog application development runbook. It assumes the user is in the PostHog monorepo with local services such as `hogli`, `phrocs`, Caddy, Kafka, SeaweedFS, `capture-replay`, and `recording-api`. If the user is debugging replay in their own product, use `diagnosing-missing-recordings` or `investigating-replay` instead.
