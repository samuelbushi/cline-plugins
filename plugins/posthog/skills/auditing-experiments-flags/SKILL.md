...
Use the available PostHog MCP read/list tools for experiments and feature flags. No SQL queries are needed for Phase 1 checks when those resource tools are available.
...
1. Fetch the entity via the available PostHog MCP read tool for experiments or feature flags.
...
1. Bulk-fetch via the available PostHog MCP list tool for experiments or feature flags.
...
1. Fetch all experiments and all flags via the available PostHog MCP list tool.
...
4. If there are more than 5 entities with findings and a notebook/artifact creation tool is available, output a notebook artifact for easier navigation. Otherwise report inline.
...
Some checks require activity logs, which may not be available via the active MCP read tool.
...
If a read or list call fails for some entities:
