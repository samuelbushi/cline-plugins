...
Run these checks against each flag fetched via the active PostHog MCP read/list tool.
...
Look at: `experiment_set` (list of linked experiment IDs), and for each experiment, check its `end_date` and `archived` status via the active PostHog MCP read tool.
