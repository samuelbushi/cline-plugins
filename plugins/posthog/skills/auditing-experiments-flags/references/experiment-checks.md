...
Run these checks against each experiment fetched via the active PostHog MCP read/list tool.
...
Look at: `feature_flag` (the linked flag object or ID), and fetch the flag via the active PostHog MCP read tool if only an ID is available.
