...
SELECT
    ...
    date_diff('DAY', toDateTime('2026-06-10 07:18:27.459311'), expiry_time) AS recording_ttl,
    greaterOrEquals(max(s._timestamp), toDateTime('2026-06-10 07:13:27.458256')) AS ongoing,
...
WHERE
    and(greaterOrEquals(s.min_first_timestamp, toDateTime('2026-06-07 00:00:00.000000')), lessOrEquals(s.min_first_timestamp, toDateTime('2026-06-10 07:18:27.458497')))
GROUP BY
    session_id
HAVING
    and(greaterOrEquals(expiry_time, toDateTime('2026-06-10 07:18:27.459136')), equals(max(s.is_deleted), 0), greater(active_seconds, 5.0))
ORDER BY
    start_time DESC,
    session_id DESC
LIMIT 50000
