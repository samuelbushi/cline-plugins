...
description: >
  Debug the PostHog Signals pipeline locally end-to-end inside the PostHog application repo. Covers emitting test signals
...
This is a PostHog application development runbook. It assumes the user is in the PostHog monorepo with Django management commands, Temporal, ClickHouse, object storage, Docker sandbox services, and Signals source paths under `products/`. If the user only wants to inspect Signals in a connected PostHog project, use `signals`, `exploring-signals-scouts`, or the relevant `signals-scout-*` skill instead.
...
The sandboxed Signals agent went idle for longer than `TASKS_INACTIVITY_TIMEOUT_SECONDS`. When unset
