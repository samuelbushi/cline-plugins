...
The list can be very large for fan-out request flows; when it exceeds the inline limit, some MCP clients persist it to a file.
...
Trace tool results are JSON. When too large to read inline, some MCP clients persist them to a file.
