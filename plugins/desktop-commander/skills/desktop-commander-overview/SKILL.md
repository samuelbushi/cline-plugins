...
Prefer Cline's built-in workspace tools for ordinary project reads, edits, and commands. Use Desktop Commander when the task specifically benefits from persistent sessions, long-running background processes, structured files such as PDF/DOCX/XLSX, large local data analysis, SSH, process management, or user-approved paths outside the current workspace.

Before the first Desktop Commander read, list, search, session inspection, process inspection, or other introspection outside the current workspace, get explicit user confirmation unless the user already requested that exact path, process, machine, or session. Before destructive filesystem operations, process termination, SSH changes, deploys, restarts, migrations, or other environment-changing commands, summarize the action and get user confirmation unless the user already explicitly requested that exact operation.
