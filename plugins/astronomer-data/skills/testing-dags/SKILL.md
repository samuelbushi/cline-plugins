...
## First Action: Confirm the Test Target

When the user asks to test a DAG, first confirm the target DAG and environment. Triggering a DAG run mutates Airflow state, so ask for explicit approval before running the trigger through MCP or CLI.

```bash
af runs trigger-wait <dag_id>
```

Avoid broad pre-flight work unless the user asks for it. If approval is granted, trigger the DAG and debug only if it fails.
...
Philosophy: once the user has approved the exact DAG/environment, test directly and debug on failure. Do not trigger production workflows without confirmation.

---

## Phase 1: Trigger and Wait

Use the matching Airflow MCP operation or `af runs trigger-wait` to test the DAG:
