...
```javascript
// Geofencing and tracking require AWS credentials, not API-key auth.
// Use a Cognito identity pool or another explicit credential provider.
const IDENTITY_POOL_ID = "us-west-2:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx";
const locationAuthHelper = await amazonLocationClient.withIdentityPoolId(IDENTITY_POOL_ID);
const locationClient = new amazonLocationClient.LocationClient(
  locationAuthHelper.getClientConfig()
);
