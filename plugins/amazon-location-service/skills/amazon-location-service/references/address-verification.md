...
async function verifyAddress(addressString, { forStorage = false } = {}) {
  try {
    const command = new amazonLocationClient.places.GeocodeCommand({
      QueryText: addressString,
      ...(forStorage ? { IntendedUse: "Storage" } : {}),
...
if (result.valid && !result.ambiguous) {
  ...
  // If persisting coordinates or standardized address data long-term, make sure
  // the verification call used IntendedUse: "Storage" by passing { forStorage: true }.
  await saveAddress({
...
async function validateAddressForm(formElement) {
  ...
  formElement.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      errorEl.textContent = `Verification error: ${error.message}`;
      statusDiv.replaceChildren(errorEl);
      submitButton.disabled = false;
    }
  });
}
