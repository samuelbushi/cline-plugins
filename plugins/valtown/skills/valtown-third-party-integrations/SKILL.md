---
name: valtown-third-party-integrations
...
3. Test the connection with a minimal script (a single fetch / SDK call that returns one record) before building features on top, after confirming the user wants to call that service and any expected quota/cost impact. This isolates auth/setup problems from feature bugs.
...
- Payments: Stripe
- Email: Gmail (for sending via a user's account; for built-in mail use `std/email` instead  -  see the `valtown-email` skill)
- Google: Google Sheets
- External databases: Neon Postgres, Supabase, Upstash (for SQLite use built-in `std/sqlite` instead  -  see the `valtown-sqlite-storage` skill)
