...
When the user wants a new Val Town project, start from a template with `remix_val`  -  do not build complex app shapes from scratch. Templates handle the boilerplate (entrypoints, imports, build config) and are kept up to date with Val Town's current patterns. Pick the closest match and customize from there.

Before calling `remix_val` or creating files, confirm the target account or organization, whether the result should be public, and the expected starting template.
...
Share the live URL after the user has approved creating or remixing the val, then customize files one at a time. Update the README last.
