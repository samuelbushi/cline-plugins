...
After editing an HTTP val, ask before calling `fetch_val_endpoint` if the request may hit live integrations, mutate state, reveal private data, or consume third-party quota. When approved, confirm it returns the expected status and body before reporting the endpoint as ready.
