...
This Cline plugin registers the Supabase MCP server at `https://mcp.supabase.com/mcp`. Do not create or edit a project `.mcp.json` for this plugin-owned MCP server.
...
2. Check plugin MCP availability:
   Verify the `supabase` plugin is installed and enabled in Cline. If the MCP tools are missing, ask the user to reinstall or re-enable the plugin rather than editing workspace MCP settings.

3. Authenticate the MCP server:
   If the server is reachable and plugin MCP settings exist but tools aren't visible, the user may need to complete Cline's MCP authorization flow for Supabase, then reload the session.

## Supabase Documentation

Before implementing any Supabase feature, find the relevant documentation. Use these methods in priority order:

1. Supabase MCP docs search tool (preferred - returns relevant snippets directly when available)
2. Fetch docs pages as markdown - any docs page can be fetched by appending `.md` to the URL path.
3. Web search for Supabase-specific topics when you don't know which page to look at.

## Making and Committing Schema Changes

For schema changes, draft reviewable migration SQL first. Before running any SQL against Supabase MCP tools or `supabase db query`, confirm the exact target project, environment, schema, operation, and rollback plan with the user, then get separate explicit approval to apply remotely.

Do not "iterate freely" on an authenticated remote project. Use local development, a branch, a disposable database, or a reviewed migration draft whenever possible. If the user explicitly approves a remote exploratory SQL statement, keep it narrow, name the target project/ref in the command summary, and avoid destructive or broad data operations.

Do NOT use `apply_migration` to change a local database schema - it writes a migration history entry on every call, which means you can't iterate, and `supabase db diff` / `supabase db pull` will produce empty or conflicting diffs. If the user asks to apply a migration to a remote project, review the complete SQL first and get explicit approval for that project/environment.

When ready to commit your changes to a migration file:

1. Run advisors -> `supabase db advisors` (CLI v2.81.3+) or MCP advisors tools. Fix any issues.
2. Review the Security Checklist above if your changes involve views, functions, triggers, or storage.
3. Generate or create the migration file with a descriptive name, then review the SQL diff.
