...
-- Do not leave EXECUTE granted to every role by default
revoke execute on function private.is_team_member(bigint) from PUBLIC;

-- Grant only to roles that must evaluate policies using the function
grant execute on function private.is_team_member(bigint) to anon, authenticated;
