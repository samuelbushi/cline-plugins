---
name: figma-generate-project-plan
description: "Generate a FigJam project plan board from a PRD plus codebase context. Interactive flow: research -> propose sections -> per-section deep research -> per-section content + block-shape proposal -> create FigJam -> skeleton -> fill -> diagrams -> wrap. Each content block (section, nested section, intro callout, table, multi-column text, sticky column, diagram section, metadata strip) has its own reference file. Use when the user asks for a project plan in FigJam, an interactive project plan, or provides a PRD and wants per-section confirmation on content and rendering."
---
...
Foundation skills to use with this workflow:

- `figma-use` -- use for all `use_figma` calls.
- `figma-use-figjam` -- use for FigJam-specific `use_figma` calls.
- `figma-generate-diagram` -- use before every `generate_diagram` call.
...
Block references (one per content type -- consult the one(s) you need before each `use_figma` fill call):
...
Also pass `skillNames: "figma-use,figma-use-figjam,figma-generate-project-plan"` on `use_figma` calls when the tool exposes that optional logging parameter.
...
Tools / refs loaded
- `use_figma` (one call).
- Consult `figma-use-figjam/SKILL.md`.
- Consult `blocks/section.md` and `blocks/metadata-strip.md`.
...
- Consult `figma-use-figjam/SKILL.md`.
- Consult whichever block refs this section uses:
...
Tools / refs loaded
- Use `figma-generate-diagram/SKILL.md` before each `generate_diagram` call.
- Consult `figma-use-figjam/SKILL.md` and `blocks/diagram-section.md` before each reparent `use_figma` call.
...
```
[ok] Project plan written to FigJam.
File: <file_url>
...
"interactive project plan", "project plan", "make a FigJam project plan", "PRD to FigJam".
