...
const newPaint = figma.variables.setBoundVariableForPaint(paintCopy, "color", variable)
// [warning] Returns a NEW paint -- must capture return value!
node.fills = [newPaint]
...
// [warning] Returns a NEW effect -- must capture return value!
...
// [warning] Returns a NEW layout grid -- must capture return value!
...
// [warning] fontSize, fontWeight, lineHeight are NOT bindable via setBoundVariable
...
// Sizing
node.resize(width, height)                     // [warning] Resets sizing modes to FIXED
...
// [warning] uri MUST be a valid URL (https://...) -- relative paths will throw
```
