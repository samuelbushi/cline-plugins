...
interface PluginAPI {
   ...
   * @remarks
   *
   * This API is roughly the equivalent of pressing Ctrl-G/CmdG in the editor, but groups the specified list of nodes rather than the current selection. You may still, of course, group the current selection by passing it as an argument:
   *
   ...
   * @remarks
   *
   * This API is roughly the equivalent of pressing Ctrl-E/CmdE in the editor, but flattens the specified list of nodes rather than the current selection. You may still, of course, flatten the current selection by passing it as an argument:
   *
   ...
   * @remarks
   *
   * This API is roughly the equivalent of pressing Ctrl-Shift-G/CmdShiftG in the editor, but ungroups the given node rather than all nodes in the current selection.
   *
...
interface AspectRatioLockMixin {
   ...
   * ```
   *
   * Caution: [warning] `targetAspectRatio` cannot be used with auto-resizing text (TextNodes where textAutoResize !== NONE).
   */
...
interface PublishableMixin {
   ...
   *
   * Caution: [warning] There is a currently a bug in Figma where the description field will appear to be missing or not up to date. Until this is fixed, the workaround is to re-publish nodes for which the description is missing.
   */
  description: string
  /**
...
interface BaseNonResizableTextMixin {
   ...
   *
   * Caution: ⚠ Did you know: not all glyphs that you might think as a "character" are actually stored as a single character in JavaScript string? JavaScript strings are UTF-16 encoded. Some characters like "[emoji]" are stored using two characters! Try it in the JavaScript console: "[emoji]".length is 2! The two characters are called "surrogate pairs". Even more mindblowing: some characters are made of multiple _emojis_. For example, "[family emoji]", which you should see in your browser as a single character, has length 5. "[family emoji]".substring(0, 2) is "[emoji]" and "[family emoji]".substring(3, 5) is "[emoji]".
   */
   ...
   * [
   *   {
   *     characters: 'emoji pair',
   *     start: 0,
   *     end: 4,
   *     letterSpacing: { unit: 'PERCENT', value: 50 }
   *   },
   *   {
   *     characters: 'emoji run',
   *     start: 4,
   *     end: 12,
