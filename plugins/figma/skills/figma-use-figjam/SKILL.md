...
Always include `figma-use-figjam` in the comma-separated `skillNames` parameter when calling `use_figma` for FigJam operations, when the tool exposes that optional logging parameter. It does not affect execution.
...
Cline registers the Figma MCP server for this plugin. Use the available Figma tools directly; if Cline exposes them with a namespace, use names like `figma__use_figma`, `figma__get_figjam`, `figma__get_screenshot`, `figma__get_metadata`, and `figma__create_new_file`.
