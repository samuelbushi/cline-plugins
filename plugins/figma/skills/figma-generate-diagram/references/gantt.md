...
Every chart needs: the `gantt` keyword, a `dateFormat` directive (`YYYY-MM-DD` for date charts, `HH:mm` for intra-day -- see section3), and at least one task with a real start. `title` is optional but strongly recommended.
...
- `dateFormat HH:mm` -- intra-day only. Tasks are time-of-day starts; see section8 for the full setup.
...
For most roadmaps, `d` and `w` are the right units. Use `M` and `y` for multi-year horizons. Use `h` and `m` only for intra-day charts (section8).
...
| `milestone` | Zero-duration marker         | Launches, gates, review points (see section7)                 |
