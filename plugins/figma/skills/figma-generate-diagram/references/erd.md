...
- Styling -- `classDef`, `class Foo styleName`, `:::styleName` inline, and `style EntityId fill:#hex,stroke:#hex`. Silently dropped (no color applied). Unlike state diagrams, styling statements don't create phantom entities -- they just have no effect. Color-code entities via `use_figma` post-generation instead (section9).
...
- Aliases in relationship lines -- see section3 gotcha. Declare entities with aliases first, then reference by ID.

## 8. Layout (same ELK as flowcharts)

ER diagrams render via the same ELK layered layout as flowcharts. The principles from [flowchart.md section5 (ELK survival guide)](./flowchart.md#5-elk-survival-guide) apply:
...
7. Use aliases for display-friendly names when entity IDs are SQL-style (`user_acct` -> alias `"User Account"`). But remember section3 -- declare aliased entities separately, reference by ID in relationships.
...
2. Every relationship uses a valid cardinality pair (section5 table) and either `--` or `..`.
...
4. No alias syntax in relationship lines -- aliases must be on the entity declaration only (section3 gotcha).
...
7. No `classDef`, `class`, `:::`, or `style` lines (dropped -- section7).
