...
- Flowchart request where the only "extension" would be color -- Mermaid subgraph styling already handles this (see [flowchart.md section4](./flowchart.md)).
...
- NOT flowchart -- use Mermaid subgraph styling via [flowchart.md section4](./flowchart.md) instead. If the user specifically wants per-node coloring that Mermaid can't do, fall through to this recipe, but start with the native path.
