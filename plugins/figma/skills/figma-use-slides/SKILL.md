...
Always include `figma-use-slides` in the comma-separated `skillNames` parameter when calling `use_figma` for Slides operations, when the tool exposes that optional logging parameter. It does not affect execution.
