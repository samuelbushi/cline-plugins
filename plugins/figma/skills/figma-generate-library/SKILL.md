...
# Design System Builder -- Figma Skill
...
Prerequisites: Use the `figma-use` guidance for every `use_figma` call. It provides Plugin API syntax rules (return pattern, page reset, ID return, font loading, color range). This skill provides design system domain knowledge and workflow orchestration.

Always include `figma-generate-library` in the comma-separated `skillNames` parameter when calling `use_figma` as part of this workflow, when the tool exposes that optional logging parameter. It does not affect execution.
...
- [bad] Starting to create anything before scope is locked with user
- [bad] Ignoring existing file conventions and imposing new ones
- [bad] Skipping `search_design_system` before planning component creation

Phase 1 anti-patterns:
- [bad] Using `ALL_SCOPES` on any variable
- [bad] Duplicating raw values in semantic layer instead of aliasing
- [bad] Not setting code syntax (breaks Dev Mode and round-tripping)
- [bad] Creating component tokens before agreeing on token taxonomy

Phase 2 anti-patterns:
- [bad] Skipping the cover page or foundations docs
- [bad] Putting multiple unrelated components on one page

Phase 3 anti-patterns:
- [bad] Creating components before foundations exist
- [bad] Hardcoding any fill/stroke/spacing/radius value in a component
- [bad] Creating a variant per icon (use INSTANCE_SWAP instead)
- [bad] Not positioning variants after combineAsVariants (they all stack at 0,0)
- [bad] Building variant matrix > 30 without splitting (variant explosion)
- [bad] Importing remote components then immediately detaching them

General anti-patterns:
- [bad] Retrying a failed script without understanding the error first
- [bad] Using name-prefix matching for cleanup (deletes user-owned nodes)
- [bad] Building on unvalidated work from the previous step
- [bad] Skipping user checkpoints to "save time"
- [bad] Parallelizing use_figma calls (always sequential)
- [bad] Guessing/hallucinating node IDs from memory (always read from state ledger)
- [bad] Writing massive inline scripts instead of using the provided helper scripts
- [bad] Starting Phase 3 because the user said "build the button" without completing Phases 0-2
