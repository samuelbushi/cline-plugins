...
async function cleanupOrphans(runId) {
  ...
  // sharedPluginData scan dramatically on large files.
  ...
  for (const page of figma.root.children) {
    ...
    // Use the sharedPluginData index to find candidates, then keep only those whose
    // run_id matches. Much faster than findAll + getSharedPluginData on every node.
    const candidates = page.findAllWithCriteria({
      sharedPluginData: { namespace: 'dsb', keys: ['run_id'] },
    })
    const tagged = candidates.filter((node) => node.getSharedPluginData('dsb', 'run_id') === runId)
