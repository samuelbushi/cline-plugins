...
Code Connect uses template files (`.figma.js` or `.figma.ts`, depending on the project's `figma.config.json`) to connect your code components to Figma designs. This API reference covers the complete template system for creating these mappings.
...
Template extensions must match the project's `figma.config.json` include patterns. Common choices:
- `Button.figma.js`
- `Card.figma.js`
