...
6. For code -> design, `use_figma` is the API. Always use [`figma-use`](../figma-use/SKILL.md) before any `use_figma` call. If the task involves building a full screen, also use [`figma-generate-design`](../figma-generate-design/SKILL.md); if it involves building components or a design system, also use [`figma-generate-library`](../figma-generate-library/SKILL.md).
