...
- Existing variables. Does the file already have a semantic color collection with iOS-flavored names (`label/primary`, `background/secondary`, `separator/non-opaque`)? Use it. If not, the names from Apple's HIG (see section4) are reasonable defaults.
...
| `Label("Text", systemImage: "x")` | An auto-layout row of `[icon, text]` -- use the file's `Icon` component or an SF Symbol glyph text node (see section5). |
...
3. Mirror each distinct `.font(.custom(_))` call as its own named Figma text style (e.g. `Brand/Display Bold 17`) rather than as inline overrides on individual text nodes -- consistent with the section6 rule that named styles beat raw font properties.
...
2. Tokens before components. If you are creating variables (per section4), do it before building any view that binds to them.
