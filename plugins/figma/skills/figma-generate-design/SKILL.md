...
Use [figma-use](../figma-use/SKILL.md) before any `use_figma` call. That skill contains critical rules (color ranges, font loading, etc.) that apply to every script you write.

Always include `figma-generate-design` in the comma-separated `skillNames` parameter when calling `use_figma` as part of this workflow, when the tool exposes that optional logging parameter. It does not affect execution.
...
- User must provide a target Figma file (URL or `fileKey`). If they don't have one yet, ask whether to create a new Figma file, then call `create_new_file` only after explicit confirmation and reuse the returned file_key. Both `use_figma` and `generate_figma_design` require an existing `fileKey`.
...
1. In parallel:
   ...
   - Run `generate_figma_design` against the same `fileKey` to capture a pixel-perfect screenshot of the running web app into that file. `generate_figma_design` always requires `fileKey` -- if the user does not yet have a Figma file, ask whether to create one, then call the `create_new_file` MCP tool only after explicit confirmation and reuse that file_key for both this workflow and the capture.
