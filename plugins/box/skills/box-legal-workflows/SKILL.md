...
> PREREQUISITE: Read the bundled `box` skill for Box MCP authentication, tool selection, and base workflows. If it is unavailable, install or enable the `box` Cline plugin; do not run external skill installers from this workflow.
...
- Extracting standard information (dates, parties, amounts) into the conversation or a draft.
- Categorizing documents by obvious type as a proposed plan.
- Creating summary reports and analysis documents in chat for user review.
- Recommending metadata values, file organization, or routing decisions without applying them.

Any action that writes to Box requires explicit user confirmation first, including metadata updates, comments, generated documents, uploaded reports, folder creation, permission changes, collaborations, shared links, or moving/copying files.
...
7. [CONFIRM: Write metadata?] Apply approved metadata (`set_file_metadata`)
8. [CONFIRM: Route for review?] Add approved comments or collaborations (`create_file_comment` + `create_collaboration`)
...
5. [CONFIRM: Create reminder?] Add approved reminder (`create_file_comment`)
6. [CONFIRM: Update notification metadata?] Apply approved tracking metadata (`set_file_metadata`)
...
4. [CONFIRM: Apply metadata?] Apply approved fields to files with `set_file_metadata`
...
4. [CONFIRM: Write decision summary to Box?]
5. [CONFIRM: Update metadata to reflect current state?]
...
|------------|--------------|-------|
| Create folders | CONFIRM, then `create_folder` | Batch for efficiency |
| Upload documents | CONFIRM, then `upload_file` | New files |
| Copy documents | CONFIRM, then `copy_file` | Organize existing |
| Grant internal access | CONFIRM, then `create_collaboration` | Viewer/Editor/Co-Owner |
...
| Write metadata | CONFIRM, then `set_file_metadata` | Persist data |
| Tag for review | CONFIRM, then `create_file_comment` | Notify attorney |
