...
| Write metadata | CONFIRM, then `set_file_metadata` | Persist extracted fields |
| Identify variances | `ai_qa_multi_file` | Compare and highlight differences |
| Create variance report | CONFIRM, then `upload_file` | Write summary doc |
| Tag owner | CONFIRM, then `create_file_comment` | Notify attorney (expiring/review) |
...
11. [CONFIRM: Upload variance report to Box?] Report: `upload_file` (contract name, risk, variances, recommendations, citations)
...
17. [CONFIRM: Update alert metadata?] Update metadata: `set_file_metadata` (expiration_alert_sent: yes)
...
- Extracting dates, parties, and standard terms into the conversation or a draft.
- Drafting variance reports for user review.
- Recommending metadata values, file organization, or routing decisions without applying them.
- Proposing batch processing plans with pacing and checkpointing.

Any Box write requires explicit confirmation first, including metadata updates, file comments, uploaded reports, generated documents, collaborations, shared links, or moving/copying files.
...
4. For each (1-2 sec pauses):
   ...
   d. [CONFIRM: Write metadata?] `set_file_metadata`
   e. [CONFIRM: Upload variance report to Box?] Create report, upload
...
7. [CONFIRM: Route to attorney?] `create_file_comment`, `create_collaboration`
...
4. For each:
   ...
   d. [CONFIRM: Create reminder comment?] `create_file_comment`
   e. [CONFIRM: Update alert metadata?] `set_file_metadata` (alert_sent: yes)
...
5. For each:
   ...
   c. [CONFIRM: Write metadata?] `set_file_metadata`
...
7. [CONFIRM: Upload report and route review?] `upload_file`, `create_file_comment`, `create_collaboration`
