...
Write checks require explicit user confirmation, user-provided target IDs, and a short-lived test folder or file. Avoid shared-link smoke tests unless the task is specifically about sharing and the user confirms the audience and expiration.

```bash
box folders:create <PARENT_FOLDER_ID> "cline-smoke-test" --json
box files:upload ./artifact.pdf --parent-id <TEST_FOLDER_ID> --json
```
