...
- When this skill says to use the Read tool, read the referenced bundled file with Cline's available file-reading capability.
- When this skill says to use WebFetch, fetch current WorkOS docs only when the user requested current documentation or live docs are necessary for the task.
- When this skill says to use the Skill tool for `workos-widgets`, load the bundled `workos-widgets` skill if Cline exposes it; otherwise read `../workos-widgets/SKILL.md` and its referenced files directly.
- Do not install the WorkOS CLI, run `workos auth login`, or perform WorkOS CLI/API mutations without explicit user approval.
