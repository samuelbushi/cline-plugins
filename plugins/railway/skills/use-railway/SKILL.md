...
description: >
  ...
  this skill whenever the user mentions Railway deployments, projects, services,
  environments, buckets, object storage, build failures, MCP, or Railway
  infrastructure operations. For sign-up, sign-in, deploy, setup, and mutation
  workflows, explain the intended Railway action and ask before running commands.
---

# Use Railway

Referenced paths such as `references/setup.md` and `scripts/railway-api.sh` are relative to this skill directory, not the user's workspace. Resolve them from the installed `use-railway` skill path before reading or running them.
...
Route by user intent *before* running preflight checks. The preflight ceremony below is for diagnostic and configuration work; it adds friction when the user just wants to ship something or sign up. In Cline, still explain the action and ask before any command that signs in, creates resources, deploys, or changes Railway state.

Deploy-from-cwd intent ("deploy", "ship", "push to Railway", "deploy this app"):
- Skip the `railway whoami` / `railway status` preflights.
- Ask for confirmation, then run `railway up` directly. It self-validates auth, signs the user in (the CLI opens a browser) if they're unauthenticated, and chains into project + service creation and deploy.
...
- If the environment can't open a browser, the CLI prints a device-code sign-in link and waits. Follow [Device-code sign-in: relay the link immediately](#account-creation--sign-in).

Signup intent ("sign me up", "create my Railway account", "register me", "get me on Railway"):
- If the current directory has a deployable app (e.g. `package.json`, `requirements.txt`, `go.mod`, `Dockerfile`, source to build), explain that `railway up` signs the user up and deploys the current directory in one flow. Ask before running it.
- Only when there is nothing to deploy, or the user explicitly says they just want an account with no deploy, use `railway login` (creates new accounts on the fly through the same OAuth surface). There is no separate signup command.
- Signup is the flow most likely to hit the device-code wait (brand-new users in sandboxed/headless environments). Follow [Device-code sign-in: relay the link immediately](#account-creation--sign-in).
...
Once per session, alongside the preflight checks, you may verify the installed Railway agent tooling is current. Root help ends with an `Agent tooling:` health section (printed to stderr, CLI 5.9+; older CLIs print nothing; skip this check rather than retrying):
...
- An update is available (`rev <a> -> <b> available`): do not update automatically. Tell the user this Cline plugin already bundles the current workflow skill, and only run `railway skills update` if the user explicitly wants to manage Railway tooling for another coding agent.
- Skills or MCP server missing (`x` lines): do not run `railway setup agent -y` automatically. This plugin already registers the Railway MCP server for Cline.
...
| Command | When to use |
|---|---|
| `railway up` | Onboarding from the current directory. Unauthenticated -> opens the browser (or device-code) to sign in / sign up. With no linked project, this can create a project + service and deploy; ask before running it. |
...
- Deploy from cwd: ask, then run `railway up` (interactive) or `railway up -y` only when the user agrees to a non-interactive create/deploy flow. Do not ask the user to sign in separately first.
- New project from cwd when already signed in: ask, then run `railway up --new`.
- Sign up with a deployable app in cwd: explain that `railway up` signs up and deploys. Ask before running it. Sign in, or sign up with nothing to deploy: ask, then run `railway login`.
...
Do NOT pass `--browserless` just because the shell is non-interactive. If the human is at this machine (a local IDE or desktop session, the common case), bare `railway login` opens their browser directly, which usually completes more reliably than relaying a device code. Being in Cline does not make the machine headless.
...
When the CLI detects an automated coding environment with a human at the keyboard, `railway up` may open the browser and skip a Railway CLI confirmation prompt. In Cline, do not treat that as permission. Ask the user before starting account creation, resource creation, or deployment. A real human still has to complete OAuth in the browser.

Device-code sign-in: relay the link immediately (CRITICAL):

When the CLI can't open a browser (sandboxed shell, container, SSH, no `DISPLAY`), unauthed `railway up` and `railway login` print a sign-in URL + short code and then block, polling for up to 10 minutes while the user completes sign-in. The code expires after 10 minutes. If foreground command output is buffered until exit, the user may not see the link until the code is already dead. Handle it like this:

1. Preferred: use Cline's available background/long-running command support when possible:
...
2. No background support: set expectations, use the longest timeout:
   ...
   - Run with the longest timeout available in the current Cline session.
...
Fully unattended (no human at all): set `RAILWAY_API_TOKEN` (account-scoped) or `RAILWAY_TOKEN` (project-scoped) instead of running an interactive login. The bundled `scripts/railway-api.sh` helper reads those variables first, then falls back to the local Railway CLI config. A brand-new user with no token and no human present cannot complete signup; there is no headless account-creation path.
...
This Cline plugin already installs the Cline skill and registers the local `railway mcp` server. Do not run Railway's agent setup, MCP install, or skill update commands for Cline unless the user explicitly asks to manage Railway tooling outside this plugin.
...
`railway setup agent -y` skips the interactive login flow. If the user is not authenticated after setup, ask before running `railway login`.

Install or update MCP and skills directly only when the user names a non-Cline target tool:

```bash
railway mcp install
railway mcp install --agent <target>
railway mcp install --agent <target> --remote
railway skills
railway skills update --agent <target>
railway skills remove --agent <target>
```

The `--remote` flag configures `https://mcp.railway.com` instead of a local `railway mcp` stdio server for the target tool.
