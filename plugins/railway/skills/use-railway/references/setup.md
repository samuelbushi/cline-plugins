...
A brand-new user with no Railway account is onboarded through the same unified OAuth flow as sign-in; the backend detects fresh accounts and adapts the consent + landing pages. Pick the command by intent, then explain the action and ask before running anything that signs in, signs up, creates resources, or deploys:

- Deploy from the current directory: ask, then run `railway up` (interactive) or `railway up -y` only when the user agrees to a non-interactive create/deploy flow. When unauthenticated it opens a browser to sign in / sign up, then creates a project + service and deploys. Do not ask the user to `railway login` first.
- New project from cwd when already signed in: ask, then run `railway up --new` (`--name <name>` to override the project name).
- Sign up with a deployable app in cwd: explain that `railway up` signs up and deploys in one flow. Ask before running it. Sign in, or sign up with nothing to deploy: ask, then run `railway login`.

`railway up` and `railway login` self-validate auth; do not run `railway whoami` before them.

Headless / SSH / CI: the CLI auto-detects these and switches to the device-code flow (RFC 8628: sign-in link + short code) on its own. Do NOT pass `--browserless` just because the shell is non-interactive. If the human is at this machine, bare `railway login` opens their browser and usually completes more reliably. Reserve the flag for machines with genuinely no browser that the auto-detection missed.

`railway up --json` / `--ci` do NOT auto-prompt an unauthed user. `--json` emits a structured `{"error":"Not signed in.","code":"NOT_AUTHENTICATED", ...}` on stdout and exits non-zero. Detect `NOT_AUTHENTICATED`, ask before running `railway login`, then retry.

Fully unattended (no human): set `RAILWAY_API_TOKEN` (account-scoped) or `RAILWAY_TOKEN` (project-scoped) instead of logging in. The bundled `scripts/railway-api.sh` helper reads those variables before falling back to local Railway CLI config. There is no headless account-creation path; a brand-new user needs a human at the browser once.
