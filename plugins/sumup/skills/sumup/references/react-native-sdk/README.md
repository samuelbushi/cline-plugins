...
```tsx
<SumUpProvider apiKey="<SUMUP_CLIENT_KEY_FROM_OFFICIAL_DOCS>">
  <PaymentScreen />
</SumUpProvider>
```

Do not put SumUp secret API keys, OAuth client secrets, or server bearer tokens in React Native code. Create the checkout on the backend first and use only the client-side value required by the current official SDK docs.
