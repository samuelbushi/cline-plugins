...
4. Report network disclosure. The HTML file embeds all findings inline, but the default template loads Tailwind, React, Babel, and fonts from public CDNs at view time. Before generating or asking the user to open an HTML report, ask whether CDN-backed rendering is acceptable. For air-gapped or sensitive environments, use the "Offline mode" guidance in Step 4 instead.
...
Inspect the live Zscaler MCP tool list first and confirm the ZPA read tools needed for the inventory are available. If a tool is missing because the service is disabled, credentials are incomplete, or the MCP server did not expose it, record that category as "Cannot Audit" and continue with the rest.

Then call these tools in parallel where possible. Capture the full result of each available tool:
...
Errors: if a tool returns an error (for example service disabled, missing scope, unavailable credentials, or an MCP runtime error), record the category as "Cannot Audit -- API error" with the error message and continue with the rest. Do not abort the whole audit.
...
- The five `ca-*` findings are optional and always present.
...
Ask the admin once, before writing any files, whether they want report files generated and where they should go. Phrase it as a single short question:
...
If the user picks download or does not specify a path, default to a workspace-relative `./reports/` directory after confirming file creation is allowed in this session. Also warn that the default HTML report template loads third-party CDN scripts and fonts at view time unless the user asks for an offline variant.
...
Write the HTML report only after the user approves report file creation. Write the JSX component only when the user explicitly asks for an embeddable React component too.

1. `zpa-baseline-audit-<slug>-<ts>.html` -- the standalone Zscaler-styled web report. Open in any browser. No build step.
2. `ZpaAuditReport-<slug>-<ts>.jsx` -- optional React component for embedding in an existing app.
...
- Do not modify any other part of the template unless the user explicitly asks for an offline/no-CDN report. The CSS, React component, and Tailwind/CDN imports are calibrated together.
