...
Tell the admin: "Rule created (ID `<id>`) and activated." If activation is not approved, say the rule was created but remains staged and is not live. If activation fails, surface the error -- the rule exists but is not live.
...
Don't pre-list rules before creating. Skip `zia_list_ssl_inspection_rules` unless the admin explicitly asks about ordering or wants to inspect existing rules -- direct create + activation offer is the default flow.
