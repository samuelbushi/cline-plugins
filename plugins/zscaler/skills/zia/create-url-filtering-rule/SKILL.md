...
Do not call `zia_activate_configuration()` from within `zia-manage-time-interval` when chaining -- defer the activation decision to the end of this skill so a single approved activation can flush both the new Time Interval and the new URL Filtering rule.
...
### Step 6: Offer activation

ZIA changes are staged until activation. After a successful create, summarize the staged rule and ask the admin whether to activate it now. Only after explicit approval, run:

```
zia_activate_configuration()
```

Tell the admin: "URL Filtering rule created (ID `<id>`) and activated." If activation is not approved, say the rule was created but remains staged and is not live. If activation fails, surface the error -- the rule exists but is not live.
...
- The activation status or staged/not-live status
...
zia-create-url-filtering-rule    (this skill -- attach the interval ID and create the rule)
        ↓
offer zia_activate_configuration() after explicit approval (one activation flushes both)
```

Don't pre-list rules before creating. Skip `zia_list_url_filtering_rules` unless the admin explicitly asks about ordering or wants to inspect existing rules -- direct create + activation offer is the default flow.
