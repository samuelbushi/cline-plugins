...
- yes Valid: `Business Hours`, `After Hours`, `Weekday Mornings`, `Weekend All Day`, `Maintenance Window`, `Lunch Break`
- [x] Invalid (will be rejected): `Mon-Fri 08:00-17:00`, `Q1 Maintenance`, `9-5 Window`, `Business Hours -- Mon-Fri`
...
### Step 4: Offer activation (only when this skill is the *terminal* step in the admin's session)

If this skill is being called as a sub-step of another ZIA create skill, do not activate yet -- the caller will handle the activation decision after attaching the interval and creating the rule. Activation flushes staged config; running it mid-chain causes two activations instead of one.

Only when this skill is the *only* thing the admin asked for (e.g. "create a Business Hours time interval and stop"), summarize the staged interval and ask whether to activate it. After explicit approval:
...
- `zia_activate_configuration()` -- ZIA-wide activation; only when this skill is terminal and the user explicitly approves
