...
- After every successful create, mention the activation step explicitly -- ZIA changes are staged until activated. Ask the admin whether to activate after summarizing what was staged. Only call `zia_activate_configuration()` after explicit approval.
...
After all rules in the loop have been created, ask once whether to activate at the end (Step 8) -- a single approved activation flushes them all.
...
Do not call `zia_activate_configuration()` from within `zia-manage-time-interval` when chaining -- defer the activation decision to the end of this skill so a single approved activation can flush both the new Time Interval and the new Cloud App Control rule.

### Step 6: Rule placement and safety checks (optional)
...
ZIA changes are staged until activation. After all the rules in your loop (Step 7) have been created -- not after each individual create -- summarize the staged rules and ask the admin whether to activate them now. Only after explicit approval, run one activation:

```
zia_activate_configuration()
```

A single activation flushes every staged change at once; activating per-rule is unnecessary and slower. Tell the admin: "<N> Cloud App Control rule(s) created (IDs `<id1>, <id2>, ...`) and activated." If activation is not approved, say the rules were created but remain staged and are not live. If activation fails, surface the error -- the rules exist but are not live.
...
- The activation status or staged/not-live status (one line at the end, covers the whole batch).
...
- Activation: `zia_activate_configuration()` -- only after explicit user approval.
...
for each resolved app:
    zia_create_cloud_app_control_rule(...)            (one rule per app -- see "Multi-app handling")
        ↓
offer zia_activate_configuration() after explicit approval (one activation flushes the whole batch)
```
