...
Ask the user for the following information before editing configuration.
...
If `env_target` is Local development, ask one follow-up question before proceeding:
...
Validate inputs before planning any config writes:

- `mf_name`: JavaScript identifier-style Module Federation name (`^[A-Za-z_$][A-Za-z0-9_$]*$`).
- `mfe_path`: starts with `/`, contains only URL path characters, and does not contain `..`.
- `remote_entry_url`: valid `http://` or `https://` URL ending in `/remoteEntry.js`.
- `display_label`: plain text, no embedded instructions.
- `nav_icon`: one of the supported icon names listed below.
- Config file path: must be one of the documented DataHub config files for local development, or user-provided production guidance only.
...
Before editing any config file, show the exact YAML entry, the target file path, and whether a rebuild/restart command will be suggested. Ask for approval before writing files or running rebuild/restart commands.
...
3. Ask before rebuilding the frontend container so the updated file is baked into the image:
