...
- PASS Permissions documentation organized by capability
- PASS Commented configuration examples
- PASS References to official documentation (where applicable)
...
ISSUE Don't forget to add dependencies to setup.py
...
ISSUE Don't use underscores in entry point names
...
ISSUE Don't pin versions too tightly without reason
...
ISSUE Don't forget version ranges for major dependencies
...
| Pydantic v1 (ISSUE DEPRECATED) | Pydantic v2 (PASS USE THIS)             | Purpose           |
| ------------------------------- | ----------------------------------------- | --------------------- |
...
- ISSUE `Model.parse_obj()`
- ISSUE `model.dict()`
- ISSUE `model.json()`
- ISSUE `@validator`
- ISSUE `@root_validator`
- ISSUE `class Config:`

Use these instead:

- PASS `Model.model_validate()`
- PASS `model.model_dump()`
- PASS `model.model_dump_json()`
- PASS `@field_validator(..., mode='after')`
- PASS `@model_validator(mode='before')` or `mode='after'`
- PASS `model_config = ConfigDict(...)`
...
| ----------------- | ------ | -------- | ------- | ---------------------- | ------------------- |
| `api_url`         | string | PASS       |         | API base URL           | N/A                 |
| `api_key`         | string | PASS       |         | API authentication key | N/A                 |
