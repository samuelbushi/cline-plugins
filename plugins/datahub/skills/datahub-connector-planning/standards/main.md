...
This is the detailed reference guide for implementing DataHub connectors. For the workflow and step-by-step process, use the connector planning skill.
...
This section provides detailed guidance for planning your source implementation. Reference these sections as needed during connector planning.
...
```
PASS Tables/Views: INFORMATION_SCHEMA.TABLES
PASS Columns: INFORMATION_SCHEMA.COLUMNS
PASS View SQL: GET_DDL('VIEW', <view_name>)
PASS Lineage: SNOWFLAKE.ACCOUNT_USAGE.OBJECT_DEPENDENCIES (view-to-table)
PASS Usage: SNOWFLAKE.ACCOUNT_USAGE.QUERY_HISTORY (requires Enterprise Edition)
PASS Audit: SNOWFLAKE.ACCOUNT_USAGE.ACCESS_HISTORY (requires Enterprise Edition)
Warning: Limitation: ACCESS_HISTORY only available in Enterprise+ editions
```
...
- PASS Your source has a concept that's fundamentally different from existing subtypes
- PASS The subtype will be useful for filtering/searching in DataHub UI
- ISSUE Don't create subtypes for minor variations (use custom properties instead)
...
1. Audit Logs / Query History (STAR BEST - shows actual usage)
...
2. View Definitions + SQL Parsing (STAR GOOD - standard approach)
...
3. System Dependency Tables (Warning: FALLBACK - limited)
...
------------------------------------------------------------------------------------------
Table -> View (via SQL)    | PASS Yes     | View DDL + SQL parsing          | None
View -> Table (query logs) | Warning: Partial | ACCESS_HISTORY                  | Enterprise Edition only
S3 -> Table (COPY)         | PASS Yes     | COPY_HISTORY                    | Last 14 days only
Column-level              | PASS Yes     | Query parsing                   | Requires query text
Pipeline -> Table          | ISSUE No      | Not available                   | N/A
```

#### 5.3 SQL Parsing Pattern

Warning: IMPORTANT: Use `SqlParsingAggregator` instead of direct `sqlglot_lineage` calls.
...
Warning: CRITICAL: Configuration design is crucial for maintainability and consistency across DataHub sources. Follow these patterns to avoid config bloat and ensure usability.
...
1. PASS Is it essential? - Does it control a core feature or connection parameter?
2. PASS Is it reusable? - Will most users of this source need this option?
3. PASS Does a standard pattern exist? - Check if other sources have similar config (use same name!)
4. ISSUE Is it a one-off edge case? - Consider if it's too specific for general use
5. ISSUE Can it be auto-detected? - Prefer smart defaults over user configuration
6. ISSUE Is it for debugging only? - Use `HiddenFromDocs` or remove from public API
...
- PASS Use `_pattern` suffix for AllowDenyPattern
- PASS Use singular form (`table_pattern`, not `tables_pattern`)
- PASS Use common names where possible (`database`, `schema`, `table`)
- ISSUE Don't invent new names for standard entities

#### Time Windows

Important: CRITICAL: ALWAYS use DataHub's standard time window classes for usage and lineage extraction.

PASS CORRECT - Use BaseTimeWindowConfig or BaseUsageConfig:
...
ISSUE WRONG - Do NOT create custom time window fields:

```python
# ISSUE DON'T DO THIS - creates inconsistency with other sources
usage_lookback_days: int = Field(
    default=7,
    description="Number of days to look back for usage statistics"
)

# ISSUE DON'T DO THIS - reinvents the wheel
...
- PASS Consistent across ALL DataHub sources (Snowflake, BigQuery, Redshift, etc.)
- PASS Supports both relative time (`"-7 days"`) and absolute timestamps
- PASS Automatic validation and timezone handling
- PASS Time bucketing (DAY vs HOUR aggregation)
- PASS Integration with stateful ingestion via `StatefulTimeWindowConfigMixin`
- PASS Standard helper methods like `.buckets()` and `.majority_buckets()`
...
- PASS Debugging flags that shouldn't be used in production
- PASS Internal implementation details
- PASS Experimental options not ready for public use
- ISSUE Don't hide commonly needed options
- ISSUE Don't use as excuse for poor design
...
Warning: CRITICAL: DataHub uses Pydantic v2. Never use deprecated v1 methods.

#### Migration Quick Reference

| Pydantic v1 (ISSUE DEPRECATED) | Pydantic v2 (PASS USE THIS)             |
| ------------------------------- | ----------------------------------------- |
...
```python
# PASS Correct - Pydantic v2
config = MyConfig.model_validate(config_dict)

# ISSUE Wrong - Pydantic v1 (deprecated)
...
#### ISSUE DON'T: Create Platform-Specific Names for Common Concepts
...
#### ISSUE DON'T: Make Everything Configurable
...
#### ISSUE DON'T: Use Strings for Enums
...
- PASS Match source system terminology
- PASS Use `SecretStr` for sensitive data
- PASS Provide descriptions for all fields
- PASS Use `AllowDenyPattern` for filtering
- PASS Inherit from appropriate base config

### Implementation

- PASS Separate API client from source logic
- PASS Use structured reporting
- PASS Implement proper error handling
- PASS Support stateful ingestion
- PASS Implement `test_connection()`
- PASS Add comprehensive docstrings
- PASS Use DataHub SDK V2 (MANDATORY for new connectors) - See dedicated section below

### Important: CRITICAL: Use DataHub SDK V2 for All New Connectors
...
- PASS ALWAYS when creating a new connector from scratch
- PASS ALWAYS when adding new entity types to a connector
- Warning: OPTIONAL when making small bug fixes to existing connectors (can keep old approach for consistency within that connector)
- Warning: OPTIONAL when adding minor features to existing connectors using old approach
...
ISSUE WRONG - Old approach (don't use for new connectors):
...
- PASS Unit tests for configuration
- PASS Mock external APIs
- PASS Golden file tests for SQL sources
- PASS Integration tests with Docker
- PASS Test filtering and edge cases

### URNs

- PASS Use consistent naming
- PASS Match upstream source URNs for lineage
- PASS Include platform instance for multi-tenant
- PASS Use typed urn where possible -> https://github.com/datahub-project/datahub/tree/master/metadata-ingestion/src/datahub/utilities/urns

### Performance

- PASS Use pagination for large result sets
- PASS Implement request retry logic
- PASS Add rate limiting
- PASS Use connection pooling
- PASS Batch API requests when possible
