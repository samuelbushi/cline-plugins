...
## Important: CRITICAL REQUIREMENTS (Read First!)

Before implementing any DataHub connector, you MUST follow these requirements:

### 1. Important: ALWAYS Use DataHub SDK V2 (MANDATORY for New Connectors)
...
- PASS MANDATORY when creating a new connector from scratch
- PASS MANDATORY when adding new entity types to any connector
- PASS PREFERRED when refactoring existing connectors
- Warning: ACCEPTABLE to use MCP pattern (Gen 2) for bug fixes in existing MCP-based connectors
- ISSUE NEVER use MCE pattern (Gen 1) - it's deprecated
...
ISSUE WRONG - Old Approach (Don't Use!):
...
| ------------------ | --------------------------------------------- | ------------------------------------ |
| MCE (Gen 1)    | `MetadataChangeEvent` with bundled aspects    | ISSUE DEPRECATED                        |
| MCP (Gen 2)    | `MetadataChangeProposalWrapper.as_workunit()` | Warning: LEGACY (still in many connectors) |
| SDK V2 (Gen 3) | `datahub.sdk.Dataset.as_workunits()`          | PASS CURRENT STANDARD                  |
...
```python
# Warning: LEGACY - MCP style (acceptable for bug fixes in existing connectors)
...
### 2. Warning: Configuration SHOULD Be in Separate File (Strongly Recommended)
...
|-- <platform_name>_config.py    # Warning: REQUIRED - Config goes here!
```
...
ISSUE BAD - Unhelpful descriptions:
...
PASS GOOD - Helpful descriptions:
...
|-- <platform_name>.py               # Main source implementation
|-- <platform_name>_config.py        # Warning: REQUIRED: Config in separate file
...
```python
# PASS GOOD: Empty __init__.py (completely empty file)

# ISSUE BAD: Don't add unnecessary comments
# This file intentionally left blank.

# ISSUE BAD: Don't add docstrings to empty init files
...
```python
# ISSUE BAD: Don't put config in main source file
# myplatform.py
...
Warning: CRITICAL: All imports MUST be at the top of the file unless you have a strong reason.
...
ISSUE BAD: Don't use late imports without reason
...
### Warning: CRITICAL: Follow Established Patterns Only
...
```python
# ISSUE WRONG - Adding Pydantic configuration not used by other sources
...
```python
# PASS CORRECT - Follow existing patterns, trust base classes
class MyPlatformConfig(ConfigModel):
    api_url: str = Field(...)
```
...
class MyPlatformConfig(ConfigModel):
    # PASS GOOD - SecretStr for sensitive data
    password: SecretStr = Field(description="Database password")
    api_key: SecretStr = Field(description="API authentication key")
    token: SecretStr = Field(description="Access token")

    # ISSUE BAD - Plain string for passwords
...
class MyPlatformConfig(ConfigModel):
    # PASS GOOD - AllowDenyPattern with sensible defaults
    database_pattern: AllowDenyPattern = Field(
        default_factory=AllowDenyPattern.allow_all,
        description="Regex patterns to filter databases"
    )
...
class MyPlatformConfig(ConfigModel):
    ...
    # PASS GOOD - Single concern: port range validation
    ...
    # PASS GOOD - Single concern: SSL configuration consistency
    ...
    # ISSUE BAD - Multiple concerns in one validator
    @field_validator("port")
    @classmethod
...
class MyPlatformConfig(ConfigModel):
    ...
    # PASS GOOD - Deprecated field with migration helper
...
def validate_host_port(cls, v: str) -> str:
    if ":" not in v:
        # PASS GOOD - Actionable error message
        raise ValueError(
    ...
    try:
        ...
        # PASS GOOD - Specific error with example
        raise ValueError(
            f"Port must be a number, got '{port_str}' in '{v}'"
        )
    return v

    # ISSUE BAD - Unhelpful error message
...
| ---------------------------------------- | -------------- | --------------------------------- |
| `SecretStr` for secrets                  | Important: BLOCKER     | `password: SecretStr`             |
| `AllowDenyPattern` for filters           | WARNING Recommended | `table_pattern: AllowDenyPattern` |
| Single concern per validator             | WARNING Recommended | Separate validators for each rule |
| Actionable error messages                | WARNING Recommended | Include expected format in errors |
| `pydantic_removed_field` for deprecation | WARNING Recommended | Graceful field migration          |
...
ISSUE BAD - Don't write comments for:
...
Warning: CRITICAL: When you use external libraries, ALWAYS add them to `setup.py`.
...
1. Pin major and minor versions
   ...
   ```python
   # PASS GOOD: Specific version range
...
# ISSUE BAD: Manual string concatenation
...
2. Don't fail entire ingestion on single errors
   ...
   ```python
   # PASS GOOD: Continue processing other datasets
   ...
   # ISSUE BAD: Fail entire ingestion
   for dataset in datasets:
...
```python
# ISSUE BAD: Don't define report in main source file
# myplatform.py
class MyPlatformSource(Source):
...
def report_warning(
    self,
    message: LiteralString,           # Warning: MUST be constant string
    context: Optional[str] = None,    # Can be dynamic (f-strings OK)
    title: Optional[LiteralString] = None,  # Warning: MUST be constant string
...
### Important: BLOCKER: LiteralString Requirement for `title` and `message`
...
```python
# PASS CORRECT - Constant strings for title and message, dynamic value in context
self.report.warning(
...
# PASS CORRECT - Entity identifier in context
self.report.warning(
...
# PASS CORRECT - Multiple dynamic values in context
self.report.warning(
...
| --------- | --------------- | -------------------------------------- | --------------------------- |
| `title`   | `LiteralString` | Short error category (2-5 words)       | ISSUE NO - must be constant    |
| `message` | `LiteralString` | Detailed explanation + fix suggestions | ISSUE NO - must be constant    |
| `context` | `str`           | Resource identifier, error details     | PASS YES - use f-strings here |
...
# ISSUE BAD messages - Not actionable
...
try:
    ...
    self.report.warning(
        ...
        exc=e,  # PASS Always include exception
    )
```
...
for table in tables:
    try:
        ...
        continue  # PASS Continue with next item
```
...
try:
    ...
    return None  # PASS Stop processing
```
...
8. Be specific in messages
   - PASS "Unable to parse view definition. Lineage will not be extracted."
   - ISSUE "Parse error"

9. Suggest solutions
   - PASS "Please verify your credentials have sufficient permissions"
...
- `self.report.warning()` - Recoverable issue, continue processing PASS
- `self.report.failure()` - Critical error, stop processing PASS
