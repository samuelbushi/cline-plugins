...
```python
# PASS Simple and correct - start with this
...
```python
# PASS Optimized - bulk fetch then filter in memory
def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
    for schema in self._get_schemas():
...
```python
# ISSUE N+1 pattern - one query per ID
for table_id in table_ids:
    details = api.get_table_details(table_id)  # N queries!

# PASS Batch pattern - one query for all IDs
...
```python
# PASS GOOD - Streaming with generators
def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
    for table in self._get_tables():
        yield self._create_workunit(table)  # Memory released after processing

# ISSUE BAD - Loading all into memory first
def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
    all_workunits = []
    for table in self._get_tables():
...
```python
# PASS Simple sequential approach - start here
def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
    for schema in self._get_schemas():
...
```python
# PASS Thread-safe: Each call creates its own connection
def _get_tables(self, schema: str) -> List[TableInfo]:
    with self._create_connection() as conn:
        return conn.query(f"SELECT * FROM tables WHERE schema = '{schema}'")

# ISSUE Not thread-safe: Shared connection
def _get_tables(self, schema: str) -> List[TableInfo]:
...
# PASS Batch request (if API supports it)
