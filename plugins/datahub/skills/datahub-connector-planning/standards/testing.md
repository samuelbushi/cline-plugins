...
ALERT TOTAL AND ABSOLUTE BLOCKER: Trivial tests are an IMMEDIATE REJECTION trigger. No exceptions. No appeals. No "but it's just one test".

Important: ABSOLUTE BLOCKER: Testing is THE primary indicator of code quality. Tests must verify BUSINESS LOGIC, not configuration defaults or trivial getters.
...
# ISSUE BAD - unittest style (DO NOT USE)
import unittest

class TestMyPlatformSource(unittest.TestCase):  # ISSUE Don't use TestCase
    def test_extracts_tables(self):
        source = MyPlatformSource(config, ctx)
        tables = list(source.get_tables())
        self.assertEqual(len(tables), 5)  # ISSUE Don't use self.assertEqual
```
...
```python
# PASS GOOD - assert statements
assert result == expected
assert len(items) == 5
assert "error" in message.lower()
assert table.name == "users"

# ISSUE BAD - unittest assertions
...
```
# PASS GOOD - pytest can discover these
test_myplatform.py
test_myplatform_config.py
test_myplatform_lineage.py

# ISSUE BAD - pytest won't discover these
myplatform_test.py
myplatform_tests.py
tests_myplatform.py
```
...
# PASS GOOD - No class at all (module-level functions)
def test_extracts_tables():
    ...
...
# ISSUE BAD - unittest.TestCase
class TestMyPlatformSource(unittest.TestCase):  # ISSUE Don't inherit TestCase
    def test_extracts_tables(self):
        ...
```
...
| --------------------------- | ---------- | ----------------------- |
| Tests in `tests/` directory | Important: BLOCKER | NOT in `src/`           |
| Use pytest                  | Important: BLOCKER | No unittest imports     |
| Use `assert` statements     | Important: BLOCKER | No `self.assertEqual()` |
| Files named `test_*.py`     | Important: BLOCKER | pytest discovery        |
| Functions named `test_*`    | Important: BLOCKER | pytest discovery        |
| No `unittest.TestCase`      | Important: BLOCKER | Regular classes         |

---

## Important: MANDATORY: Use time-machine, NOT freezegun

DataHub uses `time-machine` for time mocking, NOT `freezegun`.

ISSUE DO NOT USE freezegun:

```python
# ISSUE WRONG - Do not use freezegun
...
PASS USE time-machine:

```python
# PASS CORRECT - Use time-machine
import time_machine
...
### Warning: CRITICAL: What Makes a Good Test

Good tests verify BUSINESS LOGIC and BEHAVIOR:

- PASS Data transformation and mapping logic
- PASS Error handling and recovery
- PASS Filtering and pattern matching behavior
- PASS API response parsing and edge cases
- PASS Lineage extraction logic (see "Testing Lineage" section below)
- PASS Type conversion and custom type handling
- PASS Complex configuration interactions

Bad tests verify TRIVIAL or OBVIOUS behavior:

- ISSUE Testing default configuration values
- ISSUE Testing simple getters/setters
- ISSUE Testing that a class can be instantiated
- ISSUE Testing framework behavior (SQLAlchemy, pydantic)
- ISSUE Testing type registration without verification
- ISSUE Testing obvious equality (platform == "platform")

---

## Important: ABSOLUTE BLOCKERS: Tests That Must NOT Be Written
...
ISSUE BAD - Imports Hidden Inside Functions:
...
def test_session_handle_extracted(self, mock_session_class):
    from datahub.ingestion.source.flink.flink_sql_gateway_client import (
        FlinkSQLGatewayClient,  # ISSUE Import inside function
    )
    from datahub.ingestion.api.common import PipelineContext  # ISSUE Import inside function
...
PASS GOOD - All Imports at Top of File:
...
PASS GOOD - Test Business Logic Instead:
...
ISSUE BAD - Useless Test - WILL BE REJECTED:

```python
def test_platform_correctly_set_doris():
    source = DorisSource(ctx=PipelineContext(run_id="test"), config=DorisConfig())
    assert source.platform == "doris"  # ISSUE Just tests get_platform() returns hardcoded string
```
...
PASS GOOD - Test Platform Usage in Context:
...
PASS GOOD - Test Type Handling in Real Scenario:
...
ISSUE BAD - Testing Pydantic:

```python
def test_config_has_correct_fields():
    config = DorisConfig(host_port="localhost:9030")
    assert hasattr(config, "host_port")  # ISSUE Tests pydantic, not your code
    assert hasattr(config, "username")
```

PASS GOOD - Test Configuration Validation Logic:
...
ISSUE BAD - Testing AllowDenyPattern in Connector Tests:
...
class TestMyConnectorPatternFiltering:
    ...
    def test_catalog_pattern_allows_matching(self):
        ...
        assert config.catalog_pattern.allowed("prod_catalog")  # ISSUE Tests AllowDenyPattern, not your connector

    def test_database_pattern_deny_takes_precedence(self):
        config = MyConnectorConfig(database_pattern={"allow": [".*"], "deny": ["^test_.*"]})
        assert not config.database_pattern.allowed("test_db")  # ISSUE Tests shared utility
```
...
WARNING MINOR REQUIREMENT (Unit Tests): Connectors that extract lineage SHOULD have unit tests verifying lineage registration logic.

Important: ABSOLUTE BLOCKER (Integration Tests): Lineage MUST be comprehensively tested in integration tests with real data and golden file validation. See "Integration Tests" section below.
...
PASS GOOD - Test that schemas and views are registered correctly:
...
Important: ABSOLUTE BLOCKER: Integration tests must verify END-TO-END behavior with real database/API.

Code with incomplete or missing integration tests WILL BE REJECTED. This is non-negotiable.

### Important: ABSOLUTE BLOCKER: Comprehensive Test Data Requirements
...
ISSUE BAD - Empty or Minimal Test Data (WILL BE REJECTED):
...
Important: THIS IS AN ABSOLUTE BLOCKER - Code will be REJECTED if integration tests are this minimal.

PASS GOOD - Comprehensive Test Data:
...
1. Configure test environment to enable comprehensive testing:
   - PASS Test setup must support testing ALL features the connector implements
   - PASS Test data must be sufficient to validate metadata extraction (tables, views, schemas, lineage)
   - PASS Test environment must be configured appropriately for the source type (e.g., SQL databases may need volumes, APIs may need fixtures)
   - PASS Test data must be deterministic (same results on repeated runs)

2. Create comprehensive test data covering:
   - PASS Multiple tables (at least 3-5 for variety)
   - PASS Various column types (including platform-specific custom types)
   - PASS Views (if connector supports view lineage)
   - PASS Foreign keys/relationships (if applicable)
   - PASS Edge cases (special characters in names, NULL columns, etc.)

3. Ensure test determinism:
   - PASS Running test multiple times produces identical results
   - PASS Test data is immutable and consistent
   - PASS Golden file tests produce consistent results

4. Match production scenarios:
   - PASS Use realistic table structures
   - PASS Use realistic view definitions
   - PASS Test configurations users will actually use
...
#### Important: ABSOLUTE BLOCKER: Verification Checklist Before Claiming "Tests Pass"
...
Important: ABSOLUTE BLOCKER - Rule of thumb: If your golden file is < 5KB or has < 20 metadata events, your integration test is INCOMPLETE and code will be REJECTED.

### Important: MANDATORY: Self-Evaluation of Integration Tests
...
- PASS YES - Test setup allows extracting all metadata types I'm testing (tables, views, schemas, lineage, etc.)
- ISSUE NO - Test environment is missing configuration or data needed for full testing -> ABSOLUTE BLOCKER - MUST FIX
- ISSUE UNSURE - Check! Does the golden file contain all the metadata types the connector is supposed to extract?
...
- ISSUE File only contains container entities (catalogs, databases)
- ISSUE No dataset (table/view) entities with schemas
- ISSUE All events are `containerProperties` or `status` aspects
- ISSUE No `schemaMetadata` aspects
- ISSUE No `upstreamLineage` aspects (if connector supports lineage)
...
- PASS GOOD: Multiple tables with different column types (INT, VARCHAR, TIMESTAMP, custom types)
- PASS GOOD: Views that reference tables (for lineage testing)
- PASS GOOD: Tables with foreign keys, constraints, indexes
- PASS GOOD: Edge cases (special characters in names, NULL columns)
- ISSUE BAD: One table with one column
- ISSUE BAD: No views (if connector supports lineage)
- ISSUE BAD: Only simple types (no custom types if platform has them)
...
- ISSUE Timestamps in output change between runs -> Use `@freeze_time`
- ISSUE Random order of entities -> Sort entities consistently
- ISSUE Run IDs change -> Use fixed test run ID
- ISSUE Different results on second run -> Test data is not persisting properly
...
### Warning: Common Self-Deception Patterns (Watch Out For These!)
...
Important: NEVER create custom scripts for generating golden files. The `--update-golden-files` flag ensures:
...
Important: ABSOLUTE BLOCKER: Lineage Integration Tests
...
Important: ABSOLUTE BLOCKER: Cross-Source Lineage Tests Must Use REAL Ingestion Output
...
ISSUE BAD - Fabricated Test Data (TESTS NOTHING - WILL BE REJECTED):
...
def test_kafka_to_flink_lineage(kafka_mcps, flink_mcps):
    ...
    assert some_urn in kafka_mcps  # ISSUE Circular logic
```
...
PASS GOOD - Test With Real Ingestion Output:
...
- PASS All data transformation logic
- PASS All error handling paths
- PASS All filtering/pattern matching
- PASS All configuration validation
- PASS All custom type handling
- PASS All edge cases

Integration Tests (must exist):

- PASS Golden file test (end-to-end)
- PASS Connection test (valid credentials)
- PASS Connection test (invalid credentials)
- PASS Feature-specific tests (lineage, profiling, custom types)

What NOT to Count Toward Coverage:

- ISSUE Tests of default values
- ISSUE Tests of trivial getters
- ISSUE Tests of framework behavior
- ISSUE Tests that only verify mocks were called
...
## Important: Expected Aspects - Golden File Verification
...
## Important: Final Checklist: Before Claiming Tests Are Complete
...
PASS You can confidently say: "This test would catch real bugs in schema extraction, type mapping, and lineage"

PASS Your golden file is comparable in size/content to similar DataHub sources

PASS You've performed the self-evaluation and answered all 7 questions honestly

PASS Your test data persists across container restarts

PASS Running the test multiple times produces identical results

PASS You have ZERO trivial or anti-pattern tests
