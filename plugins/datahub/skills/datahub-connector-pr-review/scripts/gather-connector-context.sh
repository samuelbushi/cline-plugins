...
if [[ -d "$SOURCE_DIR" ]]; then
    ...
    if [[ -n "$FOUND" ]]; then
        ...
        if [[ -n "$FOUND" ]]; then
            echo "Location: Found file: $FOUND"
            IS_SINGLE_FILE=true
            SOURCE_FILE="$FOUND"
            ...
            FOUND=$(find "$SOURCE_BASE" -type f -name "*${CONNECTOR}*.py" 2>/dev/null | head -1)
            if [[ -n "$FOUND" ]]; then
                echo "Location: Found file (partial match): $FOUND"
                IS_SINGLE_FILE=true
                SOURCE_FILE="$FOUND"
                SOURCE_DIR="$(dirname "$FOUND")"
                SOURCE_FILES=("$SOURCE_FILE")
...
echo "Architecture:  BASE CLASS & INHERITANCE"
echo "-----------------------------------------------------------------"
if [[ ${#SOURCE_FILES[@]} -gt 0 ]]; then
...
if [[ -n "$GOLDEN_FILES" && -f "$EXTRACT_SCRIPT" ]]; then
    echo "Report: GOLDEN FILE ANALYSIS"
    echo "-----------------------------------------------------------------"
    echo "$GOLDEN_FILES" | while read -r gf; do
        if [[ -f "$gf" ]]; then
            echo ""
...
for aspect in checklist:
    status = 'yes' if aspect in aspects else 'no'
