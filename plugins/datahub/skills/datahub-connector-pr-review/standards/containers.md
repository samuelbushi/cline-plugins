...
## Important: CRITICAL: Two Approved Patterns
...
ISSUE DO NOT manually emit container aspects - this is incorrect and will cause issues.
...
- PASS SQL database sources (databases, schemas)
- PASS API-based sources with hierarchies
- PASS File-based sources (folders, buckets)
- PASS Any source extending `StatefulIngestionSourceBase` or `Source`
- PASS When you need fine-grained control over container aspects
...
def _emit_schema_container(
    ...
    yield from gen_containers(
        ...
        parent_container_key=database_key,  # PASS Links to parent
        description=description,
    )
```
...
## Warning: Critical: Emit Parents Before Children (Topological Order)
...
- PASS Newer sources that prefer SDK style
- PASS Sources that need a more object-oriented approach
- PASS When building entities programmatically
- PASS When you want automatic parent-child relationship handling
...
## ISSUE ANTI-PATTERN: Manual Container Aspect Emission (WRONG!)

DO NOT do this - manually emitting container aspects is incorrect:

```python
# ISSUE WRONG - Don't manually emit ContainerPropertiesClass
...
def _emit_organization_container(self, org_id: str):
    ...
    # ISSUE WRONG - Manually creating ContainerPropertiesClass
    ...
    # ISSUE WRONG - Manually emitting container properties
    yield MetadataChangeProposalWrapper(
        entityUrn=org_urn,
        aspect=container_properties,
    ).as_workunit()

    # ISSUE WRONG - Manually emitting status
    yield MetadataChangeProposalWrapper(
        entityUrn=org_urn,
...
PASS CORRECT - Use `gen_containers()` instead:
...
def _emit_organization_container(self, org_id: str):
    ...
    # PASS CORRECT - Use gen_containers()
    yield from gen_containers(
...
```python
# PASS CORRECT - Emit container first
yield from self._emit_schema_container(database, schema)
yield from self._emit_table(database, schema, table)

# ISSUE WRONG - Emit dataset before container
yield from self._emit_table(database, schema, table)
yield from self._emit_schema_container(database, schema)
```
...
```python
# PASS CORRECT - Consistent key construction
...
yield from gen_containers(
    ...
    name=database,
    sub_types=[DatasetContainerSubTypes.DATABASE],
    external_url=f"https://platform.com/database/{database}",  # PASS Include URL
)
```
...
- `src/datahub/ingestion/source/snowplow/snowplow.py` - ISSUE Manually emits `ContainerPropertiesClass` (incorrect)
