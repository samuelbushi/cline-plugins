...
| `datasetProperties`    | PASS ALWAYS              | Topic name, qualified name, custom properties (partitions, replication) |
| `schemaMetadata`       | PASS IF SCHEMA AVAILABLE | Schema from Schema Registry (Avro, Protobuf, JSON Schema)               |
| `subTypes`             | PASS ALWAYS              | Identifies entity subtype (Topic, Stream)                               |
| `container`            | PASS IF HIERARCHICAL     | Links to parent container (cluster, namespace)                          |
...
| ---------------------- | --------- | ----------------------------------------------------- |
| `dataPlatformInstance` | PASS ALWAYS | Links entity to data platform. Always required.   |
| `dataFlowInfo`         | PASS ALWAYS | Pipeline metadata (Kafka Connect connector, KSQL app) |
| `dataJobInfo`          | PASS ALWAYS | Job/task metadata                                     |
| `dataJobInputOutput`   | PASS ALWAYS | Input/output topic relationships                      |
| `status`               | AUTO AUTO   | Auto-generated for all entities                       |
