...
| `containerProperties`  | PASS ALWAYS             | For workspaces/projects/folders                            |
| `subTypes`             | PASS ALWAYS             | Identifies entity subtype (Dashboard, Chart, Report)       |
| `container`            | PASS ALWAYS             | Links entities to parent containers                        |
| `dashboardInfo`        | PASS IF DASHBOARD       | Dashboard metadata (required for all dashboards)           |
| `chartInfo`            | PASS IF CHART           | Chart metadata (required for all charts)                   |
| `ownership`            | PASS IF SOURCE PROVIDES | Dashboard/chart owners (required if source exposes owners) |
...
| ---------------------- | --------------------- | ----------------------------------------------------------- |
| `dataPlatformInstance` | PASS ALWAYS             | Links entity to data platform. Always required.         |
| `dataFlowInfo`         | PASS ALWAYS             | Pipeline/flow metadata                                      |
| `dataJobInfo`          | PASS ALWAYS             | Job/task metadata                                           |
| `dataJobInputOutput`   | PASS ALWAYS             | Job input/output relationships (datasets and upstream jobs) |
| `globalTags`           | PASS IF SOURCE PROVIDES | Tags from source system (required if source has tags)       |
| `ownership`            | PASS IF SOURCE PROVIDES | Pipeline owners (required if source exposes owners)         |
