...
| `mlModelProperties`      | PASS ALWAYS             | Model metadata (name, description, version, hyperparameters) |
| `mlModelGroupProperties` | PASS IF MODEL GROUP     | Model group/registry metadata                                |
| `ownership`              | PASS IF SOURCE PROVIDES | Model owners (required if source exposes owners)             |
| `globalTags`             | PASS IF SOURCE PROVIDES | Tags from source system                                      |
| `container`              | PASS IF HIERARCHICAL    | Links to parent container (experiment, project)              |
...
| -------------------------- | --------------------- | --------------------------------------------------- |
| `dataPlatformInstance`     | PASS ALWAYS             | Links entity to data platform. Always required. |
| `mlFeatureTableProperties` | PASS ALWAYS             | Feature table metadata                              |
| `mlFeatureProperties`      | PASS FOR FEATURES       | Individual feature metadata                         |
| `upstreamLineage`          | PASS IF SOURCE PROVIDES | Link to source datasets                             |
| `status`                   | AUTO AUTO               | Auto-generated for all entities                     |
