...
| `datasetProperties`    | PASS ALWAYS             | Collection/table name, qualified name, custom properties |
| `schemaMetadata`       | PASS IF SCHEMA INFERRED | Schema inferred from sampling or schema registry         |
| `subTypes`             | PASS ALWAYS             | Identifies entity subtype (Collection, Table, Index)     |
| `container`            | PASS IF HIERARCHICAL    | Links to parent container (database, keyspace)           |
| `containerProperties`  | PASS FOR CONTAINERS     | Database/keyspace metadata                               |
