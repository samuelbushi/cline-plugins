...
```python
# PASS GOOD - Structured dataclass
...
```python
# ISSUE BAD - Global mutable state
...
# PASS GOOD - Instance-level state
class MySource:
    def __init__(self):
...
# ISSUE BAD - Positional arguments (hard to read)
...
```python
# PASS GOOD - Empty __init__.py (most cases)
# __init__.py is empty

# PASS GOOD - Only export main source class if needed
# __init__.py
from datahub.ingestion.source.myplatform.myplatform import MyPlatformSource

# ISSUE BAD - Re-exporting everything
# __init__.py
...
### Important: BLOCKER: All Code Must Pass Mypy
...
# ISSUE WRONG - Missing type hints
def get_tables(self, schema_name):
    ...
```

### Important: BLOCKER: Avoid `Any` Type Unless Justified
...
# PASS ACCEPTABLE - Any with justification
...
# ISSUE WRONG - Lazy use of Any
def process_data(self, data: Any) -> Any:  # What types are these?
    ...
```

### Important: BLOCKER: Use `isinstance()` Instead of `cast()`
...
# PASS CORRECT - isinstance() for type narrowing
...
# PASS CORRECT - isinstance() with multiple types
...
```python
# PASS CORRECT - Specific ignore with justification
result = untyped_library.get_data()  # type: ignore[no-untyped-call]  # Third-party lib lacks stubs

# PASS CORRECT - Documented unavoidable ignore
...
# ISSUE WRONG - Blanket ignore without reason
result = some_function()  # type: ignore

# ISSUE WRONG - Ignoring to avoid fixing actual bug
...
| Rule                          | Required       | Check                                            |
| ----------------------------- | -------------- | ------------------------------------------------ |
| `ruff format` passes          | Important: BLOCKER     | `ruff format --check .`                          |
| `mypy` passes                 | Important: BLOCKER     | `mypy src/datahub/ingestion/source/myconnector/` |
| Names match source system     | WARNING Recommended | Manual review                                    |
| Use dataclasses for data      | WARNING Recommended | No tuple/dict returns                            |
| Flat inheritance              | WARNING Recommended | Max 2-3 levels                                   |
| No global state               | Important: BLOCKER     | No module-level mutables                         |
| Named arguments for 3+ params | WARNING Recommended | Manual review                                    |

### Type Safety Rules

| Rule                                | Required   | Example                                                                                           |
| ----------------------------------- | ---------- | ------------------------------------------------------------------------------------------------- |
| Type hints on public methods        | Important: BLOCKER | `def get_tables(self) -> List[Table]:`                                                            |
| Avoid `Any` without justification   | Important: BLOCKER | Use specific types or document why `Any` is needed                                                |
| Use `isinstance()` not `cast()`     | Important: BLOCKER | `if isinstance(x, str): ...`                                                                      |
| Mypy ignores need justification     | Important: BLOCKER | `# type: ignore[specific-code]  # reason`                                                         |
| Pass mypy with no errors            | Important: BLOCKER | `mypy src/datahub/ingestion/source/myconnector/`                                                  |
| `LiteralString` for report messages | Important: BLOCKER | See [Error Reporting Guide](patterns.md#-blocker-literalstring-requirement-for-title-and-message) |

### Important: BLOCKER: LiteralString in Error Reporting
...
```python
# PASS CORRECT - Constants for title/message, dynamic value in context
self.report.warning(
