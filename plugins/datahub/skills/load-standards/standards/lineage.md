...
### Important: CRITICAL: Query Log Lineage vs View Lineage
...
#### 2. Query Log Lineage (Operational) STAR DON'T FORGET THIS
...
PASS Best Practice: Implement BOTH types of lineage for complete coverage:
...
Warning: Common Mistake: Implementing only view lineage and forgetting query log lineage.
...
#### WARNING MAJOR ISSUE: Use SqlParsingAggregator for Lineage Generation
...
PASS PREFERRED: Use SqlParsingAggregator
...
Warning: ACCEPTABLE BUT NOT PREFERRED: Direct sqlglot_lineage() calls
...
- PASS SQL Databases - PostgreSQL, MySQL, SQL Server, Oracle, etc. (via `SQLAlchemySource`)
- PASS Data Warehouses - Snowflake, Redshift, BigQuery (view definitions and query logs)
- PASS Query Engines - Dremio, Trino, Hive (view definitions)
- PASS BI Tools - Tableau (Custom SQL), Looker (LookML SQL), Power BI (M Query -> SQL), Superset, Metabase, Mode
- PASS Transformation Tools - dbt (model SQL), Airflow (SQL operators)
- PASS Streaming Platforms - Kafka Connect (SQL-based connectors)
...
class MySource(StatefulIngestionSourceBase):
    ...
    def _emit_table(self, table) -> Iterable[MetadataWorkUnit]:
        ...
        # STAR Add to resolver so SQL parsing can resolve columns later
