...
Not for debugging a specific wrong answer -- use the `aws-agents-debug` skill for that.

## Input

`the user request` is optional. If provided, it scopes the skill:
...
if awk -v s="$score" -v t="$THRESHOLD" 'BEGIN{exit !(s<t)}'; then
  echo "Wrong Quality gate FAILED: score $score < $THRESHOLD"
  exit 1
fi
