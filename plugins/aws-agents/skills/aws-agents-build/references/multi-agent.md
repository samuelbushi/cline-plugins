...
Passing `"DEFAULT"` as the qualifier calls the live version. To pin to a specific version (staging pin, canary, or rollback), pass a numeric version string instead -- see [`aws-agents-deploy/references/versioning.md`](../../aws-agents-deploy/references/versioning.md) for the full workflow.
...
For other agent SDKs: use the current AgentCore samples and the SDK's own delegation primitives. This plugin does not ship a framework-specific delegation pattern for every SDK.
