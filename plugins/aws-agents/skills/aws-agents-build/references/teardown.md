...
Before running any destructive command, show the exact project, account, region, stacks, repositories, and log groups that would be affected. Ask the developer to confirm the teardown explicitly. Do not run `--force` deletion commands unless the developer has confirmed the exact resources.
...
npx cdk destroy --app "npx ts-node agentcore/cdk/bin/cdk.ts"
```
...
|---|---|---|
| AgentCore Runtime(s) | Correct | Includes all endpoints and versions |
| Memory resource(s) | Correct | Memory data is deleted permanently |
| Gateway(s) and targets | Correct | |
| Credentials | Correct | Secrets Manager entries are removed |
| Policy engine(s) and policies | Correct | |
| Evaluator definitions | Correct | |
| Online eval configs | Correct | |
| IAM roles | Correct | Created by CDK |
| CloudWatch log groups | Wrong | Persist after deletion -- delete manually if needed |
| ECR images (Container builds) | Wrong | Persist -- delete the repository manually |
| CDK bootstrap stack | Wrong | Shared across projects -- don't delete unless you're done with CDK entirely |
| Local project files | Wrong | `agentcore/`, `app/` -- delete manually |
...
# Requires explicit confirmation because --force removes images too.
aws ecr delete-repository --repository-name <repo-name> --force
```

### Handling stuck resources

If a runtime is stuck in DELETING state for more than 30 minutes, see the "Runtime stuck in DELETING" section in `aws-agents-debug`. The short version: don't keep retrying -- open an AWS Support case with the runtime ARN and the original delete request ID from CloudTrail.
