...
- Calling an API -- use Gateway (`aws-agents-connect`)
...
Credential handling: the Nova Act API key is a secret. If this is running inside an AgentCore Runtime agent, register it as a credential with the interactive prompt (`agentcore add credential --name NovaAct --type api-key`) and retrieve it with `@requires_api_key(provider_name="NovaAct")`. Do not put it in runtime env vars or paste it into shell commands. See `aws-agents-connect` Path D.
