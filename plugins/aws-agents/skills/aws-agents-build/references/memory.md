...
`the user request` is optional. If provided, use it as the memory resource name:
...
| Google ADK / custom agent SDK | BYO -- use `MemoryClient` directly | Validate end-to-end before shipping |
...
#### Google ADK and other agent SDKs -- bring your own memory integration

AgentCore Memory doesn't have a framework-specific integration for ADK or custom agent SDKs yet, and the samples repo doesn't contain a combined pattern we can point to. Use the general `MemoryClient` API and wire it into the framework's existing extension points:

- Google ADK: Expose memory operations as ADK tools (functions passed to `Agent(tools=[...])`). The ADK agent decides when to call them.
- Custom agent SDK: Wrap `query()` with a pre-call memory load and a post-call memory save. Use the SDK's system prompt or pre-call context hook as the injection point for retrieved context.
...
Before shipping a memory integration for ADK or another agent SDK, validate the end-to-end flow against a deployed agent:
