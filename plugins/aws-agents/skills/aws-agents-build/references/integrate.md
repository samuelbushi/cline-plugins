...
- Giving the agent tools to call external APIs -> use `aws-agents-connect`
- Deploying the agent -> use `aws-agents-deploy`
- Debugging agent responses -> use `aws-agents-debug`
- Securing the agent endpoint for production -> use `aws-agents-harden` (but this skill covers the client-side auth code)
...
If not deployed: "Your agent needs to be deployed before you can call it from an app. Run `agentcore deploy` first, or use the `aws-agents-deploy` skill for guidance."
...
Based on the developer's language preference (from `the user request` or ask), generate the appropriate client code.
