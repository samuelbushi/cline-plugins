...
This routes 10% of traffic to the new version. Monitor `aws-agents-optimize` eval scores and error rates before increasing the weight.
...
- If a rollback is needed because of a specific failure, use `aws-agents-debug` to diagnose first
- For staging/production best practices, see `aws-agents-harden`
- For running evals against a specific version before cutover, see [`aws-agents-optimize/references/evals.md`](../../aws-agents-optimize/references/evals.md)
