...
description: >
  ...
  Not for production hardening -- use aws-agents-harden. Not for adding
  capabilities before deploy -- use aws-agents-build or aws-agents-connect.
  Not for VPC configuration errors -- use aws-agents-build.
---
...
```
aws-agents-deploy                     # interactive -- pre-flight check or diagnose failure
aws-agents-deploy preflight           # validate config and IAM before deploying
aws-agents-deploy diagnose            # diagnose a failed deploy (paste error or read logs)
aws-agents-deploy preview             # show what deploy will create without deploying
aws-agents-deploy rollback            # roll back to a previous version
```
...
Before running any checks, explain which commands access the AWS account or preview deployment changes and ask for explicit approval. Then run these checks before `agentcore deploy`:
...
```bash
agentcore deploy
agentcore deploy -v       # verbose -- shows resource-level events
agentcore deploy --target staging   # deploy to a specific target
```
...
```bash
agentcore deploy --target staging
