...
description: >
  ...
  "add tools to agent".
  Not for inbound auth (who can call your agent) -- use aws-agents-harden.
  Not for debugging agent behavior -- use aws-agents-debug.
  Not for VPC networking errors (agent can't reach APIs due to VPC) -- use
  aws-agents-build. Not for creating or hosting a new MCP server project -- use
...
| Calling another agent via A2A | A2A is HTTP-by-design and has its own auth model | [`aws-agents-build/references/multi-agent.md`](../aws-agents-build/references/multi-agent.md), not a Gateway target |
...
The app signs the invocation with IAM SigV4 or presents a JWT. See [`aws-agents-build/references/integrate.md`](../aws-agents-build/references/integrate.md) for the app-side patterns.
...
- "My FastAPI app needs to call my agent" -> app is calling the agent -> direct invocation (not Gateway; use [`aws-agents-build/references/integrate.md`](../aws-agents-build/references/integrate.md))
...
# Omit the client secret flag so the CLI prompts interactively.
agentcore add gateway-target \
  --type mcp-server \
  --name WeatherTools \
  --endpoint https://mcp.example.com/mcp \
...
```bash
# API key
agentcore add credential --name OpenAI --type api-key
...
MCP clients that expect OAuth auto-discovery cannot auto-connect to Gateway
AgentCore Gateway does not currently implement the MCP OAuth spec endpoints (RFC 8414 OAuth Authorization Server Metadata, RFC 7591 Dynamic Client Registration). MCP clients that expect to auto-discover OAuth config and register themselves -- MCP clients that expect OAuth auto-discovery -- cannot connect without manual credential configuration. The workaround is to manually obtain the Cognito `client_id` and `client_secret` and enter them in the MCP client's advanced settings. This is a platform limitation, not a config error.
