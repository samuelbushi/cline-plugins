...
|---|---|
| "Let it look up orders in our database" | `aws-agents-connect` (add a gateway target for the order API) |
| "Remember the customer's name between sessions" | `aws-agents-build` (loads [`references/memory.md`](../../aws-agents-build/references/memory.md)) |
| "Make sure it can't say anything off-policy" | `aws-agents-connect` (loads [`references/policy.md`](../../aws-agents-connect/references/policy.md)) |
| "Put it on our website" | `aws-agents-build` (loads [`references/integrate.md`](../../aws-agents-build/references/integrate.md)) |
| "Know if it's actually helpful" | `aws-agents-optimize` |
...
For cost-sensitive use cases, consider Bedrock Nova models (e.g., `amazon.nova-micro-v1:0`, `amazon.nova-lite-v1:0`) -- significantly cheaper than Claude for simpler extractive tasks. See [`aws-agents-optimize/references/cost.md`](../../aws-agents-optimize/references/cost.md) for model selection guidance.

For a chatbot that remembers conversations, add `--memory longAndShortTerm` during scaffolding. Memory can also be added later -- see [`aws-agents-build/references/memory.md`](../../aws-agents-build/references/memory.md).
