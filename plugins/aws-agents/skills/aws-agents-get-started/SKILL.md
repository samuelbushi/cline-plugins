...
description: >
  ...
  Not for adding capabilities to existing projects -- use aws-agents-build
  or aws-agents-connect. Strands vs LangGraph in a migration context routes
  to aws-agents-build, not here. Connecting to an existing MCP server routes
  to aws-agents-connect, not here.
---
...
- Environment/prerequisite issues (CLI not found, credentials broken) -> use `aws-agents-debug`
- Adding capabilities to an existing project (memory, tools, policies) -> use `aws-agents-build` or `aws-agents-connect`
- Migrating an existing Bedrock Agent -> use `aws-agents-build` (loads [`references/migrate.md`](../aws-agents-build/references/migrate.md))
...
> If you're having trouble with installation, I can run the `aws-agents-debug` skill (which loads [`references/doctor.md`](../aws-agents-debug/references/doctor.md)) to diagnose your environment.
...
If the developer's intent is clear from `the user request` (e.g., "create a Strands agent called MyBot"), skip straight to Step 3.
...
| Provider SDK / custom agent SDK | This is a model SDK, not an agent framework. You can use it inside any framework (Strands, LangGraph, etc.) or standalone. For standalone use, wrap it in a container with the Runtime contract. |
| Coding assistants and IDE tools | These are IDE tools, not agent frameworks. They're where you *write* agent code, not what you deploy. Pick a framework from the table above for the agent itself. |
...
- Memory: Start with `none` -- memory can be added later via `aws-agents-build` (loads [`references/memory.md`](../aws-agents-build/references/memory.md)) when the developer needs it
...
|   +-- cdk/                <- CDK infrastructure (auto-managed, do not edit)
+-- app/
    +-- <AgentName>/
...
1. It will become a silent no-op if you repoint it at a gateway that isn't deployed yet. The common path is to swap the example endpoint for `os.getenv("AGENTCORE_GATEWAY_<NAME>_URL")`. That env var is only populated after `agentcore deploy`. If the developer repoints and runs `agentcore dev` before deploying, `get_streamable_http_mcp_client()` returns a client with a `None` URL and the agent starts with zero MCP tools -- no error, no warning. See the "Local dev gap" section in `aws-agents-connect` for the guard pattern: `if not GATEWAY_URL: tools = []`.
...
The reference client code in `aws-agents-connect` (Path A) shows the correct pattern for gateway-backed MCP clients once deploy has run.
...
|---|---|---|
| "How do I call it from my app?" | `aws-agents-build` | `agentcore fetch access` |
| "I want it to remember things" | `aws-agents-build` | `agentcore add memory` |
| "I want it to call external APIs" | `aws-agents-connect` | `agentcore add gateway` |
| "I want to restrict what it can do" | `aws-agents-connect` | `agentcore add policy-engine` |
| "I want to measure quality" | `aws-agents-optimize` | `agentcore add evaluator` |
| "I want to go to production" | `aws-agents-harden` | production readiness checklist |
| "I want multiple agents working together" | `aws-agents-build` | `agentcore create --protocol A2A` |
| "I need it in a VPC" | `aws-agents-build` | `agentcore create --network-mode VPC` |
