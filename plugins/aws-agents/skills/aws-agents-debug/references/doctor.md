...
- Creating a new project or getting started -> use `aws-agents-get-started`
- Deploy failures that aren't environment-related (CDK errors, IAM) -> use `aws-agents-deploy`
- Agent runtime errors -> use `aws-agents-debug`
...
Correct AgentCore CLI: 0.9.1
Correct Node.js: v20.11.0
Correct uv: 0.4.18
Correct AWS credentials: configured (account: 123456789012, region: us-east-1)
Correct Bedrock model access: Claude models enabled
Warning:  IAM permissions: missing iam:CreateRole -- deploy will fail
Wrong Docker: not running -- needed for Container builds (optional)
...
All clear? Run `aws-agents-get-started` to create your first project.
```
...
- Pointer to `aws-agents-get-started` skill when environment is healthy
