...
description: >
  ...
  Not for deploy failures -- use aws-agents-deploy. Not for performance
  tuning without errors -- use aws-agents-optimize. Not for VPC
  configuration -- use aws-agents-build. Not for observability setup or
  missing logs -- use aws-agents-optimize.
---
...
- Deploy failures (CDK errors, IAM during deploy) -> use `aws-agents-deploy`
- Scaffolding a new project -> use `aws-agents-get-started`
- Measuring quality or setting up monitoring -> use `aws-agents-optimize`

## Input

`the user request` is optional:

```
aws-agents-debug                      # interactive -- describe what's wrong
aws-agents-debug traces               # read and explain recent traces
aws-agents-debug logs                 # search recent logs for errors
aws-agents-debug memory               # diagnose memory recall issues specifically
aws-agents-debug doctor               # check environment prerequisites
```
...
### Step 3: Read traces and logs with approval

Do not ask the developer to paste logs. Instead, explain that these commands access AWS traces/logs and may return customer data, then ask for explicit approval before running them.
...
VPC connectivity issue: If the agent is in a VPC, check security group rules and route tables. See `aws-agents-build` (loads [`references/vpc.md`](../aws-agents-build/references/vpc.md)) for VPC-specific debugging.
...
4. Only after the above, request a quota increase. See `aws-agents-harden` (loads [`references/limits.md`](../aws-agents-harden/references/limits.md)) -- request it through the Service Quotas console (Amazon Bedrock AgentCore), not by filing a support ticket directly.

See `aws-agents-harden` Session lifecycle management section for the full pattern.
...
```dockerfile
# Correct Correct -- wrapped with opentelemetry-instrument
CMD ["opentelemetry-instrument", "python", "main.py"]

# Wrong Wrong -- no OTEL wrapper, logs won't appear
CMD ["python", "main.py"]
```
...
# Correct This appears in CloudWatch
logger.info("Processing request")

# Wrong This does NOT appear in CloudWatch
...
Alternative: use the SDK's async task API for fire-and-forget patterns. If the client doesn't need to wait for the result, register the work via `add_async_task` / `complete_async_task` and return the invocation immediately. See `aws-agents-harden` Long-running background tasks section.
...
|---|---|---|
| Memory misconfigured (wrong strategy, namespace, wiring) | `aws-agents-build` | Load [`references/memory.md`](../aws-agents-build/references/memory.md) |
| Agent invocation from app not working (auth, URL, streaming) | `aws-agents-build` | Load [`references/integrate.md`](../aws-agents-build/references/integrate.md) |
| VPC connectivity (can't reach RDS, no internet, AZ error) | `aws-agents-build` | Load [`references/vpc.md`](../aws-agents-build/references/vpc.md) |
| Multi-agent delegation not working | `aws-agents-build` | Load [`references/multi-agent.md`](../aws-agents-build/references/multi-agent.md) |
| Custom request headers not reaching agent code | `aws-agents-build` | Load [`references/request-headers.md`](../aws-agents-build/references/request-headers.md) |
| Cross-account invocation from an app in another account | `aws-agents-build` | Load [`references/integrate.md`](../aws-agents-build/references/integrate.md) (cross-account section) |
| Gateway auth misconfigured (401, wrong auth type) | `aws-agents-connect` | Gateway auth matrix |
| Gateway target type question (Lambda vs OpenAPI vs MCP vs API Gateway) | `aws-agents-connect` | "What Gateway is and isn't" section |
| Policy denying unexpectedly (Cedar, access denied on tool) | `aws-agents-connect` | Load [`references/policy.md`](../aws-agents-connect/references/policy.md) |
| Observability not set up (no logs, no traces appearing) | `aws-agents-optimize` | Load [`references/observability.md`](../aws-agents-optimize/references/observability.md) |
| Cold start / initialization too slow | `aws-agents-harden` | Initialization time section |
| Session lifecycle / `maxVms` / `StopRuntimeSession` | `aws-agents-harden` | Session lifecycle management section |
| Long-running background tasks being reclaimed | `aws-agents-harden` | Long-running background tasks section |
| JWT inbound auth failing (403, `allowedClients`/`allowedAudience`, issuer mismatch) | `aws-agents-harden` | Inbound auth section |
| Throttling / quota error / limit increase request | `aws-agents-harden` | Load [`references/limits.md`](../aws-agents-harden/references/limits.md) |
| Deploy artifact stale or wrong version | `aws-agents-deploy` | Redeploy workflow |
