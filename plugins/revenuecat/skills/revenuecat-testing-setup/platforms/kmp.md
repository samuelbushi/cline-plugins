...
- iOS target: see the Test Store section in `ios.md` for the `xcconfig` + `Info.plist` pattern.
- Android target: see the Test Store section in `android.md` for the `buildConfigField` per build type pattern.
...
- iOS target -> `ios.md`. Prefer a real sandbox Apple ID when exercising RevenueCat dashboard ingestion. The StoreKit Configuration File is useful for pure UI iteration but does not hit the real sandbox pipeline.
- Android target -> `android.md`. License tester on the Internal Testing track is required.
