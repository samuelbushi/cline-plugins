...
```
https://app.revenuecat.com/projects/56965ae1/charts/revenue?range=Last+90+days%3A{start_date}%3A{end_date}
```
...
| ------------- | -------------------------------------------------- |
| Last 7 days   | `range=Last+7+days%3A{start_date}%3A{end_date}`    |
| Last 28 days  | `range=Last+28+days%3A{start_date}%3A{end_date}`   |
| Last 90 days  | `range=Last+90+days%3A{start_date}%3A{end_date}`   |
| Last 365 days | `range=Last+365+days%3A{start_date}%3A{end_date}`  |
| Custom        | `range=Custom%3A{start_date}%3A{end_date}`         |

Note: The `:` between parts must be URL-encoded as `%3A`. Spaces become `+`.
Always compute `{end_date}` from the current date unless the user specifies a different end date, then compute `{start_date}` from the selected preset or custom range.
...
Calculate dates from the current date before building the URL.

```
https://app.revenuecat.com/projects/56965ae1/charts/revenue?range=Last+90+days%3A{start_date}%3A{end_date}&segment_by=country&country=US&country=DE
```
...
```
https://app.revenuecat.com/projects/56965ae1/charts/churn?range=Custom%3A2025-08-01%3A{end_date}
```
