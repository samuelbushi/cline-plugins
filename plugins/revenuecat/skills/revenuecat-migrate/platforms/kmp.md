...
`purchases-kmp` wraps `purchases-ios` and `purchases-android`. Migration on KMP is a thin shim over platform native migration, so the platform files `ios.md` and `android.md` remain the source of truth.
