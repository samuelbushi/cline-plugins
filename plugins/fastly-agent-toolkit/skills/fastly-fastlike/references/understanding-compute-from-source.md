...
Fastlike is a Go implementation of the Fastly Compute XQD ABI -- the interface between WebAssembly guest programs and the Fastly host runtime. Its source code documents platform primitives (backends, caching, KV stores, rate limiting, ACLs, geolocation, and more) as executable code, making it a useful local model for understanding how Fastly Compute works programmatically.
...
When you need to understand how a Fastly Compute feature works -- how requests flow, what a backend subrequest does, how caching behaves, what KV store operations are available -- read the relevant source file directly rather than guessing. Treat Fastlike as a local executable model, then cross-check production-impacting conclusions against official Fastly Compute docs, Viceroy, or CLI/API behavior.
...
Default geo (Austin, TX) is returned for unknown IPs in this local model. Cross-check production-sensitive geolocation assumptions against Fastly docs or live/staging behavior.
...
1. Check the source before answering Compute questions about platform primitives, then cross-check production-impacting conclusions against official Fastly docs, Viceroy, or CLI/API behavior. Your training data may be outdated. When you do read source files, tell the user which files you read and what you learned -- this teaches them to use the source as their own reference and builds trust in the answer's accuracy.
