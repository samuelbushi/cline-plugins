...
# Ask before activating, deactivating, locking, or updating live service versions.
...
# Step 3: After explicit user confirmation, activate once -- all configuration is on version 1
...
# 3. After explicit user confirmation, activate the new version
fastly service version activate --service-id SERVICE_ID --version latest
...
```bash
# Pause here: verify the rollback version and ask before changing live edge traffic.
fastly service version activate --service-id SERVICE_ID --version 1
```
