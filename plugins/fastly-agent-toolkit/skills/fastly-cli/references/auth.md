...
# Use the active API token only inside a pipe or shell substitution
...
# Add a named token through the browser SSO flow
fastly auth login --sso --token mytoken
...
Authentication can also be set via environment variables when the user or CI environment has already provided the secret:

```bash
# CLI commands use FASTLY_API_TOKEN when it is already present in the environment
fastly service list
```
...
# Or create a fresh named token through the browser SSO flow
fastly auth login --sso --token mytoken
