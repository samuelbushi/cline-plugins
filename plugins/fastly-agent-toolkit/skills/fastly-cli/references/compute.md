...
Every `fastly compute deploy` or `fastly compute publish` changes a live Fastly service. Build and test locally first, then state the target service/domain and ask for explicit confirmation before running these commands.

```bash
# Deploy after explicit user confirmation (creates service if needed)
fastly compute deploy

# Deploy to a specific service after explicit user confirmation
...
`fastly compute publish` builds and deploys in one command. Build/test locally first, then ask for explicit confirmation before publishing.

```bash
# Build and deploy in one command after explicit user confirmation
fastly compute publish

# With options, after explicit user confirmation
...
# Deploy to staging after explicit user confirmation
fastly compute deploy --env stage

# Deploy to production after explicit user confirmation
fastly compute deploy
```
...
# Pause here: validate and ask before activating the modified live service.
