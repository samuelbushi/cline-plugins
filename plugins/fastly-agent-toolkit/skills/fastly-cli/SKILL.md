...
Before running a live Fastly command that mutates production-facing state, state the target customer/service/domain and ask for explicit confirmation. This includes `fastly service version activate`, purge-all, TLS changes, origin/backend edits, WAF or rate-limit changes, Compute deploys, store deletion, product enablement, and account access changes.

Prefer read-only commands and local validation first. Do not use `--auto-yes`, `--accept-defaults`, `--non-interactive`, or `--debug-mode` on mutating commands unless the user explicitly asks for that automation and understands the effect. Never print, paste, or commit Fastly API tokens.
...
5. Activate after confirmation:

   ```bash
   # Pause here: state the service ID/domain and ask before activating live edge traffic.
