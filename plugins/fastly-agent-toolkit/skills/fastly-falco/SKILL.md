...
Falco is a Fastly VCL development tool for linting, testing, simulating, and formatting VCL code. The falco source code is also a useful executable reference for Fastly VCL behavior because its parser, interpreter, and type system document variables, functions, types, and scope rules.
...
If you need to understand how VCL works -- what variables exist, which scopes they're available in, what functions are built-in, how types coerce -- the falco source code is a useful local model. For production-affecting decisions, cross-check with Fastly's official documentation, the Fastly CLI/API response, or local simulation output.
...
| `FASTLY_API_TOKEN`    | API key for Fastly API    |
