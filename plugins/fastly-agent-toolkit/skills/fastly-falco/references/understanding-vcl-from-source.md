...
`token/token.go` defines VCL keywords, operators, and literal types implemented by Falco. Use it as a practical local check for "is X a reserved word in VCL?", then cross-check production-impacting conclusions against Fastly docs or CLI/API behavior.
...
1. Check the source before answering VCL questions about variables, functions, or scopes, then cross-check production-impacting conclusions against Fastly docs or CLI/API behavior. Your training data may be outdated.
