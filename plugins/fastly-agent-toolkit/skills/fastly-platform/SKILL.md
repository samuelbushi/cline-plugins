...
Prefer the `fastly` CLI over raw API calls -- see the fastly-cli skill. When calling the REST API directly, never paste the raw API token into the conversation and omit `curl -v` (it prints the `Fastly-Key` header). Source tokens from the existing `FASTLY_API_TOKEN` environment variable or `$(fastly auth token)` inside a shell substitution. Only use `fastly auth show TOKEN_NAME --reveal --quiet` inside a shell substitution when the user has named a stored token.

Before any live Fastly API mutation, state the target customer/service/domain and ask for explicit confirmation. This includes service activation, purge-all, TLS changes, origin/backend edits, WAF or rate-limit changes, Compute deploys, store deletion, product enablement, and account access changes. Prefer read-only inspection and local validation first.
