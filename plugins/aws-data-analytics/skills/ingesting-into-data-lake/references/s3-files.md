...
4. Prefer available `aws-mcp` tools by capability or description for S3 Tables, data processing, and IAM operations; do not assume separate MCP servers or hardcode tool names.

Use `aws-mcp` tools when available. Fall back to AWS CLI only when no matching MCP capability is available.
