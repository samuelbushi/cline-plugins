...
You MUST execute commands using available `aws-mcp` AWS API tools when connected - they
...
- You MUST prefer an available `aws-mcp` AWS API tool when connected; fall back to AWS CLI if not
...
CLI calls per database - that burns turns and tokens. Prefer an available
`aws-mcp` AWS API tool. If no suitable MCP path is available, ask for
explicit approval before running a local boto3 broad-scan helper. Write the
helper only to a temp location, run it once with `python3`, bound the output
to matching results, and delete it afterward when practical.
...
- Avoid printing sample rows, credentials, or table parameters that may contain
  secrets
...
a first choice, and it requires explicit user approval.
