...
You MUST use available `aws-mcp` AWS API tools when connected, they provide command validation, sandboxed execution, and audit logging. Fall back to AWS CLI if MCP unavailable.
...
- You MUST check whether available `aws-mcp` AWS API tools or AWS CLI are available and inform user if missing
