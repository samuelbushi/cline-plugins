...
- You MUST prefer an available `aws-mcp` AWS API tool when connected; fall back to AWS CLI only if the MCP server is unavailable
...
For analytical queries, you SHOULD propose a bounded profiling query before building the final query. Ask before executing it, and default to summarized or redacted output. Show raw sample rows (`SELECT ... LIMIT 5`) only when the user approves seeing sample data.
...
| Statement | Behavior |
|---|---|
| `SELECT`, `SHOW`, `DESCRIBE`, `EXPLAIN` | Read-oriented - ask before execution and summarize or redact results by default |
...
Example command, whether executed through `aws-mcp` or AWS CLI:

```
aws athena start-query-execution --work-group <WORKGROUP_NAME> --query-string '<sql>' --query-execution-context Database=<db>
```
