...
#### XML Views
```xml
<!-- CORRECT: Controls are auto-loaded by tag -->
<m:Button text="Click Me"/>

<!-- CORRECT: For formatters/types, use core:require -->
<ObjectListItem
    core:require="{
...
Why Custom Types:
- CORRECT: Two-way binding support (formatValue + parseValue + validateValue)
- CORRECT: Real-time validation as user types
- CORRECT: Model updates immediately on valid input
- WRONG: Custom formatters only work for one-way (display) binding
...
Use the `get_api_reference` tool from the `ui5-mcp-server` MCP server to get information on UI5 controls and APIs. This provides direct access to the official UI5 API Reference for the UI5 version in use.
...
Use the `run_ui5_linter` tool from the `ui5-mcp-server` MCP server to identify issues. It detects deprecated APIs, accessibility issues, and other potential bugs.
...
After making code changes, ask the user before running project scripts or local CLIs. If approved, run the project's linter if available:
...
- If CDS tools are available and the user approves local CLI execution: Use them to get definitions, services, and endpoints
- If no CDS tools are available or the user does not approve local CLI execution: ask for the service URL and target entity set instead of running commands.
- If approved, these commands can inspect CAP definitions:
  ```bash
...
Confirm with the user before installing dependencies. If approved, run in the CAP project root:
```bash
npm i -D cds-plugin-ui5
```

The `cds-plugin-ui5` package handles serving UI5 applications from the CAP project.
...
- Official UI5 API Reference (use `get_api_reference` from `ui5-mcp-server`)
