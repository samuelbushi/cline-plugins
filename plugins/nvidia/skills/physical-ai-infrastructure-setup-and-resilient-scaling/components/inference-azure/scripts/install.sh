...
# ---------- Test ----------
if [[ "${NVIDIA_CLINE_TEST_FOUNDRY_ENDPOINT:-}" == "1" && -n "$PRIMARY_KEY" ]]; then
  ...
  echo "Skipping authenticated endpoint test. Set NVIDIA_CLINE_TEST_FOUNDRY_ENDPOINT=1 after approval to test with the retrieved key."
fi

echo ""
echo "Foundry endpoint deployed. Use the URL above and the retrieved key from Azure to call the model."
