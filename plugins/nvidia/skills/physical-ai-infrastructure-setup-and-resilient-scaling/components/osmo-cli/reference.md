...
The focused references below preserve specialized workflow and log handling
guidance. Read them when that slice is needed, but keep execution in the
current Cline session unless the user explicitly asks for background work.

- `references/workflow-expert.md` - workflow generation, resource check, submission, failure diagnosis
- `references/logs-reader.md` - log fetching and summarization for monitoring and failure diagnosis
...
- Add useful inline annotations in cells when relevant:
  ...
  - Optionally mark the top pool in a section as `[ok] Most available`.
...
The lifecycle is split between focused workflow setup guidance and live
monitoring in the current Cline session, so the user sees real-time updates.

1. Read `references/workflow-expert.md` for setup and submission.

   Use it to write workflow YAML if needed, check resources, and submit only
   after required approvals. Do not monitor, poll status, or report results from
   that focused setup step.
   ...
   Record the workflow ID, pool name, and OSMO Web link.
...
3. Handle the outcome.
   ...
   Status" Step 2 (1 task = inline, 2+ tasks = compact batches using
   `references/logs-reader.md`). Then return to `references/workflow-expert.md`
   with the logs summary: "Workflow <id> FAILED. Here is the logs summary:
   <summary>. Diagnose and fix." It should produce a proposed fix and, after
   approval where required, a new workflow ID.
...
2. Summarize results in a table showing workflow name, pool, status, and duration.
   ...
   - [ok] COMPLETED
   - [fail] FAILED / FAILED_CANCELED / FAILED_EXEC_TIMEOUT / FAILED_SERVER_ERROR
   -  RUNNING
   -  PENDING
...
2. Get recent logs - Choose the log-fetching method based on task count
   ...
   - 2+ tasks: read `references/logs-reader.md` and summarize logs in compact
     batches. Do not paste broad raw logs into chat; preserve only concise
     progress and error evidence.
