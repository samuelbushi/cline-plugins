# OSMO Workflow Expert Reference

Use this focused reference for OSMO workflow creation, resource checking,
submission, and failure diagnosis. It does not require a separate worker;
the current Cline session should apply the workflow and return the workflow ID.

Handle workflow generation, resource selection, submission, and failure
diagnosis, then return to inline monitoring with live status updates visible to
the user.
...
1. Analyze logs - Analyze the logs summary that is provided to you
   ...
   workflows, use `references/logs-reader.md` to fetch compact log batches
   before returning here. Ask for that summary if the logs are insufficient.
...
3. Proactive review - When fixing a script error, review the ENTIRE
   ...
   not just the line that failed. Propose fixes in a single pass to
...
5. Resubmit - Submit to the same pool only after user approval when the change
   modifies workflow YAML, scripts, resource requests, paid infrastructure, or
   submitted workloads.
