...
```bash
SKILL_DIR="${SKILL_DIR:?set by Cline skill runtime}"
"$SKILL_DIR/components/cluster-azure/scripts/preflight.sh"
```

2. Generate `deploy.tfvars`

`deploy.tfvars` is local-only (gitignored); `terraform.tfvars.example` is the tracked template. On a fresh clone, `cp terraform.tfvars.example deploy.tfvars` and fill in `subscription_id` (from `az account show --query id -o tsv`) and `allowed_cidr` (list containing your public IP `/32`). Other values (region, VM sizes, GPU min/max, priority, pg SKU, K8s version) are edited here when workload needs change - defaults live in `variables.tf`. If caller IP drifts after apply, update `allowed_cidr`, review a new `terraform plan`, and apply only after explicit user approval.
...
```bash
SKILL_DIR="${SKILL_DIR:?set by Cline skill runtime}"
LOCATION=$(awk -F'"' '/^location/{print $2}' "$SKILL_DIR/components/cluster-azure/scripts/deploy.tfvars")
GPU_SKU=$(awk -F'"'  '/^gpu_vm_size/{print $2}' "$SKILL_DIR/components/cluster-azure/scripts/deploy.tfvars")
...
```bash
SKILL_DIR="${SKILL_DIR:?set by Cline skill runtime}"
terraform -chdir="$SKILL_DIR/components/cluster-azure/scripts" init
terraform -chdir="$SKILL_DIR/components/cluster-azure/scripts" plan  -var-file=deploy.tfvars
terraform -chdir="$SKILL_DIR/components/cluster-azure/scripts" apply -var-file=deploy.tfvars
```
...
```bash
SKILL_DIR="${SKILL_DIR:?set by Cline skill runtime}"
RG=$(terraform -chdir="$SKILL_DIR/components/cluster-azure/scripts" output -raw resource_group)
AKS=$(terraform -chdir="$SKILL_DIR/components/cluster-azure/scripts" output -raw aks_name)
...
Run `setup.sh` only after the user approves mutating kubeconfig, StorageClass
defaults, and Helm releases for the target AKS cluster.

```bash
SKILL_DIR="${SKILL_DIR:?set by Cline skill runtime}"
NVIDIA_CLINE_APPROVE_AZURE_CLUSTER_SETUP=1 "$SKILL_DIR/components/cluster-azure/scripts/setup.sh"
```
