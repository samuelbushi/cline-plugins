...
if { [[ -n "${NGC_API_KEY:-}" ]] || [[ -n "${HF_TOKEN:-}" ]]; } && \
   [[ "${NVIDIA_CLINE_APPROVE_K8S_SECRET_WRITE:-}" != "1" ]]; then
  cat <<'EOF'
ERROR: install.sh will write NGC_API_KEY and/or HF_TOKEN into Kubernetes
Secret objects. Review the target namespace and set
NVIDIA_CLINE_APPROVE_K8S_SECRET_WRITE=1 only after the user explicitly approves
that credential write.
EOF
  exit 1
fi
