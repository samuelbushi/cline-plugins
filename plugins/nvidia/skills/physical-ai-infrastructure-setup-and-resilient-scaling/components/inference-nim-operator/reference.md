...
NVIDIA_CLINE_APPROVE_K8S_SECRET_WRITE=1 \
  NIM_SERVICES="qwen25-14b cosmos-predict" \
  "$SKILL_DIR/components/inference-nim-operator/scripts/install.sh"
```
...
Set `NVIDIA_CLINE_APPROVE_K8S_SECRET_WRITE=1` only after the user approves
writing any configured `NGC_API_KEY` or `HF_TOKEN` into Kubernetes Secret
