...
- Never print, echo, or paste raw keys into commands, YAML, logs, or
  ...
  runtime-only secret injection. Before sharing raw transcript exports, redact
  secrets with an available scanner or manual review; this plugin does not
  bundle a transcript scanner.
- Use absolute paths. For bundled helper files, resolve paths under
  `${SKILL_DIR}` instead of the user's workspace root.
...
| OSMO CLI and workflow operations | `components/osmo-cli/reference.md` | `components/osmo-cli/scripts/`, `components/osmo-cli/references/` |
...
| File | Read when |
| --- | --- |
| `components/osmo-cli/references/workflow-expert.md` | Workflow generation, resource selection, submission, or failure diagnosis needs focused guidance. |
| `components/osmo-cli/references/logs-reader.md` | Multi-task OSMO workflow logs need compact summarization guidance. |
...
- Video Data Augmentation, Defect Image Generation, and NuRec carline
  adaptation are not bundled in this plugin. Use this skill for shared
  infrastructure, then ask the user for the workflow YAML, package, or
  repository that owns the workload-specific instructions.
- NRE, NCore, and Asset Harvester route through
  `physical-ai-neural-reconstruction`, which requires user-approved access to
  the external NuRec skill catalog before using those references.
