...
- SA Stage 2 heuristics flag validation candidates; they do not justify operations
  by themselves.
- Outlier detection (section2.1) uses authored `extentsHint` attributes when present.
...
- If copies are structurally identical at the same transform, they are
  export duplicates - the scene contains Nx the data it should.
...
```
Scene (123K prims)
+-- Discipline (x7): Architectural, HVAC, Plumbing, Electrical, Structural, Facades, Site
|   +-- Floor (x8): L1, L2, L3, L4, L5, M1, Parking, R1
...
- Spatial heuristics (section2.1, section2.2, section2.3) use only authored `extentsHint`
