...
in [section13 Acceptance Criteria](#13-acceptance-criteria) is correct.
...
- `HASH_LEVEL` *(integer, 1..4)* - fidelity of the duplicate-detection
  hash. See [section5 Hash levels](#5-hash-levels) for exact semantics.
- `MIN_SUBTREE_PRIMS` *(integer >= 1)* - exclude any candidate whose
  ...
  the instance-skipping rule from section4 applied (see section9 for the formula).
...
- `SKIP_EXISTING_INSTANCES` *(bool)* - see section4.
- `COLLAPSE_NESTED` *(bool)* - see section6.4.
- `CHECK_INSTANCEABILITY` *(bool)* - when true, run the analysis in section7
...
- `INCLUDE_ATTRIBUTE_CONNECTIONS` *(bool)* - when true, attribute
  `.GetConnections()` are checked alongside relationships in section7. When
...
the missing-`ROOT` path in section8.1.
...
The tool also computes a candidate hash per prim (see section6.2) which is
...
- the prim type name
- the prim's name (relative within the subtree - see section6.2 for treatment of
...
### 8.1 Error path
If `ROOT` is missing or invalid, OR any knob in section3.1 is out of range,
...
4. The top `TOP_N` groups, each rendered as:
   ...
   - When `CHECK_INSTANCEABILITY` is true: a verdict line and up to
     `MAX_FINDINGS_PER_GROUP` finding lines as defined in section7.5.
...
for readability, but are not required. I1 determinism (section10) only
...
- Subtree of P - P and the set of prims reached from P by recursive
  descent under the traversal rules in section4.
...
The implementation must preserve all of these. Acceptance tests in section13
exercise them.
...
- `ROOT` does not exist. Print a single error line naming the missing
  path; produce no other report content; exit cleanly. (See section8.1.)
...
- Long array attribute values (e.g. mesh point arrays). Must be
  digested per section5.1 before being included in any hash.
...
- Instanceability pass: scoped to *reported* groups only (i.e. the
  ...
  Cost is bounded by `(groups_reported x avg_copies x avg_subtree_size)`.
...
- Detecting near-duplicates with tolerances (e.g. mesh point clouds
  that differ by epsilon). That is fuzzy duplicate detection - a separate
  concern.
...
- rev 2 - Incorporates feedback from a clean-room re-implementation.
  Fixes section8/section11 ordering contradiction; tightens section5 Level 4 to include
  sample values; tightens section7.2 INTERNAL classification to require
  matching relative target sequences; adds section3.2 Defaults; promotes long-
  value digesting to mandatory with concrete thresholds (section5.1); adds
  worked example block to section7.1; mandates ascending sort within finding
  buckets and within printed candidate-path lists; clarifies section10 I1
  determinism scope; adds I8; adds section9 subtree-size formula for the
  `SKIP_EXISTING_INSTANCES=False` case; adds section11 entries for `ROOT == "/"`,
  `ROOT`-as-instance, and inactive prim handling; adds section13 acceptance
