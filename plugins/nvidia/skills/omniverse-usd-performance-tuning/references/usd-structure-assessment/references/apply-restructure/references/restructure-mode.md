...
2. For each duplicate site on the assembly root:
   ...
   b. Set `instanceable=true` on the payload root prim only when
      ...
      section"instancing-readiness gate") passes for that site's dedupe group.
...
3. For unique (non-duplicate) boundary candidates, extract as independent
   ...
   without instancing).
4. Validate all outputs per section"Authoring Requirements" below.
...
pattern described in `operation-safety.md` section"SO Operation Returns Success With
Zero Work".
