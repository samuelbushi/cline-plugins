...
> Invocation. Tool of `restructure-decision` (Phase 2e gate, mode=`restructure`) and the `optimize-loop` (mode=`ref_remap`, after Phase 4 mesh ops produce optimized sub-assets). In Cline / generic shell agents, invoke by name. In Cline, also available as `/apply-restructure`.
>
...
- For `mode=restructure`: every payload, prototype, or loadable sub-asset in
  ...
  traversal (see `restructure-mode.md` section"Authoring Requirements"). Verify
...
- `usd-structure-assessment/README.md` section2.7 + "Tools for asset extraction" - boundary identification + USD API patterns this reference builds on.
