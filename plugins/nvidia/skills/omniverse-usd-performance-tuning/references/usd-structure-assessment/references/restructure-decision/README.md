...
- A completed `usd-structure-assessment` report including:
  ...
  - The section2.7 asset-boundary identification output (when the stage is monolithic).
...
| Asset-boundary candidates | `usd-structure-assessment` section2.7 + `usd-hierarchy-dedupe-candidates` | Where the cut points are if restructure is chosen |
...
`restructure-mode.md` section"Instanced Asset Extraction" applies:
...
- Asset-boundary candidates from SA section2.7 are suggestions, not enforcement; the user can override the cut points before invoking `apply-restructure`.
...
- `usd-structure-assessment/README.md` section2.7 (Asset boundary identification) - the source of boundary candidates.
