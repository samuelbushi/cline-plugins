...
Do not run this installer automatically. It can install Node.js or Docker,
request `sudo`, change Docker group membership, build containers, write local
credentials, and create sandbox network policy. Show the command, explain the
host changes, and wait for explicit user approval. If the user asks Cline to
run it, download the script to a temporary file and inspect it before executing
it.
...
The piped installer prompts through your terminal. In CI or another
pre-approved non-interactive environment, pass explicit acceptance to the
`bash` side of the pipe:

```console
$ curl -fsSL https://www.nvidia.com/nemoclaw.sh -o /tmp/nemoclaw.sh
$ less /tmp/nemoclaw.sh
$ NEMOCLAW_NON_INTERACTIVE=1 NEMOCLAW_ACCEPT_THIRD_PARTY_SOFTWARE=1 bash /tmp/nemoclaw.sh
```
...
Review the messaging channel credentials, permissions, and network exposure
...
If you picked a different option, the `Model` line shows that provider's model and label instead. For example, you might see `gpt-5.4 (OpenAI)`, `cline-sonnet-4-6 (Anthropic)`, `gemini-2.5-flash (Google Gemini)`, `llama3.1:8b (Local Ollama)`, `nvidia-routed (Model Router)`, or `<your-model> (Other OpenAI-compatible endpoint)`.
...
This plugin bundles the quickstart and prerequisite references only. For broad
NemoClaw product overviews or sandbox lifecycle management, use the installed
`nemoclaw` CLI help or NVIDIA documentation.
