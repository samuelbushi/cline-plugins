...
If you entered a key incorrectly, use the installed `nemoclaw` CLI help or
NVIDIA documentation for credential reset commands before re-running
onboarding.
...
3. At the `Choose model [1]:` prompt, pick a curated model (for example, `cline-sonnet-4-6`, `cline-haiku-4-5`, or `cline-opus-4-6`), or pick Other... to enter any Cline model ID.

Option 5: Other Anthropic-Compatible Endpoint:

Routes inference to any server that implements the Anthropic Messages API at `/v1/messages`, including Cline proxies, Bedrock-compatible gateways, and self-hosted Anthropic-compatible servers.
