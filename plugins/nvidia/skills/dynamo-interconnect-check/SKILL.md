...
description: Validate that a Dynamo deployment's NIXL/UCX/NCCL interconnect is ready for disaggregated serving over RDMA/NVLink. Use after a disaggregated or multi-node Dynamo deployment is already up to confirm the KV transport is correct.
license: Apache-2.0
metadata:
...
- After the user-provided deployment runbook brings up a disagg or multi-node
  recipe.
...
For pods that are already crashing or unschedulable, diagnose pod events,
scheduling, image pulls, and worker logs before running transport checks.
...
```bash
python3 "$SKILL_DIR/scripts/check_interconnect.py" env recipes/<model>/<framework>/<mode>
```
...
```bash
python3 "$SKILL_DIR/scripts/check_interconnect.py" node \
...
```bash
python3 "$SKILL_DIR/scripts/check_interconnect.py" nixl \
  --namespace "${NAMESPACE}" --pod <worker-pod>
```
...
```bash
python3 "$SKILL_DIR/scripts/check_interconnect.py" env recipes/qwen3-coder-480b/sglang/disagg
```
