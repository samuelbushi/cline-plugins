...
description: Start or patch Dynamo router modes and run router endpoint smoke checks. Use for round-robin, KV-aware, least-loaded, or device-aware routing setup after a Dynamo recipe is already deployed.
license: Apache-2.0
metadata:
...
frontend service. If the recipe is not already deployed, ask the user for the
deployment runbook or recipe they want to use; this plugin does not bundle a
Dynamo recipe deployment skill.
...
```bash
python3 "$SKILL_DIR/scripts/check_router_health.py" \
...
If the endpoint is unhealthy or workers are missing, inspect the deployment,
pod readiness, and worker logs before retrying; this plugin does not bundle a
separate Dynamo troubleshoot skill.
...
Run scripts through the shell with `$SKILL_DIR` pointing at this installed
skill directory.
...
```bash
python3 -m dynamo.frontend --router-mode kv --http-port 8000 &
python3 "$SKILL_DIR/scripts/check_router_health.py" --base-url http://127.0.0.1:8000
```
...
python3 "$SKILL_DIR/scripts/check_router_health.py" --base-url http://127.0.0.1:8000 --retries 3
```
...
| Smoke chat request times out | Frontend up, workers not serving | Inspect worker pod readiness and logs before retrying |
