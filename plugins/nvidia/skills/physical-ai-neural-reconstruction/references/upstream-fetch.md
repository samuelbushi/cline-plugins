...
1. `.agents/skills/<name>/SKILL.md` (Cursor, Cline, NemoClaw)
2. `.cline/skills/<name>/SKILL.md` (Cline)
...
Before cloning, fetching, pulling, or reading upstream skill-like files, explain
the repository, branch or commit, destination path, and trust boundary to the
user, then wait for explicit approval. Prefer a user-specified commit or tag
when they need reproducibility; `main` is acceptable only after the user agrees
to use the current upstream state.
