...
1. `QODO_API_URL` from the environment or config (highest priority): If `QODO_API_URL` is present, append `/rules/v1` only when the value does not already end with `/rules/v1`.
...
## Example (curl with Python JSON builder)

```bash
# Build body with proper JSON escaping for multi-line search queries.
