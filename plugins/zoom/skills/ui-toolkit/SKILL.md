...
Background reference for the prebuilt Zoom Video SDK UI Toolkit on web. Prefer `zoom-choose-approach` first when the user might still need Meeting SDK instead.
...
- OK: Ready-to-use UI - Professional video interface out of the box
- OK: Zero UI code - No need to build video layouts, controls, or participant management
- OK: Framework agnostic - Works with React, Vue, Angular, Next.js, vanilla JS
- OK: Highly customizable - Choose which features to enable, customize themes
- OK: Built-in features - Chat, screen share, settings, virtual backgrounds included
...
```
### Must-Remember Rules
1. OK: Always generate JWT server-side
2. OK: Always call `destroy()` on cleanup
3. OK: Always use React 18 (not 17/19)
4. OK: Always import CSS file
5. Do not: Never expose SDK secret client-side
6. Do not: Never skip `onSessionClosed` cleanup
7. Do not: Never call components before `joinSession`
##  Support
- Developer Forum: https://devforum.zoom.us/
