...
Background reference for deterministic server-side Zoom automation and resource management. Prefer `zoom-plan-product`, `zoom-plan-integration`, or `debug-zoom` first, then route here for endpoint-level detail.
# Zoom REST API
...
### Warning: JWT App Type is Deprecated
...
### Warning: Meeting ID vs UUID -- Double Encoding
...
### Warning: Rate Limits Are Per-Account, Not Per-App
...
### Warning: Download URLs Require Auth and Follow Redirects
