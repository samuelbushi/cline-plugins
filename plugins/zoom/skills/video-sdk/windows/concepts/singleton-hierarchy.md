...
+--> CreateZoomVideoSDKObj() -----------------------------------> IZoomVideoSDK*
    ...
    |                                                                         |
    |  Warning: CRITICAL: Share subscription uses IZoomVideoSDKShareAction from     |
...
| Subscribe share | `shareAction->getShareCanvas()->subscribeWithView(hwnd, aspect)` Warning: From callback! |
| Get raw share | `shareAction->getSharePipe()->subscribe(res, delegate)` Warning: From callback! |
...
Warning: Share subscription is DIFFERENT from video subscription!
```cpp
