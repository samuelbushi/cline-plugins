...
### Warning: CRITICAL: Windows Message Pump Required
...
- Message pump implementation OK
- Audio connection strategy OK
- Error handling OK
...
```
Advantages:
- OK: Best quality - SDK uses optimized, hardware-accelerated rendering
- OK: No artifacts - Professional video quality
- OK: Simple code - 3 lines to subscribe
- OK: Better performance - No CPU-intensive YUV conversion
- OK: Automatic scaling - SDK handles window resizing
- OK: Aspect ratio - Built-in aspect ratio handling
...
```
Disadvantages:
- Warning: CPU intensive - YUV conversion can cause frame drops
- Warning: Artifacts - Manual rendering may show tearing/artifacts
- Warning: Complex - More code to maintain
- Warning: Performance - Slower than Canvas API
Use Raw Data When:
...
Warning: CRITICAL: Video subscription must be event-driven and manual.
Key Events:
...
```
Key Points:
- OK: Subscribe in response to events (onUserJoin, onUserVideoStatusChanged)
- OK: Always exclude current user from remote subscriptions
- OK: Unsubscribe on onUserLeave
- OK: Clean up all subscriptions on onSessionLeave
- OK: Track subscriptions in a map for lifecycle management
### Warning: Screen Share Subscription (DIFFERENT from Video!)
...
## Merged from zoom-video-sdk-windows/SKILL.md
