...
Background reference for fully custom video-session products. Prefer `zoom-plan-product` first when the boundary between Meeting SDK and Video SDK is still unclear.
...
> Need pre-join diagnostics on web? Use [zoom-probe-sdk](../probe-sdk/SKILL.md) before Video SDK `join()` to reduce first-minute failures.
...
```javascript
// Do not: WRONG: Getting stream before joining
const client = ZoomVideo.createClient();
await client.init('en-US', 'Global');
const stream = client.getMediaStream();  // Returns undefined!
await client.join(...);
// OK: CORRECT: Get stream after joining
const client = ZoomVideo.createClient();
await client.init('en-US', 'Global');
await client.join(...);
