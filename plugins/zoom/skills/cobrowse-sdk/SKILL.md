...
|------------|------|----------|----------------|
| SDK Key | Public | CDN URL, JWT `app_key` claim | OK Yes (client-side) |
| SDK Secret | Private | Sign JWTs | No No (server-side only) |
| API Key | Private | REST API calls (optional) | No No (server-side only) |
| API Secret | Private | REST API calls (optional) | No No (server-side only) |
...
### Warning: CRITICAL: SDK Secret Must Stay Server-Side
Problem: Developers often accidentally embed SDK Secret in frontend code.
Solution:
- OK SDK Key -> Safe to expose (embedded in CDN URL)
- No SDK Secret -> Never expose (use for JWT signing server-side)
```javascript
// Do not: WRONG - Secret exposed in frontend
const jwt = signJWT(payload, 'YOUR_SDK_SECRET');  // Security risk!
// OK: CORRECT - Secret stays on server
...
Solution:
- Production: Use HTTPS OK
- Development: Use a loopback host for local HTTP testing OK
- Development: Use a local HTTPS endpoint with a trusted/self-signed cert if required OK
