...
## Benefits
OK: Automatic token management - No manual refresh logic
OK: Single token for all requests - Account-wide access
OK: TTL-based expiration - Redis handles cleanup
OK: 10-second buffer - Prevents race conditions
OK: Graceful shutdown - Clears token on exit
## Testing
```bash
# Start Redis
