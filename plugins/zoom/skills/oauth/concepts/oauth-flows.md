...
```
### Key Points
OK: Simple: No redirect URIs, no user interaction
OK: Secure: Credentials stored server-side only
OK: Account-wide: Single token for all account operations
Warning: No refresh token: Just request a new token when expired (cache with TTL)
---
...
```
### Key Points
OK: User-controlled: Users authorize access to their own account
OK: Per-user tokens: Each user gets their own access/refresh tokens
OK: Refresh support: Tokens can be refreshed while the refresh token remains valid (lifetime varies; ~90 days is common)
Warning: Redirect URI must match exactly: Including trailing slash, protocol, port
Warning: State parameter required: Prevent CSRF attacks
Warning: Authorization code expires in 5 minutes: Exchange immediately
---
...
```
### Key Points
OK: No browser required: User authorizes on separate device
OK: Simple user experience: Just enter a short code
OK: Polling-based: Device polls until user authorizes
Warning: Must enable in app settings: "Use App on Device" feature flag
Warning: Device code expires in 15 minutes: User must complete authorization quickly
Warning: Respect polling interval: Returned by `/devicecode` endpoint (usually 5s)
Warning: Handle slow_down: Increase interval by 5s when requested
---
...
```
### Key Points
OK: Simplest flow: Just request token with credentials
OK: Chatbot-specific: Limited to Team Chat bot operations
Warning: No refresh token: Request new token when expired
Warning: Scope limited: Primarily `imchat:bot` scope
---
## Comparison Table
...
| Refresh Token | Do not: None | OK: ~90 days (commonly) | OK: ~90 days (commonly) | Do not: None |
| Redirect URI | Do not: Not needed | OK: Required | Do not: Not needed | Do not: Not needed |
| PKCE Support | Do not: N/A | OK: Optional | Do not: N/A | Do not: N/A |
| State Parameter | Do not: N/A | OK: Recommended | Do not: N/A | Do not: N/A |
