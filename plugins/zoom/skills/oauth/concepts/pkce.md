...
|-------------|-------------------|----------------|----------|
| Confidential | OK: Yes (server-side) | Do not: Optional | Backend servers, traditional web apps |
| Public | Do not: No (client-side) | OK: Required | Mobile apps, SPAs, desktop apps |
###Why Public Clients Can't Keep Secrets
```javascript
// Do not: INSECURE: Client secret embedded in mobile/SPA code
...
|--------|-------------|----------|---------|
| S256 | SHA256 hash of verifier | OK: Recommended | All OAuth 2.0 servers |
| plain | Verifier sent as-is (no hash) | Warning: Weaker | Legacy support only |
Always use S256:
```javascript
// OK: RECOMMENDED
...
```javascript
// Do not: WRONG: Generating new verifier on callback
app.get('/callback', async (req, res) => {
...
```
```javascript
// OK: CORRECT: Retrieve stored verifier
app.get('/callback', async (req, res) => {
