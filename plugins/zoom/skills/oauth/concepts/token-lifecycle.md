...
```
Key Points:
- OK: Cache token in Redis with TTL matching expiration
- OK: Use 10-second buffer to prevent race conditions
- OK: Single token shared across all requests
- Do not: Do NOT request new token on every API call
- Do not: Do NOT try to "refresh" (no refresh token exists)
---
...
```
Key Points:
- OK: Refresh BEFORE token expires (5-minute buffer recommended)
- OK: ALWAYS save the NEW refresh token (old one becomes invalid)
- OK: Store tokens per user in database
- OK: Encrypt tokens at rest (AES-256 minimum)
- Do not: Do NOT reuse old refresh token after refresh
- Do not: Do NOT wait for API 401 errors to trigger refresh
---
...
### Common Mistake
```javascript
// Do not: WRONG: Not saving new refresh token
...
```
Key Points:
- OK: Exchange authorization code immediately upon receiving it
- OK: Authorization codes are one-time use
- Do not: Do NOT cache or store authorization codes
- Do not: Do NOT delay token exchange
---
## Token Revocation
...
```javascript
// OK: Refresh with buffer (5 minutes before expiry)
if (expiresIn < 300000) {
  await refreshToken();
}
```
```javascript
// Do not: Wait for 401 error
try {
  await makeAPIRequest(token);
} catch (err) {
  if (err.status === 401) {
...
```javascript
// OK: Detect revoked tokens and prompt re-auth
if (error.code === 4741) {
  await deleteUserTokens(userId);
