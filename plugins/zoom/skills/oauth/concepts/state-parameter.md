...
```javascript
// Do not: WRONG: Accepting any state
app.get('/callback', async (req, res) => {
...
```javascript
// Do not: WRONG: Predictable state
const state = Date.now().toString(); // Attacker can predict!
```
```javascript
// OK: CORRECT: Cryptographically random state
const state = crypto.randomBytes(16).toString('hex');
```
### 3. Reusing State
```javascript
// Do not: WRONG: Not deleting state after use
if (state === req.session.oauthState) {
  // State remains in session - can be reused!
}
```
```javascript
// OK: CORRECT: Delete state after verification
if (state === req.session.oauthState) {
