...
| `zIndex` | `number` | `1` | Stacking order |
> Warning: Documentation inconsistency: The official Zoom guides show a `webviewId` parameter in examples, but the TypeDoc type definition (v0.16.36) does not include it. Since there is only one webview per app, this parameter may be vestigial. If in doubt, omit it.
...
- Default renderTarget: 1280x720 (configurable)
