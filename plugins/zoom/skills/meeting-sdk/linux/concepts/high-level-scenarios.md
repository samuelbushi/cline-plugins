...
OK: Always check capabilities before calling methods
OK: Use heap memory mode for raw data
OK: Handle authentication fallbacks (OBF -> ZAK -> password)
OK: Detect SDK version at compile/runtime
OK: Test with actual SDK headers, not documentation examples
OK: Setup PulseAudio for Docker/headless environments
OK: Parse error codes to adapt behavior
OK: Keep authentication tokens fresh (regenerate before expiry)
OK: Log everything for debugging version-specific issues