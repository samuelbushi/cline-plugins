...
+-------------+    +-------------+    +-------------+    +-------------+
                   ...
                   OnAuthComplete      onInMeeting
                     callback           callback
```
Warning: CRITICAL: Add Windows message loop or callbacks won't fire!
```cpp
while (!done) {
    MSG msg;
...
Warning: WITHOUT THIS, CALLBACKS WON'T FIRE!
```cpp
#include <windows.h>
#include <thread>
#include <chrono>
...
---
### 2. Windows Message Loop (Warning: MOST COMMON ISSUE)
