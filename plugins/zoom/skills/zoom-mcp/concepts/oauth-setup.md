...
the user's own MCP client configuration. This Cline plugin does not register MCP servers or
store bearer tokens.
Authentication path:
- manually complete Zoom user-level OAuth
- keep bearer tokens outside chat in the user's terminal, environment, secret manager, or MCP settings
- configure the chosen MCP entry yourself, then use [`/setup-zoom-mcp`](../../setup-zoom-mcp/SKILL.md)
...
Cline or your terminal flow so the token exchange can continue manually.
Pros:
...
- this plugin documents the server, but does not register it by default
...
## Step 5: Provide the Token to the User-Managed MCP Entry
Configure the token wherever the user's MCP client expects secrets. Environment variables are one common pattern:
```bash
...
```
Verification:
- reconnect the user-managed MCP server or restart the Cline session if the MCP client requires it
- in Cline, use [`/setup-zoom-mcp`](../../setup-zoom-mcp/SKILL.md) after the user confirms MCP is connected
...
| Client update | Update the token in the user-managed MCP configuration, then reconnect that MCP server |
Refresh exchange:
```bash
curl -X POST https://zoom.us/oauth/token \
  -u "CLIENT_ID:CLIENT_SECRET" \
...
- reconnect the user-managed MCP server if you are injecting tokens through env vars
