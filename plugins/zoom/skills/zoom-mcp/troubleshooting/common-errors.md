...
Cause: The user-managed MCP entry does not have a token available, so no Authorization header is
sent to the Zoom MCP endpoint.
Fix: set the token wherever the user's MCP client expects it, then reconnect the MCP server.
```bash
...
Fix: refresh the token, update the user-managed MCP configuration, reconnect the MCP server, and verify the MCP-specific granular scopes.
## Scope Issues
...
Fix:
- confirm the user configured the MCP server entry
- confirm the configured token source is available to the MCP client
- reconnect the MCP server or restart the Cline session if needed
