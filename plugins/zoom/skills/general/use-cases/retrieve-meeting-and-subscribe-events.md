...
app.get('/api/meetings/:meetingId', async (req, res) => {
  try {
    ...
    console.log(`OK: Now tracking meeting: ${meeting.topic} (${meeting.id})`);
    res.json({
      success: true,
      meeting: {
        id: meeting.id,
...
app.post('/webhooks/zoom', async (req, res) => {
  ...
  if (!verifyWebhookSignature(req)) {
    console.error('Do not: Invalid webhook signature');
  ...
  if (!trackedMeetings.has(meetingId)) {
    console.log(`  Ignoring event for untracked meeting ${meetingId}`);
  ...
  switch (event) {
    case 'meeting.started':
      console.log(`OK: Meeting started: ${payload.object.topic}`);
...
async function connectWebSocket() {
  ...
  ws.on('close', () => {
    console.log('Do not: WebSocket connection closed. Reconnecting...');
    ...
    ZoomMtg.init({
      ...
      success: () => {
        ...
        ZoomMtg.join({
          ...
          userName: userName,
          success: () => {
            console.log('OK: Joined meeting:', credentials.topic);
...
---
## Common Pitfalls
### Do not: Misconception: "Subscribe to Events for a Specific Meeting"
...
### Do not: Misconception: "All Events Are the Same"
