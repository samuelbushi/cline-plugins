...
- OK: Need video conferencing fast (hours, not weeks)
- OK: Want Zoom-like UI consistency
- OK: Don't have resources to build custom video UI
- OK: Need standard features (chat, share, participants, settings)
- OK: Want framework-agnostic solution (React, Vue, Angular, vanilla JS)
## When NOT to Use (Use Raw Video SDK Instead)
- Do not: Need complete custom UI control
- Do not: Building non-standard video experiences
- Do not: Need access to raw video/audio data for processing
- Do not: Want custom rendering pipeline
## Architecture
```
+-------------------------------------------------------------+
