...
5. Select events:
   - OK: `meeting.started`
   - OK: `meeting.ended`
   - OK: `meeting.participant_joined`
   - OK: `meeting.participant_left`
6. Save and activate
...
async function handleMeetingStarted(meetingId, payload) {
  ...
  console.log(`OK: Meeting started: ${object.topic} (ID: ${meetingId})`);
...
app.post('/webhooks/zoom', async (req, res) => {
  ...
  switch (event) {
    case 'meeting.started':
      ...
      console.log(`OK: Meeting started: ${meeting.topic}`);
      break;
    case 'meeting.ended':
