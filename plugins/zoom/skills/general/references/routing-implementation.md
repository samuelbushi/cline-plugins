...
interface Signals {
  ...
  zoomApps: boolean;
  zoomScribe: boolean;
  zoomSummarizer: boolean;
  zoomTranslator: boolean;
  oauth: boolean;
  rtms: boolean;
  teamChat: boolean;
...
export function detectSignals(rawQuery: string): Signals {
  ...
  return {
    ...
    zoomScribe: hasAny(q, ['zoom-scribe', 'transcribe file', 'transcribe recording', 'batch transcription', 'aiservices/zoom-scribe']),
    zoomSummarizer: hasAny(q, ['zoom-summarizer', 'summarize transcript', 'meeting recap api', 'action items', 'aiservices/zoom-summarizer']),
    zoomTranslator: hasAny(q, ['zoom-translator', 'translate text', 'batch translation', 'target_languages', 'aiservices/zoom-translator']),
...
function pickPrimarySkill(s: Signals): SkillId {
  ...
  if (s.preflight) return 'zoom-probe-sdk';
...
function buildChain(primary: SkillId, s: Signals): SkillId[] {
  ...
  if (s.rivet) chain.add('zoom-rivet-sdk');
  ...
  if (primary === 'zoom-summarizer' && s.zoomScribe) chain.add('zoom-scribe');
  if (primary === 'zoom-translator' && s.zoomScribe) chain.add('zoom-scribe');
  if (primary === 'zoom-translator' && s.zoomSummarizer) chain.add('zoom-summarizer');
  if (primary === 'zoom-summarizer' && s.zoomTranslator) chain.add('zoom-translator');
  if (s.zoomScribe || s.zoomSummarizer || s.zoomTranslator) {
  ...
  if (s.teamChatMcp && s.teamChat) {
    chain.add('build-zoom-team-chat-app');
  }
