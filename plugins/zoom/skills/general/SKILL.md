...
function detectSignals(rawQuery: string) {
  ...
  return {
    ...
    zoomScribe: hasAny(q, ['zoom-scribe', 'transcribe file', 'transcribe recording', 'batch transcription']),
    zoomSummarizer: hasAny(q, ['zoom-summarizer', 'summarize transcript', 'meeting recap api', 'action items']),
    zoomTranslator: hasAny(q, ['zoom-translator', 'translate text', 'batch translation', 'target_languages']),
...
function pickPrimarySkill(s: ReturnType<typeof detectSignals>): SkillId {
  ...
  if (s.zoomSummarizer) return 'zoom-summarizer';
  if (s.zoomTranslator) return 'zoom-translator';
  if (s.zoomScribe) return 'zoom-scribe';
...
function buildChain(primary: SkillId, s: ReturnType<typeof detectSignals>): SkillId[] {
  ...
  if (s.webhooks && !chain.includes('setup-zoom-webhooks')) chain.push('setup-zoom-webhooks');
  return chain;
}
```
Example:
- `Create a meeting, configure webhooks, and handle OAuth token refresh` ->
  `build-zoom-rest-api-app -> zoom-oauth -> setup-zoom-webhooks`
...
| Build deterministic automation/configuration/reporting with explicit request control | [build-zoom-rest-api-app](../rest-api/SKILL.md) |
| Receive event notifications (HTTP push) | [setup-zoom-webhooks](../webhooks/SKILL.md) |
| Receive event notifications (WebSocket, low-latency) | [setup-zoom-websockets](../websockets/SKILL.md) |
| Embed Zoom meetings in my app | [build-zoom-meeting-sdk-app](../meeting-sdk/SKILL.md) |
| Build custom video experiences (Web, React Native, Flutter, Android, iOS, macOS, Unity, Linux) | [build-zoom-video-sdk-app](../video-sdk/SKILL.md) |
| Build an app that runs inside Zoom client | [zoom-apps-sdk](../zoom-apps-sdk/SKILL.md) |
| Transcribe uploaded or stored media with AI Services Scribe | [zoom-scribe](../scribe/SKILL.md) |
| Summarize transcript text with AI Services Summarizer | [zoom-summarizer](../summarizer/SKILL.md) |
| Translate plain text or text files with AI Services Translator | [zoom-translator](../translator/SKILL.md) |
...
| Build Contact Center apps and channel integrations | [build-zoom-contact-center-app](../contact-center/SKILL.md) |
| Build Virtual Agent web/mobile chatbot experiences | [build-zoom-virtual-agent](../virtual-agent/SKILL.md) |
| Build Zoom Phone integrations (Smart Embed, Phone API, webhooks, URI flows) | [build-zoom-phone-integration](../phone/SKILL.md) |
| Build Team Chat apps and integrations | [build-zoom-team-chat-app](../team-chat/SKILL.md) |
| Build server-side integrations with Rivet (auth + webhooks + APIs) | [zoom-rivet-sdk](../rivet-sdk/SKILL.md) |
| Run browser/device/network preflight diagnostics before join | [zoom-probe-sdk](../probe-sdk/SKILL.md) |
...
| Build enterprise AI systems with stable API core + AI tool layer | [build-zoom-rest-api-app](../rest-api/SKILL.md) + [zoom-mcp](../zoom-mcp/SKILL.md) |
...
- If user chooses Rivet: chain `zoom-rivet-sdk` + `oauth` + `rest-api`.
...
- MCP remote server works over Streamable HTTP/SSE; use this path when the target client/agent supports MCP transports, such as Cline.
...
- Zoom MCP access is evolving; docs indicate a model where Zoom exposes product-scoped MCP servers (for example Meetings, Team Chat, Whiteboard).
...
| [Scribe Transcription Pipeline](use-cases/scribe-transcription-pipeline.md) | Transcribe uploaded files or S3 archives with AI Services Scribe using fast mode or batch jobs | [zoom-scribe](../scribe/SKILL.md) + optional [zoom-rest-api](../rest-api/SKILL.md) + optional [zoom-webhooks](../webhooks/SKILL.md) |
| [AI Services Text Intelligence](use-cases/ai-services-text-intelligence.md) | Chain Scribe, Summarizer, and Translator for transcript summary and localization workflows | [zoom-scribe](../scribe/SKILL.md) + [zoom-summarizer](../summarizer/SKILL.md) + [zoom-translator](../translator/SKILL.md) |
...
| [Meeting Bots](use-cases/meeting-bots.md) | Build bots that join meetings for AI/transcription/recording | [zoom-meeting-sdk-linux](../meeting-sdk/linux/SKILL.md) + [zoom-rest-api](../rest-api/SKILL.md) + optional [zoom-webhooks](../webhooks/SKILL.md) |
...
| [Rivet Event-Driven API Orchestrator](use-cases/rivet-event-driven-api-orchestrator.md) | Build a Node.js backend that combines webhooks and API actions through Rivet module clients | [zoom-rivet-sdk](../rivet-sdk/SKILL.md) + [zoom-oauth](../oauth/SKILL.md) + [zoom-rest-api](../rest-api/SKILL.md) |
| [Probe SDK Preflight Readiness Gate](use-cases/probe-sdk-preflight-readiness-gate.md) | Add browser/device/network diagnostics and readiness policy before Meeting SDK or Video SDK joins | [zoom-probe-sdk](../probe-sdk/SKILL.md) + [zoom-meeting-sdk](../meeting-sdk/SKILL.md) or [zoom-video-sdk](../video-sdk/SKILL.md) |
