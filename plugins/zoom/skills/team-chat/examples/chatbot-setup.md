...
async function handleButtonClick(payload, res) {
  ...
  try {
    switch (actionItem.value) {
      case 'option_a':
        await sendTextMessage(toJid, accountId, 'OK: You selected Option A');
        break;
      case 'option_b':
        await sendTextMessage(toJid, accountId, 'OK: You selected Option B');
        break;
      case 'cancel':
        await sendTextMessage(toJid, accountId, 'Do not: Cancelled');
        break;
      default:
...
- [LLM Integration](llm-integration.md) - Add Cline/GPT
...
- [Cline Chatbot Sample](https://github.com/zoom/zoom-chatbot-claude-sample)
