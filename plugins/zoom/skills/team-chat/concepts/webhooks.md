...
async function handleButtonClick(payload, res) {
  ...
  switch (actionItem.value) {
    ...
    case 'reject':
      await sendChatbotMessage(toJid, accountId, {
        body: [{ type: 'message', text: 'Do not: Rejected' }]
      });
      break;
    default:
...
```javascript
// OK: GOOD - Respond immediately, process async
app.post('/webhook', (req, res) => {
  verifyZoomWebhookSignature(req);
  // Respond immediately
...
// Do not: BAD - Slow processing blocks response
...
// Do not: BAD - Crash on unknown events
switch (event) {
  case 'bot_notification':
    return handleBotNotification(payload, res);
