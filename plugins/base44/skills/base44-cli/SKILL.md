---
name: base44-cli
description: Use this skill for Base44 CLI workflows such as project initialization, linking, scaffolding, resource configuration, type generation, deployment planning, authentication, and safe command selection.
metadata:
  sourcePackage:
    name: base44
    version: 0.0.50
...
## Cline Compatibility - Read This First

This skill activates on Base44 work or when a `base44/` folder exists. Check local project state first, then use the bundled references before external documentation.
...
3. If NO, decide between two initialization paths:
   - Provisioned app -- the Base44 app already exists because it was just provisioned through a Stripe Projects / projects.dev flow, OR `BASE44_APP_ID` (or `BASE44_PROJECTS_BASE44_APP_ID`) is already present in process environment:
...
## Cline Command Safety

Prefer project-local CLI execution and do not call `base44` directly. The CLI is installed locally as a dev dependency and must be accessed via a package manager:
...
Examples in the bundled references may show `-y` or `--yes` for CI/non-interactive usage. In Cline, do not use confirmation-bypassing flags unless the user explicitly approves that exact command. Do not inspect `.env` or `.env.local` just to detect provisioning state; those files may contain access and refresh tokens.

## Authentication Check

Before running Base44 CLI commands:

1. Check authentication status after the user approves local command execution by running:
   ```bash
   npx base44 whoami
   ```
...
This check should happen before other Base44 CLI commands, after command execution is approved.
...
- IF (user mentions "create/build app" OR "make a project"):
  ...
  - ELSE IF (app was provisioned externally -- `BASE44_APP_ID`/`BASE44_PROJECTS_BASE44_APP_ID` is already available in process environment, or a Stripe Projects / projects.dev flow just ran):
...
| Command | Description | Reference |
|---------|-------------|-----------|
| `base44 create` | Create a new Base44 project from a template | [create.md](references/create.md)  MUST READ |
...
| RLS Patterns           | Row-level security examples and operators   | [rls-examples.md](references/rls-examples.md)  READ FOR RLS |
...
- `npx base44 site deploy` - Deploy site only

## Common Workflows

### Creating a New Project

 MANDATORY: Before running `base44 create`, you MUST read [create.md](references/create.md) for:
...
| Update available message    | If prompted to update, run `npm install --save-dev base44@latest` in the project after user approval |
