...
Scaffolds a local project for an existing Base44 app. Use this when you already have a Base44 app and want to set up the local project files to work with it. The command is deterministic for CI, but in Cline it still writes files and requires explicit user approval before execution.
