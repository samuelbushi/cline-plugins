...
```typescript
// Users module for inviting users to the app.interface UsersModule {
  //   // Invite a user to the application.   // @param user_email - User's email address.   // @param role - User's role ('user' or 'admin').   // @returns Promise resolving when the invitation is sent.   // @throws Error if role is not 'user' or 'admin'.   // /  inviteUser(user_email: string, role: "user" | "admin"): Promise<any>;
}
```
