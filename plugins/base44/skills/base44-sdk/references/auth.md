...
| `redirectToLogin()` | `nextUrl: string` | `void` | Avoid - Prefer custom login UI with `loginViaEmailPassword()` or `loginWithProvider()` |
...
try {
  ...
  console.log('Access token received:', hasAccessToken);
  // Do not log response.access_token; it is a credential.
...
- Note: Prefer building custom login UI for better user experience
