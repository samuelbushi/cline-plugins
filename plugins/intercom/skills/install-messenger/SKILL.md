...
2. Identity Verification Secret (also called Messenger API Secret) -- Found on the [Messenger Security page](https://app.intercom.com/a/apps/_/settings/channels/messenger/security). This is the HMAC secret used to sign JWTs. It must never appear in frontend code or source-controlled files.

Ask for the Workspace ID. For the Identity Verification Secret, prefer that the user stores it directly as a server-side environment variable such as `INTERCOM_IDENTITY_SECRET`; do not ask them to paste the raw secret into chat. Do not proceed without the Workspace ID. If they don't have the Identity Verification Secret yet, direct them to the [Messenger Security page](https://app.intercom.com/a/apps/_/settings/channels/messenger/security) to enable it.
