...
Track progress through the debugging process with Cline's task plan or a concise checklist.
...
<decision_tree>
  IF docker_not_running:
    ASK APPROVAL, THEN RUN: docker compose up -d
    WAIT: for services to start (30-60 seconds)
  IF output_dev_not_running:
    ASK APPROVAL, THEN RUN: npx output dev
...
Based on the trace analysis, identify the error pattern and use the relevant error skill based on symptoms.
...
<decision_tree>
  ...
  ELSE:
    REVIEW: from a workflow-quality perspective for additional patterns
...
Use the user's current request and any linked files, traces, IDs, or follow-up clarifications.
