...
Track progress with Cline's task plan or a concise checklist.
...
Otherwise, ask before running `npm view @outputai/core version` and use that as the target.
...
Fetch `https://docs.output.ai/migrations`. This is the index page - it links to every hand-authored migration guide with a title keyed on the version boundary.
...
Also fetch `https://docs.output.ai/changelog` to cross-reference what shipped in the range - use it to fill in gaps the migration guides may not cover.
...
From the fetched guide(s), produce a Cline task plan or concise checklist of concrete changes, one item per change. Do not start editing yet.

For each todo, capture:
  - Which file(s) are affected (search the workspace for call sites of deprecated APIs the guide mentions).
...
Work through each todo. For every todo:
  ...
  2. Apply the change with Cline's normal file-editing tools.
  3. Mark it `completed` when done.

Do not batch completions - update the task plan or checklist after each one.
...
Then ask before running `pnpm install` (or `npm install` / `yarn install` - detect from the lockfile).
