...
Ask for approval before retrieving a workflow result. Results may contain customer data, generated content, or other sensitive payloads; summarize or save them without echoing raw values unless the user explicitly asks to see them.
...
# Get the result. Do not paste raw sensitive payloads into chat.
...
if [[ $(npx output workflow status $WORKFLOW_ID) == *"COMPLETED"* ]]; then
  npx output workflow result $WORKFLOW_ID > workflow-result.json
  echo "Workflow result saved to workflow-result.json"
else
