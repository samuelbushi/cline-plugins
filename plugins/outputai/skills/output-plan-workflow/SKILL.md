...
description: Use when the user asks to create, build, generate, scaffold, or plan a new workflow. Orchestrates the full planning process including architecture, steps, prompts, evaluators, and testing strategy.
---

Your task is to generate a comprehensive Output.ai workflow implementation plan in markdown format.

The plan can be displayed in chat or written to a file, depending on what the user requested. Do not create a plan file unless the user asked for one or approved writing it.

Track progress through the plan creation process with Cline's task plan or a concise checklist.
...
When writing a plan file, use: `.outputai/plans/YYYY_MM_DD_<workflow_name>_<task_name>/PLAN.md`
...
Analyze the user's workflow request and any provided instructions.
...
<step_output>
Output Draft Plan: in chat, or to the approved plan file.
</step_output>
...
<step_output>
Output Updated Plan: in chat, or to the approved plan file.
</step_output>
...
<step_output>
Output Reviewed Plan: in chat, or to the approved plan file.
</step_output>
...
<step_output>
Output Final Plan: in chat, or to the approved plan file.
</step_output>
...
3. Implement the workflow with the `output-build-workflow` skill, providing the plan file path, workflow name, and workflow directory.
