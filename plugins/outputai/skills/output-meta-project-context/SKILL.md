...
| Agent | Purpose |
|-------|---------|
| `workflow-planner` | Role perspective for designing workflow architecture and implementation blueprints |
| `workflow-debugger` | Role perspective for analyzing workflow execution traces and identifying issues |
| `workflow-quality` | Role perspective for reviewing code quality and validating implementations |
| `workflow-prompt-writer` | Role perspective for creating and optimizing LLM prompt templates |
| `workflow-context-fetcher` | Role perspective for gathering documentation and existing patterns |
