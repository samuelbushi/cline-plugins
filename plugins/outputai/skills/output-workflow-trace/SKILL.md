...
Ask for approval before retrieving full JSON traces. Treat traces as sensitive: inspect only the fields needed for debugging, redact secrets and customer data in summaries, and do not paste raw step inputs, outputs, or full trace payloads into chat unless the user explicitly requests them.
...
3. Check step inputs: Verify shape and relevant fields without exposing sensitive values
4. Check step outputs: Look at preceding step output shape and status without exposing sensitive values
...
4. Inspect step inputs and outputs only as needed, redacting sensitive values in any report
...
2. Review from the `workflow-quality` perspective for best practices
3. Make code fixes based on identified issues
4. Ask before re-running the workflow: `npx output workflow run <workflowName> <input>`
