...
The workflow skeleton path should come from the user's request, the selected plan, or a brief clarification question. If it has not been created yet, ask before running scaffold commands or writing the skeleton.

Please read the plan file and implement the workflow according to its specifications.

Track progress through the implementation process with Cline's task plan or a concise checklist.
...
<pre_flight_check>
  EXECUTE: Cline skill: `output-meta-pre-flight`
</pre_flight_check>
...
1. Read the plan file provided by the user or selected from `.outputai/plans/`
...
Update the selected workflow directory's `workflow.ts` with the workflow definition.
...
Update the selected workflow directory's `steps.ts` with all step definitions from the plan.
...
If the plan includes evaluator functions, implement them in the selected workflow directory's `evaluators.ts`.
...
If the plan includes LLM-based steps, create prompt templates in the selected workflow directory's `prompts/`.
...
Update the selected workflow directory's `README.md` with workflow-specific documentation.
...
Create at least one scenario file in the selected workflow directory's `scenarios/` for testing the workflow.
...
<post_flight_check>
  EXECUTE: Cline skill: `output-meta-post-flight`
</post_flight_check>
...
Workflow Name: provided by the user or inferred from the plan

Workflow Directory: provided by the user or inferred from the plan

Plan File Path: provided by the user

Additional Instructions:

Use the user's current request and any linked files, notes, or follow-up clarifications.
