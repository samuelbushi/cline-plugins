...
When current model selection matters, ask before making the network request, then fetch the 10 most recently released models per provider from the Vercel AI Gateway:
...
```
### Snapshot Fallback
If the command above returns nothing, likely because `jq`, `curl`, or network access is missing. Ask the user whether to proceed with project-local model conventions or to verify provider options another way.
