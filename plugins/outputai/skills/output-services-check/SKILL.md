...
Read-only checks such as `docker ps` and `curl` are safe diagnostics. Ask for explicit approval before starting, stopping, or restarting local services with `npx output dev` or `docker compose`.
...
IF api_not_responding:
  ...
  ASK APPROVAL, THEN RUN: output dev (if not already running)
