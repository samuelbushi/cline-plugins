...
Derive the name from the actual project -- check `Package.swift`, the `.xcodeproj` filename, or the app's product name. If the project name is unclear, ask the user rather than guessing. The examples below use `MyAPI` as a placeholder; substitute your real name wherever you see it (including `MyAPITestMocks`). Likewise, `MyApp` in the `embeddedInTarget` example stands in for whatever target name you are embedding into.
