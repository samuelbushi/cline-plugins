...
- Name the generated schema module after the project, using the `<ProjectName>API` convention (e.g. `RocketReserverAPI` for a project called `RocketReserver`). Derive the project name from `Package.swift` / the `.xcodeproj` / the app product name -- never ship the `MyAPI` placeholder. If the project name is not obvious, ask the user with `ask the user`.
...
- If Xcode MCP tools are available in the agent environment, prefer them over raw `xcodebuild` for building, running tests, and inspecting build issues after regenerating code.
