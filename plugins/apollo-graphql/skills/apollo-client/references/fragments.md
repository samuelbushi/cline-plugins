...
```tsx
// [no] Without data masking - component can access any data from parent
...
```tsx
// [yes] With data masking - component can only access its fragment data
import { UserCard_UserFragmentDoc } from "./fragments.generated";
...
- [yes] Less work: Automatic, no manual registration needed
- [no] Larger bundle: Fragments are inlined into every query that uses them

Fragment Registry:

- [yes] Smaller bundle: Fragments are registered once, referenced by name
- [no] More work: Requires manual registration of each fragment
- [no] May cause issues with lazy-loaded modules if the module is not loaded before the query is executed
- [yes] Best for deeply nested component trees where bundle size matters

Manual interpolation:

- [no] Most work: Manual imports and interpolation required
- [yes] Explicit: Clear fragment dependencies in code
...
```tsx
// [yes] Good: Each component has its own fragment
if (false) {
  gql`
    fragment UserCard_user on User {
      id
...
```tsx
// [no] Avoid: Sharing a generic fragment across components
...
```tsx
// [yes] Good: Single page-level query
if (false) {
  gql`
    query UserProfilePage($id: ID!) {
      user(id: $id) {
...
```tsx
// [no] Avoid: Multiple queries in different components
...
```tsx
// [yes] Good: Component reads fragment data
import { FragmentType } from "@apollo/client";
import { useSuspenseFragment } from "@apollo/client/react";
import { UserCard_UserFragmentDoc } from "./fragments.generated";
...
```tsx
// [no] Avoid: Component uses query hook
...
```tsx
// [yes] Good: Only necessary fields
if (false) {
  gql`
    fragment UserListItem_user on User {
      id
...
```tsx
// [no] Avoid: Requesting unused fields
if (false) {
  gql`
    fragment UserListItem_user on User {
      id
