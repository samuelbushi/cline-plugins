...
|------------------- |------------------------------ |---------------------------------- |
| Performance | [yes] Faster, inlined | [no] Slower: vtable indirection |
| Compile time | [no] Slower: monomorphization | [yes] Faster: shared code |
| Binary size | [no] Larger: per-type codegen | [yes] Smaller |
| Flexibility | [no] Rigid, one type at a time | [yes] Can mix types in collections |
| Use in trait fn() | [no] Traits must be object-safe | [yes] Works with trait objects |
| Errors | [yes] Clearer | [no] Erased types can confuse errors |
...
### [no] Avoid Dynamic Dispatch When:
...
```rust
// [yes] Use generics when possible
