...
// [no] Invalid cases
let person: Person = Builder::new().name("name".to_string()).build(); // [no] Compile error: Age required to `build`
let person: Person = Builder::new().age(30).build(); // [no] Compile error: Name required to `build`
let person: Person = Builder::new().age(30).email("myself@email.com".to_string()).build(); // [no] Compile error: Name required to `build`
let person: Person = Builder::new().build();// [no] Compile error: Name and Age required to `build`
```
...
### [yes] Use Type-State Pattern When:
