...
#### [no] Don't use a generic name for a test
```rust
#[test]
...
```
#### [yes] Use a name which reads like a sentence, describing the desired behavior
...
#### [no] Don't test multiple things in the same test
```rust
...
[no] Don't include many assertions in one test:
...
#### [no] What not to test with snapshot
...
## 5.6 [yes] Snapshot Best Practices
