...
|-------------- |------------------------------------------- |---------------------------------------------------------------- |
| Describe Why | [yes] Yes - explains tricky reasoning | [no] Not for documentation |
| Describe API | [no] Not useful | [yes] Yes - public interfaces, usage, details, errors, panics |
| Maintainable | 🚨 Often becomes obsolete and hard to reason | [yes] Tied to code, appears in generated docs and can run test cases |
...
```
* [yes] Write clear and descriptive What it does and how to use it.
* [yes] Use `# Examples` section to better explain how to use it.
* [yes] Prefer writing examples that can be tested via `cargo test`, even if you have to hide their output with starting `#`:
```rust
/// ```
/// let result = my_crate::add(2, 3);
/// # assert_eq!(result, 5);
/// ```
```
* [yes] Use `# Panics`, `# Errors` and `# Safety` sections when relevant.
...
### 📌 Best Practices
* [yes] Use examples generously -- they double as test cases.
* [yes] Prefer clarity over formality -- it's for humans, not machines.
* [yes] Prefer doc comments to explain usage, and leave implementation details to code comments if needed.
* [yes] Use `cargo doc --open` to check your output often.
* [yes] Add `#![deny(missing_docs)]` and other relevant doc lints in top-level modules if you want to enforce full doc coverage.
