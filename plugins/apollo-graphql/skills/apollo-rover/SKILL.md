...
>
> ```bash
> [no] rover schema describe my-graph@current             # error: looks for a file named that
> [no] rover graph fetch my-graph@current                 # dumps the full SDL into your context
> [yes] rover graph fetch my-graph@current | rover schema describe -
> ```
