---
name: airtable-cli
description: Use only when the user specifically asks for the `airtable-mcp` CLI, needs local Airtable scripting, or needs a fallback outside the plugin-owned Airtable MCP OAuth flow. Prefer the plugin-owned `airtable` MCP server for normal Airtable data work.
license: MIT
metadata:
    version: '1.0.0'
    author: airtable
...
In Cline, prefer the plugin-owned `airtable` MCP server for normal Airtable work. Use this CLI guidance only when the user specifically asks for `airtable-mcp`, needs local scripting, or needs a fallback outside the MCP OAuth flow. Do not install the CLI, configure tokens, or write credential files without explicit user approval.
...
4. Check access -- in `tools --json` output, check the `access` field: `read-only`, `write`, or `destructive`. Confirm with the user before running any write, schema change, bulk operation, permission change, automation change, external-surface action, or destructive tool.
