...
- `--no-agent-skills - Skip assistant skill installation. In Cline, prefer this flag unless the user explicitly asks to install upstream assistant artifacts.`
- `--quiet - Suppress output (for scripted usage)`

Examples:

```bash
# Run Cline-safe setup without installing upstream assistant skill artifacts
sentry cli setup --no-agent-skills

# Full setup can install upstream assistant skill artifacts; ask first.
sentry cli setup
