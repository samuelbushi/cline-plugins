...
- For read-only inspection, run the relevant command directly - the CLI handles authentication and org/project detection automatically. Don't pre-authenticate or look up org/project before running commands. If auth is needed, ask the user to run the interactive auth flow.
...
- In Cline, also confirm before commands that mutate Sentry resources, upload artifacts, create releases or deploys, modify dashboards or alerts, change org/team/repo settings, or call `sentry api` with POST, PUT, PATCH, or DELETE.
...
- Do not run `sentry auth token` unless the user explicitly asks to inspect credentials, and do not print the token back into chat or logs.
- If the CLI reports the wrong org/project, override with explicit `<org>/<project>` arguments
- When this skill points at `references/...`, use the `read_sentry_cli_reference` tool to load the bundled details instead of guessing flags from memory.
...
Run `sentry --help` or the relevant command's `--help` output for the complete exit-code reference.
...
- `sentry auth token` - Print the stored authentication token. Avoid from Cline unless the user explicitly asks to inspect credentials, and never echo the raw token into chat or logs.
