...
def _encode_file_path(file_path: str) -> str:
    """
    URL-encode a Domino file path while preserving directory separators.
    """
    return "/".join(urllib.parse.quote(part, safe="") for part in file_path.split("/"))
...
async def run_domino_job(user_name: str, project_name: str, run_command: str, title: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Could not parse run_command: {e}"}

    if not command:
        return {"error": "run_command must not be empty"}
...
async def get_domino_environment_info() -> Dict[str, Any]:
    ...
    indicate whether DOMINO_HOST and DOMINO_API_KEY are available.
    """
    inside_domino_workspace = _is_domino_workspace()
    info: Dict[str, Any] = {
        "inside_domino_workspace": inside_domino_workspace,
    }

    if inside_domino_workspace:
        ...
        info["project_type_note"] = (
            "The MCP server does not infer Git versus DFS from its process "
            "working directory. Inspect the active workspace paths before "
            "using DFS file tools."
        )
    else:
        ...
        missing = []
        if not domino_host:
            missing.append("DOMINO_HOST")
        if not domino_api_key:
            missing.append("DOMINO_API_KEY")
        info["domino_host"] = domino_host.rstrip("/") if domino_host else None
        info["auth_mode"] = "api_key"
        info["ready"] = not missing
        info["missing_environment"] = missing
        info["note"] = (
            "Running outside Domino. Set DOMINO_HOST and DOMINO_API_KEY in "
            "the environment before using authenticated Domino tools."
        )
...
async def upload_file_to_domino_project(
    ...
    encoded_file_path = _encode_file_path(file_path)
    url = f"{_get_domino_host()}/v1/projects/{encoded_user_name}/{encoded_project_name}/{encoded_file_path}"
