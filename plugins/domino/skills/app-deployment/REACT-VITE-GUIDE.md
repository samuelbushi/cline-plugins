...
Do not put model API bearer tokens in `VITE_*` variables. Vite embeds `VITE_*`
values into browser JavaScript, so any token there is visible to every app user.
Keep `MODEL_API_TOKEN` server-side in Domino app settings, a backend proxy, or a
trusted service that calls the model endpoint on behalf of the browser.
...
export async function predict(inputData) {
  // Preferred: call a same-origin backend route that stores MODEL_API_TOKEN
  // server-side and forwards the request to the Domino Model API.
