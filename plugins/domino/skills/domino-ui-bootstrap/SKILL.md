...
- Existing Vite + React project - leave the project alone, retrofit just the Domino bits (deps, theme provider, optional Storybook MCP notes, and project guidance).
...
2. Project root path - where shared project guidance and optional MCP notes should live. Default to the target path; set this to a parent directory when the target is a subfolder of a larger repo (monorepos, an existing project with apps under `apps/`, etc.). Ask explicitly; don't assume. If the user doesn't volunteer one, propose the target path and confirm.
3. Display name - free-form, any string. Used in UI titles and the hand-off message. Example: `Domino Frontend`.
...
5. What to do if the path has unexpected content - ask plainly once you've inspected the directory (Step 2). The options depend on what you find; see Step 2.
...
- At the project root (from Step 1 - may equal the app folder, may not): are there existing project guidance files, MCP settings, or repo-specific instructions? These usually live at the project root, not the app folder, so check there even when the app folder is a fresh empty subdirectory.
...
## Step 6 - Prepare optional Storybook MCP guidance

The Storybook MCP lets future Cline sessions look up real component props instead of guessing. In Cline, do not silently create or edit user MCP settings as part of a scaffold. Treat the Storybook MCP as optional user-managed setup unless the user explicitly asks you to configure it.

If the user asks for Storybook MCP setup, preview the target settings file and ask for approval before writing anything. The server should be named `storybook` and point at the Domino library's live Storybook:

- URL: `https://main--60c0de3f60dd96003bdcb1a1.chromatic.com/mcp`
- Transport: `http`

If existing MCP settings already contain `storybook` at a different URL, ask before changing it. If the user does not want MCP setup, continue with the safe component list in Step 9 and tell them that deeper component additions should use current Storybook docs or an explicitly configured Storybook MCP.
...
These are the components and prop shapes confirmed to work against the published `@dominodatalab/extensions-tools`. Use them for the starter screen without needing to consult the MCP. For anything beyond this set, query the Storybook MCP or current Storybook docs first - don't guess.
...
`node_modules/@dominodatalab/extensions-tools/README.md` (and any code snippets it embeds) imports from `@domino/base-components` - that's the Storybook-internal alias, not the published package name. Even though `node_modules` reads as authoritative, every import in this project must use `@dominodatalab/extensions-tools`. Project guidance written in Step 10 should remind future sessions of this, but the starter screen is the first place it can go wrong.

---

## Step 10 - Write or update project guidance

If the project already has a standard guidance file, add a Domino section there after previewing the edit and getting approval. If it does not, ask the user where they want persistent project guidance recorded; do not invent a tool-specific file name without consent.

The Domino guidance should tell future Cline sessions four things:
...
It should also describe the optional Storybook MCP lookup workflow (`list-all-documentation` -> `get-documentation` -> `get-documentation-for-story`), note that React 18 / react-router 5 versions are pinned for peer-dep reasons, and remind that all backend URLs route through `apiBase` (not root-absolute paths).

Always merge rather than overwrite - preserve whatever project-specific guidance is there, and add a Domino section. The user's existing guidance may have important info about their codebase that you'd erase by replacing it.
...
- User-specific MCP or assistant settings files. These can leak machine-specific paths and personal preferences. If you add any user-local settings while helping the user, make sure the relevant ignore entry exists or surface the missing entry to the user.
...
- If the user has committed something this skill is now telling git to ignore (e.g., they checked in `node_modules` once by accident, or user-local assistant settings are already tracked), don't run `git rm` on their behalf - surface it and let them decide.
...
- That future component additions should go through the Storybook MCP or current Storybook docs.
...
- Test cases over conviction. When unsure whether a component or prop exists, query the Storybook MCP or current Storybook docs. Don't ship invented props.
...
- Building anything beyond a minimal app before Storybook references are available. If the user asks for more than a minimal application while the MCP is not available, only scaffold the boilerplate plus a minimal app, then ask them whether to configure Storybook MCP or proceed from current docs.
