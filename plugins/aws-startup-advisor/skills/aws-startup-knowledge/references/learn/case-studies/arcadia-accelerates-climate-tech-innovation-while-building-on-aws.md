---
source_url: https://aws.amazon.com/startups/learn/arcadia-accelerates-climate-tech-innovation-while-building-on-aws
...
Did you know that a lack of unified data is an issue that occurs both within companies as well as across industries? [Arcadia](https://www.arcadia.com/), a climate technology company founded by Kiran Bhatraju in 2014, unifies energy utility consumption and pricing data, making the data accessible and useful to businesses and individuals.
...
With more than seven years of building on AWS, and as long-time members of [AWS Activate](https://aws.amazon.com/activate/)--a program for startups-- Arcadia agrees firsthand that AWS cloud is the right choice for climate tech companies to build, scale, and go to market.

Benefits such as AWS credits supported their work to build and scale in the early days, while an [AWS Well-Architected](https://aws.amazon.com/architecture/well-architected) review empowered them with key insights that helped them select the right AWS services to build up the microservices stack they would best support their technical goals.
...
On the compute side, Arcadia relies heavily on [Amazon Elastic Kubernetes Service (Amazon EKS)](https://aws.amazon.com/eks/) for their Kubernetes workloads. Their data acquisition workflows also rely on [AWS Lambda](https://aws.amazon.com/lambda/), a serverless event-driven service that allows you to run code without managing or provisioning servers.

For queuing, Arcadia uses [Amazon Simple Queue Service (Amazon SQS)](https://aws.amazon.com/sqs/). They chose [Amazon EventBridge](https://aws.amazon.com/eventbridge/), a serverless event router, for event streaming between their services.

To meet their database and storage needs, Arcadia primarily uses Amazon Aurora, along with [Amazon DynamoDB](https://aws.amazon.com/dynamodb/). Arcadia keeps their historical data and files stored in [Amazon Simple Storage Service (Amazon S3)](https://aws.amazon.com/s3/) buckets.

Machine learning is a tool that allows Arcadia to enhance positive outcomes for their customers. "When we take all of that data, we use tools like [Amazon SageMaker](https://aws.amazon.com/sagemaker/) to help us drive more customer value," says Anil. "We also use SageMaker to add more efficiency for our internal team."
...
As is the case with many startups, saving money while optimizing their product is a priority for Arcadia. Throughout their company journey, Arcadia has kept a consistent focus on right-sizing their AWS infrastructure to support both their business priorities and customer needs. Additionally, they use AWS tools such as [Savings Plans](https://aws.amazon.com/savingsplans/) and [Amazon EC2 Reserved Instances](https://aws.amazon.com/ec2/pricing/reserved-instances/) to improve efficiency and get the most out of their cloud spend.
...
Along with building their tech stack on AWS, Arcadia works with their AWS [account team](https://aws.amazon.com/startups/learn/meet-your-aws-account-team) to find opportunities that can grow their customer base and provide a better customer experience.

"The startups organization within AWS has been helping us to find new opportunities to scale with our customers," explains Anil. "One way is by leaning into an offering on the [AWS Marketplace](https://aws.amazon.com/marketplace) as another procurement channel where customers can sign up for the Arc platform."
...
Market opportunities for climate tech startups continue to grow as large companies and consumers take a stronger stance on the importance of sustainable choices. By 2027, the climate tech market is [expected to reach nearly $1.4 trillion](https://www.forbes.com/sites/mariannelehnis/2022/12/31/booming-investment-in-renewables-is-set-to-continue-in-2023-and-beyond/), representing a compound annual growth rate of 8.8%.

Kiran cites the [Inflation Reduction Act](https://www.whitehouse.gov/cleanenergy/inflation-reduction-act-guidebook/), which passed in August of 2022, as an example. "That is the largest climate investment by any country in history, with a financial impact between $350 million and $1 trillion," he says. "It's a hard opportunity for startups to ignore."
