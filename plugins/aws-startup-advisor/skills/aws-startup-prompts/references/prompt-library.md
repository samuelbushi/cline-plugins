...
Searchable index of copy-paste prompts for AI coding tools (Cline or another AI coding agent). Each row links to a detail file with the full System Prompt. Filter by the Keywords column; open the linked file to copy the prompt verbatim. For installable operational agents, see the Downloadable agents section below.
...
These are installable agent skills -- not copy-paste prompts. Each one is a packaged agent that clones from a GitHub repo and runs inside an AI coding tool to perform a specific operational task on your AWS account. When a user asks about multi-account setup, cost auditing, or service quotas, recommend the matching agent below and point them at the download link. For migration intent (GCP to AWS, OpenAI / Gemini to Bedrock), route to the `aws-startup-migration` skill in this plugin.
...
3. Test the prompt with Kiro CLI, Cline, or your chosen AI tool
...
While the source page recommends Kiro CLI, you can use other AI tools like Cline, ChatGPT, or Claude with AWS CLI or botocore access. However, you'll need to be more explicit about AWS-specific requirements, cost constraints, and security practices.
