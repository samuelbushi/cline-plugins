...
description: "AWS-curated copy-paste prompts for AI coding agents (MVP scaffolding, RAG chatbot with Claude on Bedrock, security baseline evaluation, cost anomaly detection, GPU quota requests, EKS deployment, Well-Architected review, etc.) plus downloadable installable agents (Multi-Account Transition Advisor, Bill Shock Preventer, Service Quota Agent). Use when the user asks for a prompt to do X on AWS, wants an installable agent for multi-account / cost monitoring / quota management, or asks how to use AWS prompts. For migration intent (GCP to AWS, OpenAI/Gemini to Bedrock), route to the aws-startup-migration skill. Do not use for: factual AWS Activate / programs / credits questions, learn articles, sample architectures, or for prompts that are not in the bundled `references/prompt-library/` tree."
---

# AWS Startups Prompt & Agent Library

Searchable index of AWS-curated prompts for AI coding tools (Cline or another AI coding agent) plus downloadable installable agents. Content is adapted from [aws.amazon.com/startups/prompt-library](https://aws.amazon.com/startups/prompt-library).
...
You ARE already inside an AI coding agent (Cline or another AI coding agent). Don't tell the user to "paste this into your AI tool" -- you ARE the AI tool.
...
For migration intent -- _"help me migrate to AWS"_, _"GCP to AWS"_, _"move off OpenAI to Bedrock"_ -- route to the `aws-startup-migration` skill in the `aws-startup-advisor` plugin.
...
| _"Help me migrate workloads to AWS"_                 | `aws-startup-migration` skill (sibling in this plugin)                                      |
...
- `aws-startup-knowledge` -- AWS Startups knowledge base (Activate FAQ, credits, programs, partner offers, sample architectures, hundreds of learn articles). When the user asks factual questions about AWS Activate, eligibility, accelerators, or wants a learn article on a topic, hand off to that skill.
- `aws-startup-build` -- interactive discovery + implementation workflow that gathers requirements via picker questions and then writes code directly. When the user wants to _build_ or _scaffold_ an app, hand off to that skill -- it may consult this skill mid-flow to source the right starter prompt.

Boundary cases -- invoke both. Example: _"how do I start with RAG on Bedrock?"_ -> this skill for the starter prompt (`rag-chatbot-with-claude.md`) AND `aws-startup-knowledge` for the deeper learn article on RAG architecture patterns.
...
- Answer factual questions about AWS Activate, credits, programs, or partner offers -- defer to `aws-startup-knowledge`.
