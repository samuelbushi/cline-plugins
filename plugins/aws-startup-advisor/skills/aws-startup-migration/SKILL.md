...
7. Feedback checkpoint: After a phase completes, check if feedback is due (see rules below). This runs before advancing to the next phase. Feedback is local-only in this Cline plugin; do not submit telemetry or open external survey links.

   - After Discover (if `phases.feedback` is `"pending"`): Output to user:
     "Would you like to generate a local anonymized migration trace now? It stays in `.migration/` and is not submitted anywhere. It never includes resource names, file paths, account IDs, or secrets.
     [A] Generate local trace now
   ...
   - After Estimate (if `phases.feedback` is `"pending"`): Output to user:
     "Would you like to generate a local anonymized migration trace now?
     [A] Yes, generate local trace
...
- Optional local feedback trace generation with anonymized aggregate data
