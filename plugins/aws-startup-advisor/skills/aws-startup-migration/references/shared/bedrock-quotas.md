...
## The 5x Burndown Trap

Claude models (3.7+) on Bedrock consume quota at 5x the rate for output tokens. This is the single most common surprise for teams migrating from OpenAI (which has no output multiplier).
...
| ------------------ | ------------------ | ------------- | -------------------------- |
| RAG / long context | 80:20              | ~111K         | Need 18x increase to match |
| Balanced chat      | 60:40              | ~77K          | Need 26x increase to match |
| Code generation    | 20:80              | ~48K          | Need 42x increase to match |

Formula: `Effective TPM = TPM_quota / (input_ratio + output_ratio x 5)`

Nova, Llama, DeepSeek, and Mistral models have 1x burndown (no multiplier). Output-heavy workloads that hit Claude quota limits may benefit from routing to these models for applicable tasks.
...
| `"medium"`                | Claude (5x burndown)               | `"medium"`   | "Monitor TPM usage during parallel run; quota increase may be needed at peak" |
...
If code sets `max_tokens: 4096` (common OpenAI default) but typical responses are 200 tokens, each request reserves 20x more quota than needed -- dramatically reducing concurrency.

Migration action: Set `max_tokens` to ~1.5x expected output length. This is a code change that should be flagged in the migration guide when `quota_risk` is `"medium"` or `"high"`.
...
- [ ] `max_tokens` set to ~1.5x expected output (not model maximum)
...
```
Target TPM = (peak_tokens_per_minute_on_source_provider) x burndown_multiplier
```

Where `burndown_multiplier` = 5 for Claude models (because output tokens consume 5x quota), 1 for all others. If the user doesn't know their peak TPM, estimate from daily volume:

```
Estimated peak TPM = (daily_tokens / active_hours / 60) x 3  (3x for peak headroom)
```
