...
| `"medium"`                | Claude (5x burndown)               | `"medium"`   | Flag: "Monitor TPM usage; quota increase may be needed at peak"                   |
| `"medium"`                | Nova / Llama / other (1x burndown) | `"low"`      | No action                                                                         |
...
1. Embeddings + generation model detected -- If `models[]` contains both an embeddings model (capabilities_used includes `"embeddings"`) AND a text generation model:
   > Warning: "Migrating the embedding model (e.g., text-embedding-3-small -> Titan Embeddings v2) requires re-embedding all documents in your vector store. Plan for re-indexing time and temporary storage. Test retrieval quality with the new embeddings before switching generation model."
   > Warning: "Verify vector index dimension compatibility before cutover. OpenAI text-embedding-3-small outputs 1536 dimensions; Titan Embeddings v2 is configurable (256, 512, or 1024). A mismatch will cause insert failures at the vector store layer -- your index must be rebuilt at the target dimension before any data is written."
...
2. Models at different price tiers -- If `models[]` contains both a mini/nano/lite model AND a flagship model (infer from model_id naming: `*-mini`, `*-nano`, `*-lite` vs flagship):
   > Warning: "These models appear to work as a cascade or routing pattern (cheap model for classification/filtering, expensive model for generation). Test the Bedrock replacement pair together -- validate that the cheaper model's classification accuracy is preserved with its Bedrock equivalent before testing the expensive model."

3. More than 3 models -- If `models[]` count > 3:
   > Warning: "Multiple models detected ([count]). Recommend a tiered migration strategy: migrate and validate one model at a time, starting with the lowest-risk (highest-volume, simplest task). See Part 1B for tiered routing recommendations."

4. Text generation + image generation -- If `models[]` contains both text generation AND image generation capabilities:
   > Warning: "Image generation migration (e.g., DALL-E/gpt-image -> Nova Canvas) requires separate evaluation. Image quality is subjective -- plan for human evaluation alongside automated metrics."

5. Speech models -- If `models[]` contains speech-to-text or text-to-speech capabilities:
   > Warning: "Speech model migration targets different AWS services (Whisper -> Amazon Transcribe, TTS -> Amazon Polly or Nova Sonic) with different pricing models and APIs. These are not Bedrock model swaps -- they require separate integration work."
