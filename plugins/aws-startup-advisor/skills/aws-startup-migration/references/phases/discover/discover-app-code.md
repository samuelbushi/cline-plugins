...
- Model identifiers -- Look for model name strings passed to constructors or API calls:
  ...
  - Gemini 1.5 Legacy: Normalize `gemini-1.5-flash*` -> `model_id: "gemini-1.5-flash"` and `gemini-1.5-pro*` -> `model_id: "gemini-1.5-pro"`. Set `lifecycle_status: "legacy"` and `eol_date: "2025-09-24"` on these model entries in `ai-workload-profile.json`. Surface in the AI Context Summary during Clarify: "Warning: Gemini 1.5 Flash/Pro is past EOL (Sep 2025). Plan to upgrade the source model to 2.5 Flash/Pro as part of or before the AWS migration."
