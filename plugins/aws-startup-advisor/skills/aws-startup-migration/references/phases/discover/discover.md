...
Before creating or modifying `.migration/`, ask for explicit approval to create a local migration state directory and write planning artifacts under it. If approval is denied, stop and explain that the migration workflow requires local state files to resume safely.
...
- If found -> Ask for explicit approval before reading, parsing, summarizing, or processing billing/cost/usage exports. Explain that billing exports can include sensitive account, project, SKU, usage, and spend data. If approval is denied, skip billing discovery and continue with any Terraform or app-code discovery already approved.
...
2. Before writing or running a helper script, ask for explicit approval to create and execute a temporary local extraction script under `$MIGRATION_DIR`.
3. Write a script to `$MIGRATION_DIR/_extract_billing.py` (or `.js` / shell -- use whatever runtime is available) that:
...
4. Run the script: try `python3 _extract_billing.py` first. If `python3` is not found, try `python _extract_billing.py`. If neither is available, delete the script and fall back to loading `references/phases/discover/discover-billing.md`.
5. Write the script's JSON output to `$MIGRATION_DIR/billing-profile.json` with this exact schema:
...
6. Delete the script file after successful execution.
