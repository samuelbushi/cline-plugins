...
- Ask for explicit user confirmation before running production deploys, store submissions, EAS credential changes, EAS secret writes, signing changes, or paid/long-running cloud jobs.
- Prefer preparing configuration, validating locally, and explaining the exact command before executing live EAS, App Store Connect, or Google Play actions.
- Keep Apple, Google, Expo, and backend credentials in EAS secrets, environment variables, or platform secret stores, not in source files or chat transcripts.
