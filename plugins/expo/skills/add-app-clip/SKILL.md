...
The parent app's bundle ID becomes `com.<company>.<app-name>` and the Clip's is automatically derived as `<parent>.clip` (for example, `com.example.myapp.clip`).

## Cline Safety Notes

- Ask for explicit user confirmation before registering bundle IDs, creating App Store Connect records, deploying with EAS Hosting, setting up credentials, or submitting builds.
- Prepare and review configuration changes before invoking Apple, Expo, or store-side mutations.

## 1. Set `bundleIdentifier` and `appleTeamId`

The target creation helper warns if these are missing. Add the real bundle ID and Apple team ID to `app.json`:
...
Use the package runner already used by the project and confirm the current `@bacons/apple-targets` command from its docs before running it. For Bun projects, the helper is:
...
Pick a good icon or reuse the existing one defined in the app - check it with `npx expo config` or the project's equivalent Expo command under the `icon` or `ios.icon` key.
...
```json
{
  "expo": {
    "ios": {
      "associatedDomains": [
        "applinks:example.expo.app",
        "appclips:example.expo.app"
      ]
    }
  }
}
...
module.exports = (config) => ({
  type: "clip",
  icon: "./assets/app-clip-icon.png",
  entitlements: {
    "com.apple.developer.associated-domains": ["appclips:example.expo.app"],
  },
});
```
...
This logs in to the Apple Developer account, registers the real parent bundle ID, creates the App Store Connect entry, and prints:
...
```json
{
  ...
  "appclips": {
    "apps": ["TEAMID1234.com.example.myapp.clip"]
  },
...
<meta
  ...
  content="app-id=1234567890, app-clip-bundle-id=com.example.myapp.clip, app-clip-display=card"
/>
```
...
```sh
curl https://example.expo.app/.well-known/apple-app-site-association
```
  ...
  "apple": {
    "appClip": {
      "defaultExperience": {
        ...
        "reviewDetail": {
          "invocationUrls": ["https://example.expo.app/", null, null]
        },
        "info": {
          "en-US": {
...
- Parent app target: `com.example.myapp`
- App Clip target: `com.example.myapp.clip`, lives in `targets/clip/`
- AASA hosted at `https://example.expo.app/.well-known/apple-app-site-association`
