...
- Before saving charts, present each chart name, project, definition, filters, and visualization type to the user and wait for explicit confirmation.
- Save confirmed charts with `Amplitude:save_chart_edits` before building dashboard
...
- Present the batch of chart definitions and wait for explicit confirmation before calling `save_chart_edits`
- Save confirmed charts in batches using `save_chart_edits` (more efficient)
...
Before calling `Amplitude:create_dashboard`, present the dashboard name, target project, section layout, chart IDs, rich text blocks, and chart display configuration to the user. Wait for explicit confirmation.
