...
Reading and analysis operations are lower risk than writes, but live Amplitude data is still sensitive. Follow the Cline Safety section, keep reads scoped to the user's request, and use the taxonomy audit procedures in this skill for data-quality reviews.
...
Deprecation must always follow a phased process. Use the deprecation guidance in this skill and wait for explicit user approval before recommending or applying any write.
...
These are the Amplitude MCP server tools available for taxonomy work. Use the corresponding Amplitude MCP tool in Cline when taking each action.
...
Use the workflow sections in this skill for data quality audits, governance analyses, event verification, deprecation workflows, tracking plan generation, and tool usage strategy.
