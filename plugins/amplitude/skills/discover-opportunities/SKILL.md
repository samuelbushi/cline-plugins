...
description: >
  Discovers product opportunities by analyzing Amplitude analytics, experiments, session replays, and customer feedback. Synthesizes evidence into prioritized, actionable opportunities with RICE scoring. Use when the user asks to find opportunities from Amplitude data, identify product gaps in analytics, understand where users are dropping off, or create a data-driven backlog from product metrics. Do not trigger for general product brainstorming without Amplitude or analytics context.
---
