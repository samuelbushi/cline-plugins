...
name: debug-replay
description: >
  Turns Amplitude Session Replay bug evidence into reproducible steps by finding error sessions, extracting interaction timelines, and identifying the common action sequence that precedes the failure. Use when the user explicitly wants to investigate a bug, error spike, user session, or reproduction path with Amplitude Session Replay. Do not trigger for general bug reports unless Amplitude, replay, product analytics, or session data is part of the request.
---
