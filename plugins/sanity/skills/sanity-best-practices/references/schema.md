...
- [bad] Bad: `bigHeroText`, `redButton`, `threeColumnRow`, `color`, `fontSize`
- [good] Good: `heroStatement`, `callToAction`, `featuresSection`, `status`, `role`

The test: "If we redesigned the site, would this field name still make sense?"
- `threeColumnLayout` -> [bad] Fails (what if we go to 2 columns?)
- `features` -> [good] Passes (features are features regardless of layout)
...
// [bad] Wrong - index keys break Visual Editing
...
```typescript
// [good] Author is reusable and independently editable
defineField({
  name: 'author',
  type: 'reference',
  to: [{ type: 'author' }]
...
```typescript
// [good] SEO is document-specific, not shared
defineField({
  name: 'seo',
  type: 'object',
  fields: [
...
```typescript
// [good] Correct - relationship comes from a lookup
...
// [bad] Wrong - IDs encode relationships and source data
