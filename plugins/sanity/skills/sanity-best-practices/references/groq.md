...
// [good] Option A: groq tag (provides highlighting automatically)
import groq from "groq";
const QUERY = defineQuery(groq`*[_type == "post"]`);

// [good] Option B: Comment prefix (for plain template literals)
const QUERY = defineQuery(/* groq */ `*[_type == "post"]`);

// [good] Also valid: Just defineQuery (TypeGen works, but no editor highlighting)
...
// Warning: Order BEFORE slice, not after!
*[_type == "post"] | order(publishedAt desc)[0...10]  // [good] Correct
*[_type == "post"][0...10] | order(publishedAt desc)  // [bad] Wrong order
```
...
|---------|-------------|---------|
| `_type == "x"` | [good] Yes | `*[_type == "post"]` |
| `_id == "x"` | [good] Yes | `*[_id == "abc123"]` |
| `slug.current == $slug` | [good] Yes | `*[slug.current == "hello"]` |
| `defined(field)` | [good] Yes | `*[defined(publishedAt)]` |
| `references($id)` | [good] Yes | `*[references("author-123")]` |
| `field->attr == x` | [bad] No | Resolves reference for every doc |
| `fieldA < fieldB` | [bad] No | Compares two attributes |
...
```groq
// [bad] Slow: Resolves reference for every document
*[_type == "post" && author->name == "Bob Woodward"]

// [good] Fast: Direct _ref comparison
...
```groq
// [bad] Slow: Two separate subqueries
*[_type == "category"]{
  "parentTitle": parent->title,
  "parentSlug": parent->slug.current
}

// [good] Fast: Single subquery, merged
*[_type == "category"]{
...
```groq
// [bad] Slow: Must sort and skip 10,000 docs
*[_type == "article"] | order(_id)[10000...10020]

// [good] Fast: Cursor-based, only fetches 20
...
```groq
// [bad] Returns ALL fields including unused ones, metadata, revisions
*[_type == "post"]

// [good] Only fetch what the component needs
*[_type == "post"]{
  _id,
  title,
  "slug": slug.current,
...
```groq
// [bad] Not optimizable (computed attribute)
*[_type == "person"]{
  "fullName": firstName + " " + lastName
} | order(fullName)

// [good] Optimizable (original attribute)
