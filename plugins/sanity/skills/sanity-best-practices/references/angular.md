...
[bad] Bad: Fetching full-size images without width constraints.

```html
<img [src]="post.mainImage | sanityImage" />
```

[good] Good: Specifying width and using `auto('format')` for WebP/AVIF delivery.
...
[bad] Bad: Using `isPlatformBrowser()` in templates to conditionally render content  -  causes hydration mismatch.

[good] Good: Using `@defer` or `afterNextRender()` for browser-only code.
