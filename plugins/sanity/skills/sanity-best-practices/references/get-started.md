...
Before deploying, show the detected Sanity project ID, dataset, exact command, and expected effect. Wait for explicit user approval before running the command.
...
Before calling MCP content creation or edit tools, show the target project, dataset, document types, and document payload summary. Wait for explicit user approval before creating or editing Content Lake documents.
...
The Cline Sanity plugin registers the `sanity` MCP server automatically. If MCP content tools cannot see new types or fields, deploy the schema first with `npx sanity schema deploy`, then retry the MCP operation.
