...
| Scenario | Clean? | Why |
|----------|--------|-----|
| Comparing strings (`if (x === 'y')`) | [good] Yes | Stega breaks equality |
| Using as object keys | [good] Yes | Keys won't match |
| Using as HTML IDs | [good] Yes | Invalid characters |
| Passing to third-party libraries | [good] Yes | May validate input |
| Rendering text (`<h1>{title}</h1>`) | [bad] No | Breaks click-to-edit |
| Passing to `<PortableText />` | [bad] No | Handles internally |
| Passing to image helpers | [bad] No | Handles internally |
