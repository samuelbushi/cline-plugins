...
[good] Correct  -  use GROQ filters to exclude processed documents:
```typescript
defineDocumentFunction({
...
[good] Correct  -  use `@sanity/client` v7.12.0+ for automatic lineage headers:
```typescript
import { createClient } from '@sanity/client'
...
[bad] Incorrect  -  no recursion guard:
```typescript
defineDocumentFunction({
  name: 'update-post',
  event: {
...
Cost = invocations x (memory GB x duration seconds). Default is 1GB memory. A function averaging 1GB and 40ms duration can run ~500k invocations within 20K GB-seconds. [Monitor usage at the organization level](https://www.sanity.io/manage).
...
export const handler = documentEventHandler(async ({ context, event }) => {
  ...
  await fetch(WEBHOOK_URL, {
    ...
    body: JSON.stringify({
      text: `[note] New content published: *${event.data.title || event.data._id}* (${event.data._type})`,
    }),
  })
})
```
...
Use a Sanity API token with the minimum permissions needed to deploy the Blueprint. Prefer a dedicated robot token for CI/CD unless the specific API or organization policy requires a personal token.
