...
- `threeColumnFeatures` -> [bad] Fails (what if 2 columns?)
- `features` -> [good] Works (describes the content's purpose: a list of product features)
- `blueHighlightBox` -> [bad] Fails (what if we go purple?)
- `callout` -> [good] Works (describes the content's role: an attention-grabbing aside)

## Sanity Implementation

```typescript
// [bad] Avoid presentation-focused names
defineField({ name: 'bigHeroText', type: 'string' })
defineField({ name: 'fontSize', type: 'number' })
defineField({ name: 'backgroundColor', type: 'color' })

// [good] Use meaning-focused names
