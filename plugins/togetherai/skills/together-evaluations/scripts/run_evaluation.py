...
Usage:
    ...
    python run_evaluation.py --type classify --eval-model openai/gpt-5 \
        --eval-model-source external --eval-external-api-token-env OPENAI_API_KEY
...
def build_judge_config(args: argparse.Namespace, default_template: str) -> dict[str, Any]:
    ...
    if args.judge_external_api_token_env:
        config["external_api_token"] = read_secret_from_env(args.judge_external_api_token_env)
...
def build_model_config(
    *,
    ...
    max_tokens: int,
    temperature: float,
    external_api_token_env: str | None = None,
    ...
    if external_api_token_env:
        config["external_api_token"] = read_secret_from_env(external_api_token_env)
...
def read_secret_from_env(name: str) -> str:
    """Read a secret from an environment variable instead of a CLI argument."""
    value = os.environ.get(name)
    if not value:
        raise RuntimeError(f"Set {name} before using the matching external provider token option")
    return value
...
def run_classify(args: argparse.Namespace, dataset: list[dict[str, Any]]) -> None:
    ...
    if args.eval_column:
        ...
        model_to_evaluate = build_model_config(
            ...
            external_api_token_env=args.eval_external_api_token_env,
...
def run_compare(args: argparse.Namespace, dataset: list[dict[str, Any]]) -> None:
    ...
    if args.model_a_column and args.model_b_column:
        ...
        model_a = build_model_config(
            ...
            max_tokens=args.max_tokens,
            temperature=args.temperature,
            external_api_token_env=args.model_a_external_api_token_env,
        ...
        model_b = build_model_config(
            ...
            max_tokens=args.max_tokens,
            temperature=args.temperature,
            external_api_token_env=args.model_b_external_api_token_env,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--judge-external-api-token-env", help="Environment variable holding the external judge API key")
    ...
    parser.add_argument("--eval-external-api-token-env", help="Environment variable holding the external evaluation target API key")
    ...
    parser.add_argument("--model-a-external-api-token-env", help="Environment variable holding the external model A API key")
    ...
    parser.add_argument("--model-b-external-api-token-env", help="Environment variable holding the external model B API key")
