...
Usage:
    ...
    python manage_cluster.py create --name my-cluster --region us-central-8 --gpu-type H100_SXM --num-gpus 8 --driver-version CUDA_12_6_560 --i-understand-costs
    python manage_cluster.py demo --i-understand-costs --delete
...
def parse_args() -> argparse.Namespace:
    ...
    create_parser.add_argument(
        "--i-understand-costs",
        action="store_true",
        help="Required acknowledgement before creating billable GPU infrastructure",
    )
    ...
    demo_parser.add_argument("--scale-to", type=int, help="Optional GPU count to scale to after creation")
...
def main() -> None:
    ...
    if args.command == "create":
        if not args.i_understand_costs:
            raise SystemExit("Creating GPU clusters is billable. Pass --i-understand-costs after reviewing the target region, GPU count, and cleanup plan.")
        create_cluster(
            name=args.name,
            region=args.region,
    ...
    if not args.i_understand_costs:
        raise SystemExit("The demo creates billable GPU infrastructure. Pass --i-understand-costs after reviewing the defaults and cleanup plan.")
    if not args.delete:
        print("Warning: demo will leave the cluster running. Pass --delete to clean it up at the end.")
