...
```bash
ANIMATION_MAP=$(find "$HOME" -path '*/skills/hyperframes/scripts/animation-map.mjs' -type f 2>/dev/null | head -1)
node "$ANIMATION_MAP" <composition-dir> --out <composition-dir>/.hyperframes/anim-map
```
...
GSAP patterns and effects are in the `/hyperframes-gsap` skill.
