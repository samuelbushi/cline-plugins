...
- `/hyperframes-gsap` or `/hyperframes-gsap-core`, `/hyperframes-gsap-timeline`, `/hyperframes-gsap-plugins` --- deeper GSAP reference
...
- `the bundled contrast script` --- audit color contrast (WCAG)
- `/hyperframes-lottie`, `/hyperframes-three`, `/hyperframes-waapi`, `/hyperframes-animejs`, `/hyperframes-css-animations` --- if beat uses these engines
...
- Sub-agent: "Storyboard says cells at 56px but they read too small at 1920x1080. I used 96px." Accept - edit STORYBOARD.md beat N's cell size to 96px.
...
- - "Hold with breathing" implemented as `y: +-1---2px` or `scale: 1.01` --- invisible at 1920x1080+ scale. If continuous motion is required, use camera dolly (scale 1.0 - 1.05), parallax pan (x/y +-30---80px), or progressive reveals.
