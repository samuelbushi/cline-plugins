...
[ ] animation-map.json generated          → locate `animation-map.mjs` under the installed `skills/hyperframes/scripts/` directory and run it against `<project-dir>`; confirm every beat has events listed and no bbox/flag warnings
[ ] w2h-verify report                     → locate `w2h-verify.mjs` under the installed `skills/website-to-hyperframes/scripts/` directory and run it against `<project-dir>`; paste the FULL output (every row, every percent) verbatim into your final user-facing summary -- see "w2h-verify -- the source of truth" below
...
```bash
W2H_VERIFY_SCRIPT=$(find "$HOME" -path '*/skills/website-to-hyperframes/scripts/w2h-verify.mjs' -maxdepth 10 2>/dev/null | head -1)
node "$W2H_VERIFY_SCRIPT" <project-dir>
```

(Locate the installed script from a project subdirectory with the `find "$HOME" ...` command above.)
