...
[ ] animation-map.json generated          - locate and run the bundled `animation-map.mjs`; confirm every beat has events listed and no bbox/flag warnings
[ ] w2h-verify report                     - locate and run the bundled `w2h-verify.mjs`; paste the FULL output (every row, every percent) verbatim into your final user-facing summary --- see "w2h-verify --- the source of truth" below
...
```bash
W2H_VERIFY=$(find "$HOME" -path '*/skills/website-to-hyperframes/scripts/w2h-verify.mjs' -type f 2>/dev/null | head -1)
node "$W2H_VERIFY" <project-dir>
```

If the script cannot be found, say so explicitly and fall back to manual checks instead of inventing a path.
...
Scale snapshot count to the video --- not a fixed number. Formula: `max(beats x 3, ceil(duration_seconds / 2))`. A 3-beat 10s video: max(9, 5) = 9 frames. An 8-beat 60s video: max(24, 30) = 30 frames. Aim for at least 3 frames per beat: entrance, hold, and near-exit.
...
Open the Studio URL in a browser via Playwright (or another browser tool you have). Play start-to-end at 1.0x speed (NOT scrubbed). Confirm:

```
[ ] Played full video front-to-back at 1.0x --- actually played, not scrubbed
...
- Snapshots cover: <N> frames out of <video_duration x fps> total (<percentage>% coverage)
...
| `--resolution`    | `landscape` (1920x1080), `portrait` (1080x1920), `landscape-4k` (3840x2160), `portrait-4k` | Aliases: `1080p`, `4k`, `uhd`                                                      |
