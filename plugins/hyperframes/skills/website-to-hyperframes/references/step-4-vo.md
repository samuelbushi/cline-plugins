...
# Measure: seconds / words x total script words = estimated full audio length
```
...
- ElevenLabs: "Add `ELEVENLABS_API_KEY=your-key` to a local `.env` file in the project root, then tell me when it is set."
- HeyGen: "Add `HEYGEN_API_KEY=your-key` to a local `.env` file in the project root, then tell me when it is set."

Do not ask users to paste API keys into chat. If a key appears in chat accidentally, treat it as sensitive, avoid repeating it, and move credential setup back to local environment variables.
...
- Use the REST API only when `ELEVENLABS_API_KEY` is already available in the local environment:
...
- Use the v3 REST API only when `HEYGEN_API_KEY` is already available in the local environment:
