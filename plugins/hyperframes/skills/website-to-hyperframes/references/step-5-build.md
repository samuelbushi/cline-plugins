...
```bash
SFX_DIR=$(find "$HOME" -path "*/skills/website-to-hyperframes/assets/sfx" -type d 2>/dev/null | head -1)
cp -r "$SFX_DIR" <project-dir>/sfx/
```
...
Do not assume the user has an upstream HyperFrames repository checkout. If the project was scaffolded by the current HyperFrames CLI and already includes the shader runtime, use that. If it does not, prefer a non-shader CSS transition or install the needed runtime through the documented HyperFrames package/registry flow for the user's installed version.

Full `index.html` shape --- every field matters, but use the shader runtime path that exists in the user's project:

```html
<script src="https://cdn.jsdelivr.net/npm/hyperframes-gsap@3.14.2/dist/hyperframes-gsap.min.js"></script>
<script src="<project-shader-runtime-path>"></script>
...
If your runtime supports parallel sub-agents or delegated workers: dispatch one sub-agent per beat --- 3 to 4x faster than building sequentially. For 3+ beats, always dispatch in parallel. For 1---2 beats, sequential is fine.
...
where N follows the snapshot formula: `max(beats x 3, ceil(duration_seconds / 2))`. This is the canonical snapshot that Step 6's DoD uses --- not any individual sub-agent's intermediate snapshots.
