---
name: hyperframes-website-video
description: |
...
Lint, validate, take snapshots scaled to video length (formula: `max(beats x 3, ceil(duration_seconds / 2))`), and review each one. Fix issues before delivering. Deliver the localhost Studio project URL --- only render to MP4 on explicit user request.
