...
Recommended setup: Use this plugin's `aws-mcp` server when connected for sandboxed execution, audit logging, and enterprise controls.
...
Execute commands using available tools from this plugin's `aws-mcp` server when connected - it provides sandboxed execution, audit logging, and observability. When the MCP server is not available, fall back to the AWS CLI or shell as needed.
