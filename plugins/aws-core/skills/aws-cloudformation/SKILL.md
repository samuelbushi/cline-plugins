...
Critical: Pre-deployment validation errors are retrieved via `aws cloudformation describe-events --change-set-name <arn-or-name> --region <region>`. Do NOT use `describe-stack-events` - that API does not return validation errors. Note: `describe-events` is a newer API - if the command is not recognized, upgrade the AWS CLI to the latest version.
