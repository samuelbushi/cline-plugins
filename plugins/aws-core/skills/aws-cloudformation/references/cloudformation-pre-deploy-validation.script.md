...
Validation errors are exposed through the new `describe-events` API scoped to the change set. This procedure uses an available `aws-mcp` AWS API tool or the AWS CLI to invoke these APIs directly.

Important: The legacy `describe-stack-events` API does NOT return validation errors. You MUST use `describe-events --change-set-name <arn-or-name>` to retrieve validation results.
...
- You MUST check in this order of preference:
  1. An available `aws-mcp` AWS API tool for CloudFormation calls, if connected
...
- You MUST use `aws cloudformation describe-events --change-set-name <arn-or-name> --region <region>` via an available `aws-mcp` AWS API tool or the AWS CLI
...
- For each validation event, You MUST extract:
  ...
  - `ValidationStatus` - `FAILED` or `SKIPPED`
...
Validation results:
  No VALIDATION_ERROR events were returned.
...
`describe-stack-events` returns events after the stack begins provisioning. It does NOT include pre-deployment validation errors. You MUST redirect the user to `describe-events --change-set-name <arn-or-name>`.
