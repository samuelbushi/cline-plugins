...
Recommended setup: Use this plugin's `aws-mcp` server when connected for sandboxed execution, audit logging, and enterprise controls. See: https://docs.aws.amazon.com/aws-mcp/
...
You MUST ALWAYS use a script or calculator tool for any math on data returned from API calls. Write a Python script that performs the calculation and prints the result. If this plugin's `aws-mcp` server exposes a `run_script` tool, use it. Otherwise, run the script locally.
