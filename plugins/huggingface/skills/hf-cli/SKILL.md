...
This command inventory was generated from `huggingface_hub v1.19.0`. This Cline plugin already bundles the relevant skills, so do not run `hf skills add` or `hf skills update` to install skills for Cline.
...
- `hf skills add` - Download a Hugging Face skill and install it for an AI assistant outside this bundled Cline plugin. `[--claude --global --dest PATH --force --format [auto|human|agent|json|quiet]]`
...
- `hf skills update` - Update installed Hugging Face marketplace skills outside this bundled Cline plugin. `[--claude --global --dest PATH --format [auto|human|agent|json|quiet]]`
