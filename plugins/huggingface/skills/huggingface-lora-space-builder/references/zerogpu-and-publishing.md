...
- User already authenticated and the LoRA repo is private: try `api.repo_info(repo_id, token=cached)`. If it succeeds, the existing token has the right scope - proceed. If it fails, ask the user to authenticate outside chat with broader access.
- No cached token: ask the user to run `hf auth login` locally or set `HF_TOKEN` in their environment. One ask, with the explanation: "I need Hugging Face authentication with write scope. Please run `hf auth login` or set `HF_TOKEN`, then tell me when it is ready." The same authenticated session will be reused for publishing.
...
- `HfHubHTTPError: 401/403` on `upload_file`. The authenticated session lacks write scope. Fix: ask the user to run `hf auth login` with a write-scoped token or set `HF_TOKEN`, then retry.
