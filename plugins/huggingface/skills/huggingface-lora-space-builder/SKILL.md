...
- If no cached token, or the cached token can't read this private repo, ask the user to authenticate outside chat with `hf auth login` or set `HF_TOKEN` in their environment, then retry.

When asking for authentication (and only when you actually need to ask):

> I need Hugging Face authentication with write scope to read this LoRA if it is private/gated and to publish the Space. Please run `hf auth login` locally or set `HF_TOKEN` in your environment, then tell me when it is ready.

The same authenticated session will be reused for publishing in the final phase, so this is a one-time setup step.
...
- 401/403 on `upload_file`: the authenticated session does not have write scope. Fix: ask the user to run `hf auth login` with a write-scoped token or set `HF_TOKEN`, then retry.
