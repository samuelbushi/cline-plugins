...
If the user wants to run the same eval remotely on Hugging Face Jobs, use `hf-mcp` or `hf-cli` Jobs guidance with one of the local scripts in this skill.
...
- use `hf-mcp` or `hf-cli` Jobs guidance if the user wants remote compute.
...
5. If the user wants remote execution, use `hf-mcp` or `hf-cli` Jobs guidance with the same script + args.
...
then use `hf-mcp` or `hf-cli` Jobs guidance and pass it one of these scripts plus the chosen arguments.
...
| `13B+` | high-memory local GPU or Hugging Face Jobs via `hf-mcp` / `hf-cli` |
...
- CUDA or vLLM OOM:
  ...
  - if necessary, use Hugging Face Jobs via `hf-mcp` / `hf-cli`
