...
If you want to run these same scripts remotely, use Hugging Face Jobs via `hf-mcp` or `hf-cli` and pass one of the scripts in `scripts/`.
...
When local hardware is not enough, use Hugging Face Jobs via `hf-mcp` or `hf-cli` and run one of these scripts remotely. Keep the script path and args; move the orchestration there.
