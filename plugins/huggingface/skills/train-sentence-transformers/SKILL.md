...
- Ask for the target Hub repo and visibility before publishing. Default to private unless the user explicitly requests public. On HF Jobs (ephemeral env) ALSO enable in-trainer push (`push_to_hub=True` + `hub_strategy="every_save"`); details in `references/hf_jobs_execution.md`.
