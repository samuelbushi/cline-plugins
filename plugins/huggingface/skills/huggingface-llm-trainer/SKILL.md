...
1. Prefer the `hf_jobs()` MCP tool for remote Jobs - Prepare submissions using `hf_jobs("uv", {...})`, NOT bash `trl-jobs` commands. The `script` parameter accepts Python code directly. Do NOT save to local files unless the user explicitly requests it. Pass the script content as a string to `hf_jobs()`. Because Hugging Face Jobs are paid/background work, prepare the script, target repo, hardware flavor, timeout, visibility, and estimated cost, then ask for explicit user confirmation before submitting.
...
3. Provide job details after confirmed submission - After submitting, provide job ID, monitoring URL, estimated time, and note that the user can request status checks later.
...
- Authenticated login: Check with `hf_whoami()` or `hf auth whoami`
- HF_TOKEN for Hub Push - Training environment is ephemeral, must push to Hub or ALL training results are lost
...
- Do not ask the user to paste a write token into chat. Prefer MCP auth, `hf auth login`, or an existing `HF_TOKEN` environment variable.
...
IMPORTANT: Training jobs run asynchronously, can take hours, and may incur cost.
...
2. Present the exact submission plan: dataset, target repo, visibility, hardware flavor, timeout, estimated cost, and whether Hub push is enabled
3. Ask for explicit confirmation before calling `hf_jobs()` or starting any paid/background job
4. After confirmation, submit using the `hf_jobs()` MCP tool with script content inline - don't save to file unless user requests
5. Report submission with job ID, monitoring URL, and estimated time
6. Wait for user to request status checks - don't poll automatically
...
UV scripts use PEP 723 inline dependencies for clean, self-contained training. This is the primary approach for Cline.
...
When to use: Default choice for all training tasks in Cline, custom training logic, any scenario requiring `hf_jobs()`
...
When to use: User working in terminal directly outside Cline, quick local experimentation
Repository: https://github.com/huggingface/trl-jobs

In Cline context, prefer using the `hf_jobs()` MCP tool (Approach 1) when available.
...
3. Verify auth with the Hugging Face MCP account tool or `hf auth whoami`
...
- Authentication errors → Check the Hugging Face MCP account tool or `hf auth whoami`, token permissions, and the secrets parameter
...
7. Use UV scripts (Approach 1) - Default to `hf_jobs("uv", {...})` with inline scripts; TRL maintained scripts for standard training; avoid bash `trl-jobs` commands in Cline
