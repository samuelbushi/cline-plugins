...
```bash
curl -s "https://huggingface.co/api/datasets?filter=benchmark:official&limit=500" \
...
```bash
curl -s "https://huggingface.co/api/datasets/<namespace>/<repo>/leaderboard" \
...
```bash
# REST API
curl -s "https://huggingface.co/api/models/org/model1" \
  | jq '{safetensors, tags, cardData}'
