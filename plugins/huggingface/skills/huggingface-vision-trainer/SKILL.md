...
- `hf-mcp` and `hf-cli` - Hugging Face Jobs infrastructure, authentication, hardware flavors, timeout management, cost estimation, secrets, environment variables, scheduled jobs, and result persistence.
- `huggingface-llm-trainer` - TRL-based language model training (SFT, DPO, GRPO). Use that skill for text/language model fine-tuning.
...
- Use `hf-mcp` and `hf-cli` for token, secret, and Jobs command details.
...
For the full hardware flavor list, use the Hugging Face MCP tools or `hf jobs hardware`. For cost estimation, run `scripts/estimate_cost.py`.
