...
The Hub supports raw agent session traces from compatible coding agents. Upload them to Hugging Face Datasets as original JSONL files and the Hub can auto-detect supported trace formats, tag the dataset as `Traces`, and enable the trace viewer for browsing sessions, turns, tool calls, and model responses.
...
hf upload <namespace>/<repo> /path/to/agent/sessions traces/<project-or-cwd> --type dataset
```
