...
```js
// BAD Wrong - result stream never consumed; socket is left open
const resultSet = await client.query({ query: 'SELECT ...' })
// result is abandoned without calling .json(), .text(), .stream(), or .close()

// BAD Wrong - stream created but not fully piped/iterated
...
// OK Correct - consume via .json()
const resultSet = await client.query({ query: 'SELECT ...' })
const data = await resultSet.json()

// OK Correct - consume via async iteration
const resultSet = await client.query({
  query: 'SELECT ...',
  format: 'JSONEachRow',
})
...
// OK Correct - explicitly close; this destroys the underlying socket immediately
const resultSet = await client.query({ query: 'SELECT ...' })
resultSet.close()
```
...
> Warning If you still get errors after increasing, lower the value, not raise it.
...
> Warning Node.js caps total received headers at ~16 KB. After ~70-80 progress headers, an exception is thrown. For a query running longer than roughly `http_headers_progress_interval_ms * 75`, this limit will be hit - use a longer interval or the fire-and-forget approach below.
