...
### 1. mapbox__search_and_geocode_tool
...
### 2. mapbox__category_search_tool
...
### 3. mapbox__reverse_geocode_tool
...
| ------------------------------- | ----------------------- | ------------------------ |
| "Find Starbucks on Main Street" | mapbox__search_and_geocode_tool | Specific brand name      |
| "Find coffee shops nearby"      | mapbox__category_search_tool    | Generic category, plural |
| "What's at 37.7749, -122.4194?" | mapbox__reverse_geocode_tool    | Coordinates to address   |
| "Empire State Building"         | mapbox__search_and_geocode_tool | Specific named POI       |
| "hotels in downtown Seattle"    | mapbox__category_search_tool    | Generic type + location  |
| "Target store locations"        | mapbox__search_and_geocode_tool | Brand name (even plural) |
| "any restaurant near me"        | mapbox__category_search_tool    | Generic + "any" phrase   |
| "123 Main St, Boston, MA"       | mapbox__search_and_geocode_tool | Specific address         |
| "electric vehicle chargers"     | mapbox__category_search_tool    | Industry category        |
| "McDonald's"                    | mapbox__search_and_geocode_tool | Brand name               |
...
mapbox__category_search_tool only (1-25, default 10)
...
### types Parameter (mapbox__search_and_geocode_tool)
...
### auto_complete Parameter (mapbox__search_and_geocode_tool)
...
```javascript
// BAD
mapbox__category_search_tool({ category: 'starbucks' });
// "starbucks" is not a category, returns error

// GOOD
mapbox__search_and_geocode_tool({ q: 'Starbucks' });
```
...
```javascript
// BAD
mapbox__search_and_geocode_tool({ q: 'coffee shops' });
// Less precise, may return unrelated results

// GOOD
mapbox__category_search_tool({ category: 'coffee_shop' });
```
...
mapbox__category_search_tool({ category: 'restaurant' });

// GOOD - Biased to user location
mapbox__category_search_tool({
  category: 'restaurant',
...
mapbox__search_and_geocode_tool({
  q: 'pizza',
...
mapbox__search_and_geocode_tool({ q: 'museums' });
...
-> Coordinates -> Address
  -> mapbox__reverse_geocode_tool

-> Address -> Coordinates
  -> mapbox__search_and_geocode_tool with types: ["address"]
```
