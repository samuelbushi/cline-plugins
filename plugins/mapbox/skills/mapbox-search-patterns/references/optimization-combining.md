...
```
1. mapbox__category_search_tool → Get POIs
2. mapbox__distance_tool (offline) → Calculate distances
3. mapbox__bearing_tool (offline) → Get directions
```
...
```
1. mapbox__category_search_tool({category: "hospital", proximity: user_location})
   → Returns 10 hospitals with coordinates
2. mapbox__distance_tool(user_location, each_hospital)
...
```
1. mapbox__search_and_geocode_tool({q: "Space Needle"})
   → Get destination coordinates
2. mapbox__directions_tool({from: user_location, to: space_needle_coords})
...
```
1. mapbox__search_and_geocode_tool({q: "warehouse"})
   → Get warehouse coordinates
2. mapbox__isochrone_tool({coordinates: warehouse, time: 30, profile: "driving"})
   → Get 30-minute delivery zone polygon
3. mapbox__point_in_polygon_tool(customer_address, delivery_zone)
...
```
1. mapbox__category_search_tool({category: "restaurant", limit: 10})
   → Get restaurant coordinates
2. mapbox__static_map_image_tool({
...
1. Invalid category → Use `mapbox__category_list_tool` to see valid categories
...
```
1. mapbox__category_search_tool({category: "taco"}) → No results
2. Check: Is "taco" a valid category?
   → Use mapbox__category_list_tool → See "mexican_restaurant" is valid
3. Retry: mapbox__category_search_tool({category: "mexican_restaurant"}) → Success
```
...
Get valid categories: Use `mapbox__category_list_tool`

```
mapbox__category_list_tool()
```
