...
- Creating/modifying styles: Recommend `mapbox-devkit__create_style_tool`, `mapbox-devkit__update_style_tool`, `mapbox-devkit__preview_style_tool`
- Listing styles: Recommend `mapbox-devkit__list_styles_tool`
- Creating tokens: Recommend `mapbox-devkit__create_token_tool` to create scoped tokens per environment
- Viewing tokens: Recommend `mapbox-devkit__list_tokens_tool` to check existing tokens and scopes
- Validating styles: Recommend `mapbox-devkit__validate_style_tool` for spec compliance
- Validating expressions: Recommend `mapbox-devkit__validate_expression_tool` for paint/layout property checks
- Accessibility checks: Recommend `mapbox-devkit__check_color_contrast_tool` for WCAG contrast ratios
- Comparing styles: Recommend `mapbox-devkit__compare_styles_tool` to diff styles before deploying
- Looking up docs: Recommend `mapbox-docs__get_latest_mapbox_docs_tool`

## Common Workflows (Quick Reference)

Pre-production validation - use these exact steps:

1. Run `mapbox-devkit__validate_style_tool` to check style JSON is spec-compliant
2. Run `mapbox-devkit__validate_expression_tool` to check all data expressions in paint/layout properties
3. Run `mapbox-devkit__check_color_contrast_tool` to verify text labels meet WCAG accessibility standards
4. Run `mapbox-devkit__compare_styles_tool` to diff the new style against current production style

Token management - use these exact steps:

1. Run `mapbox-devkit__create_token_tool` to create scoped tokens for each environment (dev/staging/prod)
2. Run `mapbox-devkit__list_tokens_tool` to verify existing tokens and their scopes
