...
- Always validate external GeoJSON with `mapbox-devkit__validate_geojson_tool` before using as a source

When writing expressions:

- Validate expressions with `mapbox-devkit__validate_expression_tool` as you write them
...
- Check foreground/background contrast with `mapbox-devkit__check_color_contrast_tool`
...
- Use `mapbox-devkit__compare_styles_tool` to generate a diff report
...
- Run `mapbox-devkit__optimize_style_tool` to reduce file size
...
2. Validate with mapbox-devkit__validate_geojson_tool
...
Solution: Validate expressions with `mapbox-devkit__validate_expression_tool` during development
Prevention: Add expression validation to pre-commit hooks or CI/CD

### Poor Text Readability

Problem: Text labels are hard to read on map
Solution: Check contrast with `mapbox-devkit__check_color_contrast_tool`, adjust colors to meet WCAG AA
...
Solution: Run `mapbox-devkit__optimize_style_tool` to remove redundancies and simplify
...
Solution: Validate with `mapbox-devkit__validate_geojson_tool`, fix coordinate issues, verify structure
...
Solution: Use `mapbox-devkit__compare_styles_tool` to generate diff report
...
| --------------------------- | ---------------------- | -------------------------- |
| `mapbox-devkit__validate_geojson_tool`     | Adding GeoJSON sources | Valid/invalid + error list |
| `mapbox-devkit__validate_expression_tool`  | Writing expressions    | Valid/invalid + error list |
| `mapbox-devkit__check_color_contrast_tool` | Styling text labels    | Passes/fails + WCAG levels |
| `mapbox-devkit__compare_styles_tool`       | Reviewing changes      | Diff report with paths     |
| `mapbox-devkit__optimize_style_tool`       | Before deployment      | Optimized style + savings  |
