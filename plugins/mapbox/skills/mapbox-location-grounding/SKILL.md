...
If `mapbox__ground_location_tool` is available, use it - it handles reverse geocoding, POI search, place details enrichment, isochrone, and a static map image in one call:

```
mapbox__ground_location_tool(
  longitude, latitude,
...
Do not call `mapbox__reverse_geocode_tool`, `mapbox__category_search_tool`, `mapbox__place_details_tool`, or `mapbox__isochrone_tool` separately - they are already composed inside this tool.
...
If `mapbox__ground_location_tool` is not available, build the grounded response by composing these tools in order:

#### Step 1 - Establish place context

```
mapbox__reverse_geocode_tool(longitude, latitude, types: "neighborhood,locality,place")
```
...
```
mapbox__search_and_geocode_tool(query, proximity: {longitude, latitude}, limit: 10)
```

For generic categories:

```
mapbox__category_search_tool(category, proximity: {longitude, latitude}, limit: 10)
```
...
```
mapbox__place_details_tool(mapbox_id, attribute_sets: ["visit"])
```
...
```
mapbox__static_map_image_tool(longitude, latitude, zoom: 14)
```
...
For questions about travel between two locations, use `mapbox__directions_tool`. For questions about reachable area, use `mapbox__isochrone_tool`. For comparing travel times across multiple locations, use `mapbox__matrix_tool`.
...
Returns a polygon showing the area reachable within each time threshold. Combine with `mapbox__category_search_tool` or `mapbox__ground_location_tool` to answer "what restaurants are within a 10-minute walk?" - use the isochrone to define the boundary, then search within it.
...
Use `mapbox__matrix_tool` - do NOT call `mapbox__directions_tool` separately for each pair. Matrix batches all pairs in a single API call.
...
Returns a matrix of durations and distances between every source/destination pair. Rank results by duration to find the nearest location. One `mapbox__matrix_tool` call replaces N separate `mapbox__directions_tool` calls.

### Proximity search with routing context

For "find [category] within X minutes of [location]":

1. Geocode the origin if needed: `mapbox__search_and_geocode_tool(address)`
2. Get the reachable area: `mapbox__isochrone_tool(coordinates, profile, contours_minutes)`
3. Search for POIs: `mapbox__ground_location_tool(longitude, latitude, query: "category")` - the isochrone
   contours tell the user what's within each time band

Or more directly: `mapbox__ground_location_tool` returns both nearby POIs and isochrone reachability in one
...
- Coordinates required for proximity search - if the user provides an address, geocode it first with `mapbox__search_and_geocode_tool` before running category search.

## Anti-Patterns to Avoid

- Answering "what's near X?" from training data without calling search tools
- Estimating travel times without calling `mapbox__directions_tool` or `mapbox__isochrone_tool`
- Hallucinating business names, hours, or ratings
- Calling `mapbox__reverse_geocode_tool` + `mapbox__category_search_tool` + `mapbox__place_details_tool` separately when `mapbox__ground_location_tool` is available
