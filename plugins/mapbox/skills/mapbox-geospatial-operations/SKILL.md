...
| -------------------------------------------------- | ----------------------------------- | ---------------------------------------- |
| Straight-line distance (as the crow flies)         | `mapbox__distance_tool`                     | Accurate for geometric distance, instant |
| Driving distance (as the crow drives)              | `mapbox__directions_tool`                   | Only routing knows actual road distance  |
| Walking/cycling distance (as the crow walks/bikes) | `mapbox__directions_tool`                   | Need specific path network               |
| Travel time                                        | `mapbox__directions_tool` or `mapbox__matrix_tool`  | Requires routing with speed data         |
| Distance with current traffic                      | `mapbox__directions_tool` (driving-traffic) | Need real-time traffic consideration     |

Example: "What's the distance between these 5 warehouses?"

- As the crow flies → `mapbox__distance_tool` (10 calculations, instant)
- As the crow drives → `mapbox__matrix_tool` (5×5 matrix, one API call, returns actual route distances)
...
| ---------------------------- | ----------------------------------------------------- | ------------------------------------------------------------- |
| "Within X meters radius"     | `mapbox__distance_tool` + filter                              | Simple geometric radius                                       |
| "Within X minutes drive"     | `mapbox__isochrone_tool` → `mapbox__point_in_polygon_tool`            | Need routing for travel-time zone, then geometric containment |
| "Inside this polygon"        | `mapbox__point_in_polygon_tool`                               | Pure geometric containment test                               |
| "Reachable by car in 30 min" | `mapbox__isochrone_tool`                                      | Requires routing + traffic                                    |
| "Nearest to this point"      | `mapbox__distance_tool` (geometric) or `mapbox__matrix_tool` (routed) | Depends on definition of "nearest"                            |

Example: "Are these 200 addresses in our 30-minute delivery zone?"

1. Create zone → `mapbox__isochrone_tool` (routing API - need travel time)
2. Check addresses → `mapbox__point_in_polygon_tool` (geometric - 200 instant checks)
...
| ----------------------------------- | ----------------------------------- | --------------------------------- |
| A to B directions                   | `mapbox__directions_tool`                   | Turn-by-turn routing              |
| Optimal order for multiple stops    | `mapbox__optimization_tool`                 | Solves traveling salesman problem |
| Clean GPS trace                     | `mapbox__map_matching_tool`                 | Snaps to road network             |
| Just need bearing/compass direction | `mapbox__bearing_tool`                      | Simple geometric calculation      |
| Route with traffic                  | `mapbox__directions_tool` (driving-traffic) | Real-time traffic awareness       |
| Fixed-order waypoints               | `mapbox__directions_tool` with waypoints    | Routing through specific points   |

Example: "Navigate from hotel to airport"

- Need turn-by-turn → `mapbox__directions_tool`
- Just need to know "it's northeast" → `mapbox__bearing_tool`
...
| ------------------------- | ---------------- | ------------------------ |
| Simple circular buffer    | `mapbox__buffer_tool`    | Geometric circle/radius  |
| Travel-time zone          | `mapbox__isochrone_tool` | Based on routing network |
| Calculate area size       | `mapbox__area_tool`      | Geometric calculation    |
| Simplify complex boundary | `mapbox__simplify_tool`  | Geometric simplification |
| Find center of shape      | `mapbox__centroid_tool`  | Geometric centroid       |

Example: "Show 5km coverage around each store"

- 5km radius → `mapbox__buffer_tool` (geometric circles)
- "What customers can reach in 15 min?" → `mapbox__isochrone_tool` (routing-based)
...
- Routing APIs: Consider batch operations (mapbox__matrix_tool)

Large operations (> 1,000 calculations):

- Geometric tools: May need optimization but still fast
- Routing APIs: Definitely use batch tools (mapbox__matrix_tool handles up to 25×25)
...
| ----------------------------------- | -------------------- | ---------------------------------------- |
| Real-time geofencing (every second) | Geometric checks     | `mapbox__point_in_polygon_tool` (instant)        |
| Route planning (one-time)           | Full routing         | `mapbox__directions_tool` or `mapbox__optimization_tool` |
| Periodic proximity checks           | Geometric distance   | `mapbox__distance_tool`                          |
| Live traffic routing                | Routing with traffic | `mapbox__directions_tool` (driving-traffic)      |

Architecture note for high-frequency geofencing: If the application calls containment checks at very high rates (e.g., 50 vehicles every 2 seconds = 25 checks/second), calling MCP tools over the network adds agent-reasoning overhead that makes it impractical. In those cases, recommend using Turf.js directly in-process (`turf.booleanPointInPolygon`) for the hot path, and reserve MCP tools for peripheral tasks like zone definition (`mapbox__isochrone_tool`), rerouting (`mapbox__directions_tool`), or visualization (`mapbox__static_map_image_tool`).
...
1. Search stores → `mapbox__category_search_tool` (returns distances automatically)
2. Create coverage zone → `mapbox__buffer_tool` (5km geometric circle)
3. Visualize → `mapbox__static_map_image_tool`
...
1. Geocode addresses (if needed) → Use `mapbox__search_and_geocode_tool` to convert any street addresses to coordinates. Even when coordinates are already provided, mention this as an optional pre-step - real-world delivery lists often contain a mix of addresses and coordinates.
2. Optimize route → `mapbox__optimization_tool` (TSP solver - reorders stops to minimize total drive time)

Why `mapbox__optimization_tool` and NOT these alternatives:

- `mapbox__directions_tool` only routes A → B (or through fixed-order waypoints). It does NOT reorder stops - if you pass 8 stops, it routes them in the order given, which is almost never optimal.
- `mapbox__matrix_tool` gives travel times between all pairs of stops (8×8 = 64 values), but it does NOT compute the optimal ordering. You'd need to solve TSP yourself on top of the matrix - `mapbox__optimization_tool` does this for you in one call.

Always mention `mapbox__search_and_geocode_tool` as a useful companion for geocoding delivery addresses before optimization.
...
1. Create delivery zone → `mapbox__isochrone_tool` (30-minute driving)
2. Check each address → `mapbox__point_in_polygon_tool` (200 geometric checks)
...
1. Clean GPS trace → `mapbox__map_matching_tool` (snap to bike paths)
2. Get distance → Use API response or calculate with `mapbox__distance_tool`
...
1. Create buffers around each location → `mapbox__buffer_tool`
2. Calculate total area → `mapbox__area_tool`
3. Or, if time-based → `mapbox__isochrone_tool` for each location
...
mapbox__distance_tool({ from: A, to: B });
...
mapbox__point_in_polygon_tool({ point: location, polygon: boundary });
```
...
mapbox__distance_tool + calculate 20min * avg_speed

// CORRECT: Actual routing with road network
mapbox__isochrone_tool({
...
mapbox__bearing_tool({ from: hotel, to: airport });
...
```
1. mapbox__directions_tool → Get route geometry
2. mapbox__buffer_tool → Create corridor around route
3. mapbox__category_search_tool → Find POIs in corridor
4. mapbox__point_in_polygon_tool → Filter to those actually along route
```
...
```
1. mapbox__category_search_tool → Find 10 nearby locations
2. mapbox__distance_tool → Calculate straight-line distances (geometric)
3. For top 3, use mapbox__directions_tool → Get actual driving time
```
...
```
1. mapbox__isochrone_tool → Create travel-time zone (routing)
2. mapbox__point_in_polygon_tool → Check hundreds of addresses (geometric)
```
...
2. Does it require traffic awareness?
   YES → Use mapbox__directions_tool or mapbox__isochrone_tool with traffic profile
   NO → Continue

3. Is it a geometric/spatial operation?
   - Distance between points (straight-line) → mapbox__distance_tool
   - Point containment → mapbox__point_in_polygon_tool
   - Area calculation → mapbox__area_tool
   - Buffer/zone → mapbox__buffer_tool
   - Direction/bearing → mapbox__bearing_tool
   - Geometric center → mapbox__centroid_tool
   - Bounding box → mapbox__bbox_tool
   - Simplification → mapbox__simplify_tool
...
- `mapbox__distance_tool` - Straight-line distance between two points
- `mapbox__bearing_tool` - Compass direction from A to B
- `mapbox__midpoint_tool` - Midpoint between two points
- `mapbox__point_in_polygon_tool` - Is point inside polygon?
- `mapbox__area_tool` - Calculate polygon area
- `mapbox__buffer_tool` - Create circular buffer/zone
- `mapbox__centroid_tool` - Geometric center of polygon
- `mapbox__bbox_tool` - Min/max coordinates of geometry
- `mapbox__simplify_tool` - Reduce geometry complexity

### Routing & Navigation (APIs)

- `mapbox__directions_tool` - Turn-by-turn routing
- `mapbox__matrix_tool` - Many-to-many travel times
- `mapbox__optimization_tool` - Route optimization (TSP)
- `mapbox__isochrone_tool` - Travel-time zones
- `mapbox__map_matching_tool` - Snap GPS to roads
