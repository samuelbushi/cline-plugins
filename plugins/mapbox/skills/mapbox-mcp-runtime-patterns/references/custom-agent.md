...
class CustomMapboxAgent {
  ...
  async findPropertiesWithCommute(
    ...
    const isochrone = await this.callTool('mapbox__isochrone_tool', {
    ...
    const isInRange = await this.callTool('mapbox__point_in_polygon_tool', {
  ...
  async findRestaurantsNearby(location: [number, number], radiusMiles: number) {
    // Search restaurants
    const results = await this.callTool('mapbox__category_search_tool', {
      category: 'restaurant',
    ...
    for (const restaurant of restaurants) {
      const distance = await this.callTool('mapbox__distance_tool', {
...
class GeospatialService {
  ...
  async aiSearchNearby(userQuery: string): Promise<string> {
    ...
    return await this.agent.execute(userQuery, [
      this.mcpServer.tools.mapbox__category_search_tool,
      this.mcpServer.tools.mapbox__directions_tool
    ]);
  }
  ...
  async calculateDistance(from: Point, to: Point): Promise<number> {
    // No API cost, instant
    return await this.mcpServer.callTool('mapbox__distance_tool', {
      from,
      to,
      units: 'miles'
    });
