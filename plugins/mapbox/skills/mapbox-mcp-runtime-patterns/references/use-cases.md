...
async findPropertiesByCommute(
  ...
  const reachableArea = await mcp.callTool('mapbox__isochrone_tool', {
  ...
  for (const property of properties) {
    const inRange = await mcp.callTool('mapbox__point_in_polygon_tool', {
    ...
    if (inRange) {
      ...
      const directions = await mcp.callTool('mapbox__directions_tool', {
...
async canDeliver(
  ...
  const deliveryZone = await mcp.callTool('mapbox__isochrone_tool', {
  ...
  const canDeliver = await mcp.callTool('mapbox__point_in_polygon_tool', {
  ...
  const route = await mcp.callTool('mapbox__directions_tool', {
...
async buildItinerary(
  ...
  const attractionsWithDistance = await Promise.all(
    attractions.map(async (attr) => ({
      ...attr,
      distance: await mcp.callTool('mapbox__distance_tool', {
        from: hotel,
        to: attr.location,
        units: 'miles'
      })
