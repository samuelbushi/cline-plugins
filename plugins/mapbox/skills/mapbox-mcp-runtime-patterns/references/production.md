...
class CachedMapboxMCP {
  ...
  async callTool(name: string, params: any): Promise<any> {
    ...
    const offlineTools = ['mapbox__distance_tool', 'mapbox__point_in_polygon_tool', 'mapbox__bearing_tool'];
...
for (const location of locations) {
  const distance = await mcp.callTool('mapbox__distance_tool', {
    from: userLocation,
    to: location
  });
}
...
const distances = await Promise.all(
  locations.map((location) =>
    mcp.callTool('mapbox__distance_tool', {
      from: userLocation,
      to: location
    })
  )
...
const matrix = await mcp.callTool('mapbox__matrix_tool', {
...
Problem: "How far is it from A to B?" - Could trigger either `mapbox__directions_tool` OR `mapbox__distance_tool`

```typescript
// ❌ Ambiguous descriptions
{
  name: 'mapbox__directions_tool',
  description: 'Get directions between two locations'  // Could mean distance
}
{
  name: 'mapbox__distance_tool',
...
Problem: "Find coffee shops nearby" - Could trigger `mapbox__category_search_tool` OR `mapbox__search_and_geocode_tool`
...
{
  name: 'mapbox__category_search_tool',
...
{
  name: 'mapbox__search_and_geocode_tool',
...
Problem: "Where can I go in 15 minutes?" - Could trigger `mapbox__isochrone_tool` OR `mapbox__directions_tool`

```typescript
// ❌ Confusing
{
  name: 'mapbox__isochrone_tool',
...
const searchPOITool = new DynamicStructuredTool({
  ...
  DO NOT use for:
  - Specific named places (use mapbox__search_and_geocode_tool instead)
  - Addresses (use mapbox__search_and_geocode_tool or mapbox__reverse_geocode_tool)`
...
- Use mapbox__distance_tool for straight-line distance ("as the crow flies")
- Use mapbox__directions_tool for route distance along roads with traffic
- Use mapbox__category_search_tool for finding types of places ("coffee shops")
- Use mapbox__search_and_geocode_tool for specific addresses or named places ("123 Main St", "Starbucks downtown")
- Use mapbox__isochrone_tool for "what can I reach in X minutes" questions
- Use offline tools (mapbox__distance_tool, mapbox__point_in_polygon_tool) when real-time data is not needed
...
const localOps = {
  distance: 'mapbox__distance_tool', // Turf.js
  pointInPolygon: 'mapbox__point_in_polygon_tool', // Turf.js
  bearing: 'mapbox__bearing_tool', // Turf.js
  area: 'mapbox__area_tool' // Turf.js
};

// Use API tools when necessary (requires token, slower)
const apiOps = {
  directions: 'mapbox__directions_tool', // Mapbox API
  geocoding: 'mapbox__reverse_geocode_tool', // Mapbox API
  isochrone: 'mapbox__isochrone_tool', // Mapbox API
  search: 'mapbox__category_search_tool' // Mapbox API
};
...
class MockMapboxMCP {
  async callTool(name: string, params: any): Promise<any> {
    const mocks = {
      mapbox__distance_tool: () => '2.5',
      mapbox__directions_tool: () => JSON.stringify({
        duration: 1200,
        distance: 5000,
        geometry: {...}
      }),
      mapbox__point_in_polygon_tool: () => 'true'
    };
