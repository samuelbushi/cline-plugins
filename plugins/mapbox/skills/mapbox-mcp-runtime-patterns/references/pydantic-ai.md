...
# Use agent - MCP tools (mapbox__directions_tool, etc.) are available automatically
async def main():
...
> Key point: With `MCPServerHTTP`, you do NOT define tools manually - the agent discovers them from the MCP server. The server exposes tools like `mapbox__directions_tool`, `mapbox__category_search_tool`, `mapbox__isochrone_tool`, etc.

### How the Agent Calls mapbox__directions_tool

When the agent processes a directions query, it will call `mapbox__directions_tool` with these parameters:

```python
# The agent automatically calls mapbox__directions_tool like this:
{
    "coordinates": [
