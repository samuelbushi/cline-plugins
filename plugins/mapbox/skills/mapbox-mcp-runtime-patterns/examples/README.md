...
### 1. Pydantic AI Custom Tool Example
...
This runnable example intentionally wraps the remote MCP endpoint with typed Pydantic AI tools. For the simpler direct-MCP setup, use `references/pydantic-ai.md`, which shows `MCPServerHTTP`.

Features:

- Type-safe tool definitions around selected Mapbox MCP tools
- Environment variable support
- Lower-level JSON-RPC integration with the remote MCP server
...
- All tools use the remote Mapbox MCP server
...
| --------------- | ---------- | ------------------- | ---------- | ----------- |
| Pydantic AI | Python     | Production apps     | Medium     | High        |
| CrewAI      | Python     | Multi-agent systems | High       | Medium      |
| Smolagents  | Python     | Lightweight agents  | Low        | Medium      |
| Mastra      | TypeScript | Typed agents        | Medium     | High        |
| LangChain   | TypeScript | Conversational AI   | High       | Medium      |
...
All examples connect to the remote Mapbox MCP Server at `https://mcp.mapbox.com/mcp`.

The tool names below are raw MCP tool names used by direct framework clients and example wrappers. Inside Cline, the same tools are exposed with the server prefix, such as `mapbox__directions_tool`, `mapbox-devkit__create_style_tool`, or `mapbox-docs__get_latest_mapbox_docs_tool`.
