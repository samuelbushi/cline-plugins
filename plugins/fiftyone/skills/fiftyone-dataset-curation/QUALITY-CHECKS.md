...
# Samples wider than 2x their height
long_filter = F("metadata.width") > 2 * F("metadata.height")

# Samples taller than 2x their width
...
Threshold guideline: CLIPScore < 21.8 (cosine similarity x 100) indicates poor alignment (from published CLIP research). Tune to dataset.
