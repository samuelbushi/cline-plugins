...
# If not installed, ask for approval before downloading and enabling it:
download_plugin(
...
- Solution: Ask for approval, then install with `download_plugin()` and `enable_plugin()`
...
- After installing umap-learn, restart the active Cline session or user-managed MCP server and retry
