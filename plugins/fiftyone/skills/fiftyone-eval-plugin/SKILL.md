...
Static review comes first. Do not install, enable, import, or execute third-party plugin code until the user explicitly confirms that dynamic validation is wanted and understands the trust boundary.
...
When the plugin is trusted enough for dynamic validation, verify it against the actual FiftyOne plugin system. Use configured MCP tools to list operators, get schemas, and check registration when available; otherwise inspect manifests and use local FiftyOne CLI/Python checks. Do not execute untrusted plugin code just to complete the checklist.
