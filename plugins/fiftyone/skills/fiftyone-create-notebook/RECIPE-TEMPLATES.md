...
### Cell 5 -- code
```python
# Review duplicates instead of deleting by default
dups = results.duplicates_view()
dups.tag_samples("candidate_duplicate")
print(f"Tagged {len(dups)} candidate duplicate samples for review")

# Optional after manual review and explicit confirmation:
