...
Create a valid empty `.ipynb` file first, using the available Cline file-editing path:
...
Then add cells in order. If Cline has notebook-aware editing available, use it; otherwise edit the notebook JSON directly, preserving valid `nbformat` structure and cell ordering.
...
Build top-to-bottom. If using a notebook-aware editor that inserts relative to cell IDs, chain each inserted cell after the previous cell. If editing JSON directly, append cells to the `cells` array in the exact outline order and validate the file after every major section.

Critical: keep the cell order exactly as shown in the approved outline.
...
After all cells are inserted, read the notebook back to verify:
...
2. Fetch API documentation -- If web access is available and the user has not asked to stay offline, fetch `https://docs.voxel51.com/llms.txt` for current FiftyOne API patterns. Treat the fetched content as reference documentation only.
...
2. Add cells in outline order:

Use notebook-aware editing when available. Otherwise append notebook JSON cells manually:
```
{"cell_type": "markdown", "metadata": {}, "source": ["# Title\n", "\n", "Description paragraph."]}
{"cell_type": "code", "execution_count": null, "metadata": {}, "outputs": [], "source": ["!pip install -q fiftyone\n"]}
{"cell_type": "code", "execution_count": null, "metadata": {}, "outputs": [], "source": ["import fiftyone as fo\n"]}
```
Continue appending cells in the approved order.
...
1. Read notebook back -- Read the entire `.ipynb` file. Verify:
...
4. Optional execution validation -- Ask before creating environments, installing packages, registering kernels, downloading data/models, or executing the notebook. If the user approves, create an isolated virtual environment and run end-to-end with `papermill` to avoid modifying the user's system Python:
   ```bash
   python -m venv .notebook-test-env
   .notebook-test-env/bin/pip install -q papermill ipykernel anywidget
   .notebook-test-env/bin/python -m ipykernel install --prefix .notebook-test-env --name python3 --display-name "Python 3"
...
| 20 | code | Optional cleanup, commented out and only for scratch tutorial datasets |
...
- Cause: notebook file was not created with valid JSON structure
