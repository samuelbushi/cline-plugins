...
> WARNING: Re-encoding overwrites files on disk. Show the user the exact paths that will be modified and get explicit confirmation before running.
...
> WARNING: Export important datasets before downgrading.
...
> WARNING: After applying this fix, restart any running FiftyOne App or Python processes -- env var changes only affect new processes. Check for stale processes: `ps aux | grep fiftyone`. Confirm with user before killing them.
