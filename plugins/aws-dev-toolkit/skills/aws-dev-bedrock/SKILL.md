...
2. Use the `awsknowledge` MCP tools to verify current Bedrock model availability, pricing, and features (these change frequently)
...
Resource creation belongs in IaC. Use the `aws-dev-iac-scaffold` skill for templates.
...
### Related Skills
- `aws-dev-cost-check` - Broader AWS cost analysis beyond Bedrock
- `aws-dev-iac-scaffold` - IaC templates for Bedrock resource creation
- `aws-dev-security-review` - Security audit for Bedrock configurations and guardrail policies
