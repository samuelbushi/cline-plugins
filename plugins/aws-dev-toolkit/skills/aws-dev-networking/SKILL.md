...
- `aws-dev-security-review` - Network security posture, security group audits, NACLs
- `aws-dev-iam` - VPC endpoint policies, resource-based access control
- `aws-dev-ec2` - Instance placement, security groups, and subnet selection
- `aws-dev-ecs` - awsvpc networking, task-level security groups, service discovery, ECR endpoint requirements
- `aws-dev-eks` - Pod networking, secondary CIDRs, CNI configuration, IP address planning
- `aws-dev-lambda` - Lambda VPC configuration, ENI usage, endpoint requirements
- `aws-dev-rds-aurora` - Database subnet groups, isolated subnet placement
