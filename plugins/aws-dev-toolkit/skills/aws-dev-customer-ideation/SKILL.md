---
name: aws-dev-customer-ideation
...
- Migrate: Moving from another cloud or on-prem → use `aws-dev-migration-gcp-to-aws` or `aws-dev-migration-azure-to-aws` skills
- Optimize: Already on AWS, needs improvement → use `aws-dev-cost-check` and `aws-dev-architect` skills
...
```
Best for: AI-powered features, chatbots, document processing. Cost: Per Bedrock invocation (token-based). Use `aws-dev-bedrock` skill for detailed estimates.
...
5. Cost Estimate: Rough monthly range (use `aws-dev-cost-check` skill for precision)
6. Next Steps: Use the `aws-dev-iac-scaffold` skill to generate starter code, then iterate
...
3. Ignoring cost from the start: Model costs before building. Use `aws-dev-bedrock` for AI workloads. Set up AWS Budgets immediately.
