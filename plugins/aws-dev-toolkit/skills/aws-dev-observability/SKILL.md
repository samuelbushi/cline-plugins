...
- `aws-dev-lambda` - Lambda metrics, Embedded Metric Format, and X-Ray active tracing
- `aws-dev-ecs` - Container Insights, task-level metrics, and ECS service alarms
- `aws-dev-eks` - Control plane logging, Prometheus, and Container Insights for Kubernetes
- `aws-dev-cloudfront` - CloudFront access logs and cache metrics
- `aws-dev-api-gateway` - API Gateway latency and error monitoring
- `aws-dev-networking` - VPC Flow Logs, Route53 health checks, and Transit Gateway metrics
