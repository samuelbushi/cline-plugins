...
5. Use the `awsknowledge` MCP tools to verify current EKS versions, add-on compatibility, or feature availability
...
- `aws-dev-ecs` - Simpler container orchestration alternative when Kubernetes is not required
- `aws-dev-ec2` - Instance types, Spot strategy, and ASG config for self-managed nodes
- `aws-dev-networking` - VPC design, pod networking (secondary CIDRs), and security groups
- `aws-dev-iam` - IRSA, Pod Identity, and node role configuration
- `aws-dev-observability` - CloudWatch Container Insights, Prometheus, and control plane logging
- `aws-dev-lambda` - Serverless alternative for event-driven or low-traffic workloads
