...
2. Use the `awsknowledge` MCP tools to verify current IoT Core limits, Greengrass component versions, and service quotas
...
### Related Skills
- `aws-dev-lambda` -- Lambda functions as IoT rule actions and Greengrass components
- `aws-dev-step-functions` -- Orchestrating multi-step device provisioning and remediation workflows
- `aws-dev-dynamodb` -- Device metadata storage design, partition key strategy, TTL configuration
- `aws-dev-s3` -- Telemetry archival, lifecycle policies, Athena integration for batch queries
- `aws-dev-messaging` -- SQS/SNS integration with IoT rules for decoupled processing and alerting
- `aws-dev-observability` -- CloudWatch metrics, alarms, and dashboards for IoT fleet monitoring
- `aws-dev-iam` -- IAM roles for IoT rules engine actions, Greengrass token exchange, and fleet provisioning
- `aws-dev-networking` -- VPC endpoints for IoT Core, private connectivity for Greengrass core devices
- `aws-dev-security-review` -- Security audit of IoT policies, certificate management, and Device Defender configuration
