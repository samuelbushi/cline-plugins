...
Use the discovery questions from the `aws-dev-customer-ideation` skill as your reference menu.
...
Apply the `aws-dev-architect` skill's process:
...
Invoke the `aws-dev-security-review` skill to validate the proposed architecture. Review should cover:
- IAM least privilege
...
Use the `aws-dev-cost-check` skill or `awspricing` MCP tools to produce a rough monthly cost range. Include:
- Baseline cost (steady state)
- Scale cost (at projected peak)
- Cost optimization opportunities (Savings Plans, Spot, right-sizing)

For AI/ML workloads, also invoke the `aws-dev-bedrock` skill.
...
## Next Steps
1. [Scaffold IaC with the `aws-dev-iac-scaffold` skill]
2. [Set up CI/CD]
...
- `aws-dev-architect` - Well-Architected design evaluation and service selection
- `aws-dev-customer-ideation` - Discovery questions and requirements gathering
- `aws-dev-security-review` - Mandatory security validation for proposed architectures
- `aws-dev-cost-check` - Cost estimation and optimization analysis
- `aws-dev-challenger` - Pushback and alternative perspective on proposed designs
