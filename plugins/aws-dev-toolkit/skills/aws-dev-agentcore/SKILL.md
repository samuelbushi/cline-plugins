...
2. Use the `awsknowledge` MCP tools to verify current AgentCore quotas, regional availability, and API changes
...
### Related Skills
- `aws-dev-bedrock` - Bedrock cost modeling and model selection for agent workloads
- `aws-dev-strands-agent` - Strands Agents SDK scaffolding (deploys to AgentCore Runtime)
- `aws-dev-security-review` - IAM, network, and encryption audit for agent infrastructure
- `aws-dev-networking` - VPC design for agents accessing private resources
- `aws-dev-observability` - CloudWatch/X-Ray deep-dive for agent monitoring
- `aws-dev-step-functions` - Alternative orchestration for deterministic multi-step workflows
...
Include estimated monthly cost range using the `aws-dev-cost-check` skill or the `awspricing` MCP tools.
