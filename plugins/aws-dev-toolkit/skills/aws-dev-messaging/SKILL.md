...
2. Use the `awsknowledge` MCP tools to verify current service limits and features
...
- `aws-dev-lambda` - Lambda as SQS/SNS/EventBridge consumer, event source mappings
- `aws-dev-step-functions` - Orchestrated saga pattern, workflow coordination
- `aws-dev-dynamodb` - DynamoDB Streams as event source, event sourcing store
- `aws-dev-observability` - Queue depth alarms, DLQ monitoring, message age alerts
- `aws-dev-api-gateway` - API Gateway to SQS/SNS integration for async APIs
