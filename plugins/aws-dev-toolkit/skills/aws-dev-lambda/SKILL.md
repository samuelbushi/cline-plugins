...
- `aws-dev-api-gateway` -- API Gateway configuration, routing, authorization, and Lambda integration patterns.
- `aws-dev-dynamodb` -- Table design, access patterns, streams, and DynamoDB-Lambda integration.
- `aws-dev-step-functions` -- Orchestrating Lambda functions with state machines instead of direct invocation chains.
- `aws-dev-messaging` -- SQS, SNS, and EventBridge patterns for async Lambda triggers.
- `aws-dev-observability` -- CloudWatch metrics, alarms, dashboards, and X-Ray tracing beyond Powertools.
- `aws-dev-iam` -- Least-privilege execution roles, resource policies, and cross-account access for Lambda.
