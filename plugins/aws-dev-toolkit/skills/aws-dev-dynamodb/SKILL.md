...
2. Use the `awsknowledge` MCP tools to verify current DynamoDB limits and features
...
- `aws-dev-lambda` - Lambda with DynamoDB Streams event source mapping
- `aws-dev-api-gateway` - API Gateway direct integration with DynamoDB
- `aws-dev-messaging` - DynamoDB Streams feeding event-driven architectures
- `aws-dev-cost-check` - DynamoDB capacity mode cost analysis, reserved capacity
- `aws-dev-iam` - Fine-grained access control with DynamoDB condition keys
