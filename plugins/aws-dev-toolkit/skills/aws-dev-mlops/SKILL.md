...
2. Use the `awsknowledge` MCP tools to verify current SageMaker instance types, limits, pricing, and feature availability
...
### Related Skills
- `aws-dev-bedrock` - Foundation model customization, fine-tuning, and Bedrock-native inference
- `aws-dev-eks` - Kubernetes cluster design for Kubeflow or self-hosted MLflow deployments
- `aws-dev-lambda` - Serverless compute for lightweight ML inference or pipeline triggers
- `aws-dev-step-functions` - Workflow orchestration for simple ML pipelines without SageMaker Pipelines
- `aws-dev-s3` - Data lake design, artifact storage, lifecycle policies for training data and model artifacts
- `aws-dev-iam` - Least-privilege roles for SageMaker execution, cross-account model deployment
- `aws-dev-observability` - CloudWatch dashboards, alarms, and logging for ML infrastructure
- `aws-dev-cost-check` - Detailed cost analysis, Savings Plans recommendations, and Spot vs On-Demand comparison
- `aws-dev-ec2` - Instance type selection for self-managed training clusters or custom inference servers
...
Include estimated monthly cost range using the `aws-dev-cost-check` skill.
