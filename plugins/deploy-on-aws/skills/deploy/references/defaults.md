...
> Warning: App Runner is in maintenance mode - AWS has moved App Runner to maintenance mode. Do not recommend App Runner for new workloads. Use ECS Express Mode instead.
...
Use the `amplify_docs` topic in the `aws-knowledge` MCP server for framework-specific guidance
