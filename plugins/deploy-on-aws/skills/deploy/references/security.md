...
Use the `aws-knowledge` MCP topic `vpc_patterns` for multi-AZ production layouts.
...
Consult the `aws-iac` MCP server for IAM policy patterns by service.
...
- `aws-knowledge` topics: `vpc_patterns`, `iam_best_practices`, `security`
- `aws-iac` for CDK security constructs (L2 constructs apply many defaults)
