...
5. Use the example entries in the table below as conceptual guidance for edge routing and layout patterns. You may read the selected bundled example from this skill's `references/` directory, but do not proactively read `.drawio` files from the user's project unless they ask you to reference or modify them.
...
2. Run the `validate_aws_drawio` Cline tool on the new file. It applies the bundled post-processing pipeline, validates XML structure and AWS4 shape names, and returns a preview URL when validation passes.
3. If validation fails, fix errors and rewrite
4. Re-run `validate_aws_drawio` until validation passes
5. If export format requested, run draw.io CLI (see `references/cli-export.md`)
...
5. Run `validate_aws_drawio` against the saved file:
   a. It applies deterministic post-processing fixes
   b. It validates XML structure, AWS shapes, edges, and geometry
   c. If validation passes, it returns a draw.io preview URL without opening a browser
6. If validation fails, fix the errors and rewrite the file
7. Only after validation passes, present the preview URL returned by `validate_aws_drawio`
...
- ALWAYS set `container=1;pointerEvents=0;` on VPC, subnet, and other real grouping shapes. Region groups are the exception and MUST use `container=0`.
