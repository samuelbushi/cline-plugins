...
SAFE_LOCATION_RE = re.compile(r"^[A-Za-z0-9-]+$")


async def run_command_async(cmd: list[str]) -> str:
  """Runs a command asynchronously and returns the stdout."""
...
async def create_and_wait_for_scan(
  ...
  """
  if not SAFE_LOCATION_RE.fullmatch(location):
    logging.error("[%s] Invalid location: %s", table_id, location)
    return

  parts = table_id.split(".")
  if not all(SAFE_ID_RE.fullmatch(part) for part in parts):
    logging.error("[%s] Invalid table identifier. Skipping.", table_id)
    return
