...
Use these standards while editing Liquid, CSS, JavaScript, and HTML in a Shopify theme. If local diagnostics are useful and the workspace is trusted, run the plugin's `liquid_theme_diagnostics` tool after meaningful changes; it runs bounded `shopify theme check` output rather than a persistent language server.
