...
Use this skill directly when editing Shopify theme files. Cline does not provide a persistent Liquid language-server surface; when diagnostics are useful and the user is working in a trusted local theme workspace, run the plugin's `liquid_theme_diagnostics` tool to execute bounded `shopify theme check` output.
