...
Use these patterns while creating or reviewing accessible Shopify theme components. If local diagnostics are useful and the workspace is trusted, run the plugin's `liquid_theme_diagnostics` tool after meaningful theme changes; do not start unrelated Shopify CLI commands just because the skill is active.
...
Rules:
- Keep every actionable control keyboard reachable, including quick add
- Avoid duplicate tab stops for the same destination; make secondary actions explicit
