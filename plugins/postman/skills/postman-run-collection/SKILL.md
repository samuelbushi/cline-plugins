---
name: postman-run-collection
description: Run Postman collection tests with the local Postman CLI and diagnose failures.
---
...
Before running the command, summarize the collection, target environment, request scope, base URL if known, and whether the run may hit non-local services or mutate data. Ask for confirmation before broad collection runs, non-local targets, credentials, file output, or any run that may affect production-like data. Show the command with secret values redacted rather than any exact secret-bearing command.
...
Not authenticated:
"Postman CLI requires authentication. Ask before running `postman login`"
...
- Ask before broad, non-local, credentialed, or side-effecting collection runs
