...
This workflow does not use `the bundled GraphQL validator`; that tool validates GraphQL only, not app TOML or extension config files.
...
- After `the bundled GraphQL validator` succeeds, inspect its output for a `Required scopes: ...` line.
