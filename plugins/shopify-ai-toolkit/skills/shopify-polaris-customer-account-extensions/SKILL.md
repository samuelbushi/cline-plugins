...
If this is an extension workflow and the target is unknown, search for `extension targets` first. Validation requires `--target <extension-target>` for this skill.
...
```bash
scripts/validate.mjs --code '...' [--version YYYY-MM] --target <extension-target>
```
