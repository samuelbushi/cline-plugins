...
var getCompilerOptions = (jsxImportSource, packageRoot) => ({
  ...
  ...packageRoot ? {
    ...
    // build-files/ts. Keep the assets unchanged and teach TS to resolve those
    // internal type-only imports against the extracted build tree.
    baseUrl: packageRoot,
    paths: {
      "@shopify/ui-extensions/src/surfaces/*": [
        "node_modules/@shopify/ui-extensions/build-files/ts/surfaces/*"
      ]
    }
  } : {}
});
...
function resolveTypesDataDirectory(currentDir) {
  ...
  if (currentDir.includes("dev-mcp") && currentDir.includes("dist-files") && !currentDir.includes("shopify-dev-tools")) {
    return path2.join(currentDir, "data", "types");
  }
  if (currentDir.includes("/dist-files") || currentDir.includes("\\dist-files")) {
    const distFilesIndex = currentDir.lastIndexOf(path2.sep + "dist-files");
    if (distFilesIndex !== -1) {
      const distRoot = currentDir.substring(0, distFilesIndex + 10);
...
function surfaceMatcher(extensionSurfaceName) {
  ...
  const surfaceEntries = new Set(
    surfaceNames.map((s) => `build-files/ts/surfaces/${s}.d.ts`)
  );
  const surfaceSubtreePrefixes = surfaceNames.map(
    (s) => `build-files/ts/surfaces/${s}/`
  );
  const topLevelPrefix = "build-files/ts/";
  return (rel) => {
...
function loadTargetSpecificComponents(assetRoot, packageName, virtualEnv, packageRoot, extensionSurfaceName, extensionTarget, shopifyWebComponents) {
  const surfaceRel = `build-files/ts/surfaces/${extensionSurfaceName}`;
  ...
  loadDtsTree(
    ...
    shopifyWebComponents,
    (rel) => {
      if (!rel.startsWith("build-files/ts/")) return false;
      const rest = rel.slice("build-files/ts/".length);
