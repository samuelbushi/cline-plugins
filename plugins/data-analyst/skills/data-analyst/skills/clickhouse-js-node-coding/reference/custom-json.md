...
- OK No new dependency to install.
- OK Only promotes known to be 64-bit integers to `BigInt`.
- BAD Requires Node `>= 20.16` / `>= 21.7` for the reviver `context.source`.
  On older Node, prefer `json-bigint` or upgrade Node.
- BAD Outgoing `stringify` still uses default `JSON.stringify`, which throws
