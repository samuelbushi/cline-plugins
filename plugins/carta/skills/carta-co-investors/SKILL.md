...
Use the plugin-owned `carta` MCP server in Cline. Treat any Cowork, Claude Preview, `.claude/launch.json`, `mcp__cowork__...`, or `preview_*` instructions below as optional host-specific source notes. In Cline, prefer local files in the current workspace, surface paths for the user to open, and ask before writing report artifacts or running broad warehouse queries. Resolve bundled scripts and references relative to this skill directory instead of relying on `CLAUDE_PLUGIN_ROOT`.
...
- Never use tables for lists of user actions -- use `ask the user` instead.
...
Tell the user: `Artifact generated: $WORKSPACE/carta-co-investors.html`

### Step A4: Surface local report

Do not write `.claude/launch.json`, start a preview server, or call preview tools in Cline. The generated HTML file is the deliverable.

If `uv` is unavailable, do not fabricate the HTML. Offer Mode B text analysis instead, or ask the user to install `uv` and rerun the artifact generation command. `uv` reads the bundled script metadata and resolves Python dependencies consistently; this Cline plugin bundles the scripts, but it does not install `uv`.
...
> investor breakdown."
>
> Local report: `$WORKSPACE/carta-co-investors.html`
>
