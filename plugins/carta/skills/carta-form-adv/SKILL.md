...
Use the plugin-owned `carta` MCP server in Cline. Generate local HTML and `.xlsx` files as the deliverable, surface their paths, and ask before writing report artifacts or running broad warehouse queries. Ignore source-host preview instructions such as Cowork, Claude Preview, `.claude/launch.json`, `mcp__cowork__...`, or `preview_*`. Resolve bundled scripts and references relative to this skill directory instead of relying on `CLAUDE_PLUGIN_ROOT`.
...
You MUST complete every step on every invocation. Step 3 is not optional -- the local files are the deliverable, not a supplementary export.
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/carta-form-adv/references/filing-guide.md` and execute its local-file sub-steps in order, every time:
...
4. Surface the generated HTML and `.xlsx` paths for the user to open locally.
...
1. User names a specific fund, no match found -> use `ask the user` to present up to 4 candidate fund names. Do not expand scope until the user picks one.
...
| User cannot open HTML | Browser/file association issue | "The HTML file is at the path above. Open it in a browser, or use the Excel filing reference instead." |
