...
- Ask before logging in, submitting forms, purchasing, posting, uploading, or changing remote state.
- Do not enter credentials or personal data unless the user explicitly provides and approves that action.
- Use read-only extraction when possible, and close sessions with `firecrawl interact stop` when finished.
...
```bash
# Session 1: Login and save state only after the user approves
