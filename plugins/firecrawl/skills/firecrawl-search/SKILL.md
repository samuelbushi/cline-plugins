...
description: |
  Web search with full page content extraction through Firecrawl. Use when the user explicitly asks to use Firecrawl search, needs search results saved to disk, needs full-page content scraped from search results, or needs Firecrawl search options such as news filters, categories, or JavaScript-rendered page extraction. Do not trigger for ordinary quick web lookup unless Firecrawl is requested.
---
...
Search costs credits. Firecrawl supports structured feedback with `firecrawl search-feedback <id>`. The first feedback per search may refund 1 credit and helps improve search quality. Because feedback sends search quality details to Firecrawl, ask the user before submitting it.
...
- `--silent` is useful when submitting approved feedback because rejected or expired feedback should not interrupt the main task.
...
`--silent` suppresses output so feedback does not distract from the main task.
