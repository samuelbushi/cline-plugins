...
description: |
  Extract clean markdown from known URLs with Firecrawl, including JavaScript-rendered SPAs. Use when the user explicitly asks for Firecrawl, wants persisted page output, needs multiple concurrent URL scrapes, needs formats such as links/screenshot/json, or built-in web lookup is insufficient. Do not trigger for ordinary quick webpage reading unless Firecrawl is requested.
---
...
- Write large outputs to `.firecrawl/` and read only relevant excerpts.
- Ask before scraping private, login-gated, or sensitive pages.
- Prefer Cline's normal web lookup for simple current facts unless the user asks for Firecrawl or needs Firecrawl-specific behavior.
