...
- Prefer `scrape` or `crawl` for simple extraction; use `agent` for genuinely complex structured extraction.
- Set `--max-credits` for exploratory or broad runs.
- Ask before extracting personal, private, gated, or sensitive data.
...
```bash
# Extract structured data with a credit cap
