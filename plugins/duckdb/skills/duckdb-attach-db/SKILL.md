...
description: >
  Attach a DuckDB database file for use with duckdb-query.
...
Database path: use the path the user provided in the current request.
...
If the requested database path is a relative path, resolve it against `$PWD` to get an absolute path (`RESOLVED_PATH`).

```bash
REQUESTED_DB_PATH="<path from the user request>"
case "$REQUESTED_DB_PATH" in
  /*) RESOLVED_PATH="$REQUESTED_DB_PATH" ;;
  *) RESOLVED_PATH="$PWD/$REQUESTED_DB_PATH" ;;
esac
```
...
- File not found -> ask the user if they want to create a new empty database (DuckDB creates the file on first write). If the parent directory also does not exist, ask before creating it or stop. Never rewrite the path to `/$(basename "$REQUESTED_DB_PATH")` if path resolution fails.
...
- Confirm the database is now active for `duckdb-query`
