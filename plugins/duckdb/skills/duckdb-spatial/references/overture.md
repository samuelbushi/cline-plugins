...
Use the STAC catalog at `https://stac.overturemaps.org/catalog.json` to find the current release before querying. Examples below use `<RELEASE>` as a placeholder.
...
CROSS JOIN LATERAL (
    SELECT * FROM read_parquet('s3://overturemaps-us-west-2/release/<RELEASE>/theme=places/type=place/*')
