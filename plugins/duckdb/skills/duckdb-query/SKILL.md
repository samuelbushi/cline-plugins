...
Input: use the SQL or data question from the current user request.
...
Ad-hoc mode (bounded to the referenced local file paths):
...
Do not set `enable_external_access=false` for ad-hoc file scans; DuckDB uses external-access plumbing for file readers and `httpfs`, and disabling it can break legitimate reads.
...
- Missing extension (e.g. `Extension "X" not loaded`): delegate to `duckdb-setup <ext>`, then retry.
...
- Persistent or unclear DuckDB error: use `duckdb-docs <error message or relevant keywords>` to search the documentation for guidance, then apply the fix and retry.
