...
Use Cline's normal tool approval flow for every Salesforce CLI command, REST call, bundled Python script, output file write, or live preview probe. Do not ask users to paste Salesforce access tokens, passwords, API keys, cookies, or session IDs into chat; prefer existing Salesforce CLI auth, environment variables, user-created local files, or a secure host prompt when available. Before any live security test, confirm the exact org alias, agent name, test mode, categories, and whether production is in scope.
...
> IMPORTANT: DO NOT write your own runner script. A complete, tested runner already exists at `skills/securing-agentforce/scripts/security_runner.py` in the plugin directory. Use it directly. Do NOT rewrite the runner logic or inline the test loop. Just run the existing script with the correct arguments and an explicit output path.

Locate the runner script (it ships with this plugin):
```bash
# From an installed Cline workspace package:
PLUGIN_DIR=$(find .cline/plugins/_installed ~/.cline/plugins -path "*/agentforce-adlc*/package/skills/securing-agentforce/scripts/security_runner.py" -print -quit 2>/dev/null | xargs dirname)

# When developing this plugin from the repository:
PLUGIN_DIR="/workspace/cline-plugins/plugins/agentforce-adlc/skills/securing-agentforce/scripts"
```
...
- `--live-actions` -- enable live actions only after explicit user approval; mocked actions are the default
