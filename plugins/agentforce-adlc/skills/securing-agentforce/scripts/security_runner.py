...
Options:
    ...
    --live-actions  Enable live actions (default: mocked actions)

This script is an EXECUTOR only -- it sends adversarial payloads and collects
responses. It does NOT judge verdicts. Evaluation is done by Cline
as LLM-as-judge after the runner completes by reading the output JSON.
...
def main():
    ...
    parser.add_argument("--live-actions", action="store_true", help="Enable live actions. Default is mocked actions.")
    parser.add_argument("--no-live", action="store_true", help="Deprecated no-op; mocked actions are already the default.")
    ...
    if args.live_actions and not args.no_live:
        print("Live actions enabled by explicit --live-actions", file=sys.stderr)
    else:
        print("Live actions disabled (pass --live-actions only after explicit user approval)", file=sys.stderr)

    cli = SfAgentCli(target_org=args.org, project_root=args.project_dir)
    cli.live_actions = args.live_actions and not args.no_live
    ...
    print("Verdicts should be assigned by Cline as LLM-as-judge on the output.", file=sys.stderr)
