...
Use Cline's normal tool approval flow for every Salesforce CLI command, REST call, test spec write, metadata deploy, or live preview run. Do not ask users to paste Salesforce access tokens, passwords, API keys, cookies, or session IDs into chat; prefer existing Salesforce CLI auth, environment variables, user-created local files, or a secure host prompt when available. Present generated test plans before execution, and require explicit authorization for production orgs or live actions.
