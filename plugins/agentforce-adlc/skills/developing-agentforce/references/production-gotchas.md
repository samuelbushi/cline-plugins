...
[MISSING] WRONG Syntax (causes compile error):
```yaml
before_reasoning:
   instructions: ->      # [MISSING] NO! Don't wrap with instructions:
...
```yaml
before_reasoning:
   set @variables.x = True   # [OK] Direct content under the block
```
...
```bash
# [MISSING] ERROR when using source tracking:
...
# [OK] WORKAROUND - Use CLI directly:
