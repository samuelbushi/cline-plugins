...
The `discover.py` script is bundled with this Cline plugin. Locate the plugin root first, then run the helper from that package.

```bash
PLUGIN_ROOT=$(find .cline/plugins/_installed ~/.cline/plugins -path "*/agentforce-adlc*/package/scripts/discover.py" -print -quit 2>/dev/null | sed 's#/scripts/discover.py##')
python3 "$PLUGIN_ROOT/scripts/discover.py" -o <org-alias> --agent-file <path-to-agent-file>
python3 "$PLUGIN_ROOT/scripts/discover.py" -o <org-alias> --agent-dir force-app/main/default/aiAuthoringBundles
python3 "$PLUGIN_ROOT/scripts/discover.py" -o <org-alias> --agent-file MyAgent.agent --validate-io
```
