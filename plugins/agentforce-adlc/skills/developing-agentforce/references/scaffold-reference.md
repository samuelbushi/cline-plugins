...
The `scaffold.py` script is bundled with this Cline plugin. Locate the plugin root first, then run the helper from that package.

```bash
PLUGIN_ROOT=$(find .cline/plugins/_installed ~/.cline/plugins -path "*/agentforce-adlc*/package/scripts/scaffold.py" -print -quit 2>/dev/null | sed 's#/scripts/scaffold.py##')
