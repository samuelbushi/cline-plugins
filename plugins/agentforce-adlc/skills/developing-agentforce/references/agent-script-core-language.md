...
> WARNING `timestamp` and `currency` compile as variable types but are absent from official GA documentation and should NOT be used. Prefer `date` for date/time variables and `number` for currency values.
...
```agentscript
# [OK] CORRECT -- simple if/else
if @variables.status == "pending":
    run @actions.notify_pending
else:
    run @actions.notify_complete

# [MISSING] WRONG -- else if not supported
if @variables.count < 5:
    run @actions.small
else if @variables.count < 10:
    run @actions.medium

# [MISSING] WRONG -- nested if inside else is also invalid
if @variables.status == "pending":
