...
WARNING: CRITICAL: Agent Script is NOT AppleScript, JavaScript, Python, or any other
...
Use Cline's normal tool approval flow for every Salesforce CLI command, REST call, bundled helper script, metadata edit, deploy, publish, activate, permission assignment, or live-action preview. Do not ask users to paste Salesforce access tokens, passwords, API keys, cookies, or session IDs into chat; prefer existing Salesforce CLI auth, environment variables, user-created local files, or a secure host prompt when available.

When a reference mentions `scripts/discover.py`, `scripts/scaffold.py`, or another bundled helper, locate this plugin package first. From this repository the plugin root is `/workspace/cline-plugins/plugins/agentforce-adlc`; from an installed Cline workspace package, search `.cline/plugins/_installed` for an `agentforce-adlc*/package` directory. Ask before running helper scripts or copying generated files into the user's Salesforce project.

References to `developing-agentforce`, `testing-agentforce`, `securing-agentforce`, and `observing-agentforce` mean the corresponding Cline skills in this plugin, not slash commands.
