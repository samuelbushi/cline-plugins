...
If using an installed Cline plugin package, locate the package first:
```bash
PLUGIN_ROOT=$(find .cline/plugins/_installed ~/.cline/plugins -path "*/agentforce-adlc*/package/skills/observing-agentforce/apex/AgentforceOptimizeService.cls" -print -quit 2>/dev/null | sed 's#/skills/observing-agentforce/apex/AgentforceOptimizeService.cls##')
cp "$PLUGIN_ROOT/skills/observing-agentforce/apex/AgentforceOptimizeService.cls" \
   <project-root>/force-app/main/default/classes/
cp "$PLUGIN_ROOT/skills/observing-agentforce/apex/AgentforceOptimizeService.cls-meta.xml" \
   <project-root>/force-app/main/default/classes/
```
