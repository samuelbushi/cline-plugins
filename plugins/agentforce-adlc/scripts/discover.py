...
TARGET_TYPE_PATTERN = "flow|apex|retriever|prompt|generatePromptResponse|externalService"
...
def extract_targets(agent_file: Path) -> list[tuple[str, str, str]]:
    ...
    targets = []
    for match in re.finditer(rf'target:\s*"?({TARGET_TYPE_PATTERN})://([^"\s]+)"?', content):
...
def extract_actions(agent_file: Path) -> list[dict]:
    ...
    for line in content.splitlines():
        ...
        # Detect target line
        target_match = re.match(rf'target:\s*"?({TARGET_TYPE_PATTERN})://([^"\s]+)"?', stripped)
...
def _check_prompt_templates(names: list[str], target_org: str) -> dict[str, bool]:
    """Check which prompt templates exist in the org."""
    records = _query_org(
        "SELECT DeveloperName FROM PromptTemplate WHERE Status = 'Active'",
        target_org,
    )
    org_templates = {r["DeveloperName"] for r in records if "DeveloperName" in r}
    return {name: name in org_templates for name in names}


def _check_external_services(names: list[str], target_org: str) -> dict[str, bool]:
    """Check which external service registrations exist in the org."""
    records = _query_org(
        "SELECT DeveloperName FROM ExternalServiceRegistration",
        target_org,
    )
    org_services = {r["DeveloperName"] for r in records if "DeveloperName" in r}
    return {name: name in org_services for name in names}
...
def discover(agent_file: Path, target_org: str, validate_io: bool = False) -> DiscoveryReport:
    ...
    checkers = {
        ...
        "prompt": (_check_prompt_templates, "SELECT DeveloperName FROM PromptTemplate WHERE Status = 'Active'", "DeveloperName"),
        "generatePromptResponse": (_check_prompt_templates, "SELECT DeveloperName FROM PromptTemplate WHERE Status = 'Active'", "DeveloperName"),
        "externalService": (_check_external_services, "SELECT DeveloperName FROM ExternalServiceRegistration", "DeveloperName"),
    }
...
def print_report(report: DiscoveryReport) -> None:
    ...
    # Found targets
    if report.found:
        print(f"\n[OK] Found ({len(report.found)}):")
        for t in report.found:
            print(f"   {t.target}")

    # Missing targets
    if report.missing:
        print(f"\n[MISSING] Missing ({len(report.missing)}):")
        for t in report.missing:
            print(f"   {t.target}")
            for s in t.suggestions:
                print(f"      Hint: Did you mean: {s.name} ({s.similarity:.0%} match)?")

    # I/O mismatches
    if report.io_mismatches:
        print(f"\nWARNING  I/O Mismatches ({len(report.io_mismatches)}):")
