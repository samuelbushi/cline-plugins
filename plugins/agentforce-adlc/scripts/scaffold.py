...
def _scaffold_apex(
    ...
    if is_callout:
        ...
        if _needs_auth_metadata(description_text):
            result.warnings.append(
                f"Action '{cls_name}' appears to need credentials. No secret metadata was generated; "
                "configure Salesforce Named Credentials or External Credentials before implementing the callout."
            )
...
def print_result(result: ScaffoldResult) -> None:
    ...
    if result.files_created:
        print(f"\n[OK] Created {len(result.files_created)} file(s):")
