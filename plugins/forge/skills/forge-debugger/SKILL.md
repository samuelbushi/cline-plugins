...
## Cline Execution Policy

Use read-only diagnostics first. Ask before commands that install packages, deploy, install, upgrade scopes, mutate production/staging state, start tunnels, write credentials, or change customer data. When a safe local fix is clear and the user asked Cline to fix it, make the edit and report what changed.

Commands requiring an interactive terminal (`forge login`, `forge tunnel`) must be run by the user in their own terminal - tell them exactly what to run and why.
...
- Own safe local fixes when the user asked Cline to fix the app; hand off interactive or high-impact commands with exact instructions.
- Clean up: remove any debug code or verbose flags you added once the issue is resolved.
- npx fallback: if the user approves downloading and executing npm package code, `npx @forge/cli` can be used as a drop-in replacement for Forge commands.
...
If the installed version is behind the latest major version, ask before upgrading:
...
Match the error against this table first. If you find a match, apply safe local fixes directly when the user asked Cline to fix the app; ask before deploy/install/upgrade/tunnel/package-install actions.

| Error / Symptom | Root Cause | Fix |
|-----------------|-----------|-----|
| "App is not installed on this site" | `forge install` wasn't run, or ran against wrong site | Ask for the Atlassian site URL if not already known, then ask before running `forge install --non-interactive --site <url> --product <jira\|confluence> -e development` |
| Blank panel / Custom UI white screen | Frontend build not run before deploy | Ask before running build/deploy commands such as `cd static && npm run build && cd .. && forge deploy -e development` |
...
| 403 / "Permission denied" / "Unauthorized" | OAuth scope missing from manifest | Add the least-privilege scope to `manifest.yml`, then ask before `forge deploy -e development && forge install --non-interactive --site <url> --upgrade` |
...
| "forge: command not found" | CLI not installed | Ask before `npm install -g @forge/cli` |
| `ENOENT` or missing files on deploy | `npm install` not run in app directory | Ask before package installation and deploy, for example `cd <app-dir> && npm install && forge deploy -e development` |
| "Rate limit exceeded" | Too many API calls in resolver | Add exponential backoff; check for resolver being called in a loop |
| "App tunnel disconnected" | `forge tunnel` connection dropped | Ask the user to re-run `forge tunnel` in their terminal; check VPN isn't blocking websocket connections |
...
| `npm install -g` permission error / cannot install forge globally | No sudo or write access to global npm directory | After user approval, use `npx @forge/cli` as a drop-in replacement for Forge commands. No global install needed |
