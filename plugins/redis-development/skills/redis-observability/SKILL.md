...
| Topic | Command |
|---|---|
| Slow commands | `SLOWLOG GET 10` / `SLOWLOG LEN` |
...
- `SLOWLOG RESET` clears diagnostic history. Use it only after exporting needed entries and getting explicit confirmation.
...
for entry in redis.slowlog_get(10):
    print(f"{entry['duration']}us  {entry['command']}")
```
