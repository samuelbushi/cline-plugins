...
SLOWLOG GET 10
SLOWLOG LEN
# SLOWLOG RESET clears history; run only after export and explicit confirmation.
...
Do not run `SLOWLOG RESET` during incident triage unless the user has confirmed the current slow log is exported or no longer needed.
...
for query in slow_queries:
    print(f"Duration: {query['duration']}us, Command: {query['command']}")
```
