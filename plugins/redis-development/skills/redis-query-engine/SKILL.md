...
The classic mistake is using `TEXT` for a category or status field because "it's a string." `TAG` is 10x faster for those.
...
Useful management commands: `FT.INFO`, `FT._LIST`, `FT.ALIASADD/UPDATE/DEL`. Treat `FT.DROPINDEX` as destructive for application query surfaces; use it only in development or after explicit production confirmation and rollback planning.
