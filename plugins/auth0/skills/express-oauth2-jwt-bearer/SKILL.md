...
| Python API (FastAPI) | `auth0-fastapi-api` |
| Python web app (Flask) | `auth0-flask` |
| Node.js API using the older `express-jwt` package | Prefer `express-oauth2-jwt-bearer` for new work; review legacy `express-jwt` manually |
...
>    ```
>
> 3. Configure Auth0  --  follow `references/setup.md`. If the user already provided their Auth0 Domain and API Audience in the prompt, use them directly  --  skip the bootstrap script and do NOT ask the user to re-confirm. Otherwise, offer automatic setup via bootstrap script or manual setup.
>
...
>    ```
>
> 8. Failcheck: If the server fails to start or tokens are rejected unexpectedly, check `references/api.md` for common issues. After 5-6 failed iterations, ask the user for more details about their environment.
...
- [auth0-fastapi-api](../auth0-fastapi-api)  --  JWT validation for FastAPI APIs
- [auth0-flask](../auth0-flask)  --  Auth0 login for Flask web apps
