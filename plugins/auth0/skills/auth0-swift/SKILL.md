...
> Agent instruction: Follow these steps in order. If you encounter an error at any step, attempt to fix it up to 5 times before asking the user for guidance. Always search existing code first  --  if there are existing login/logout handlers, hook into them rather than creating new ones.
>
> IMPORTANT  --  Credential privacy: Never echo Auth0 credentials (domain, client ID, client secret) in your response text or terminal output. Write them directly into config files using the Cline file-editing tools. When running Auth0 CLI commands that produce output containing these values, redirect output to a file and read it programmatically. For example:
> ```bash
...
>
> If none are found, ask in a plain prompt: _"Which dependency manager does your project use  --  Swift Package Manager, CocoaPods, or Carthage?"_
>
...
> - If no `Auth0.plist` exists and no credentials were provided: Ask the user in a plain prompt: _"How would you like to configure Auth0?"_
...
> 3. Ask the user in a plain prompt: _"Which callback URL scheme would you like to use?"_
...
> Extract `DEVELOPMENT_TEAM` from `project.pbxproj` (10-character value, e.g. `ABC12DE34F`). If not found, ask in a plain prompt: _"What is your Apple Team ID? (developer.apple.com → Account → Membership Details)"_
> ```bash
...
> Agent instruction: Search the project for `@main struct` (SwiftUI) or `AppDelegate`/`UIViewController` (UIKit) to detect the UI framework. If ambiguous, ask in a plain prompt: _"Does your app use SwiftUI or UIKit?"_ Then follow only the matching path below.
>
