...
> - If no `Auth0.plist` exists: Ask the user in a plain prompt which setup path they prefer:
...
>
> IMPORTANT  --  Credential privacy: Never echo Auth0 credentials (domain, client ID, client secret) in your response text or terminal output. Write them directly into config files using the Cline file-editing tools. When running Auth0 CLI commands, redirect output to a temporary file:
> ```bash
...
>
> 1. Ask the user in a plain prompt: _"Please place your `Auth0.plist` file (containing your ClientId and Domain) in the project root directory and let me know when it's ready. You can download it from Auth0 Dashboard → Applications → your app → Settings → scroll to bottom → 'Download Auth0.plist'."_
...
>
> If none are found, ask the user in a plain prompt: _"Which dependency manager does your project use  --  Swift Package Manager, CocoaPods, or Carthage?"_ Then follow only the matching section below.
