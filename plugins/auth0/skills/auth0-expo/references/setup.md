...
> Agent instruction:
>
> Important: Never echo Auth0 credentials (domain, client ID) in your response text. Write them directly into config files using the Cline file-editing tools.
>
> Credential check (always first):
> If the user's prompt already provides Auth0 credentials (domain, client ID), use them directly  --  do NOT ask the user to re-confirm. Go straight to writing the app.json config and proceeding with integration.
>
> If credentials are NOT provided, ask:
