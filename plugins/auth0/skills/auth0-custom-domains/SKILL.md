...
Ask questions as plain conversational text. Avoid structured-choice prompts except for a single yes/no confirmation immediately before a destructive or irreversible action (create, PATCH, delete). For everything else:
...
- Tier 1 Cloudflare: Cloudflare MCP connected in Cline. If not, prompt the user to add `https://mcp.cloudflare.com/mcp` as an HTTP MCP server in Cline's MCP settings, then authorize it in the browser.
