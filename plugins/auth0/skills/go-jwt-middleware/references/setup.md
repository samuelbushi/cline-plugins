...
> If the user's prompt already provides Auth0 credentials (domain and audience), use them directly. Write the `.env` file and proceed to integration. Do NOT ask the user to re-confirm.
>
...
>    - If a `.env` file already exists, ask the user for confirmation in a plain prompt: _"A `.env` file already exists in this project. Can I add the Auth0 configuration to it?"_
