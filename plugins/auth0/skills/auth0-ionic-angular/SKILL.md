...
> Agent instruction: Follow these steps in order. Always use a plain prompt to let the developer choose between Automatic Setup and Manual Setup before proceeding  --  even if credentials are already provided in the prompt.
>
...
> Agent instruction: Always present the setup choice in a plain prompt  --  even if the user has already provided credentials:
>
> ```
> a plain prompt:
...
> npx cap sync
> ```
> If the build fails, investigate errors and fix (up to 5-6 iterations). If still failing, ask the user for help.
