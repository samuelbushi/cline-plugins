...
> Regardless of whether the user has already provided credentials in their prompt, always use a plain prompt to let the developer choose between Automatic and Manual setup:
>
> ```
> a plain prompt:
...
# Linux
echo "Install Auth0 CLI from https://github.com/auth0/auth0-cli#installation"
exit 1
```
...
Ask the developer for the path to their `.env` file:
...
> Agent instruction: Read the file at the path the user provides. Extract the values for `AUTH0_DOMAIN` and `AUTH0_CLIENT_ID` by parsing `KEY=VALUE` lines. If the file is missing either key, ask the user to provide the missing value. Accept common variations: `DOMAIN` / `AUTH0_DOMAIN`, `CLIENT_ID` / `AUTH0_CLIENT_ID`.
