...
> - Never write actual credential values into any file using the Cline file-editing tools  --  always use placeholders and instruct the user to substitute their real values.
...
> ./gradlew build
> ```
> or `mvn package`. If build fails, diagnose and fix. After 5-6 failed attempts, use a plain prompt to get help.
